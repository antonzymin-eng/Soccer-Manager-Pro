...
- **`Possession.ControlHeight` ↔ `GroundControlHeight` cross-spec routing — OPEN** — *June 3, 2026.* `BallPhysicsConstants.Possession.ControlHeight` (Ball Physics #1 §3.1.2) and `FirstTouchConstants.GroundControlHeight` (First Touch #4) declare the same physical concept (max ball height for ground possession control) at the same value (0.50 m), both tagged `[GT]`, with no `[CROSS]` mirror chain between them. Per Spec #20 §4.2 routing rule one must be the authority and the other must mirror. Surfaced by the Ball Physics AR-2 L-5 / AR-3 L-5 reviews; XML doc on both constants warns about the drift hazard. **Action:** decide which spec is the authority (recommend Ball Physics #1 since the constant is physical, not first-touch-specific), then patch the non-authority side to `[CROSS]` mirror. Until landed, designers MUST tune both values together or accept silent drift between possession and first-touch acceptance gates. Investigate `Possession.ControlRadius` / `ControlVelocity` / `ChallengeRadius` for the same hazard while the routing decision is being made.
