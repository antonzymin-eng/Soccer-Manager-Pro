...
**15 active** / 41 resolved. *Re-filed August 2, 2026 — eight entries archived (six closed-but-unmoved, plus a duplicated pair); three titles amended to lead with what remains open rather than what has landed. August 4, 2026 — the wiring-backlog entry added at the head; later same day, the football-judgment remediation entry added above it. August 7, 2026 — the two-red-scenario-locks entry added at the head (the B9c gate run's finding).*

- **Two `MatchEngine.Tests` scenario locks are RED on origin/main — `sim_match_engine_close_chance` (meanCosine −0.119 vs bound −0.10) and `sim_match_engine_keeper_contact` (deepDiveEarly = 1) — verified by execution at `9b8a7b4`; provenance not bisected, the ERR-008-021/-022/-023 shot-geometry chain is the likely perturbation source; owned by the realism track, NOT quarantine material.** Every CI run and full local gate on any branch cut from main reports these two failures — treat them as the known floor and do not attribute them to unrelated work.
