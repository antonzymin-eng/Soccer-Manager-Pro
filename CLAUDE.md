...
**15 active** / 41 resolved. *Re-filed August 2, 2026 — eight entries archived (six closed-but-unmoved, plus a duplicated pair); three titles amended to lead with what remains open rather than what has landed. August 4, 2026 — the wiring-backlog entry added at the head; later same day, the football-judgment remediation entry added above it. August 7, 2026 — the id-collision residue entry added (opened by the new `tools/spec-ci` gate).*

- **Duplicate version numbering — 12 files carry a version number on two rows or two `**Version:**` fields; baselined by the new gate, not reconciled. Opened August 7, 2026.** The `tools/spec-ci/check-id-collisions.sh` gate landed today and its first run surfaced these; `tools/spec-ci/known-id-collisions.txt` holds all 17 entries with per-entry notes. **The one worth knowing** is `docs/specs/season-competition-loop/section-3.md`, which carries **two v0.7 rows and two v0.8 rows** — the live residue of the `ERR-030-007`-filed-twice case (back-props at #42's and #32's approvals both took v0.7; #45's and #44's both took v0.8). The step-number half of that collision was found and reconciled as `ERR-030-022` when twenty-three back-props landed together; the version-history rows were never looked at, and until today nothing could have looked. The rest: six pairs in `CHANGELOG-src.md` (same-day supersede entries re-using the prior number rather than advancing it), one each in `match-engine-design.md`, `path-to-playable-roadmap.md`, `decision-tree/section-3-1.md` and `positioning-ai/section-6.md`, plus five files (`collision-system/section-2.md`, `first-touch/section-{2,6,7,8}.md`) carrying two `**Version:**` fields — the same header-drift class recorded in `spec-error-log.md`'s own v1.36 housekeeping note. **Deliberately baselined rather than renumbered:** the root rule is that a duplicate already shipped in approved text stays preserved verbatim as errata, since six approved specs citing a number is a stronger constraint than tidiness — so reconciling these is an owner call, not an implementation-pass call. **Nothing new can join them**: the gate fails on any duplicate not in the baseline, and its header forbids adding a line to silence one your own branch introduced. Cost of leaving them: a reader of #30 §3's version history cannot tell which v0.7 is authoritative without reading both
