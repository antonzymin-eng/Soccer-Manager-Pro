...
- **Deterministic Simulation (#16) — IN REVIEW (Tier 1 Conditional Approval)** — *May 14, 2026.* Lead-developer Tier 1 sign-off granted May 14, 2026; `SPEC_INDEX.md` row 16 flipped `IN PROGRESS → IN REVIEW` atomically. Per §9 v1.1 two-tier approval model, Tier 1 covers self-contained spec content (§1–§7, §9.1, §9.2, §9.5 #1–#3, §9.5 #5) and allows downstream implementation work on `DeterministicRngService`, `SnapshotPayload` writer, and §3.6.1 phase-ownership pipeline to proceed against the spec text as it stands today. **Upstream-spec gate for Tier 2 (§9.4.3) is also cleared as of May 14, 2026:** #9 Fixed64 `IN REVIEW` (May 6), #17 Event System `APPROVED` (May 13, beyond gate), #18 Performance Optimization `IN REVIEW` (May 14), #19 Testing Strategy `IN REVIEW` (May 12). **Tier 2 (`APPROVED`) remains gated on two deliverables (per §9.4.2 v1.3):** (i) `golden-vectors/serialize-canonical-corpus.md` authoring (Tier 2; requires §3.2.4.1 finalization; estimated 1–2 days); (ii) §8.3.1 cross-spec audit-row re-audit against actual upstream spec text (all four rows advanced `deferred dependency → pending re-audit` in §8 v1.1; promotion to `complete` strips the `[TBD-NORMATIVE: pending #N]` suffix and closes ERR-017-001 atomically with the #17 row). **Item (iii) (§9.5 #4 chicken-and-egg) RESOLVED in §9 v1.3 (May 14, 2026)** — criterion split into spec-level sub-condition (gates #16 Tier 2; hand-verifiable today against the cited RFC / paper / §3.2.4.1 prose without `src/`) and implementation-level sub-condition (folds into Stage 0 `FR-DS-009-GATE` per §5.5; NOT a blocker on #16). Residual Tier 2 work for #4(a) and #4(b) **CLEARED May 14, 2026** via byte-exact hand-verification pass — F-HKDF-01 (single-nibble typo in Test Case 1 OKM, `…3400720 0 8d5b…` vs canonical `…3400 7208 d5b…`) filed and fixed in `hkdf-sha256-kat.md` v1.1; SipHash file v1.1 verified clean against Aumasson & Bernstein 2012 Appendix A with no findings. §9.5 #4(c) (`SerializeCanonical` corpus) remains blocked on §3.2.4.1 finalization (subsumed by item (i)). Plus §9.3 review-checklist sign-offs (lead-dev Tier 2, QA-automation, platform-certification) outstanding. **Downstream effects:** unblocks #17 → #18 → #19 promotion path (each is now Tier-2-gated on this spec's `APPROVED`).
