...
- **EntityId no-reuse cross-spec back-propagation pending** — *May 3, 2026.* Deterministic Simulation #16 §3.2.5 declares a normative cross-spec constraint binding APPROVED specs Agent Movement (#2) and Decision Tree (#8) to guarantee EntityId uniqueness for the lifetime of a match (no despawned-ID reuse). Filed as `ERR-016-002` in `docs/tracking/spec-error-log.md`. Reciprocal `XC-002-NNN` and `XC-008-NNN` references not yet added to #2 and #8 — constraint is currently floating. Touches APPROVED specs; requires lead-developer authorization for the minor revision. See also `docs/specs/deterministic-sim/third-pass-fix-log.md` finding M-F.
- **Deterministic Simulation #16 third-pass critique resolved (specs only)** — *May 3, 2026.* Third adversarial pass (4 H, 6 M, 6 L, 1 cross-cutting) addressed in §1, §3, §4, §5, §9 at v0.9. Full per-finding fix log: `docs/specs/deterministic-sim/third-pass-fix-log.md`. Status remains `IN PROGRESS`; final approval still gated on §8.3 deferred dependencies (#9, #17, #18, #19) reaching `IN REVIEW`. Three golden-vector files (`hkdf-sha256-kat.md`, `siphash-2-4-kat.md`, `serialize-canonical-corpus.md`) named in §9.5 #4 acceptance criterion are not yet authored.
