...
- **Deterministic Simulation (#16) — IN REVIEW (Tier 1 Conditional Approval)** — *May 14, 2026.* Lead-developer Tier 1 sign-off granted May 14, 2026; `SPEC_INDEX.md` row 16 flipped `IN PROGRESS → IN REVIEW` atomically. Per §9 v1.1 two-tier approval model, Tier 1 covers self-contained spec content (§1–§7, §9.1, §9.2, §9.5 #1–#3, §9.5 #5) and allows downstream implementation work on `DeterministicRngService`, `SnapshotPayload` writer, and §3.6.1 phase-ownership pipeline to proceed against the spec text as it stands today. **Upstream-spec gate for Tier 2 (§9.4.3) is also cleared as of May 14, 2026:** #9 Fixed64 `IN REVIEW` (May 6), #17 Event System `APPROVED` (May 13, beyond gate), #18 Performance Optimization `IN REVIEW` (May 14), #19 Testing Strategy `IN REVIEW` (May 12). **§9.5 #4 (all three spec-level sub-conditions) CLEARED May 14, 2026** — (a) HKDF-SHA256 KAT (`hkdf-sha256-kat.md` v1.1, F-HKDF-01 filed & fixed: single-nibble typo in Test Case 1 OKM, `…3400720 0 8d5b…` vs canonical `…3400 7208 d5b…`); (b) SipHash-2-4 KAT (`siphash-2-4-kat.md` v1.1, no findings against Aumasson & Bernstein 2012 Appendix A); (c) `SerializeCanonical` corpus (`serialize-canonical-corpus.md` v1.0, 45 entries covering every §3.2.4.1 type kind plus all five domain-tagged preimages, generated by an embedded reproducer Python script per CLAUDE.md "no fabricated values"; D-01 cross-checks against the §3.2.4.1 worked-byte-example and matches). §9.5 #4 checkbox now ticked; §9.5 implementation-readiness-gate checkbox also ticked at the spec level (§9 v1.5). **Tier 2 (`APPROVED`) remaining gates (per §9.4.2):** (i) §8.3.1 cross-spec audit-row re-audit against actual upstream spec text (all four rows advanced `deferred dependency → pending re-audit` in §8 v1.1; promotion to `complete` strips the `[TBD-NORMATIVE: pending #N]` suffix and closes ERR-017-001 atomically with the #17 row); (ii) §9.3 review-checklist sign-offs (lead-dev Tier 2, QA-automation, platform-certification). **Downstream effects:** unblocks #17 → #18 → #19 promotion path (each is now Tier-2-gated on this spec's `APPROVED`).
