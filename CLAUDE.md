...
> **Last Updated:** May 17, 2026 (Pressing AI #13 APPROVED; Defensive AI #14 advanced `NOT STARTED → IN PROGRESS` via section files v0.1 (all 11 files); ERR-014-001..004 pre-filed. Counts: 16 APPROVED, 2 IN REVIEW, 1 IN PROGRESS, 1 NOT STARTED.)
...
- **Defensive AI (#14) — IN PROGRESS** — *May 17, 2026.* All 11 section files v0.1 drafted from `outline-detailed.md` v1.0 (PR #81, merged May 17, 2026). KD-5 Option B resolved: `TacticalContext.MarkDirective?` nullable field via ERR-014-001 (mirrors #13's `PressDirective?` precedent). Q2 resolved (#3 §3.3 agent-agent collision surface); Q3 resolved (attribute normalisation `(attr−1)/19.0f` for [1–20] → [0,1]); Q4 resolved (GK zone as `distToOwnGoal` scalar, GK_EXPECTED_ZONE_MAX_X = 15.0 m); Q5 confirmed (`DOMAIN_TAG_DEFENSIVE_AI = 0x1A [CROSS-PENDING]` per ERR-012-001 Phase B/C block). 37 FRs; 27 constants (22 `[GT]` all promoted from `[EST]` with Appendix A derivations; 4 `[CROSS]`/`[CROSS-PENDING]`). ≥85 tests; ≤0.12 ms per-tick budget. **Outstanding (gates `IN PROGRESS → IN REVIEW`):** (i) PASS-1 adversarial review (`defensive-ai/adversarial-review-section-files-v1.md`) + v0.2 fix pass; (ii) ERR-014-001 `TacticalContext.MarkDirective?` amendment in #8 §2.2.6 ratified by lead developer; (iii) ERR-014-004 `DOMAIN_TAG_DEFENSIVE_AI = 0x1A` allocated in #16 §3.4 v1.0.4 patch; (iv) lead-developer R-01..R-05 sign-off. ERR-014-002 (MARK_ASSIGNED channel) and ERR-014-003 (LINE_STEPPED channel) are Stage 1 deferred — do not block `IN REVIEW`.
