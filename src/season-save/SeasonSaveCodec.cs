...
// Created:  2026-07-22
// Modified: 2026-08-06 (#29/#41 T1: the frame gains the training and medical sub-blobs;
//           SEASON_SAVE_FORMAT_VERSION 2 -> 3 per FR-TR-018 / FR-MD-017)
// Author:   —
...
namespace TacticalDirector.SeasonSave
{
    ...
    /// <c>matchPresent</c> flag and five length-prefixed opaque sub-blobs, in order: the living-world
    /// composite (always), the season state (always), the #29 training block (always), the #41 medical
    /// block (always), and the match save (only when a match was in progress). Serialized through
    ...
    public static class SeasonSaveCodec
    {
        ...
        /// <summary>
        /// Decodes a season blob produced by <see cref="Encode"/> into its five opaque sub-blobs.
        /// Fail-loud (throws) on: a null blob; a <see cref="SeasonSaveConstants.SEASON_SAVE_FORMAT_VERSION"/>
        /// mismatch (KD-4, no Stage-0 migration — a v1 pre-#30-T1 or v2 pre-#29/#41-T1 file is rejected
        /// here); a <c>matchPresent</c> flag that is neither 0 nor 1; a length prefix (world, season,
        /// training, medical, or match) that would read past the blob; or any trailing bytes after the
        ...
        /// <see cref="TacticalDirector.InjuriesMedical.MedicalSaveCodec.Decode"/> / the match decode
        /// path.
        /// </summary>
        public static SeasonSaveBlobs Decode(byte[] blob)
        {
            ...
            return new SeasonSaveBlobs(worldBlob, seasonBlob, trainingBlob, medicalBlob, matchBlob);
        }
