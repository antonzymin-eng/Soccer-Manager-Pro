    ...
    public sealed class MatchClientConstantsTests
    {
        ...
        public void RequireAtLeast_RefusesBelowTheMinimum_AndAnythingNonFinite()
        {
            ...
            foreach (float bad in new[] { 0.5f, 0f, -1f, float.NaN })
            {
                Assert.Throws<InvalidOperationException>(
                    () => MatchClientConstants.RequireAtLeast(bad, 1f, "BallMaxHeightScale"), "value " + bad);
            }
        }
        ...
        public void TheMessageNamesTheConfigKey_SoABootFailureIsActionable()
        {
            ...
            var ex = Assert.Throws<InvalidOperationException>(
                () => MatchClientConstants.RequireAtLeast(0.5f, 1f, "BallMaxHeightScale"));

            StringAssert.Contains("BallMaxHeightScale", ex.Message);
        ...
        {
            Assert.GreaterOrEqual(MatchClientConstants.BallMaxHeightScale, 1f);
            Assert.Greater(MatchClientConstants.PossessionRingRadiusM, MatchClientConstants.AgentMarkerRadiusM);
            Assert.Greater(MatchClientConstants.BallHeightScalePerMetre, 0f,
                "a non-positive ramp makes the height cue and its documented saturation point meaningless");
        }
    }
}
