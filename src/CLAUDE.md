...
The authoritative layer taxonomy is Spec #20 §3.5.2. The ten tiers and their members
are reproduced here verbatim — do not infer tier membership from folder order or spec
number. All 35 production assembly folders in `src/` are placed.

| Tier | Assemblies |
|---|---|
| 0 **Foundation** | project-constants, deterministic-sim, event-system |
| 1 **Physics** | ball-physics, agent-movement, collision-system, first-touch, pass-mechanics, shot-mechanics, heading-mechanics, goalkeeper-mechanics |
| 2 **Configuration** | tactical-instructions |
| 3 **Mechanics** | positioning-ai, pressing-ai, defensive-ai, attacking-ai |
| 4 **AI** | decision-tree, perception-system |
| 5 **Data** | player-database |
| 6 **Composition** | match-engine |
| 7 **Management** | living-world, player-progression, training-system, injuries-medical, discipline, season-save |
| 8 **Presentation** | match-viewer, match-analytics |
| 9 **Client** | match-client-core, ui-framework, client-app, match-client-unity, match-client-web |

The tier 0 assemblies are the cross-cutting foundations: referenceable by everything,
referencing nothing but each other.

**A tier is a ceiling, not a licence.** An individual spec may forbid a reference the
tier order permits — `discipline` (#44) sits in Management but must not reference
`match-engine` or `season-save`; the composition root mediates. Where a spec is
stricter than the tier, the spec wins.

**Test assemblies are not members of the order** (`src/*/[Tt]ests/`). A test assembly
legitimately references upward — `event-system.Tests` references `decision-tree` — so
...
`project-constants` is tier 0, not infrastructure: it is a real assembly that every
tier reads, and the order places it. Spec #20 is a style guide with no assembly — there
is no `src/code-standards/`, and it must not be listed as one.
...
**Intra-tier references are permitted; intra-tier cycles are not** (FR-CS-046a). An
assembly MAY reference another in the same tier — `pressing-ai → positioning-ai` is the
standing example — but the reference graph as a whole MUST remain acyclic. That is not
merely convention: Unity rejects circular `.asmdef` references, and `tools/dotnet-ci`
emits one `<ProjectReference>` per `.asmdef` reference, so a cycle fails the Linux
compile gate too. The rule is written down because the enforcement is a build error
whose message explains what broke but not why the constraint exists.
...
**Migration status** *(as of 2026-06-30 — see the SNAPSHOT correction below before relying on any
figure in this paragraph)***:** 509 of the 520 `[GT]` declarations the root `CLAUDE.md` OPEN ISSUES entry
...
- `decision-tree` / `living-world` / `positioning-ai` / `pressing-ai` carry `[GT]` constants that
  ...
  > **CORRECTION, 2026-08-10 — the "confirmed by grep" claim above is FALSE for `pressing-ai`.**
  > `src/pressing-ai/PressingAIConstants.cs` carries **28** `// TODO: replace with config loader`
  > markers — the largest concentration in the tree — so it did not "never pick up the marker"; it
  > is simply unmigrated. Re-measured 2026-08-10; the other three named catalogues do have 0 markers.
  > The `TacticalInstructionsConstants.cs` bullet above also understates its own case: that file has
  > **zero** `Config.GetX` calls and 12 markers, so more than the 11 array-valued constants are
  > unmigrated there.

**Migration status is a SNAPSHOT, not a standing rule (added 2026-08-10).** The 17-catalogue list
below is the state on **2026-06-30**. It is not a description of the tree today, and it does not
carve out anything created since. **A `<Spec>Constants.cs` created after 2026-06-30 is unmigrated by
default** — it was never in the 520 denominator, so its absence from the list means nothing. Check
the catalogue, not this section. (`PlayerProgressionConstants.cs` is `Created: 2026-07-24`, which is
the whole reason it is missing here — it was never *excluded*.)

Measured tree-wide 2026-08-10 — **71** live `TODO: replace with config loader` markers across
**9** catalogues (`pressing-ai` 28, `tactical-instructions` 12, `player-progression` 11,
`player-database` 7, `tactical-presets` 6, `training-system` 2, `injuries-medical` 2,
`defensive-ai` 2, `first-touch` 1), and **10** catalogues with zero `Config.GetX` calls (the four
...
composition root calls `Bind`, which none does, so none of this is a live defect):
- **`POINT_COST` is mis-tagged.** It is `[GT]`, but its initializer *is* `DAYS_PER_YEAR` (a
  `[FIXED]` const) and `PlayerProgressionConstantsTests` asserts the equality as a MUST. A constant
  a test forbids from being set independently is `[DERIVED]` (root `CLAUDE.md` tag table, FR-CS-021),
  which removes it from the migration set entirely. `docs/specs/player-progression-lifecycle/appendices.md`
  carries the same wrong tag, so this is a back-prop, not a code-only fix.
- **Migrating `POINT_COST` would make `POINT_COST = 0` in a config file hang the process** on the
  first `AdvanceDay` of every career: `GrowthProjection`'s drain loop is `while (cursor <= -POINT_COST)`
  with `cursor += POINT_COST` and a `void DrainOnePoint` — no exit, and every seeded player starts at
  cursor 0. (The spend loop above it is safe; it breaks via `TrySpendOnePoint`.) Same class as
  `RecoveryDaysPerTickBase = 0` (AR pass 12), but unrecoverable rather than silent.
- **`PA_MIN` / `ABILITY_MAX` are part of a save-file acceptance predicate.** `ProgressionSaveCodec`
  refuses a decoded `PotentialAbility` outside `[PA_MIN, ABILITY_MAX]` as "corrupt save". Making
  either config-settable lets a config edit retroactively invalidate saves that were valid when
  written — a value outside the format deciding the format's validity (`ERR-029-005`/`ERR-041-009`
  class). Needs an owner decision, not a `GetInt` call.

Any real migration must also add **fail-loud guards at the consuming sites**, not rely on the
existing catalogue locks: those locks run config-unbound, so they see the fallbacks forever while a
shipped config violates them — `ERR-041-003` and AR pass 10's severity-split finding, twice-filed.
And `NEW_GAME_PA_HEADROOM` must be left alone: its marker is `TODO: replace with the #47 authored
value`, not the loader TODO — a config key there is a lever #47 is contracted to delete.
