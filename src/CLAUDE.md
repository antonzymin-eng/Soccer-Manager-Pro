...
| `src/tactical-instructions/` assembly (Spec #21) | Spec #21 (Tactical Instructions) is APPROVED (June 20, 2026), but **no source exists yet** — T0 scaffolding (the `TacticalDirector.TacticalInstructions` enums + `TeamTactic`/`PlayerTactic`/`PlayerInstructions` structs + identity factories) is the first landable slice (behaviour-neutral). Seams into #8/#11–#15 + the consumer-side `TacticTranslation` maps land at Stage 1 (#21 §7.2 T2–T3); runtime activation gated on the `[GT]` config-loader + match-engine Phase C/D. Add the folder to the tree above when T0 lands. |
