...
│   ├── ScenarioIndex.cs               ← sealed class + ScenarioIndexEntry: immutable in-memory root manifest; duplicate paths rejected; runner refuses unindexed scenarios (§3.3.6 / FR-TS-028)
│   ├── ScenarioRunner.cs              ← sealed class: §3.3.3 single entry point Run(manifestPath, seed) — index resolution + A.1 / FR-TS-070 load-time validation → IScenario.Run
│   └── Tests/
