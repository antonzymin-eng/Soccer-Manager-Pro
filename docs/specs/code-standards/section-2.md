...
| FR-CS-017 | Every constant declaration **MUST** carry its root `CLAUDE.md` tag (`[GT]`, `[EST]`, `[FIXED]`, `[DERIVED]`, `[CROSS]`, or `[CROSS-PENDING]`) in the XML doc comment immediately preceding the declaration. | MUST | §3.2.2; root `CLAUDE.md` — "Constant Tags" | §3.2.2 |
