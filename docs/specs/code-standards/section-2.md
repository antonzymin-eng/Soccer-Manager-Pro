...
- **ID** — unique identifier; renumbering after publication is forbidden (§2.2.10).
...
| FR-CS-008 | Code **MUST** target **C# 9.0**, the language version fixed by the certified Unity 6000.4.9f1 host recorded in `docs/tracking/certification-platform.md` (`ERR-020-008`). A supplemental compiler or synthetic project **MUST NOT** widen the permitted language surface beyond that governing Unity compiler. | MUST | §3.1.3; `docs/tracking/certification-platform.md` | §3.1.3 |
...
| 1.5 | August 18, 2026 | Claude Code | **Adversarial-review round-7 findings M1 + M6.** M1: `ERR-020-006` and `ERR-020-007` were cited nowhere in the spec they patch (only in `spec-error-log.md`) — the v1.4 row's "round-6 H6/H7" text is now also cross-cited by ERR id at its fix sites: FR-CS-017's row (§2.2.2) and FR-CS-022's row (§2.2.2), plus §3.2.1/§3.2.3/§4.2 in the sibling section files. M6: the v1.4 row's "218 … `grep -rn 'CROSS-PENDING' docs/specs/ \| wc -l`" clause offered a command as live proof of a figure it no longer returns (245 today, since every citation of the tag — including the v1.4 row's own text — keeps raising it); annotated in place per the annotate-don't-rewrite convention rather than rewritten, with 218 re-derived against the pre-fix commit (`git grep -c 'CROSS-PENDING' 9b841d1^ -- docs/specs`) instead. No FR text, count, or conformance level changed by this row. | — |
...
| 1.9 | September 14, 2026 | Codex | **`ERR-020-008` — FR-CS-008 activation drift corrected.** The July 19 certification of Unity 6000.4.9f1 satisfied the FR's documented activation condition, but this catalogue still called the rule inactive. FR-CS-008 is now active and fixes the allowed language surface at C# 9.0; supplemental/synthetic compilers may not widen it. This erratum is effective immediately against the approved Code Standards baseline and is independent of the pending A3.1a/A3.1b architecture amendment. | — |
