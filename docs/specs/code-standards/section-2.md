...
| FR-CS-017 | Every constant declaration **MUST** carry its root `CLAUDE.md` tag (`[GT]`, `[EST]`, `[FIXED]`, `[DERIVED]`, or `[CROSS]`) in the XML doc comment immediately preceding the declaration. | MUST | §3.2.2; root `CLAUDE.md` — "Constant Tags" | §3.2.2 |
...
| FR-CS-046b | An assembly seated in the ten-tier order **MUST NOT** reference an out-of-band **Infrastructure** assembly (`performance-optimization`, `testing-strategy`) at runtime; an Infrastructure assembly **MUST NOT** reference any assembly above tier 0 (Foundation). | MUST | §3.5.2 | §3.5.2 |
