...
```
T2a current:  ClubFinances (#40) ──▶ DeterministicSim (#16)     [canonical save framing]
...
#27 dependency; it does not change that ownership direction.
...
already changed. The byte layout below is unchanged by T1b or T2a — the frame nests this block opaquely and
T2a only supplies initial records, so `FINANCE_SAVE_FORMAT_VERSION` stays 1.
