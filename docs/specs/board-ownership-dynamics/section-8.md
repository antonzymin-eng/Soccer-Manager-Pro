...
| **ERR-045-001** | `deterministic-sim/section-3.md` §3.4 | `_RESERVED_0x2D_` placeholder + `SubsystemOrdinals.BoardOwnership = 95`, **RESERVED not promoted** — #45's minimal tier is draw-free, and a named tag over a zero-draw stream is the phantom surface FR-LW-031 forbids (the `_RESERVED_0x29_` #40 / `_RESERVED_0x21_` #29 precedent). | Namespace reservation; no `DETERMINISM_DIGEST_VERSION` bump |
