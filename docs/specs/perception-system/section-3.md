...
governing perception output production. This section is the implementation authority for
...
`FilteredView` and `PerceptionDiagnostics` struct definitions. All constants are audit-tagged.
All formulas include worked examples with numerical verification.
...
| 1.3 | April 11, 2026 | **Architectural rework — separation of filter output from filter metadata.** (1) §3.7 replaced: `PerceptionSnapshot` split into `FilteredView` (9 fields — consumer output) and `PerceptionDiagnostics` (7 fields — filter metadata). (2) `FilteredView` contains only pure data: identity, ball state, visible agents (forward arc), blind-side agents, forced-refresh flag. (3) `PerceptionDiagnostics` contains filter internals: EffectiveFoVAngle, PressureScalar, BlindSideWindowActive, BlindSideWindowExpiry, ShoulderCheckAnimData. NOT delivered to Decision Tree. (4) `PerceivedAgent` reduced to 4 fields: `IsInBlindSide` removed (separate array makes it redundant), `LatencyCounterAtConfirmation` moved to editor-only debug struct. (5) `BlindSideWindowExpiry` type confirmed as `float` (match time in seconds). (6) `ForcedRefreshThisTick` added to `FilteredView` — resolves XC-4.5-01. (7) Summary (line 1148) corrected from "12 fields" to match actual struct counts. (8) Constants table summary (line 1157) corrected: 17 constants total (12 [GT], 2 [DERIVED], 3 [CROSS]). |
...
Step 7: BuildFilteredView() + BuildDiagnostics()
        Assemble FilteredView struct (§3.7.1) — consumer output for Decision Tree.
        Assemble PerceptionDiagnostics struct (§3.7.2) — filter metadata (NOT for DT).
        Copy FilteredView by value to Decision Tree intake.
...
```csharp
// Persistent state across heartbeats — lives in PerceptionSystem, not FilteredView
...
// It is stored in PerceptionDiagnostics for debug/telemetry use (§3.7.2)
// It is NOT delivered to the Decision Tree, and NOT used to degrade ball visibility,
// recognition latency, or shoulder check timing
```
