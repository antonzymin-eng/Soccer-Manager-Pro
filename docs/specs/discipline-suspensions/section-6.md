...
**Last Updated:** August 16, 2026, latest of all (v0.5 — reviewed-findings pass, finding L10: §6.2's two
line-number citations (`AvailabilityComposition.cs:162`, `DisciplineRules.cs:265`) were both already
wrong — line numbers drift across every later edit to those files and are not stable citations, the
class this project's own `L6` corrections elsewhere in this spec exist to fix. Both replaced with
member-name citations (`MarkSuspended`'s `Availability.MarkSuspended` call site inside
`AvailabilityComposition.Compose`; `DisciplineRules.OnClubFixturePlayed`'s descending walk). The
`OnClubFixturePlayed` bullet also gained the O(|clubPlayerIds|) `IsOnClubRoster` roster scan
`ERR-044-014` added, which sits ahead of the already-priced O(|fieldedPlayerIds|) `WasFielded` scan and
had no bullet at all. The v0.3/v0.4 entries below and the matching version-history rows quote the two
stale citations as part of describing what those passes landed; annotated in place rather than
rewritten, per this file's own convention.)
**Last Updated (prior):** August 15, 2026, later still (v0.4 — reviewed-findings pass, extending `ERR-044-011`:
...
- **`MarkSuspended`** — O(squad size): one pass writing the suspension mask. This is #44's actual
  production path, called from the `Availability.MarkSuspended` call site inside
  `AvailabilityComposition.Compose`, once per resolved squad — **twice per fixture, both clubs, on
  both resolution paths** (FR-DC-010), not once per managed-fixture selection.
...
- **`OnClubFixturePlayed`** — **O(total discipline entries across every club, not just the calling
  one)**: the descending walk in `DisciplineRules.OnClubFixturePlayed` visits every row of
  `DisciplineState` once. For each row still carrying `BanMatchesRemaining > 0` — across the whole
  league, membership not yet having been decided — it runs an O(|clubPlayerIds|) linear scan
  (`IsOnClubRoster`, `ERR-044-014`) to decide roster membership by presence rather than by id
  derivation; only for a row that is both club-owned and still banned (typically 0–2) does it then
  run a further O(|fieldedPlayerIds|) ≈ O(11) linear scan of the fielded eleven to check the
  ERR-044-003 stage 1 exemption (`WasFielded`). The two scans share one helper (`ContainsId`,
  `ERR-044-011`).
...
| 0.3 | 2026-08-15 | — | **`ERR-044-011`**, reviewed-findings pass: §6.2's first bullet renamed `OnTapRecord` → `ObserveTick` (verified against `src/discipline/CardLedgerFold.cs`, which has never had a member named `OnTapRecord` — the pseudocode name §3.1 used before M27's rewrite) and restated to describe buffering rather than direct application; `OnClubFixturePlayed`'s complexity corrected from "O(active-ban entries for the club), typically 0–2" to O(total entries across every club) — `DisciplineRules.cs`'s descending `for` loop walks the whole state once per call — with the O(11) `WasFielded` scan per active ban named explicitly. See `spec-error-log.md` `ERR-044-011`. *(L10, August 16, 2026: this row's `DisciplineRules.cs:265` line citation was already drifting and is not a stable citation — replaced with a member-name citation at §6.2 live text; see v0.5.)* |
| 0.4 | 2026-08-15 | — | **Reviewed-findings pass, extending `ERR-044-011`.** §6.2's `FilterAvailable` bullet priced a method with zero production call sites (`grep -rn "FilterAvailable" src/` outside `src/discipline/` returns nothing) at a scope — "once per managed-fixture selection" — ERR-044-002 (August 13, 2026) had already withdrawn: FR-DC-010 has covered both clubs of every fixture on both resolution paths since that fix. The real production surface, `Availability.MarkSuspended`, called at `src/season-save/AvailabilityComposition.cs:162` once per resolved squad (twice per fixture, both resolution paths — `SeasonLoop.SelectAvailable`), had no bullet at all. New `MarkSuspended` bullet added at the correct cost and call count; `FilterAvailable`'s bullet kept but relabelled non-production, matching §2.2's `Availability` block. No new ERR id — extends `ERR-044-011`. See `spec-error-log.md` `ERR-044-011`. *(L10, August 16, 2026: this row's `AvailabilityComposition.cs:162` line citation was already drifting and is not a stable citation — replaced with a member-name citation at §6.2 live text; see v0.5.)* |
| 0.5 | 2026-08-16, latest of all | — | **Reviewed-findings pass, finding L10.** §6.2's two line-number citations (`AvailabilityComposition.cs:162`, `DisciplineRules.cs:265`) replaced with member-name citations — `MarkSuspended`'s bullet now cites the `Availability.MarkSuspended` call site inside `AvailabilityComposition.Compose`; `OnClubFixturePlayed`'s bullet now cites `DisciplineRules.OnClubFixturePlayed`'s descending walk by name. The `OnClubFixturePlayed` bullet also gained the O(|clubPlayerIds|) `IsOnClubRoster` roster scan `ERR-044-014` added (§2.3 F2's club-membership walk), which runs on every league-wide row still carrying an active ban, ahead of the already-priced O(|fieldedPlayerIds|) `WasFielded` scan that runs only for a row both club-owned and still banned; both share `ContainsId` (`ERR-044-011`). The v0.3/v0.4 rows above, which quote the two stale citations while describing what those passes landed, are annotated in place rather than rewritten. |
#endregion
