...
- **Stage 0+1:** Unity Profiler + Superluminal / Tracy (or equivalent;
  selection criteria parallel Spec #19 §6.1 (`TBD-NORMATIVE`) — must
  support deterministic re-play, must emit per-frame breakdown, must
  support headless / batch-mode capture for CI).
...
exempted via the `[HotPathAllocExempt]` governance identifier, declared
here in Spec #18 §3.7.5. Spec #20 §3 mandates zero-allocation on hot
paths (the policy authority) but does not declare the
`[HotPathAllocExempt]` attribute name. The concrete C# attribute
definition is a Stage 0+1 deliverable per KD-5; coordinate with the
Spec #20 section author at first `src/` commit to ensure the exemption
mechanism aligns with the #20 §3 anti-pattern exception list.
