...
| FR-PO-053 | One-shot allocations exempt via the `[HotPathAllocExempt]` attribute (declared in Spec #18 §3.7.5; first-implementation site at first `src/` commit) MUST cite a rationale and require lead-developer sign-off. | MUST | KD-10 | §5.6 | Stage 0+1 |
