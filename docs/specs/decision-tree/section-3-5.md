...
//   - Limited to the §3.1.5.3 look-ahead distance (DRIBBLE_LOOKAHEAD_M [GT] = 5.0m;
//     §3.1.5.3 is the authority — the 8.0m "DRIBBLE_MAX_TARGET_DISTANCE" formerly
//     cited here was never declared in any constant table)
...
//   no-move fallback. Its low BaseUtility (0.28 [GT] — §3.2.5.1 is the
//   authority; the 0.25 cited here pre-dated the §3.2.5.1 raise) ensures
//   it wins only when all other scored candidates are lower.
...
**Note on axis convention (corrected June 11, 2026 audit):** Ball Physics #1 §1.2 is
Z-UP — X is goal-to-goal, Y is touchline-to-touchline, Z is height — so the ground
plane is XY, NOT XZ; the previous claim that "Ball Physics uses XZ" was wrong. Agent
Movement #2 and the implemented `MovementCommand` operate on Vector2 XY pitch
coordinates directly, so no Vector2→Vector3 conversion exists in the implemented
dispatch path; the snippet above retains the historical pseudocode shape only.
...
|---|---|---|---|
| FM-DT-14 | `AgentAction.ActionType` does not match any dispatch branch | Exhaustive switch default | Log error with `AgentId` and `ActionType`. Produce HOLD command to keep agent safe. This should not occur if §3.1 option generation is correct. (Renumbered from FM-DT-09, which §3.1.1.3 had already allocated to the possession-uncertainty warning — ERR-008-007, June 11, 2026 audit.) |
