...
| XC-046-002 | #30 §3.4 `AdvanceAndPlayNextRound` / `EmitMatchOutcome` | `Table.ApplyResult(result)` → `EmitMatchOutcome(result)` → `f.Played := true`. The `result` in hand at that instant carries the scoreline — the **only** moment an accurate match item can be captured (ERR-030-025). |
