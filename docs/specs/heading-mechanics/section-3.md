...
| `HEADING_CONTACT_BUFFER_CAPACITY` | `[GT]` | count | [22, 32] | 22 | Pre-sized per-frame `ICollisionEventConsumer` capacity. ERR-010-004: W3 can publish one `AGENT_BALL` observation for every Stage-0 agent in one frame, so effective capacity MUST be at least `MAX_AGENTS` (22 today); runtime clamps a lower configured value up to `MAX_AGENTS`. Allocated once; no per-tick heap allocation. |
...
| 0.9     | September 23, 2026 | — | **ERR-010-004 / PR #439 Codex closure.** W3's 22-agent same-frame fan-out made the 16-event buffer structurally undersized. Candidate/range now start at 22 and runtime capacity is floored to `MAX_AGENTS`; overflow remains fail-closed beyond effective capacity. No gameplay formula, RNG, schema, or draw-order change. |
