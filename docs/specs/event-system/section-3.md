...
| `ERR_EVT_UNREGISTERED_ORDINAL` | `0x1706` | `[GT]` | Design-fixed sub-class (below). Publish or subscribe before the owning spec's `EventBusRegistrar.Initialize()` ran — `EventOrdinalCache<T>.Ordinal` is still 0 (the CLR default). FR-EVT-020; AR-2 M-1. In `EventSystemConstants.cs` since v1.1 (May 30, 2026) with no catalogue row until now (ERR-017-006, extending ERR-017-005, reviewed-findings pass, 2026-08-15) — ERR-017-004's exact defect recurring on a code carrying its own name in this section's design-fixed-`[GT]` note below. |
| `ERR_EVT_ORDINAL_COLLISION` | `0x1707` | `[GT]` | Design-fixed sub-class (below). Two distinct `IEventC` types mapped to the same ordinal via erroneous `RegisterExternalRow` calls; thrown by `CosmeticChannel.Subscribe` when the dispatcher already bound at the ordinal is typed for a different event. AR-4 L. In `EventSystemConstants.cs` since v1.2 (May 31, 2026) with no catalogue row until now (ERR-017-006). |
...
| 1.0.5   | August 15, 2026, later | Claude Code | Reviewed-findings pass, L3. New §3.10 row `FOUL_ORDINAL_NONE = 0xFFFF`, `[FIXED]` — `CardIssuedEvent.FoulOrdinal`'s "no associated foul" sentinel, which had no catalogue home anywhere in this spec (it existed only as prose in `CardIssuedEvent.cs`) despite being ERR-017-004's exact defect shape on the sibling payload field of the same event. Declared in `src/event-system/EventSystemConstants.cs` beside the `CARD_KIND_*` rows and mirrored `[CROSS]` in `src/match-engine/MatchEngineConstants.cs`; the one production call site (`MatchEngine.cs`'s `foulOrdinal: 0xFFFF` literal) is outside this pass's ownership and still needs repointing at the new mirror. **DISCHARGED August 16, 2026 (reviewed-findings pass, L7):** the repointing landed on this branch — `MatchEngine.cs` now reads `MatchEngineConstants.FoulOrdinalNone` at its one production call site (verified by grep, `MatchEngine.cs` v1.x header + line 5286), so no repointing remains outstanding. |
| 1.0.6   | August 15, 2026, later still | Claude Code | ERR-017-006 (extends ERR-017-005; reviewed-findings pass, M2). §3.10's preamble claims every constant this spec declares appears in the catalogue with its source tag — false for two of the four codes ERR-017-005's own design-fixed-`[GT]` note names: `ErrEvtUnregisteredOrdinal` (`0x1706`, in code since v1.1) and `ErrEvtOrdinalCollision` (`0x1707`, since v1.2) had no catalogue row at all. Two rows added, both `[GT]` under the existing design-fixed sub-class, citing `EventSystemConstants.cs`'s own doc comments (FR-EVT-020 / AR-2 M-1; `CosmeticChannel.Subscribe` / AR-4 L). No code change; no tag or value moved. |
| 1.0.7   | August 16, 2026 | Claude Code | Reviewed-findings pass, L4 + L7 (one row covers both — both touch only this table). L4: rows 1.0.5 and 1.0.6 were appended out of chronological order (1.0.6, dated "later still", sat above 1.0.5, dated "later") — swapped so the table reads chronologically; rows are appended, never reordered, so this is a positional fix only, no row's own text altered by the swap. L7: row 1.0.5's "still needs repointing" claim annotated in place as DISCHARGED — the named call site (`MatchEngine.cs`'s `foulOrdinal: 0xFFFF` literal) was repointed to `MatchEngineConstants.FoulOrdinalNone` on this branch. No tag, value, or normative text changed. |
