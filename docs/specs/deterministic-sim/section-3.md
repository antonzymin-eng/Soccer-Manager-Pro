...
| `_RESERVED_0x20_` | `0x20` | [FIXED] | **Reserved, not yet allocated** — held for Player Progression & Lifecycle #28 per the management-layer roadmap §6 off-pitch determinism block (`SubsystemOrdinals` 82). Distinct from the permanently-orphaned `0x18`/`0x1C`: this value WILL be assigned when #28 is authored and promoted; until then it MUST NOT be reused by any other subsystem. The gap exists because Season Loop #30 (Wave 1) reached the catalogue before #28/#29 (Wave 2), per the roadmap's contiguous-block reservation (season-competition-loop-design.md §5). No code const yet. |
| `_RESERVED_0x21_` | `0x21` | [FIXED] | **Reserved, not yet allocated** — held for Training System #29 per roadmap §6 (`SubsystemOrdinals` 83). Same reserved-pending-promotion status as `_RESERVED_0x20_`; MUST NOT be reused; assigned when #29 is authored and promoted. No code const yet. |
