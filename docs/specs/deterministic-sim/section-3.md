...
| `_RESERVED_0x2B_` | `0x2B` | [FIXED] | **Reserved — held for Youth Academy & Intake #42 per roadmap §6 (`SubsystemOrdinals.YouthAcademy = 93`); MUST NOT be reused.** #42 was authored + APPROVED July 24, 2026; its minimal tier **does** have a genuine draw site (the `youth.intake` cohort generation, which drives #28's `RegenGenerator` from #42's own stream), but that site does not exist until **#42 T2**, so the row is **reserved, not promoted** — registering a stream with zero draw sites now would be the phantom-surface class FR-LW-031 forbids (`world.arcs` precedent). Promotes to `DOMAIN_TAG_YOUTH_ACADEMY = 0x2B` at #42 T2's first intake. **Filed retroactively July 25, 2026 under ERR-045-001:** #42's approval correctly deferred the *promotion* but omitted this A-04 *placeholder* (the `_RESERVED_0x21_` #29 / `_RESERVED_0x29_` #40 pattern — a deferred promotion still requires a placeholder), leaving an unmarked gap; closed here alongside `0x2C`/`0x2D` exactly as v1.0.13 closed `0x23`–`0x28`. No code const. |
| `_RESERVED_0x2C_` | `0x2C` | [FIXED] | **Reserved — held for Competition Structure #43 per roadmap §6 (`SubsystemOrdinals` 94); MUST NOT be reused.** Placeholder pending #43's promotion (A-04 gap rule; filed under ERR-045-001 so the block stays contiguous through `0x2D`). Competition/cup **draws** are a documented stochastic site (roadmap §3 #43), so this is expected to promote at #43's first draw. No code const. |
| `_RESERVED_0x2D_` | `0x2D` | [FIXED] | **Reserved — held for Board & Ownership Dynamics #45 per roadmap §6 (`SubsystemOrdinals.BoardOwnership = 95`); MUST NOT be reused.** #45 was authored + APPROVED July 25, 2026 and its **minimal tier is draw-free** — board confidence is a deterministic integer per-mille drift toward a target assembled from #30's committed "on track?" projection — so **it registers NO RNG stream and this row is deliberately NOT promoted** (ERR-045-001; the #29 `_RESERVED_0x21_` / #40 `_RESERVED_0x29_` precedent). The reservation stands for #45's **deep-tier takeover events** (the genuine first draw site), which promote it at **#45 T3**. When it promotes it registers **one** subsystem-wide stream (siteId `board.takeover`, a fixed entity sentinel), **not** one per club, with the club folded into a position-independent keyed action ordinal over `(clubId, worldDay, purpose)` at a fixed radix — so no cursor is serialized and #45 never contributes to the `MaxRngStreams` bound (#45 FR-BD-021/022). No code const. |
