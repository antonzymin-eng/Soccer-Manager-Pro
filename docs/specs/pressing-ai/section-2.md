...
**Created:** May 17, 2026
**Last Updated:** September 21, 2026 (v0.4 — ERR-013-012: FR-PR-010 synchronized with §3.1.2 / ERR-013-009 and ERR-013-011: possessing-team direction, own-team-passer exclusion, EventBus-stamped `PassAttemptEvent.Tick` recency, bounded completion of an already-started discrete-event dwell, and explicit commit-at-threshold semantics.)
**Version:** 0.4
**Status:** DRAFT
...
| FR-PR-010 | A `PassAttemptEvent` (#5 §2 FR-10) may **start** `BACKWARD_PASS` dwell only when its passer is not on the pressing team, its EventBus-stamped `Tick` lies in `[N-AI_PHASE_STRIDE,N)` at AI physics tick `N`, and its pass direction satisfies `dot(normalize((e.TargetPosition − passerPosition).xy), -attackingDirection) < BACKWARD_PASS_THRESHOLD [GT]`, where `passerPosition = perception.agents[e.AgentID].position` and `attackingDirection` is the pressing team's forward vector. Once such an event starts dwell, that already-started discrete-event dwell remains logically raw-true only until it reaches `TRIGGER_DWELL_TICKS`; reaching `TRIGGER_DWELL_TICKS` commits `BACKWARD_PASS` per §3.2. The event need not remain in the recency window and cannot start another dwell later. | MUST | KD-7 / §3.1.2 |
...
| #5 Pass Mechanics §2 FR-10 | `PassAttemptEvent` ring | events | `BACKWARD_PASS` source; payload: `AgentID`, `PassType`, `TargetPosition`, `FrameNumber`; EventBus-stamped `Tick` is the authoritative 60 Hz recency clock and must satisfy `e.Tick ∈ [N-AI_PHASE_STRIDE,N)` only to **start** a new dwell at AI physics tick `N`; once started, the bounded discrete-event dwell may complete after the event ages out; a stale retained event cannot start or restart dwell; #13 derives pass direction from `perception.agents[e.AgentID].position → e.TargetPosition`. |
...
| F7 | Stale `PassAttemptEvent` outside the current AI-owned EventBus window | `e.Tick ∉ [N-AI_PHASE_STRIDE,N)` at AI physics tick `N` | Do not let the stale retained event start or restart `BACKWARD_PASS` dwell. If that same event already started a bounded dwell while fresh, allow only that already-started dwell to remain logically raw-true until `TRIGGER_DWELL_TICKS`, then ordinary release applies. | T-U-010 / T-U-010a |
...
| 0.4 | September 21, 2026 | OpenAI | ERR-013-012: synchronized FR-PR-010, the #5 input contract and stale-event failure arm with §3.1.2 / ERR-013-009 / ERR-013-011 — `BACKWARD_PASS` uses the possessing-team frame (`-attackingDirection`), rejects pressing-team passers, permits only a fresh EventBus-stamped `Tick ∈ [N-AI_PHASE_STRIDE,N)` event to start dwell, commits when the bounded dwell reaches `TRIGGER_DWELL_TICKS`, and cannot be restarted by the stale retained event. T-U-010 locks completion/no-restart; T-U-010a locks window acceptance/rejection. No runtime/schema/RNG change. |
