...
**Created:** May 17, 2026
**Last Updated:** September 20, 2026 (v0.4 — ERR-013-012: FR-PR-010 synchronized with §3.1.2 / ERR-013-009 and ERR-013-011: possessing-team direction, own-team-passer exclusion, EventBus-stamped `PassAttemptEvent.Tick` recency, and bounded completion of an already-started discrete-event dwell.)
**Version:** 0.4
**Status:** DRAFT
...
| F7 | Stale `PassAttemptEvent` outside the current AI-owned EventBus window | `e.Tick ∉ [N-AI_PHASE_STRIDE,N)` at AI physics tick `N` | Do not let the stale retained event start or restart `BACKWARD_PASS` dwell. If that same event already started a bounded dwell while fresh, allow only that already-started dwell to remain logically raw-true until `TRIGGER_DWELL_TICKS`, then ordinary release applies. | T-U-010a |
...
| 0.4 | September 20, 2026 | OpenAI | ERR-013-012: synchronized FR-PR-010, the #5 input contract and stale-event failure arm with §3.1.2 / ERR-013-009 / ERR-013-011 — `BACKWARD_PASS` uses the possessing-team frame (`-attackingDirection`), rejects pressing-team passers, and permits only a fresh EventBus-stamped `Tick ∈ [N-AI_PHASE_STRIDE,N)` event to start dwell; an already-started bounded dwell may complete after the event ages out, and the retained event cannot restart it. No runtime/schema/RNG change. |
