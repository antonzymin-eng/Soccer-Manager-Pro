...
**Created:** May 12, 2026
**Last Updated:** September 4, 2026 (A3.3 conformance correction; v1.0 APPROVED May 15, 2026 — approved baseline remains in force)
**Version:** 1.0.5
...
| 9.1.2 | All FR-TS-### present in §2.2 with conformance level and activation stage | **Approved baseline (May 15):** FR-TS-001 … 085. **Current A3 candidate (September 4):** `docs/specs/testing-strategy/section-2.md` contains 97 unique contiguous rows FR-TS-001 … 097, each with level and activation stage. FR-TS-011 and FR-TS-062 are **substantive normative amendment candidates**, not documentation-only synchronization: FR-TS-011 changes determinism-suite consumption from an every-CI reading to pipeline-scoped consumption, and FR-TS-062 changes when the double-run flake MUST applies. Neither change is approved by this branch; A3.4 owner reapproval is required. D2/D3 and FR-TS-078/079 are implementation/tool-state synchronization candidates. | [x] |
...
|---|-------|----------|--------|
| 9.2.1 | Cite-not-redefine rule audited (no #16 / #18 / #20 restatements) | Manual review of `section-3.md` and `section-6.md`; `grep -rn "Tier A\|Tier B\|Tier C" docs/specs/testing-strategy/section-3.md docs/specs/testing-strategy/section-6.md` | [x] |
| 9.2.2 | Every FR row resolves to a §5.x verification mechanism | **Approved baseline (May 15):** §5.6 covered FR-TS-001 … 085. **Current A3 candidate (September 4):** `docs/specs/testing-strategy/section-5.md` §5.6 covers FR-TS-001 … 097 and records implemented auditors, D2/D3 tool pins, pipeline topology, and the deliberately still-open D9/per-tier-coverage boundaries. A3.4 must re-run before reapproval. | [x] |
| 9.2.3 | Every approval-checklist row in *this* checklist cites either a file path, a resolvable local § citation, or a check name (KD-6 self-application) | This table; automated auditor walk per §5.3 using `tools/checklist-auditor.py` | [x] |
...
| 9.2.12 | A3 creates no architecture/evidence enforcement claim: that gate remains inactive until A8 and `SPEC_INDEX.md` remains unchanged until A3.4 | §4.5; §5.2; §6.2; §9.4; `docs/specs/SPEC_INDEX.md` | [x] |
| 9.2.13 | Pipeline conformance claims are behavior-locked rather than prose-only: timeout, staged snapshot, hook-install refusal, auditor approval-state behavior, tool pins, owner-held-RED drift/unexpected-green handling, and Linux/Windows nightly split have tooling tests | `tools/tests/test_testing_pipeline_conformance.py`; CI `Spec hygiene checks` / `Tooling unit tests` | [x] |
...
| 9.2.15 | Structural pipeline definitions are not substituted for operational acceptance evidence | §4.5.1 requires a successful measured pre-commit run within 60 s on the certified developer host; §4.5.3 requires an actually executed certified Windows/Unity nightly job. Timeout/YAML presence alone do not close either condition. | [x] |
...
| 9.3.2 | Lead-developer sign-off captured | **Approved-baseline evidence:** May 15, 2026 sign-off remains valid for the operative baseline. **A3 amendment:** fresh owner sign-off is pending A3.4; none is claimed by this candidate. See this §9.3 row and §9.4. | [x] |
| 9.3.3 | `spec-error-log.md` updated with any cross-spec drift discovered during drafting | `docs/tracking/spec-error-log.md` | [x] |
| 9.3.4 | `SPEC_INDEX.md` status updated atomically with sign-off | **Approved-baseline evidence:** May 15 approval was synchronized in `docs/specs/SPEC_INDEX.md`. **A3 amendment:** intentionally unchanged; A3.4 owns atomic reapproval/status synchronization. | [x] |
| 9.3.5 | `file-manifest.md` version/state metadata reflects the landed section files | **Landing-only check.** This PR is still a candidate and has not landed on `main`; final `docs/tracking/file-manifest.md` synchronization is performed against the actual merge result rather than pre-claiming a landed state. | [ ] |
...
| 9.3.9 | FR-TS-011 and FR-TS-062 substantive amendment decisions receive explicit owner reapproval rather than being smuggled through as implementation synchronization | Pending A3.4 owner decision; this row is intentionally not checked by the A3.3 candidate. | [ ] |
| 9.3.10 | Operational pipeline acceptance evidence exists for the two structurally-defined but not yet proven conditions | Pending: successful ≤60 s certified-host pre-commit run and successful certified Windows/Unity #16 nightly execution. | [ ] |

## 9.4 Decision

- **Current A3 amendment state:** **AMENDMENT DRAFT.** The May 15, 2026 approved baseline remains operative. This candidate implements/corrects the FR-TS-075/079 topology and supporting evidence; it does not grant fresh approval, change `SPEC_INDEX.md`, or activate the architecture/evidence gate. A3.4 owns coordinated reapproval/status synchronization and the explicit owner decision on the substantive FR-TS-011 / FR-TS-062 amendments; A8 owns architecture/evidence enforcement activation.
- **Current candidate pipeline state:** the versioned pre-commit/PR/nightly surfaces exist; D2 and D3 tool choices are pinned; automated checklist/schema auditors exist; PR CI calls the stable composition runner; the owner-held RED is verified separately and an unexpected green blocks; Linux is explicitly non-certifying; the authoritative full #16 suite is assigned to the pinned self-hosted Windows/Unity nightly job. **Two operational acceptance facts remain unproven:** successful completion of the pre-commit composition within 60 seconds on the certified developer host, and a successful run of the certified Windows/Unity nightly job on an actually registered/configured runner. D9, the §5.5 per-tier coverage mapper, the dedicated flake layer, and A8 remain separate work.
- **Auditor blocking boundary:** the tools survey the repository on every composed run. Findings block only when the affected changed non-legacy spec is in `APPROVED` candidate state; amendment drafts / in-review specs remain visible findings until their approval transition, and #1–#8 remain KD-4 survey-only. This implements FR-TS-042/052's approval-blocking semantics without turning unrelated code PRs into retroactive documentation migrations.
...
- **Approval evidence:** file paths, resolvable local § citations, or programmatically-verifiable sources (KD-6 self-application).
...
- v1.0.5 (2026-09-04): **A3.3 review/evidence correction.** Explicitly classifies FR-TS-011 and FR-TS-062 as substantive normative amendment candidates requiring A3.4 owner reapproval; records the pre-commit timing and certified-Windows execution facts as still-unproven operational acceptance conditions; aligns checklist/schema auditor blocking with approval-state semantics; and records unexpected owner-held-RED greens as blocking. No fresh approval is claimed.
