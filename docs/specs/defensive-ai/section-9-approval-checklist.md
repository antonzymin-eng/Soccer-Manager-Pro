...
**Created:** May 17, 2026
**Last Updated:** August 12, 2026 (v0.5 — KD-6 revised (`ERR-014-006`, wiring backlog W2): §9.4 item 3's resolution amended in place — the original "#8 mediates; #3 owns contact physics" half of KD-6 was never implementable; superseded by §3.6.5's abstract-duel outcome resolution. NOTE: this patch does not recount §9.1 items 4 (constant catalogue: now 37 entries, 32 `[GT]` + 4 `[CROSS]` + 1 `[FIXED]` — a `[FIXED]` tag is new to this spec's catalogue), 7 (data structures: now 7 structs/enums + 2 views, `TackleDuelInputs` and `TackleOutcome` added), or 32 (glossary: now 18 entries, `appendices.md` Appendix F) — all three evidence counts are now stale as a downstream consequence of this revision and are flagged here for a dedicated recount pass rather than corrected in this one, to avoid an uncoordinated edit to an APPROVED gate document's verification counts.)
**Version:** 0.5
**Status:** APPROVED
...
| 3 | Tackle ownership ambiguity (who owns tackle contact vs. intent) | H | KD-6 resolved (as of v0.1, May 17, 2026): #14 produces `TackleIntentRequest` (intent); #8 mediates; #3 owns contact physics. No type enum in physics layer. **AMENDED August 12, 2026 (KD-6 revised — `ERR-014-006`, wiring backlog W2): the "#8 mediates; #3 owns contact physics" half of this resolution was never implementable — #8's `ActionType` ordinal space is exhausted and #3 defers slide-tackle collision to Stage 2. #14 now resolves the tackle OUTCOME itself as well as the intent (new §3.6.5, an abstract attribute duel; #3's contact physics becomes the Stage 2+ fallback). The "no type enum in physics layer" half is unaffected and remains true of both `TackleIntentRequest` and the new `TackleOutcome`.** | `section-1.md` §1.5 KD-6; `section-3.md` §3.6, §3.6.5; `section-4.md` §4.5.2 |
...
| 0.5 | August 12, 2026 | AI agent (wiring backlog W2) | KD-6 revised (`ERR-014-006`): §9.4 item 3's resolution corrected in place (rowless correction, not a new finding) — the "#8 mediates; #3 owns contact physics" half of the original KD-6 resolution was never implementable; #14 now resolves the tackle outcome itself (§3.6.5). §9.1 items 4, 7, and 32 are now stale as a downstream consequence (constant-catalogue count, struct/enum count, and glossary count all grew) and are flagged, not recounted, in this pass. |
