...
clauses, no multi-day negotiation, **no autonomous AI-club bidding**. **Deep (Stage 3)** = the same valuation
**identity** reads #33 personality (a multiplicative bias) and adds agents, clauses, loans, wage structures,
multi-day negotiation, and stochastic rival bidding — **one code path**, each deep feature defaulting to its
Stage-2 identity via `deepTransfersEnabled`.
...
- **Counterparty personality (#33).** #33 supplies `PersonalityProfile` traits + `MoraleOf` — **read-only,
  deferred** (#33 §7.3 names #31 a read-only consumer). Minimal makes **no** #33 read; personality is the
  deep-tier modulation of the valuation identity (KD-1).
...
- **KD-1 (valuation as the identity #33 modulates).** The Stage-2 counterparty valuation is a **pure
  deterministic integer function** over #27 attributes + age + a club-need signal (§3.1). #33 personality and
  #28 CA enter the **deep tier as a multiplicative bias** on this identity, **never a replacement path**; a
  #33-unconfigured negotiation reproduces the deterministic valuation exactly (`personalityMult ≡ 1000‰`).
