...
**Last Updated:** September 10, 2026 (v2.2 — **`ERR-030-050`** review correction: #40 finance is loop-held T1b state, restored through `financesOrNull`; empty is legal pre-T2 and a non-empty set exactly matches `SeasonState.ClubIds`. Prior update follows)
**Last Updated (prior):** September 10, 2026 (v2.1 — **`ERR-030-049`**, #40 T1b: FR-SN-021's `Save` signature —
the row that names itself the single normative copy — gains the eleventh, REQUIRED, null-rejecting
`finances` argument and `Load`'s return gains the reconstructed `Finances`, with the deliberate
ABSENCE of a `financesWired` companion stated and reasoned (FR-FN-025 vs FR-DC-017). §2.2's
data-structure list is deliberately NOT extended: `SeasonLoop` holds no #40 type at T1b — the block
travels through `SeasonSaveManager`/`SeasonSaveContents` only — and the loop-held state arrives with
the resume seam at #40 T2, which is when the ERR-030-035 M21 class would apply)
**Last Updated (prior):** August 15, 2026, later (v2.0 — **`ERR-030-042`**, the §2 half of a reviewed High:
F9's parenthetical described the depleted-squad back-fill as "least-injured first, selector-probed",
the same single-ordering-key description §3.4 carried, while `AvailabilityComposition.Reinstate` has run
a TWO-TIER rule since the #44 C1/C2 landing — injured first, a suspended player only once no injured one
remains. F9 now names both tiers and points at §3.4 for the rule and for the zero-default trap that makes
a single-key implementation invert it silently. `section-3.md` v2.6 in the same commit; no code change)
**Last Updated (prior):** August 15, 2026 (v1.9 — M26, the spec half of #44's adversarial-review round 4
(`open-issues.md`): FR-SN-021's `Save` signature was nine arguments against the code's real ten after
ERR-030-039 promoted `disciplineWired` onto the public form — refreshed to the true ten-argument
signature and to name this row as the single normative copy, since §4.4 already points here rather than
restating it and Appendix B/B.1 covers only the byte layout)
**Last Updated (prior):** August 13, 2026, later still (v1.8 — M21, a fifth adversarial-review pass over the
...
| FR-SN-021 | `SeasonSaveManager.Save`/`Load` MUST carry the season alongside the world and optional match; capture MUST complete before the file is opened (the blob-before-file precedent). *(Signature as amended through #29/#41 T1–T2, the balance pass, #28 T1, #44 T1 and #44's adversarial-review round-4 fix — refreshed at ERR-030-039 after ERR-030-035 (#44 T1, roadmap C1) had gone one landing stale the moment the public form's own `disciplineWired` guard turned out to be bypassable; this row is the single normative copy — §4.4 points here rather than restating it, and Appendix B/B.1 covers only the byte layout, not the call:)* `Save(world, season, matchOrNull, path, trainingClubs, medicalClubs, appearanceClubs, progression, discipline, disciplineWired, finances)` — the three career-block sets, `progression` (a `ProgressionEngine`, #28 KD-4), `discipline` (a `DisciplineState`, #44 Appendix B) and `finances` (a `ClubFinanceEntry[]`, #40 FR-FN-020, added at #40 T1b / ERR-030-049) are all REQUIRED and null-rejecting (T1 AR: an omitted set must not compile into an empty save; `progression`, `discipline` and `finances` follow the identical rule at #28 T1 / #44 T1 / #40 T1b); `disciplineWired` (a `bool`, ERR-030-039) is a tenth, REQUIRED, never-defaulted parameter carrying the fact "a #44 subsystem is driven behind this save at all" — it is NOT derivable from `discipline` itself, because FR-DC-017 drops a tally row the instant it reaches `(0, 0)`, so an empty `DisciplineState` is indistinguishable between "legitimately drained to zero" and "no subsystem wired", and a prior form that hardcoded this flag `true` bypassed the destination-guard it exists to drive — plus the `Save(loop, matchOrNull, path)` overload for external callers; `Load(path, …) → SeasonSaveContents` (world, season, the three career-block sets, the reconstructed `Progression` — never null, #28's restored roster authority per KD-4 — the reconstructed `Discipline` — never null, #44's restored tally state — the reconstructed `Finances` — never null, #40's restored per-club entries in ascending `ClubId` order, empty until #40 T2 bootstraps them — optional match). **#40 deliberately gets no `financesWired` companion to `disciplineWired`**: FR-FN-025 makes a club's entry permanent once `CreateInitial` creates it, so an emptied destination is unambiguously a drop and Appendix B.1's emptiness key is sound for it — the flag exists for #44 only because FR-DC-017 makes an empty tally canonical there. | MUST | KD-1 |
...
| F9 | The composed availability filters leave a club unable to field the formation even with EVERY squad member pressed back in (ERR-030-029 — the §3.4 depleted-squad rule's terminal case) | throw (`InvalidOperationException`) at selection — a roster-integrity bug, not a football outcome; the back-fill itself is the defined non-throwing path, and it is **tiered** (ERR-030-042): the merely injured first (ascending remaining recovery, ties by earliest roster position), then — only once no injured player remains and the club still cannot field the formation — the suspended (earliest roster position), the selector probed after each. The tier is a separate discriminator from the ordering key, which #41 alone writes; see §3.4 for the rule and for why deriving one from the other inverts it silently |
| F10 | A #28 player's `BirthWorldDay` anchor sits AHEAD of the world clock (`ERR-028-019` back-prop, AR pass 6 M2(b)) — a DIFFERENT invariant from F8's cursor-vs-clock rule, checked ahead-only, never for lag: an anchor arbitrarily far in the PAST is the ordinary case for a generated player (#28 §3.1.1 / ERR-028-006), but one ahead of the clock is corrupt state — #28's own `GrowthProjection.AdvanceDayForPlayer` fails loud on the resulting negative `ageDays` (#28 §3.1.1's M2(a) guard), but only once a day step reaches the player; this is the boundary that refuses the pairing BEFORE that. `ProgressionSaveCodec`'s own value gates (#28 §3.5) cannot enforce this half — the codec has no world clock to bound the anchor's upper end against, only the format's own `uint.MaxValue` ceiling | throw at `SeasonLoop` composition AND at `SeasonSaveManager.Save`/`.Load` (`InvalidOperationException`) — `PlayerCareerStates.RequireBirthWorldDayWithinClock`, one shared owner both boundaries delegate to, mirroring F8's own parallel-surface discipline |
