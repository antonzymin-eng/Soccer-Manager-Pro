...
| FR-SN-021 | `SeasonSaveManager.Save`/`Load` MUST carry the season alongside the world and optional match; capture MUST complete before the file is opened (the blob-before-file precedent). *(Signature as amended through #29/#41 T1–T2, the balance pass, #28 T1 and #44 T1 — refreshed at ERR-030-035 (#44 T1, roadmap C1) after ERR-030-032 (AR pass 5 over the #28 landing) had gone one landing stale again the moment §4.4 pointed at it as the single owner:)* `Save(world, season, matchOrNull, path, trainingClubs, medicalClubs, appearanceClubs, progression, discipline)` — the three career-block sets, `progression` (a `ProgressionEngine`, #28 KD-4) and `discipline` (a `DisciplineState`, #44 Appendix B) are all REQUIRED and null-rejecting (T1 AR: an omitted set must not compile into an empty save; `progression` and `discipline` follow the identical rule at #28 T1 / #44 T1) — plus the `Save(loop, matchOrNull, path)` overload for external callers; `Load(path, …) → SeasonSaveContents` (world, season, the three career-block sets, the reconstructed `Progression` — never null, #28's restored roster authority per KD-4 — the reconstructed `Discipline` — never null, #44's restored tally state — optional match). | MUST | KD-1 |
