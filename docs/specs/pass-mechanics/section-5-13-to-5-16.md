...
| 1.3 | June 13, 2026 | — | Dotnet-CI quarantine burn-down (ERR-005-001, First Touch ERR-004-006 family). Three §5 expected outputs re-derived from the normative §3.2.7/§3.3.4/§3.5.3 model against the authoritative §3.1.4 profile table (the §5.11 NUnit suite had never compiled until June 11, so these expectations were never executed). §5.3 PV-003 lower bound `[14,20] → [13,20]` (true 13.333 m/s at Lofted distMax=60; the 14 floor used the stale §3.2.9 distMax). §5.12 VS-001 velocity `≈11.5 → ≈10.2/9.7 m/s` (unreachable for an 8 m Ground pass; caps ~10.45 at K=20) and error `≤0.8° → ≈1.05°` (unreachable; elite Ground error floors at 0.861°). §5.12 VS-003 velocity `≈10.5 → ≈11.1/9.9 m/s` and angle `≈55° → ≈45°` (apexChip=4.5 m yields θ=atan(1)=45°, not 55°). Production code unchanged — verdict TEST/SPEC, not PRODUCTION. Corrects the velocity-ordering inversion (chip now > short ground pass). |
