...
| FR-SC-002 | `EstimateFor(in PlayerRecord, int knowledgeBand)` MUST be the **single** view function (one code path): the minimal tier calls it with `band = KNOWLEDGE_BAND_MAX` for every player; the deep tier with the resolved band (§3.1). No consumer branches on `fogEnabled`. | MUST | KD-8 |
...
| FR-SC-019 | On a roster **re-key** (#31 transfer) or **retirement/regen** (#28), the overlay entry for the affected `PlayerId` MUST be dropped (buy → own-squad rule covers it; sell → knowledge reset, a named Stage-3 simplification); a view query for a `PlayerId` not resolvable in the pool MUST fail loud (F1) — never silent staleness. | MUST | KD-6 |
...
| **F4** | A decoded overlay entry with `KnowledgeBand ∉ [0, KNOWLEDGE_BAND_MAX]`, a non-ascending/duplicate `PlayerId`, or an assignment referencing a player with no resolvable record | **Fail loud** — corrupt/incoherent overlay state (FR-SC-017, the #22 `RestoreLatched` strict-order precedent). |
