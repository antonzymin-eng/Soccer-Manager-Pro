...
- **Owner-held close-chance exclusion cardinality is inconsistent across pinned main and #416 despite identical discovered MatchEngine test sets. RECORDED, NOT FIXED.** Main run `35133104970` logs `Name!=sim_match_engine_close_chance` yet executes 514 ordinary cases before running the owner-held test separately; #416's comparable ordinary sweep executes the expected 513. This is a Testing Strategy/tooling selection-accounting remainder, not evidence of a missing #416 test. *Owning record: `open-issues.md`.*
