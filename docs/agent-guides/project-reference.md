...
**Branch cleanup/reachability rule.** A local Git result may support a branch-deletion or evidence-reachability decision only after `python3 tools/dotnet-ci/check_branch_ancestry.py --repo . --ancestor <ref> --descendant <ref>` accepts the repository as non-shallow. If the checkout is shallow, make no local ancestry claim: obtain full history or use an authoritative remote/API comparison. Fetching refs alone does not satisfy this rule. Current-tree blob absence is not proof that historical evidence is unreachable.
