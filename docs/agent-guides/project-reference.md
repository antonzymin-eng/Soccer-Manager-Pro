# Project agent reference — expanded
...
> **Purpose:** Historical snapshot of the detailed repository maps, catalogues, narratives, and issue index moved out of `CLAUDE.md` on August 27, 2026. This file preserves context; it is not an always-read rule file.
> **Precedence:** The compact root `CLAUDE.md`, owning specifications, and live tracking documents override this snapshot. Load only a relevant section, and verify dates, counts, statuses, and paths against their owning files before acting.
...
`src/CLAUDE.md` is the compact authoritative coding guide. It routes expanded coding detail on demand.
...
| `club-finances` | **#40** Club Finances & Economy | T0 + T1a/T1b + T2a/T2b + T3a foundation landed. #30 now owns production bootstrap, season-boundary settlement, and finance read/query/command routing through `SeasonLoop`; T3b1 daily identity wiring, T3b2 deterministic revenue tuning and T3c+ stochastic/deep economics remain deferred. |
...
- `src/CLAUDE.md` is the compact authoritative coding guide; this snapshot preserves its former expanded taxonomy and examples.
...
| Green Linux gate, broken Unity build | On September 12, 2026 `main` passed CI but had 22 compile errors in the Unity editor: missing direct asmdef references hidden by MSBuild's transitive references, plus NUnit 3.14-only assertions (`Assert.Multiple`, `Does.Not.Contain(<int>)`) that Unity's bundled NUnit 3.5 lacks | **Unity governs** (owner decision, September 12, 2026): verify in the Unity 6000.4.9f1 editor with a forced `Assets/Scripts` reimport — a plain Refresh sees nothing through the junction. The Linux gate stays as a supplement. Procedure: `coding-reference.md` → BUILD AND TEST COMMANDS |
...
**Branch cleanup/reachability rule.** A local Git result may support a branch-deletion or evidence-reachability decision only after `python3 tools/dotnet-ci/check_branch_ancestry.py --repo . --ancestor <ref> --descendant <ref>` accepts the repository as non-shallow and evaluates the stored commit graph (replacement refs disabled; legacy grafts rejected). For **remote** branch deletion, the descendant must be a freshly fetched remote-tracking ref such as `refs/remotes/origin/main`, an authoritative remote OID, or an authoritative remote/API comparison; local `main` is sufficient only for local-branch cleanup because a local-only merge does not prove remote publication. If the checkout is shallow, make no local ancestry claim. Fetching refs alone does not satisfy this rule unless the shallow boundary is removed. Current-tree blob absence is not proof that historical evidence is unreachable.

**32 active** / **58 archived**. *(September 26, 2026 — W8 ERR-011-017 clock-correction review files the keeper baseline-slot defect: #11 is fed the keeper's own position, so recovery cooldown never holds. Active 31 → 32; archive unchanged at 58.)* *(September 26, 2026 — PR #461 owner approval accepts the 35 m W8 B LongKick target while filing realistic punt length as a separate #5 Lofted-trajectory follow-up. Active 30 → 31; archive unchanged at 58.)* *(September 25, 2026 — PR #457 files two owner-rule gaps surfaced by the W8 goalkeeper-distribution review (#456): substitution timing and goal-kick distribution; both are indexed below. Active 28 → 30; archive unchanged at 58.)* *(September 23, 2026 — PR #439 close-out: the `ShotExecutor` FM-03 containment debt that PR #439 filed in `open-issues.md` is now indexed below, and W3 lands wired but dormant. Active 27 → 28; archive unchanged at 58.)* *(September 21, 2026 — PR #433 mechanically closes the PR #424 SHA-256 manifest remainder and narrows, but does not close, the shallow-clone ancestry hazard. Every tracked evidence directory must now register an integrity contract; full `SHA256SUMS` coverage is derived from the Git index, while W12 / PR416 keep their owning verifiers. The ancestry CLI refuses shallow history, but deletion enforcement remains procedural rather than server-enforced. Active 28 → 27; archive 57 → 58. The historical owner-held pinned-main leakage anomaly also remains open.)* *(September 19, 2026 — PR #416 Step 5 cleanup is archived after PR #427's post-delete-capable checker/workflow and durable records reached `main`: active 28 → 27, archive 56 → 57. The same follow-up pass files a new shallow-clone ancestry/reachability hazard after a shallow agent clone falsely reported a fully merged branch as unrelated: active 27 → 28; archive remains 57.)* *(September 18, 2026, later — PR #424 records SHA-256 evidence-manifest enforcement as a separate live remainder: the current 7/7 manifest verifies, but no workflow/tool enforces its digests or coverage; the repository-wide gate remains scoped to a separate follow-up PR. Active 27 → 28; archive unchanged at 56.)* *(September 18, 2026 — Step 3.3 closes the Ball Physics v2.10 elevated-`Rolling` distributional characterization residual against current main: one active issue archived, with the PR #422 cleanup remainder retained. Active 28 → 27; archive 55 → 56, re-derived mechanically from the owning files.)* *(September 18, 2026, post-#421 — PR #416 evidence cleanup is now a distinct live remainder: 24 evidence refs still exist; branch-only harness/helper/enforcement artifacts must be archived before deletion, and the narrow/state-only candidate source forms remain ref-dependent unless captured durably. Step 4 remains closed. Active 27 → 28; archive unchanged at 55.)* *(September 18, 2026 — PR #416 closeout review files two bounded residuals: the owner-held close-chance ordinary-sweep exclusion/cardinality inconsistency and the uncharacterized distributional effect of Ball Physics v2.10 elevated-`Rolling` semantics. Active 25 → 27; archive unchanged at 55; both figures re-derived from the owning files.)* *(September 17, 2026 — PR #416 / `ERR-007-004` was filed directly to `open-issues-resolved.md`: the zero-distance Perception #7 FoV-bearing defect is closed by the co-location rule and exact live-head regression evidence. Active remains 25; archive 54 → 55, re-derived from the owning files.)* *(September 11, 2026, later — the #40 Club Finances T3a landing (PR #400, `ERR-040-003`) files one new entry, `AccrueDailyRevenue` landing as a production surface with no production caller, bounded to T3b1's identity-zero #30 wiring slice; active 24 → 25, the archive unchanged at 54. Both figures re-derived by direct count at this commit.)* *(September 11, 2026 — the UX F4 validation-protocol landing (PR #394) files one new entry, the `check_drift.sh` truncated-chain blind spot, taking active 23 → 24; the archive is unchanged at 54. Both figures re-derived by direct count at this commit.)* *(September 10, 2026 — PR #389 filed the resolved non-authoritative localization T0 landing/revert incident to `open-issues-resolved.md`, taking archive membership 53 → 54 while active remains 23; both figures were re-derived by direct count on this branch.)* *(September 7, 2026 — audio G0 accepted by owner; counts unchanged because `ERR-051-001`, D48 and D49 remain live. The governing audio plan/amendment are now v1.5/v0.5.)* *(September 6, 2026 — the audio G0 planning landing files one new entry, taking the active count 21 → 22; the archive is unchanged at 53. Both figures re-derived by direct count at this commit (`grep -c '^- \*\*'` returns **22** on `open-issues.md` and **53** on `open-issues-resolved.md`), and this index now carries **22** bullets.)* *(The second number counts **archive membership**, not resolved issues: `open-issues-resolved.md` also holds superseded parallel records, which its own entries annotate as "not a resolved issue". It was published as "resolved" until September 4, 2026, which was semantically false — the count was mechanically right and the word was wrong. No resolved-vs-superseded split is published, because no reliable mechanical test for one exists: grepping the archive for supersession markers returns 11 of 53 bullets, and most are false positives matching unrelated prose.)* *September 4, 2026, later still — the System XI rename landing (PR #359) filed one new entry, taking the active count 20 → 21; the archive is unchanged at 53. Both figures re-derived by direct count at this commit (`grep -c '^- \*\*'` returns **21** on `open-issues.md` and **53** on `open-issues-resolved.md`), and this index now carries **21** bullets.* *September 4, 2026, latest — **this count had been stale at 15/46 since at least August 22, 2026, and nothing caught it because the check that exists to catch it was looking at the wrong file.** `check_drift.sh`'s open-issues count check read the claim out of root `CLAUDE.md`, which has carried no OPEN ISSUES section since the compact restructure, so it printed `UNPARSED` on every run instead of comparing anything — a check reporting nothing, read as a check finding nothing. The script now reads this file, which is where the claim actually lives, and distinguishes "surface carries no claim" from "claim disagrees". Both figures are re-derived by direct count at this commit (`grep -c '^- \*\*'` returns **19** on `open-issues.md` and **53** on `open-issues-resolved.md`), and they now agree with the 19 bullets in the index below. The active count fell 21 → 19 in the same commit: this file carried three entries for the football-judgment proxy review, and two superseded parallel records were moved to the archive verbatim (see `open-issues.md`), which is also why resolved rose 51 → 53.* *August 22, 2026, latest — TWO ENTRIES ARCHIVED, and the index-vs-record discrepancy this file recorded earlier the same day is closed with them. **(1) `EnvironmentFingerprint.floatModelHash`:** both remainders on the replay-identity contract are now closed — `ERR-016-009` (`buildHash`) earlier the same day, and `ERR-016-010` (`SaveManager` writes `Fingerprint = null`) later the same day, the latter turning out to be a §3.9.2 normative-layout contradiction in four respects rather than one missing field. Archived whole and verbatim with a resolution annotation. **(2) `#29 Training / #41 Injuries & Medical`:** verified resolved against the code and filed straight to the archive, having never had an `open-issues.md` entry to move — which is exactly the discrepancy the `landing-history.md` split had recorded and left for an owner call. Counts re-derived by direct count after the change — `grep -c '^- \*\*'` returns **15** on `open-issues.md` and **46** on `open-issues-resolved.md`, and this index now carries **15** bullets, so the two agree for the first time in the record.* *August 22, 2026 — DE-DUPLICATION: the football-judgment proxy review had **two** active entries in `open-issues.md` and **two** bullets in this file, so the active count had been double-counting one issue. The second of each was the record of the concurrent `claude/football-judgment-proxy-review-pq12dz` branch (PR #305), whose competing `ERR-008-021` fix the August 7 merge did NOT keep. This file now carries one bullet; `open-issues.md` carries one entry; the PR #305 record was moved to `open-issues-resolved.md` **verbatim**, under a blockquote annotation explaining that it is a superseded parallel record rather than a resolved issue, with two of its facts (the genuine CI-run-404 verification, and the unlanded AR-1 H-1 single-goalkeeper-candidate selection) carried forward into both survivors. Precedent: this file's own August 2, 2026 archiving of "a duplicated pair". Counts re-derived by direct count after the change — `grep -c '^- \*\*'` returns **16** on `open-issues.md` and **44** on `open-issues-resolved.md`.* *August 12, 2026 — the A4a calibration entry added at the head, taking the active count to 17 (re-derived by direct count of `open-issues.md`: `grep -c '^- \*\*'` returns 17).* *Re-filed August 2, 2026 — eight entries archived (six closed-but-unmoved, plus a duplicated pair); three titles amended to lead with what remains open rather than what has landed. August 4, 2026 — the wiring-backlog entry added at the head; later same day, the football-judgment remediation entry added above it. August 7, 2026 — a two-red-scenario-locks entry was filed at the head from the B9c gate run and RESOLVED the same evening at the `80d97c8` merge: main's `b162a00` had already diagnosed both failures per-seed and rebaselined both bands by owner call, 45 minutes before the entry was filed. It lived ~1 hour; see the archive. August 8, 2026 — the tree-wide header/version hygiene backlog entry added at the head from `tools/recurring-defect-lint.py`'s first sweep, taking the active count to 15; RESOLVED later the same day by a dedicated hygiene pass (owner chose the hygiene pass over a CI ratchet) — all 275 ERRORs fixed, `python3 tools/recurring-defect-lint.py --repo .` now reports 0 ERROR tree-wide — taking the active count back to 14. **CORRECTION, August 10, 2026: the "14 active" claim above was already wrong before this branch existed** — a direct count of `docs/tracking/open-issues.md` (`grep -c '^- \*\*'`) returns **15**, not 14, against the unchanged 43 resolved; no entry in the file accounts for the sixteenth-vs-fifteenth discrepancy, so the prior figure was never re-derived after some earlier edit. This branch then added one further active entry (the `#28 Player Progression` bullet's owning record, filed to `open-issues.md` to match this file's own index bullet — see above), taking the true count to **16 active / 43 resolved**, re-derived by direct count of both files on 2026-08-10.* *(September 8, 2026 — PR #362 synchronized with current `main`; the UX and audio live records now coexist. Counts re-derived on the merged tree: **23 active / 53 archived**.)*

- **Keeper #11 baseline slot is fed the keeper's own position; `Recovering → Set` is always immediate and the recovery cooldown never holds. RECORDED, NOT FIXED.** *Owning record: `open-issues.md`.*
- **Goalkeeper LongKick 35 m target is approved for W8 B; realistic punt length remains open against #5's Lofted trajectory.** PR #461 owner approval accepts the semantic target without claiming the current 22 m/s / zero-spin trajectory reaches it. Close after B records intended target versus actual first ground contact and the evidence supports a separate #5 trajectory decision. *Owning record: `open-issues.md`.*
- **Substitutions take effect mid-play instead of waiting for a stoppage. OWNER RULE RECORDED, NOT IMPLEMENTED.** Opened September 25, 2026 by PR #457. Owner rule: request at any time, execute at the next stoppage; `MatchEngine.SubstitutePlayer`, the client command drain and match-flow design note §6 all apply it immediately. No W8 effect.
- **Goal kicks do not apply the owner's keeper-distribution rules, and the keeper is not necessarily the taker. RECORDED, NOT FIXED; deliberately outside W8.** Opened September 25, 2026 by PR #457. Goal kicks go through the restart path (nearest eligible taker, ordinary Decision Tree foot action); #21 does not state whether `GkDistributionPolicy` covers them, and no play-out instruction exists yet.
- **`ShotExecutor` FM-03 still logs a CONTACT-time possession loss as an Error although active-W2 play reaches it as a normal cancellation; two long SeasonSave discipline tests suppress all unexpected Error logs around their engine spans. RECORDED, NOT FIXED.** Opened September 22, 2026 by PR #439. *Owning record: `open-issues.md`.*
- **Shallow clone ancestry guard LANDED AS PROCEDURE; external/manual deletion enforcement remains unverified. NARROWED, NOT CLOSED.** `tools/dotnet-ci/check_branch_ancestry.py` refuses shallow histories, disables replacement refs and rejects legacy grafts. Remote deletion additionally requires a freshly fetched remote-tracking descendant or authoritative remote OID/API result; local `main` is only sufficient for local cleanup. Residual: no hook/server-side control proves the guard was used before an external deletion claim. *Owning record: `open-issues.md`.*
- **Owner-held RED topology is now proven and fail-closed; the historical pinned-main leakage mechanism remains open.** PR #420 exact-head run `35420947427` proves `sim_match_engine_close_chance` occurred **0×** in the ordinary executed-result set and **1×** in the dedicated set, and the verifier now mechanically rejects leakage or duplicate/missing dedicated execution. Durable provenance is in `docs/tracking/evidence/pr420-owner-held-isolation.md`. The still-open remainder is narrower: explain or bound why pinned-main run `35133104970` logged the same exclusion filter while showing the 514-case ordinary cardinality. The post-#420 lifecycle follow-up also makes an empty owner-held ledger a valid terminal state: with no held rows, no exclusion is applied and the ordinary suite owns the result. **That terminal state is now active:** on September 20, 2026 the owner retired `sim_match_engine_close_chance`; the historical 0× ordinary / 1× dedicated PR #420 proof remains provenance, while current execution is expected to be ordinary-only. The historical pinned-main leakage issue stays open because retirement does not explain the old excluded-run 514-case census. *Owning record: `open-issues.md`.*
- **`check_drift.sh` cannot detect a truncated append-only tracking chain — proved by a near-miss on PR #394. RECORDED, NOT FIXED.** An automated edit deleted 5,669 lines of `docs/tracking/CHANGELOG.md` (the whole 234-entry header chain) and every existing check passed on the result; reverted byte-identically, nothing lost. The guard and the edit-discipline rule are both owed.
- **Audio & Sound Design (#51) — G0 ACCEPTED September 7, 2026; `ERR-051-001` RECORDED, NOT FIXED; T2/G3 hard-blocked on #48 T0.** No `src/audio/` assembly and no runtime audio exist; the game is silent by design. `AssetRef`/`CueParams` are named by #51 §2.2 but declared nowhere, and the repair is pinned to T0 under C6. *Owning record: `open-issues.md`.*
...
- **Match-engine wiring backlog — W1/C1/W2/W4/W5/W6/W7/W12 wired/active; W3 WIRED BUT DORMANT (PR #439; #441 owns Head reachability); W8–W10 remain Class-A wiring work, and the KD-W1 `[GT]` freeze still applies.** Index title refreshed September 23, 2026 at the PR #439 close-out; it previously still described the August state (W1/C1/W2 built-but-disabled, W3–W10 without a production caller). *Full August narrative: `docs/tracking/landing-history.md` §5. Owning records: `match-engine-wiring-backlog.md` v1.32, `open-issues.md`.*

- Testing Strategy #19 FR-TS-043 conformance gap — `tools/checklist-auditor.py` and `tools/spec5-schema-auditor.py`, two §7.1 Stage 0+1 deliverables, DO NOT EXIST at a stage the repo reached long ago; with `tools/run-tests-local.sh` that is all three of §7.1's named scripts. RECORDED, NOT FIXED, filed as `ERR-019-003`. *Owning record: `open-issues.md`.*
- Testing Strategy #19 FR-TS-075 / FR-TS-079 conformance gap — two of the three mandatory CI pipelines DO NOT EXIST and the Appendix E local-gate script named by a MUST is absent; RECORDED, NOT FIXED, filed as `ERR-019-001`. *Owning record: `open-issues.md`.*
- Spec #20 KD-4 still declares Stage 0 verification manual-only and static-analysis tooling "not required", against a tree carrying live blocking checks; modernization deferred out of A3.1b and owed at A3.4. *Owning record: `open-issues.md`.*
- Branding-only spec edits have no exemption from root `CLAUDE.md`'s "append a version-history row to every modified spec file" rule; the System XI rename (PR #359) landed 59 spec files — 47 of them carrying a Version History section — with zero rows. RECORDED, NOT FIXED; owner decision owed. *Owning record: `open-issues.md`.*
- #44 Discipline & Suspensions — C1/C2 LANDED August 13, 2026 (`src/discipline/` is the 35th production assembly, suspensions live end to end); REMAINDER: T3, the #30-owned quick-sim card synthesis. *Owning record: `open-issues.md`.*
- Depleted-squad cover — `ERR-044-003` stage 1 LANDED August 15, 2026; stages 2 and 3 are both blocked, on #42 having a `src/` assembly and on widening the packed `PlayerId` space. *Owning record: `open-issues.md`.*
- DisciplineConfig restructure — DEFERRED by owner (Fable escalation) ruling, August 16, 2026, from the #44 adversarial-review rounds 8–9 M2 finding. *Owning record: `open-issues.md`.*
- `EnvironmentFingerprint.floatModelHash` — hasher, §4.8.3 Mono mapping and the §4.8.2 MXCSR gate all LANDED; see the owning entry for the remainder. *Owning record: `open-issues.md`.*
...
- Close-chance creation — the DRIBBLE-direction defect FIXED (ERR-008-018, §5.Z.24: the average final-third dribble pointed AWAY from goal); REMAINDER: the funnel itself did not move — the ball still enters the box on 5% of final-third episodes, and the bound is now localized to #8 §3.1.3 generating PASS candidates only at a teammate's CURRENT POSITION, so the tree cannot pass to a place. **Amended Aug 17, 2026:** the Acceptance-3 regression blamed on the -021/-022/-023 shot-lane chain is REFUTED by bisect (chain effect −0.027 ± 0.039, t = −0.70; the −0.119 that drove the rebaseline is the 4.6th percentile of its own two-seed estimator) — the historical `sim_match_engine_close_chance` failure was attributable to C1 / `ERR-012-011`, while the September 20 owner decision retires the held-red exception after current exact evidence passes the unchanged floors; KD-W1 still has nothing to pull back on the shot lane (`close-chance-creation-design.md` §11 / v2.4)
- Foul/card rate — the "~7 red cards per 9 minutes" figure this index carried for months is STALE (resolved July 2026); the live entry records the re-measurement it prompted and the real drift that found. *Owning record: `open-issues.md`.*
- Goalkeeper Mechanics (#11) / Heading Mechanics (#10) engine integration — Phase 1 (opt-in) LANDED; the GK/Heading attribute projections now have a live consumer — REMAINDER: AGENT_BALL duel fan-out now WIRED BUT DORMANT via W3 / PR #439 (no production Head participant — #441), DT-emitted HEADER (ordinal 8 → composure-noise rebaseline), attribute-modulated save commit
...
- UX planning package (`docs/design/ux-high-level-plan.md` **v1.3** + `ux-detailed-plan.md` **v1.8**) — converged and routed from root `CLAUDE.md`; **F0 closed September 6, 2026 and F1 may begin** — corrected the same day from "F1 blocked on owner acceptance while PR #362 is draft", which the plan's own text contradicts: tracking close-out was F1's only prerequisite. Owner acceptance gates UX *production* from S0-C onward, not the F1 audit. **F1 and F2 complete September 7, 2026** (`ux-baseline-evidence.md` v0.3, `ux-experience-architecture.md` v0.1); F3 next. F2's finding is a client dependency, not a UX one: a career shell cannot reuse `Pop()` to return from Post-Match, because `NavigationShell.Pop()` asserts nothing about its destination and `ReturnToMainMenu()` lands correctly today only by stack-root accident. **F3 complete September 7, 2026** (`ux-shared-system.md` v0.3 at that milestone; current artifact v0.4 after the status-authority correction, the shared S0/S1 interaction system); F4 was next at that date, and the plan's header is re-derived at v1.3 after v1.2 left it stating v1.1. F3's claims were verified: `src/ui-framework/` has no focus/keyboard model, `SPEC_INDEX.md` titles #49 a *seam + template-contract slice*, and `src/` has no `Audio*` symbol, so #51 has no consumer. **September 9, 2026 — the high-level plan is corrected to v1.2 before merge:** its §8 F1 effort band still read 1–2 working days against the detailed plan's 3–4, because §8 duplicated the bands rather than citing §10.2; it now names the detailed plan as the authority it mirrors, and all twelve rows were compared — F1 was the only one that differed. **Owner acceptance of the package remains the one item owed before PR #362 merges.** **September 9, 2026 — ACCEPTED BY THE PROJECT OWNER**, discharging the one condition open on this workstream since F0 and the last item owed before PR #362 merges. UX production from S0-C onward is authorized; **F4 is still the next work package**. New §17 of the execution authority bounds the decision: no Gate A–J is passed by it, Gate G still binds at two independent participants, S0 Gate J still needs B8/B9b/B10 host evidence, the §10.1 role owner and Gate-G participants remain F4 outputs, §8.1's `FUTURE-BLOCKED` S2 rows are unmoved, and the career-return navigation move is still the client workstream's. The open issue is amended, not archived — F4 through S1 are unstarted. **September 11, 2026 — F4/privacy revision:** Anton Zymin is the accountable UX owner; participant names/contact details are intentionally omitted in favor of stable anonymous slots and privacy-safe attestations. The v1.5 branch separately moves confirmation of participant availability from F4 exit to pre-Gate-F; that is a deliberate timing relaxation, not a consequence of anonymity. Two distinct real participants remain mandatory before Gate G can pass, and P5b remains gated on S0 Gate I. **September 21, 2026 — S0 Gate A completed in `ux-s0-pm1-journey.md` v0.4; Gate B is next.** Current phase is not otherwise duplicated here; see `ux-detailed-plan.md` and `open-issues.md`. *Owning record: `open-issues.md`.*
