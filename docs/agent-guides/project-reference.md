...
- **Ball Physics v2.10 elevated-`Rolling` semantics remain trajectory-sensitive and not distributionally characterized. RECORDED, NOT FIXED.** #416's perception correction closes the HOLD pathology and no Ball Physics rollback is justified, but the ablation lattice proves the v2.10 ordering can materially resample normal-play trajectories. Measure that population effect before future trajectory calibration depends on it. *Owning record: `open-issues.md`; diagnosis: `w6-elevated-stationary-ball-fix.md`.*
