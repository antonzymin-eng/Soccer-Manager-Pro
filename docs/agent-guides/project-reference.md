...
- UX planning package (`docs/design/ux-high-level-plan.md` **v1.2** + `ux-detailed-plan.md` **v1.4**) — converged and routed from root `CLAUDE.md`; **F0 closed September 6, 2026 and F1 may begin** — corrected the same day from "F1 blocked on owner acceptance while PR #362 is draft", which the plan's own text contradicts: tracking close-out was F1's only prerequisite. Owner acceptance gates UX *production* from S0-C onward, not the F1 audit. **F1 and F2 complete September 7, 2026** (`ux-baseline-evidence.md` v0.3, `ux-experience-architecture.md` v0.1); F3 next. F2's finding is a client dependency, not a UX one: a career shell cannot reuse `Pop()` to return from Post-Match, because `NavigationShell.Pop()` asserts nothing about its destination and `ReturnToMainMenu()` lands correctly today only by stack-root accident. **F3 complete September 7, 2026** (`ux-shared-system.md` v0.3, the shared S0/S1 interaction system); F4 next, and the plan's header is re-derived at v1.3 after v1.2 left it stating v1.1. F3's claims were verified: `src/ui-framework/` has no focus/keyboard model, `SPEC_INDEX.md` titles #49 a *seam + template-contract slice*, and `src/` has no `Audio*` symbol, so #51 has no consumer. **September 9, 2026 — the high-level plan is corrected to v1.2 before merge:** its §8 F1 effort band still read 1–2 working days against the detailed plan's 3–4, because §8 duplicated the bands rather than citing §10.2; it now names the detailed plan as the authority it mirrors, and all twelve rows were compared — F1 was the only one that differed. **Owner acceptance of the package remains the one item owed before PR #362 merges.** **September 9, 2026 — ACCEPTED BY THE PROJECT OWNER**, discharging the one condition open on this workstream since F0 and the last item owed before PR #362 merges. UX production from S0-C onward is authorized; **F4 is still the next work package**. New §17 of the execution authority bounds the decision: no Gate A–J is passed by it, Gate G still binds at two independent participants, S0 Gate J still needs B8/B9b/B10 host evidence, the §10.1 role owner and Gate-G participants remain F4 outputs, §8.1's `FUTURE-BLOCKED` S2 rows are unmoved, and the career-return navigation move is still the client workstream's. The open issue is amended, not archived — F4 through S1 are unstarted. *Owning record: `open-issues.md`.*
