...
**Evidence snapshot:** `main` at `ddd221c9e5df13de13191e1a44352d8797768735`
...
# 2. Evidence correction since the F1 baseline

The F1 baseline correctly classified P4b as `HOST-UNVERIFIED`, but its supporting wording pre-dates the
September 12 Unity repair. PR #404 verified the repository in Unity 6000.4.9f1 after a forced
`Assets/Scripts` reimport with **0 compile errors** and successful assembly reload.

That does **not** make P4b `HOST-VERIFIED`: `MatchClientBehaviour.cs` still has not completed the required
scene/runtime proof (scene boot, render behavior/performance and interaction checks). This packet therefore
uses the narrower evidence statement:

**P4b = landed + Unity-compiler-verified / scene-runtime-unverified.**

The existing qualifier remains `HOST-UNVERIFIED` until the runtime certificate closes; Gate A must not use
"never compiled on Unity" as its rationale any longer.
...
| A-17 | Field/agent state | `MatchFrameView` ball position, possession, agent positions/cues, substitutions used, restart banner | projection live; P4a render model landed; P4b compiles on pinned Unity, runtime proof still open | `DESIGNABLE / HOST-UNVERIFIED` | UX may prototype from real captured output; Gate J still needs host/runtime evidence |
...
### S0-A-002 — P4b evidence wording changed on September 12

**Finding:** The older UX text saying P4b had never compiled on the pinned Unity host is stale after PR #404.
The pinned editor now compiles/reloads the project with zero errors, but the P4b scene/runtime certificate is
still open.

**Disposition:** retain `HOST-UNVERIFIED`, with the explicit qualifier **compiler-verified / runtime-unverified**.
Do not claim Gate J evidence from compilation alone.
...
**PASS — S0 Gate A is complete on evidence snapshot `ddd221c9`.**
...
The pass does **not** mean P5b, P4b runtime verification, on-host P6, #48, #49 or #51 runtime work is complete.
