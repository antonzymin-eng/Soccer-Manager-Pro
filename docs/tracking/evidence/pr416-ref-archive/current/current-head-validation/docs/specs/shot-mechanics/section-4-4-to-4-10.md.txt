...
**CANCELLED state behaviour:** The WINDUP interrupt flag is only polled while the executor is
in `WINDUP`. If the executor has already advanced to `CONTACT`, the flag itself is not used to
retroactively cancel the shot. However, `CONTACT` still re-checks authoritative possession
immediately before `ApplyKick()` (§4.2.4 FM-03). If a tackle has actually removed possession by
then, the shot is cancelled with no kick and a warning; this is normal active-W2 gameplay, not an
interrupt-routing error (**ERR-006-005**). A flag with possession still intact does not cancel the
CONTACT action.
...
| 1.5 | September 17, 2026 | Claude (AI) / Anton | ERR-006-005: active W2 makes CONTACT-time possession loss a reachable normal-play cancellation after the last WINDUP interrupt poll. §4.2.4 FM-03 now logs warning, cancels without ApplyKick, and remains distinct from the WINDUP-only ShotCancelledEvent. §4.4.2 clarifies that a CONTACT tackle flag alone is ignored while authoritative possession loss still cancels at the safety re-check. |
