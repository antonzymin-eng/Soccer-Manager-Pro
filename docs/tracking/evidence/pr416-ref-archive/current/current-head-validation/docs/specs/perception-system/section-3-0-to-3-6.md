...
| 1.5 | September 17, 2026 | ERR-007-004: §3.5.1 defines zero XY displacement between observer and ball as having no physical bearing. Co-location therefore satisfies only the FoV angle gate; the ordinary range and shadow-cone occlusion gates remain active. This closes the PR #416 Controlled-holder blind spot without changing any [GT], RNG, or persistence contract. |
...
2. **Visibility otherwise uses the same range and occlusion gates.** The FoV bearing
   gate has one geometric exception: if the ball and observer are co-located in XY, their
   displacement vector has zero magnitude and therefore no physical bearing. Feeding that zero
   vector through the ordinary bearing calculation invents a world-axis direction. In the
   zero-displacement case only, the FoV angle gate is satisfied; range and shadow-cone occlusion
   remain active. **ERR-007-004.**
...
bool ballOccluded = ballInFoV &&
    CheckOcclusionForEntity(BallState.Position, nearbyOpponents)
