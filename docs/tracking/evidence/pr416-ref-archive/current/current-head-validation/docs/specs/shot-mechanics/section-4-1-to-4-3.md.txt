...
| FM-03 | Agent loses possession after shot initiation but before `ApplyKick()` at CONTACT | Possession re-check immediately before `ApplyKick()` at CONTACT | Log warning. Transition to CANCELLED; do not call `ApplyKick()`; do not publish `ShotCancelledEvent`. Under active W2 this can be normal gameplay when a tackle lands after the final WINDUP interrupt poll or while the executor is already in CONTACT. **ERR-006-005.** |
