...
namespace TacticalDirector.ShotMechanics
{
    ...
    public sealed class ShotExecutor
    {
        ...
        private void ExecuteContact(float matchTime, int frameNumber, ref BallState ball)
        {
            ...
            // ERR-006-005 / FM-03: re-check possession immediately before ApplyKick — §4.2.4.
            // A W2 tackle can legitimately land after the final WINDUP interrupt poll or while the
            // executor is already in CONTACT. Cancellation is therefore expected gameplay, not an
            // interrupt-routing failure. ShotCancelledEvent remains WINDUP-tackle-only (§4.7.1).
...
// | 1.11    | 2026-09-17 | —      | ERR-006-005: CONTACT-time possession loss remains Cancelled/no kick, but logs |
// |         |            |        | Warning because active W2 can legitimately tackle after the last WINDUP poll. |
