...
namespace TacticalDirector.MatchEngine
{
    ...
    internal static class MatchEngineInPossGateScenarios
    {
        ...
        // PR #416 corrected-baseline capture 35286928656 on production head a7f2b77:
        // floor(0.80 * 15,830) = 12,664; floor(0.80 * 16,423) = 13,138.
        // These are population-stability guards, not football-realism targets.
        private static readonly int[] MinFinalThirdSamples =
        {
            12_664,
            13_138,
        };
        ...
        private static void RunInPossGate(ScenarioContext context)
        {
            ...
            for (int s = 0; s < Seeds.Length; s++)
            {
                ...
                int minSamples = MinFinalThirdSamples[s];
                context.Envelope.CheckTrue(
                    "final-third-phase-samples-non-vacuous-" + seedLabel,
                    samples >= minSamples,
                    "seed=" + seedLabel + " samples=" +
                    samples.ToString(CultureInfo.InvariantCulture) +
                    " (floor " + minSamples.ToString(CultureInfo.InvariantCulture) +
                ...
                // Retain the observed count/share line as a diagnostic against the now-frozen
                // per-seed population floors.
                Console.WriteLine(
