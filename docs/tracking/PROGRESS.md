...
**Last Updated:** May 31, 2026 (Stage 1 wiring AR-4 complete: 3 findings (2H+0M+1L) all fixed. H-1: EventLedger.CompareKey removed EventTypeOrdinal from sort key — it violated FM-017-002 canonical order (ProducingPhaseIndex, SubsystemOrdinal, EntityId, IntraPhaseDrawIndex); type ordinal was inserted as 4th key, corrupting dispatch order for same-(phase,subsystem,entity) multi-type publishes. H-2: EventLedger.SerializeLedger now sorts Tier A/B slot indices by FM-017-002 key before emitting — was using insertion (publication) order, making digest bytes publication-order-dependent and contradicting the EventBus XML doc claim "FM-017-002 order". L-1: structSize<=0 fallback in EventBus.PublishAuthoritative promoted from silent Unsafe.SizeOf<T>() estimate to unconditional throw — fallback was dead code (ordinal≠0 guarantees type went through RegisterRow<T> which always sets structSize>0). Files: EventLedger.cs v1.3, EventBus.cs v1.4. Prior May 30: AR-3 complete: 3 findings (0H+1M+2L) all fixed. M: CosmeticChannel added upper-bound guard structSize > MaxEventSlotBytes (prevents ArgumentOutOfRangeException crash on oversized Tier C struct); L: structSize <= 0 guard changed from silent return to throw (silent data loss worse than exception); L: EventRegistry.IsRegistered now requires StructSize > 0 so placeholder RegisterRowRaw rows return false until Initialize() is called (misleading predicate fix). Files: CosmeticChannel.cs v1.5, EventRegistry.cs v1.3. AR-3 otherwise clean. Prior same day: AR-2 complete: 9 findings (3H+3M+3L) all fixed. H: MaxEventSlotBytes 128→160 (HeaderExecutedEvent/SaveAttemptedEvent/DecisionMadeEvent struct overflows); M: Debug.Assert hot-path alloc promoted to unconditional if/throw in EventBus + CosmeticChannel (FR-EVT-048); GoalkeeperRushEvent maxPerTick 2→8 (InFlight fires each 60Hz frame); int? TargetReceiverId → int sentinel -1 in DistributionExecutedEvent (determinism); L: raw int literals replaced with SubsystemOrdinals/PhaseId typed constants in all 6 EventBusRegistrar files; subscribe-before-init guard added to EventBus.Subscribe<IEventA/B>; decision-tree.asmdef: added HeadingMechanics + GoalkeeperMechanics refs. Files: EventSystemConstants.cs v1.1, EventBus.cs v1.3, CosmeticChannel.cs v1.4, EventRegistry.cs v1.2, DistributionExecutedEvent.cs v1.2, all 6 EventBusRegistrar.cs v1.1, decision-tree.asmdef. AR-1 prior same day: 3H fixed. Prior same day: Stage 1 kick-off wiring batch (32 files). Event System #17 coded — 20 files (18 .cs + 2 asmdef), AR-1: 3H+3M+2L; AR-2: 1L; AR-3 clean.)  
...
| 8 | Decision Tree | ~55+ | 🔒 LOCKED | Feb 27 | Apr 27, 2026 | Sections 1–9 and appendices drafted; §9 v1.2 lead-developer signed off Apr 27, 2026 at draft-level quality gate. Comprehensive audit candidate before implementation. **Implementation complete May 29, 2026** at `src/decision-tree/` (36 files: 35 .cs + 1 asmdef; AR-1: 2H+3M+4L all fixed; AR-2: full sweep clean). |
...
| # | Specification | Pages | Status | Started | Completed | Notes |
|---|---------------|-------|--------|---------|-----------|-------|
| 10 | Heading Mechanics | 15-18 | ✅ APPROVED | May 16, 2026 | May 16, 2026 | Section files v0.1 → v0.3 same day; outline PASS-1 + section-files PASS-1 cycles complete; ERR-010-001 closed via #16 §3.5 v1.0.2 patch landing `DOMAIN_TAG_HEADING = 0x16`. **Implementation complete May 28, 2026** at `src/heading-mechanics/` (25 files: 24 .cs + 1 asmdef; AR-1 + AR-2 adversarial review clean). |
| 11 | Goalkeeper Mechanics | 20-25 | ✅ APPROVED | May 16, 2026 | May 18, 2026 | Section files v0.1 (May 16) → v0.2 (May 18 APPROVED); ERR-011-001 resolved: `DOMAIN_TAG_GOALKEEPER = 0x1D` (#16 §3.4 v1.0.5); R-01..R-05 signed May 18, 2026. **Implementation complete May 28, 2026** at `src/goalkeeper-mechanics/` (36 files: 35 .cs + 1 asmdef; AR-1: 5H+1M fixed; AR-2: 2M fixed; AR-3 clean). |
...
| 15 | Attacking AI | 18-22 | ✅ APPROVED | May 17, 2026 | May 18, 2026 | Section files v0.1 (May 17) → v0.2 (May 18 APPROVED); ERR-015-001/002/005 back-props closed; R-01..R-05 signed May 18, 2026. **Implementation complete May 29, 2026** at `src/attacking-ai/` (24 files: 23 .cs + 1 asmdef; AR-1: 2H+4M fixed; AR-2: 2L fixed; AR-3 clean). |
| 16 | Deterministic Simulation | 20-25 | 🔒 LOCKED | May 2, 2026 | May 14, 2026 | All 9 sections + appendices complete; §9 v1.7 Tier 2 APPROVED May 14, 2026. Tier 1 (Conditional) granted earlier same day; Tier 2 cleared all §9.4.2 gates: §9.5 #4(a)/(b)/(c) spec-level SATISFIED (golden-vector files hkdf-sha256-kat v1.1, siphash-2-4-kat v1.1, serialize-canonical-corpus v1.0); §8.3.1 cross-spec re-audit complete (§8 v1.2; all four rows promoted); ERR-017-001 closed atomically via `DOMAIN_TAG_EVENT_LEDGER = 0x15` allocation in §3.4 v1.0.1; §9.3 sign-offs (lead-developer Tier 2, QA-automation, platform-certification) granted with Stage-0 host-platform-pin caveat. **Implementation complete May 29, 2026** at `src/deterministic-sim/` (23 files: 21 .cs + 2 asmdef; AR-1: 4H+4M fixed; AR-2: 1L fixed; AR-3: 1L fixed; AR-3 clean). |
...
| # | Specification | Pages | Status | Started | Completed | Notes |
|---|---------------|-------|--------|---------|-----------|-------|
| 17 | Event System | ~35 | 🔒 LOCKED | May 13, 2026 | May 13, 2026 | All 10 section files + appendices at v0.3; section-files PASS 1+2 adversarial reviews resolved; lead-developer sign-off May 13, 2026. ERR-017-001 fully resolved May 15, 2026. **Implementation complete May 30, 2026** at `src/event-system/` (20 files: 18 .cs + 2 asmdef; AR-1: 3H+3M+2L fixed; AR-2: 1L fixed; AR-3 clean). Key: ring buffer + BFS dispatch + Appendix A event structs + Tier C immediate dispatch. **Stage 1 wiring complete May 30, 2026**: EventRegistry rows 0x0C–0x17 seeded; 6 consuming specs wired via EventBusRegistrar.cs. **Stage 1 wiring AR-1 complete May 30, 2026**: 5 findings (3H+1M+1L); 3H fixed — zero-ordinal guard added to EventBus.PublishAuthoritative, CosmeticChannel.Publish, CosmeticChannel.Subscribe (EventBus.cs v1.2, CosmeticChannel.cs v1.3); 1M open (IsRegistered inconsistency), 1L open (dead structSize guard). AR-2 in progress. |
...
- **Pressing AI (#13) initial implementation (May 29, 2026):** 21 files (20 .cs + 1 asmdef) authored at `src/pressing-ai/`. AR-1 adversarial review: 3H+1M+1L all fixed (H-1: `IsActive` field added to `PressingAgentSnapshot` + guards in 6 files; H-2: unit mismatch `TriggerEvaluator` — `len` vs `len²` vs `SpacingEpsilonM2`; H-3: `PrimaryPressSelector` eligibility uses ball-carrier not interception point per §3.3/#8 §3.1.8.2; M-1: missing `using TacticalDirector.PositioningAI` in `PressingAITick`; L-1: `PressTrigger` doc comment stale method name). AR-2 full sweep clean.
- **Defensive AI (#14) initial implementation (May 29, 2026):** 19 files (18 .cs + 1 asmdef) authored at `src/defensive-ai/`. AR-1 adversarial review: 2H+1M all fixed (H-1: SQUAD_SIZE literal→PressingAIConstants mirror; H-2: ValidThroughTick staleness in dwell-lock path; M-1: GkEntityId<0 guard in LastManDetector). AR-2 full sweep clean.
- **Attacking AI (#15) initial implementation (May 29, 2026):** 24 files (23 .cs + 1 asmdef) authored at `src/attacking-ai/`. AR-1 adversarial review: 2H+4M all fixed (H-1: MinEffectiveRadiusM moved from SupportHeuristic local const to catalogue; H-2: magic run-depth/lateral literals promoted to MinRunDepthM/MaxRunDepthM/MaxLateralOffsetM constants; M-1: AttackHysteresis.Update CandidateDwell reset on current-role re-preference; M-2: WidthHolder near-side WeakSide re-promotion guard; M-3: Math.Round(double)→Mathf.RoundToInt; M-4: dead firstLossThisTick removed). AR-2: 2L doc fixes. AR-3: AttackAngleEpsilon (0.01f) extracted to catalogue — clean.

**Status:**
- **Specification phase:** ✅ COMPLETE (20/20 specs approved).
- **Implementation phase:** Specs #1–#8 + #10 + #11 + #12 + #13 + #14 + #15 all have initial implementations and adversarial review passes. Last coded + AR-clean: Attacking AI (#15). Next: remaining Mechanics layer specs or cross-cutting infra.
...
| Pressing AI #13 Implementation | — | ✅ COMPLETE | May 29, 2026 |
| Defensive AI #14 Implementation | — | ✅ COMPLETE | May 29, 2026 |
| Attacking AI #15 Implementation | — | ✅ COMPLETE | May 29, 2026 |
...
1. Implementing remaining specs: #9 (Fixed64 — Stage 5+ stub only), #16 (Deterministic Sim), #17 (Event System), #18 (Perf Optimization), #19 (Testing Strategy), #20 (Code Standards governance). Specs #1–#8 + #10–#15 all complete.
2. (Previously #1) Verify all AR findings resolved in Defensive AI (#14) implementation — ✅ DONE.
...
**May 30, 2026:**
- Stage 1 wiring AR-1 adversarial review complete: 5 findings (3H+1M+1L). 3H fixed in EventBus.cs (v1.2) and CosmeticChannel.cs (v1.3) — zero-ordinal guard added to PublishAuthoritative, CosmeticChannel.Publish, CosmeticChannel.Subscribe (debug builds only; FR-EVT-048 preserved). 1M open (IsRegistered true for placeholder rows while ordinal cache is 0). 1L open (dead structSize guard in CosmeticChannel.Publish).
