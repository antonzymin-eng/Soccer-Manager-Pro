...
**Last Updated:** August 17, 2026 — **Owner decision pass over the seven standing decisions: two shipped, five held, one premise found false. Documentation only — **no `src/` code changed and no `.asmdef` moved** (the one file touched under `src/` is `src/CLAUDE.md`, the coding guide), so no gate run is owed and no suite count moves.** Files changed (**17**, this manifest included): `CLAUDE.md` (OPEN ISSUES counts 20/44 → **18/46**, both re-derived by direct count; two resolved bullets removed; the tags bullet re-titled; the `src/CLAUDE.md` pointer at line 92 now states the adopted ten-tier order plus its two easy-to-miss rules); `src/CLAUDE.md` (`### Assembly Layer Taxonomy` + `### Reference Direction` rewritten over the ten-tier order — the ⚠️ 16-unlisted staleness banner retired, the `code-standards` phantom infrastructure row struck, `project-constants` restated as tier 0 rather than infrastructure); `docs/specs/code-standards/section-2.md` **v1.0.1 → v1.1** (FR-CS-046 restated against the tier order, its published double negative repaired, **FR-CS-046a** registered as a sub-clause — FR span and 73-FR count unchanged; header `DRAFT` → `APPROVED`); `docs/specs/code-standards/section-3.md` **v1.0.1 → v1.1** (§3.5.2 replaced with the ten-tier table, labelled arrows, the tier-is-a-ceiling rule, the test-assembly exclusion and the adoption verification record; header `Version 1.0 / DRAFT` → `v1.1 / APPROVED`); `docs/specs/code-standards/section-5.md` **v1.0.1 → v1.0.2** (§5.4.5 checklist item 1; header `DRAFT` → `APPROVED`); `docs/specs/season-competition-loop/section-3.md` **v2.11 → v2.12** (both §3.4 open owner questions closed inline — the eighteen-player trigger kept and `ERR-030-044`'s open half closed; beyond-cap degrade-to-greedy kept and fixture-refusal rejected); `docs/tracking/spec-error-log.md` **v2.42 → v2.43** (`ERR-020-002` and `ERR-020-003` both ✅ RESOLVED, index rows and bodies, plus a new "Adoption" section recording the four new placements, the re-verification, the two added rules, the files changed and the deliberately-unswept outline files); `docs/tracking/injury-aging-research-alignment-design.md` **v0.4 → v0.5** (SIGNED OFF; §11 steps 1–2 marked complete); `docs/tracking/league-bootstrap-design.md` **v1.4 → v1.5** (KD-7a **S9** hold; S7 condition 4 corrected to post-*arming*; the rowless-edit gap since July 25 recorded rather than retro-rowed); `docs/tracking/gk-conversion-at-contact-design.md` **v1.0 → v1.1** (**KD-CC6a** — `pointQuality` parked until W1's rush geometry is measured); `docs/tracking/foul-discipline-balance-design.md` **v1.0 → v1.1** (§7 item **2a** — the tripwire recorded as FIRED, the August-13 re-measurement, the hold and its accepted cost); `docs/tracking/match-engine-wiring-backlog.md` **v1.9 → v1.10** (§5 note: W2's *arming* gates three separate held decisions, so W4 → W12 → W6 unblocks three decisions rather than one subsystem). Plus `docs/tracking/open-issues.md` (seven advisories → owner decisions; two entries moved out; the tags entry rewritten with the verified findings and the re-creation recipe), `docs/tracking/open-issues-resolved.md` (**+2**: the assembly taxonomy and the injury/aging supplement, each re-titled to lead with its resolution per this file's convention), `docs/tracking/CHANGELOG.md` and `docs/tracking/CHANGELOG-src.md` **v2.123 → v2.124**. **The taxonomy adoption's verification, re-run against the live `.asmdef` graph rather than carried over from the August-2 draft: 35/35 assembly folders placed, 0 upward references, 105 downward, 38 intra-tier, graph acyclic.** **No file was created or deleted.**
