...
| `src/performance-optimization/TraceChannelDescriptor.cs` | F.0 sealed descriptor (11 fields); constructor enforces SamplingRule=PerNTicks ⇒ SampleN>0 invariant per AR-1 L-1. Extracted from TraceChannel.cs in AR-1 H-1 |
...
| `src/performance-optimization/RegressionGate.cs` | v1.1 — static class: PassesPerPrCheck(baselineMs, currentMs) / PassesAbsoluteDriftCheck(milestoneMs, currentMs) / Evaluate(baseline, current, milestoneMs) → RegressionResult; implements FR-PO-031 §3.5.2 + §3.5.6. AR-1 M-1 Evaluate now reuses helpers; AR-1 M-2 degenerate (≤0/NaN) baseline or milestone fails closed (float.NaN milestone is the explicit skip-drift signal) |
