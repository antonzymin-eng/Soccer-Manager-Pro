...
**Last Updated:** May 17, 2026 (Pressing AI #13 v0.2 PASS-1 adversarial-review fix pass complete; all 17 findings resolved (6 H / 7 M / 4 L); `SPEC_INDEX.md` row 13 flipped `NOT STARTED → IN REVIEW`; ERR-013-007 / ERR-013-008 filed in `spec-error-log.md` v1.18)  
...
| 13 | `docs/specs/pressing-ai/` | IN REVIEW (May 17, 2026) — Section files v0.2 / PASS-1 adversarial-review fix pass complete. All 17 findings resolved (6 H / 7 M / 4 L). Key fixes: AR-S1-H1 (`#8 §1.4.21` → `#8 §1.3.2`); AR-S1-H2 (`FR-08` → `FR-10`; `passVelocity` → position-derived direction); AR-S1-H3 (fatigue inversion: `Stamina ≤` → `Fatigue <`); AR-S1-H4 (GetPhase / GetLine staged to Stage 1+ via ERR-013-007 / ERR-013-008); AR-S1-H5 (FR-PR-023 category error + `r.perceivedPressure` → `geometricPressureOn(r)`); AR-S1-H6 (`THREAT_PRESSURE_NORMALIZER` added). ERR-013-007 / ERR-013-008 filed in `spec-error-log.md` v1.18. `DOMAIN_TAG_PRESSING_AI = 0x19 [CROSS-PENDING]`. Outstanding (gates `IN REVIEW → APPROVED`): OI-001 (ERR-013-001 mechanism); OI-003 (Appendix A `[EST]` derivations); OI-004 (R-01..R-05 sign-off); OI-006 (domain-tag allocation); OI-008 / OI-009 (#12 back-prods); OI-010 (#19 prefix conformance grep). |
