...
| `src/performance-optimization/SessionManifest.cs` | v1.1 — sealed class: all §3.3.2 required fields (GitSha / Seed / EnvironmentFingerprint #16 §4.8 / PlatformPin / ScenarioManifestId / SessionStartUtc / SessionEndUtc / HardwareCounters / HarnessVersion); `IsComplete()` validator used by §3.4.4 baseline validator (AR-2 L-1: also validates HardwareCounters.CpuModel/ThermalState — default-struct slip-through closed) |
