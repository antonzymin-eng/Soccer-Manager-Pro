...
| `src/season-save/SeasonSaveConstants.cs` | `[FIXED] SEASON_SAVE_FORMAT_VERSION = 4` — the outermost format version (KD-4; 1→2 season sub-blob at #30 T1, 2→3 training/medical at #29/#41 T1, 3→4 appearance at the balance pass); + `APPEARANCE_SAVE_MAGIC` ("APPR") / `APPEARANCE_SAVE_FORMAT_VERSION` (ERR-029-005 magic-before-version) + `[FIXED] APPEARANCE_BITMASK_MAX_WINDOW_DAYS = 31` (AR pass 5 L4) |
| `src/season-save/SeasonSaveBlobs.cs` | Deframe result: `WorldBlob` + `SeasonBlob` + `TrainingBlob` + `MedicalBlob` + `AppearanceBlob` (all always) + `MatchBlob` (null if no in-progress match) — six opaque byte sub-blobs (KD-2/KD-3, FR-SN-019, FR-TR-018, FR-MD-017, ERR-041-010(b)) |
...
| `src/season-save/SeasonSaveCodec.cs` | Pure static frame codec: `Encode(worldBlob, seasonBlob, in TrainingBlock, in MedicalBlock, in AppearanceBlock, matchBlobOrNull)` / `Decode(byte[]) → SeasonSaveBlobs` — a SEASON_SAVE_FORMAT_VERSION-gated frame (v4) + matchPresent flag + six length-prefixed opaque sub-blobs, five mandatory and the match optional (each keeps its own version gate); overflow-safe `Require` bound + fail-loud on null/version/flag/length/trailing (KD-7/KD-8) |
