...
**`Discipline.Tests`: 118 passed / 0 failed (at landing: 83), four mutants killed.** `SeasonLoopDisciplineTests.cs`
(in `src/season-save/tests/`, see below) adds +14 wiring locks. Full account:
`docs/tracking/spec-error-log.md` v2.17.
...
| `tests/SeasonLoopDisciplineTests.cs` | **New.** +14 wiring locks, every one failing if its wiring point is reverted, incl. a real 90-minute engine fixture pairing observer-neutrality with a positive control (#44 §5's T-DC-NEU-001 as written is green if the fold never runs at all — ERR-030-014's shape one layer up) |
| `PlayerCareerStates.cs` | **Modified.** `SelectAvailable` split into `MarkUnavailable` (a removal SET) + the new `AvailabilityComposition` |
