...
| `src/match-engine/tests/ShotOutcomeDiagnosticTests.cs` | §5.Z.18 shot-outcome-distribution diagnostic (env-gated `TD_SHOT_DIAGNOSTIC=1`, assertion-free). Reports per full match: shots (keeper shot-notification rising edges), goals and goals/shot, on-target goal-mouth crossings, off-target exits, fast-ball body contacts against the KD-6 deflection gate, and the shot-tick ball-speed range. Measured the pre-fix baseline (15.3 goals/match, 0 fast contacts) and the post-fix distribution (12.3, 560–612) |
