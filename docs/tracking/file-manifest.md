...
**Last Updated (prior):** August 8, 2026, **#28 Player Progression T1 + T2a (roadmap D1 part one) — `spec-error-log.md` v1.97**. **New (10 files + their `.meta` sidecars):** `src/player-progression/ProgressionEngine.cs` v1.0 (the KD-7 sole writer — `SeedFrom` / `FromBlocks`, the FR-PG-021 batch `AdvanceDay`, `Snapshot`/`Restore`, the KD-4 `SquadFor` projection, the FR-PG-023 `LifecycleView`, and the ERR-041-019 global-id guard); `src/player-progression/ProgressionSaveCodec.cs` v1.0 (the §3.5 block, magic-led per ERR-028-004); `src/player-progression/ClubCareerStates.cs` v1.0 (per-club unit, keyed by `PlayerRecord.PlayerId` with no parallel id array); `src/player-progression/TrainingInputBatch.cs` v1.0 (+`ClubTrainingInputs`); `src/player-progression/LifecycleViewModel.cs` v1.0; `src/player-progression/tests/ProgressionEngineTests.cs` v1.0; `src/season-save/ProgressionBlock.cs` v1.0 (typed frame handle); `src/season-save/ProgressionSquads.cs` v1.0 (the `ISquadProvider` projection — owned here because `ISquadProvider` is a `match-engine` type #28 §4.1 forbids #28 to reference); `src/season-save/tests/SeasonLoopProgressionTests.cs` v1.0 (the slot-1 wiring proof). **Modified:** `src/player-progression/PlayerLifecycle.cs` v1.1, `PlayerProgressionConstants.cs` v1.1, `RegenGenerator.cs` v1.3; `src/season-save/SeasonLoop.cs` v1.7, `SeasonSaveCodec.cs` v1.6, `SeasonSaveBlobs.cs` v1.4, `SeasonSaveContents.cs` v1.5, `SeasonSaveConstants.cs` (`SEASON_SAVE_FORMAT_VERSION` 4 → 5), `SeasonSaveManager.cs`, `PlayerCareerStates.cs` (+`GatherTrainingInputs`), `season-save.asmdef` (+`TacticalDirector.PlayerProgression`); the three season-save suites' call sites; `tests/LeagueBootstrapTests.cs` (the retired-rule test NARROWED, not deleted). **Docs:** #28 §2/§3/§5, #30 §2/§3/appendices, `spec-error-log.md` v1.97 (ERR-029-006 resolved; ERR-028-003/-004/-005 + ERR-030-030 filed and resolved), `path-to-playable-roadmap.md` (D1 ◑), root `CLAUDE.md`, `CHANGELOG-src.md` v3.00.
