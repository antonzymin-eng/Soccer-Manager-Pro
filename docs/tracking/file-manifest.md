...
**Last Updated:** May 14, 2026 (Performance Optimization #18 section files v0.3 — PASS-2 adversarial review `ERR-018-012` … `ERR-018-018` resolved; status remains `IN REVIEW`)  
...
| 16 | `docs/specs/deterministic-sim/` | IN REVIEW (May 14, 2026) — Tier 1 Conditional Approval; lead-developer sign-off granted. Section-9 v1.5 records §9.5 #4(c) `serialize-canonical-corpus.md` v1.0 authored (45 entries; covers every §3.2.4.1 type kind + all five domain-tagged preimages; embedded reproducer Python script per CLAUDE.md no-fabricated-values rule); §9.5 #4 checkbox now ticked (all three spec-level sub-conditions SATISFIED). Tier 2 (`APPROVED`) remaining gates: (i) §8.3.1 audit-row re-audit; (ii) §9.3 review-checklist sign-offs. `golden-vectors/` now contains `hkdf-sha256-kat.md` v1.1, `siphash-2-4-kat.md` v1.1, `serialize-canonical-corpus.md` v1.0. |
