...
**Last Updated:** August 10, 2026 — **The shot-lane regression, bisected — doc sync.** No production code changed. `close-chance-creation-design.md` → **v1.6** (new §11: the 18-seed × 6-tree bisect separating the Acceptance-3 shot-lane regression, refuted as a mechanism, from the live C1/`ERR-012-011` regression, confirmed and quantified — `−0.189 ± 0.038, t = −5.05, 16/18 seeds` — that is actually failing `sim_match_engine_close_chance`), `spec-error-log.md` → **v1.100** (ERR-008-021 retitled (a)/(b) for a double-allocated id; the missing ERR-008-023 body entry written), `src/match-engine/tests/MatchEngineCloseChanceScenarios.cs` → **v1.2** (comments only — records the bisect; no predicate/bound/seed change). Those three landed together in a prior session on this branch; **this entry syncs every other tracking document they touch:** `match-engine-wiring-backlog.md` → **v1.4** (C1's close-chance cost quantified in the C1 entry + a new version-history row), `open-issues.md` (both the wiring-backlog/C1 entry and the close-chance-creation entry amended in place — neither opened nor closed), root `CLAUDE.md` (the two matching OPEN ISSUES index lines amended, index-length), `CHANGELOG.md` (new head entry) and `CHANGELOG-src.md` (new head entry + VERSION HISTORY row 2.104), this manifest. No `SNAPSHOT_SCHEMA_VERSION` change, no new RNG stream / domain tag / draw site / draw-order change — nothing here executes differently. Full gate not re-run (doc-only); the .NET 8 SDK installs cleanly from the Ubuntu archive in this environment per `src/CLAUDE.md`, unrelated to this pass.
...
| `docs/tracking/close-chance-creation-design.md` | Design supplement (**v1.6**, Aug 10, 2026 — **LANDED**) — the close-chance-creation pass (§5.Z.24): the premise check whose FIRST premise survived (312 raw final-third crossings vs 311 dwell-filtered episodes, so the 6.5% denominator was never chatter), the two measured halves (mean attackers in the penalty area **0.11** with the deepest composed target slot at **22.8 m**, and the average final-third dribble pointing AWAY from goal at cosine **−0.302**), the `ERR-008-018` back-prop, KD-CC1..CC8, the #15 §4.5.2 run overlay REFUSED, §10's post-C1 re-measurement (box geometry / aerial-reception bounds ahead of the #8 pass-to-a-place lever), and **§11 (Aug 10, 2026): the 18-seed × 6-tree shot-lane bisect** — the Acceptance-3 regression is a resample (`−0.027 ± 0.039`, refuting both named suspects), and the live `sim_match_engine_close_chance` failure is C1/`ERR-012-011` (`−0.189 ± 0.038, t = −5.05, 16/18 seeds`) |
| `docs/tracking/match-engine-wiring-backlog.md` | **AUDIT** (**v1.4**, Aug 10, 2026) — the built-but-unwired inventory for the match engine and the 18 assemblies it composes. Three-pass method (comment sweep → whole-tree production-caller count → manual triage) and its stated blind spot (it sees method-level dormancy, never gate-level). **W1 and C1 wired**; 9 built subsystems still have no production caller. C1's close-chance cost is now **quantified**: `−0.189 ± 0.038, t = −5.05, 16/18 seeds` over the 18-seed bisect in `close-chance-creation-design.md` §11 — a real, directional regression with a named mechanism (the `InPoss` `PullFactor` column), not a bound-tuning question; it is the confirmed live cause of `sim_match_engine_close_chance` failing. Establishes **KD-W1** (no `[GT]` landing for an unwired subsystem) and **KD-W2** (match engine only; the 22 assembly-less specs stay with `path-to-playable-roadmap.md`), and proposes the wire-order → calibrate-once sequence |
