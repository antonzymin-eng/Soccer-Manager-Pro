...
### `src/testing-strategy/` — Spec #19 (25 files: 23 .cs + 2 asmdef)
...
| `src/testing-strategy/TestingStrategyConstants.cs` | v1.3 — §3.10 governance constant catalogue (pyramid bounds, coverage thresholds, flake windows, pre-commit budget) + SCENARIO_MANIFEST_FORMAT_VERSION (FR-TS-070) |
...
| `src/testing-strategy/ScenarioIndex.cs` | Immutable in-memory root manifest (ScenarioIndexEntry: path + manifest + scenario); duplicate paths rejected; the runner refuses unindexed scenarios (§3.3.6 / FR-TS-028) |
| `src/testing-strategy/ScenarioRunner.cs` | §3.3.3 single entry point Run(manifestPath, seed): index resolution + load-time validation (A.1 fields, FR-TS-070 format version) → delegates to IScenario.Run; Stage 0 index injected in code, Stage 0+1 adds the index.<ext> file loader (D1) |
...
| `src/testing-strategy/Tests/ScenarioRunnerTests.cs` | ScenarioRunner contract tests: index refusal, format-version rejection, kebab-case validation, implicit-pass rejection, failure diagnostics, NaN in_range, exception capture, seed plumbing (KD-7), per-invocation hermeticity |
