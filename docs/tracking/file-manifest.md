...
**Last Updated:** September 3, 2026 — **A3.1b post-merge review findings on PR #351 resolved.**
Modified: Code Standards `outline.md`, `outline-mid.md`, `outline-detailed.md`, `section-1.md`, `section-5.md`, `docs/tracking/CHANGELOG.md`, and this manifest. KD-4 is restated in the authoritative `section-1.md` (v1.2) and matched across all three outline tiers; §5.5's FR-CS-074 row names the A4 canonical-selector/identity resolver again alongside the owner/point/state resolver; the last two `future `src/CLAUDE.md`` residues are swept. The maintained `section-5.md` pointer in the frozen header chain below moves **v1.6 → v1.7**, and the outline-tier inventory row is refreshed — it had gone stale for `outline-detailed.md`, `section-5.md` and `section-8.md` without the consistency check reporting it. `SPEC_INDEX.md` remains untouched pending A3.4. No production code, schema, executable semantics, workflow, or enforcement changed.
