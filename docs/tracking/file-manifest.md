...
| `docs/tracking/gk-rush-trigger-design.md` | Design supplement (v1.0, Aug 4, 2026 — **LANDED**) — wiring backlog **W1**, the keeper rush trigger: why the rush cannot be a Decision Tree action (`ActionType` ordinal 8 overflows the 3-bit composure-noise field — the W9 deferral reason), the last-man test and the intercept-race solve behind `RushArmed`, the `SaveArmed` priority exclusion, the no-new-engine-state latch argument, the `ERR-011-009` back-prop (a rush that reached its target had no §3.1.1 exit), the five un-calibrated `[GT]`s, and the **measurement that was not run** (no .NET SDK in the authoring environment) |
