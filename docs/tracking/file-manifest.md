...
| `tools/assembly-tier-check.py` | Mechanical guard for the Spec #20 §3.5.2 ten-tier assembly order (FR-CS-046 / FR-CS-046a / FR-CS-046b; cited as the enforcement in `code-standards/section-5.md` §5.4.5 and `section-3.md` §3.5.2). Parses the tier table OUT OF `section-3.md` §3.5.2 rather than hard-coding a copy (a duplicate table would be a second surface to keep in sync — the recurring defect class this repo keeps filing); enumerates every production `src/<folder>/*.asmdef` and fails on: a production folder absent from the table, a table entry naming a missing folder, a folder seated twice, any upward reference, any ordered-tier reference into Infrastructure, any Infrastructure-sourced reference off tier 0, and any cycle. Prints the recomputed downward/intra-tier/upward reference breakdown so §3.5.2's adoption verification re-runs on every invocation. `python3 tools/assembly-tier-check.py --repo .`; exit 0 pass / 1 fail / 2 unparseable. Created August 17, 2026; inventoried August 18, 2026 (reviewed-findings pass H12 — it landed with no inventory row anywhere). v1.3 (August 18, 2026, checker-tools pass): the ten numbered tier rows are asserted against FR-CS-046's stated sequence, parsed from `section-2.md` (swapping two tier numbers previously PASSED); production→test references and unresolvable reference names now FAIL (both were previously ignored as "external"); the docstring's CI claim corrected to `ci.yml`'s real triggers — pushes to `main` and PRs into `main`, NOT "every push" (⚠️ corrected August 18, 2026, reviewed-findings pass — this row told an owner to fix `docs/specs/code-standards/section-3.md` §3.5.2's matching sentence, but `git show 3c9dc67 --stat` shows that file was corrected in the SAME commit that wrote this row; §3.5.2 now reads "on every push to `main` and every pull request targeting `main`" with its own dated correction note, so nothing here remains for its owner to do). |
...
| `tools/doc-claim-check.py` | **Executable-claim checker** (CREATED August 18, 2026, AR round 8) — closes the defect class rounds 6-8 kept finding and review alone could not stop finding: **verification prose is never itself verified.** Two checks, both oracle-free. (1) It EXECUTES the verification commands the documents quote and diffs the stated value: root `CLAUDE.md` cited a grep that refutes its own claim, §9.2 Q-04 ratified "three IDs" against six, §9.1 C-02's value broke inside the commit claiming to re-run every checklist command, `ERR-020-006` offered a live command as proof of a historical 218. (2) It resolves `Type.MEMBER` references inside **csharp** spec fences against the same file's own declarations — the class that let `ERR-020-001`'s rename dangle in Appendix C for three months and let AR round 7's own rename dangle one section below the line it fixed. Commands are untrusted document text and `ci.yml` runs this on `pull_request`, so that text reaches a CI runner: allow-listed read-only binaries, `git` restricted to read-only subcommands, no shell, redirection/substitution/chaining refused. Scope is deliberately floored at single-integer output and same-file identifier resolution, over both claim shapes this repo writes — command-then-value ("`cmd` → 18") and value-first (the count stated before the command that checks it — the form Spec #20 §5.4.5 states the `src/` assembly count in; added at round 12, and deliberately DESCRIBED rather than quoted here, since restating a live claim on a second surface is how one figure becomes two that drift apart); **every declined claim is counted AND named**, because rounds 5-6 both found this project's checkers hiding real defects behind silent skips. *(⚠️ Corrected August 19, 2026, adversarial-review round 9 — three of this row's own claims were false as written, each disproved by reproduction rather than by reading. **(1)** "allow-listed read-only binaries only" was not a safety property: the list gates `argv[0]` alone, and `sed -i` was demonstrated rewriting a file in the working tree while the tool printed PASS — `python3 -c`, `find -delete`, `sort -o`, `rg --pre`, `git -c` and `awk 'BEGIN{system()}'` were reachable the same way. The property now rests on the allow-list PLUS a named write/execute escape-hatch refusal per binary, with `sed` dropped outright because its write lives in its script rather than a flag. **(2)** "globs expanded in-process rather than by a shell" understated a defect: expansion ignored shell QUOTING, so `find . -name '*.md' \| wc -l` ran as `find . -name CLAUDE.md doc.md`, failed, and — exit status being unchecked — was reported as "document says 12; command returns 0" against a correct document. Quoted tokens are no longer expanded and a failed segment is declined, not compared. **(3)** "every declined claim is counted AND named" was false in the commit that wrote it: an unlisted BINARY was dropped by a third, silent `continue`; 9 live instances are now counted and named. v1.0 → **v1.1**.)* Wired into `ci.yml`'s `Spec hygiene checks` job under `if: always()`, which the two checkers beside it now also carry (round-8 tooling M3 — a failure of one was masking the others). **Dated-record model (round 13):** a claim inside an append-only region — the frozen header chain, `spec-error-log.md`'s body below `## Error Index`, the resolved archive — records what the command returned AT THE TIME, so a later mismatch there is EXCUSED rather than failed, counted and NAMED like every decline, and pierced by an explicit "now"/"currently"/"today". The regions are imported from `doc-consistency-check.py`'s `frozen_chain_span`/`record_regions`, not restated, so the two tools cannot disagree about which bytes are frozen. v1.3. |
