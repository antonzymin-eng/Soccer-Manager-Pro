...
**Last Updated:** August 12, 2026, later still again — **Wiring backlog W2 (tackles), STEP 1 of N: the tackle-intent census instrument, landed BEFORE any wiring.** W2's premise is that reading `DefensiveAITick.GetTackleIntentRequests()` unlocks a tackle. But #14's `TackleIntentEvaluator` aims its intent at each HOLD_SHAPE agent's **mark-assignment target**, gated only on `TackleEligibleRadiusM` (3 m) — possession is not one of its gates — while a tackle can only ever be attempted on the carrier. Nothing in this tree has measured how often the two coincide, so the wiring plan rested on an unmeasured premise; that is exactly how **C1** landed a real gate whose consumers were inert, and the backlog's own §1.1 books this firing-rate class of measurement as **W12**. **NEW (1 + meta):** `src/match-engine/tests/TackleIntentDiagnosticTests.cs` v1.0 (env-gated `TD_TACKLE_DIAGNOSTIC=1`, assertion-free per the ERR-030-014 convention; total / COMMIT / on-carrier / both / 2 m / 1 m separation bands over the §5.Z.20 seed corpus). **Modified:** `src/match-engine/MatchEngine.cs` → **v1.66** (`CensusTackleIntents` called once per team per AI stride, plus six `_woodworkStrikes`-class counters and the `TestOnly_TackleIntentCensus` accessor). **No behaviour change**: observation only, nothing reads the counters from a gameplay path. **No `SNAPSHOT_SCHEMA_VERSION` change** (not serialized — zero after a restore by design), **no RNG stream / domain tag / draw site / draw-order change, no `[GT]`** (KD-W1 holds). **Gate: NOT YET RUN on this tree** — the pre-change baseline run was still in `MatchEngine.Tests` when this was written, and building under a live test phase is the recorded binary-swap hazard (AR pass 10). Nothing here may be cited as compiled until it is.
