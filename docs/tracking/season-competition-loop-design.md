...
| 0.4 | 2026-08-07 | **The #29/#41 balance pass, #30 side**: ERR-030-027 pins where the round sits (§3.3.2 — the fixture day's own KD-2 slots run pre-round inside `AdvanceAndPlayNextRound`; the post-round re-run is a cursor no-op), closing ERR-030-026's deferred half without touching #41's step contract. ERR-041-010(b): the frame gains the mandatory `APPR` appearance sub-blob (`SEASON_SAVE_FORMAT_VERSION` 3 → 4); `SeasonLoop` records both clubs' fielded XIs per fixture through the new `SquadRating.StartingElevenPlayerIds`; `PlayerCareerStates` owns the third state set with the same copy/reconcile discipline. Appendix B v0.5 also repairs the T1 omission (training/medical blocks were never recorded in its frame sketch). |
