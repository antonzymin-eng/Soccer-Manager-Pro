...
**Timing rule for the two landings:** In **B**, schedule each policy's commit early enough that the #5 CONTACT occurs **strictly before** the old 360-frame keeper-possession ground-drop guard; include tactical dispatch and ms→frame rounding in the worst-case budget, measured against the actual serialized `_gkHoldTicks`/Resolve ordering rather than assuming the 10 Hz `_claimTick` started on the same frame. `Execute` accepting at 5.9 s is not a release. If a late/injected commit overruns the guard, the guard drops possession, #5 cancels at CONTACT, and no distribution event may publish; ordinary policy scheduling must never depend on this fallback. In **C**, hand control continues through windup and ends only at actual CONTACT/release. A CONTACT at exactly 480 frames after the precise hand claim is legal; if control survives to frame 481, the offence wins **before** any pass CONTACT, even if the intent was committed earlier. This ordering must be locked for both keeper ends and save/restore.
