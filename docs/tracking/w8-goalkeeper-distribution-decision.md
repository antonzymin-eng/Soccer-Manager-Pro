...
|---|---|---|---|
| 0.19 | 2026-09-26 | clock correction landing | Owner direction (September 26, 2026): land the ERR-011-017 clock correction as its own behaviour-changing landing before the rest of B, so its effect is measured separately on the frozen six seeds. The correction exposed that `HandsOnBall` has no exit but `Distributing`; the owner chose to land the approved #11 §3.8.3 possession-change teardown with it rather than accept a 6 s post-pass hand state. Producer, #5 request, DT suppression and the B schema bump stay in B. |
