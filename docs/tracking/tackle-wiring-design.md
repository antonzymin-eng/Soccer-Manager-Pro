...
8. **The two defensive clamps in `TackleOutcomeResolver` are structurally unreachable at the shipped
   `[GT]`s, and no test on the resolver's output can detect their removal.** Max raw `foulShare` is
   `Base + AggressionK` = 0.26 against a 0.95 ceiling; max `engage` is
   `Base + CommitmentK + ProximityK` = 0.55 against a uniform ceiling of 1. **Proven by mutation**:
   deleting both clamps left all twelve resolver locks green — and the first attempt at "fixing" the
   two tautological tests the review found was *itself* tautological for the same reason, which the
   mutation run is what caught. This is the `RushCommitFatiguePenaltyM` situation (W1 §7 item 6): a
   guard with no reachable input. `TheTwoDefensiveClampsAreStructurallyUnreachable_AndThisLocksThatFact`
   now asserts the *headroom* instead, so the moment a retune makes either guard live, the test fails
   and says a real lock is owed. **Do not read the clamps as tested.**

9. **`CleanShare` has a plateau, and it is a football-judgment §6 P1/P2 violation — RECORDED, NOT
   FIXED.** Against a maxed dribbler, every tackler at or below normalized 0.5 (raw attributes **1
   through 10**) has exactly zero chance of a clean win and is indistinguishable from every other. The
   fix is a no-plateau ramp — the ERR-008-019 half-width precedent — which changes a `[GT]`-governed
   shape and is therefore the calibration pass's business under KD-W1, not a wiring pass's. Locked by
   `CleanShareHasAPlateauAgainstAnEliteCarrier_RECORDED_NOT_FIXED` so it cannot widen unnoticed, and so
   the pass that fixes it has a failing test to delete.

10. **`HeadingMechanics.CancelIntent` is still uncalled, and W2 does NOT cover it.** The backlog's own
    Class-C table predicted it would become load-bearing here (*"a tackle should cancel a header"*).
    A challenge interrupts a pass and a shot windup but not a header. Explicitly not covered rather
    than silently skipped, which is what §5 constraint 8 asked for: a tackled agent mid-header-commit
    keeps his header. It needs an interrupt path #10 does not have, which is its own item.

11. **The reach test is height-blind.** `TryResolveTackles` builds its separation from `_ball.Position`
    x/y only, so a ball 3 m overhead but within reach laterally counts as reachable. Harmless while the
    challenge ships disabled and while aerial balls are unreceivable anyway (backlog C7), and wrong the
