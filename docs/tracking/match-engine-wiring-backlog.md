...
> **UPDATED September 22, 2026 (v1.25):** The frozen #435 source-complete post-W2 characterization has executed on exact landed `main` `c56e5e1a…` and is captured durably at `docs/tracking/evidence/foul-card-six-seed-20260922/`. Six full matches produced **50 fouls / 5 cautions / 1 straight red / 0 second-yellow dismissals**, with **42 collision calls + 8 W2 tackle calls**, **42 priced-candidate identity checks / 0 mismatches**, and both tackle/cooldown-bypass counters at zero. That zero does **not** disposition the cooldown-bypass owner decision because the corpus contains only eight tackle fouls. This is characterization, not §5 final-calibration acceptance; no gameplay `[GT]` moved. KD-W1 remains controlling while W3, W8, W9 and W10 are still unwired, and all preregistered trajectory/contact-stream invalidators retain **remeasure, do not widen**.
>
> **UPDATED September 21, 2026 (v1.24):** Post-PR #433 sequencing advances into the foul/card calibration phase by **freezing the measurement contract first** at `docs/tracking/foul-card-w3-w9-preregistration.md`. No result-bearing calibration run had been observed at that point and no gameplay `[GT]` moved. The preregistration makes the post-W2 diagnostic source-complete (collision/referee vs already-adjudicated W2 tackle fouls), freezes six full-match seeds, and requires the same foul/card census across W3/W9 plus their shared `AGENT_BALL` contact-feed effects. KD-W1 remains unchanged: W3, W8, W9 and W10 are still unwired, so current-head measurement may characterize the stream but the final constant fit waits for the complete engine; after a listed invalidation trigger the rule is **remeasure, do not widen**.
...
|---|---|---|---|
| 1.25 | 2026-09-22 | — | Frozen #435 six-seed foul/card characterization captured durably: run 35765635130 on exact main c56e5e1a; 50 fouls / 5 cautions / 1 straight red / 0 second-yellow dismissals; 42 collision + 8 tackle calls; 42 identity checks / 0 mismatches. Cooldown-bypass zero remains non-dispositive; no `[GT]` fit; KD-W1 and remeasure-don't-widen unchanged. |
