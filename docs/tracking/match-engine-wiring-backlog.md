...
- **C1 — ✅ FIXED August 8, 2026 (`ERR-012-011`).** #12 committed `InPoss` on 9.5% of final-third
  ...
  **The named creation lever remains C4.** Two new Class-A items fell out of this and are listed
  below.
  **Quantified August 10, 2026** (`close-chance-creation-design.md` §11, an 18-seed × 6-tree
  bisect run to separate C1 from the -021/-022/-023 shot-lane chain): C1's close-chance cost is
  real and directional, not a bound-tuning question — mean final-third dribble cosine
  **−0.189 ± 0.038 across 18 paired seeds, t = −5.05, 16 of 18 seeds down**, reproducing the
  gate's −0.165 on the acceptance scenario's own seed pair exactly. **C1 is the live cause of
  `sim_match_engine_close_chance` failing on main today**; the shot-lane chain measured
  separately over the same corpus has no directional effect (−0.027 ± 0.039). The mechanism is
  the `InPoss` `PullFactor` column already named above. The "GATE: FAILED" entry in
  `open-issues.md` and the failing bound below are therefore correctly attributed to C1, not
  awaiting a bound-tuning fix — an owner call on the mechanism itself is still what is needed.
...
|---|---|---|---|
| 1.4 | 2026-08-10 | — | **C1's close-chance cost quantified**, via the bisect run recorded at `close-chance-creation-design.md` §11: mean final-third dribble cosine −0.189 ± 0.038 over 18 paired seeds (t = −5.05, 16/18 down), reproducing the gate's −0.165 on the acceptance scenario pair. Confirms C1 — not the -021/-022/-023 shot-lane chain, whose measured directional effect over the same 18 seeds is −0.027 ± 0.039 — is the live cause of `sim_match_engine_close_chance` failing. No bound moved; no code changed in this revision. |
