...
> **UPDATED September 25, 2026 (v1.32):** W8 investigation is decision-gated at `docs/tracking/w8-goalkeeper-distribution-decision.md`: #21 policy input is incomplete, #11/DT producer ownership hits the ordinal-8 boundary, the hand-only Law-12 clock conflicts with the broader engine stall guard, FR-GK-043/F-05 have live implementation gaps, #11 §3.8.3 names nonexistent `PassMechanics.ConsumePassIntent`, and the current #11 distribution path emits an event without a Pass Mechanics executor. No W8 gameplay code or `[GT]` change is authorized until the four owner decisions and preregistration land.
>
...
>
> **UPDATED (prior) September 23, 2026 (v1.29):** PR #447 begins durable W3 evidence archival. `docs/tracking/evidence/pr439-w3/` now preserves the six-seed structural pre-W3 comparator and final landing rerun plus exact run/artifact/production provenance and attribution limits; `docs/tracking/evidence/pr439-ref-archive/` records all 16 live `evidence/pr439-*` heads with an observation time. Phase 1 does **not** authorize deletion: branch-exclusive history, cited-run heads, explicit dispositions and a live-ref completeness verifier remain required. W3 remains **wired but dormant**; #441 and #440 are unchanged.
>
> **UPDATED (prior) September 23, 2026 (v1.28):** W3 lands via PR #439 as **wired but dormant**. The #442/#443 six-seed W2 test contract reached `main` as merge `6876c5c1…` and was merged into #439 without rebasing (merge `9668643c…`, via merge-only PR #444); every cited W3 evidence SHA remains in branch history. PR #439's review-closure commits `16233d1`/`bab4cd4` add `ERR-010-004` (Heading #10 contact buffer default/floor raised from 16 to `MAX_AGENTS` so W3's 22-agent same-frame fan-out fits; overflow still fails closed) and `ERR-011-014` (possession acquisition and restart awards hard-cancel live `ClaimIntent` state immediately). Frozen six-match corpus rerun `35925236129` on production head `bab4cd41…` (identical instrument and seeds) reproduces run `35814050060` at `f40f0853…` exactly — all 264 per-seed and aggregate counters, so the W3 figures below stand for the landing head. The six-seed tackle corpus reproduces the pinned #439 evidence arm exactly: `Won=3 Loose=12 Foul=2 Missed=73 dispossessions=3`. The W3 row status and classification below are otherwise unchanged; #441 still owns Head reachability and #440 remains deferred.
>
> **UPDATED (prior) September 23, 2026 (v1.27):** Owner classification for PR #439 is **W3: wired but dormant**. Production `AGENT_BALL` fan-out and the keeper Hand path are live; the frozen six-match run `35814050060` at production SHA `f40f0853909cc2a42190023fea1da72d25409b12` recorded 215,083 fan-out events, 1,297 claim-eligibility episodes, 1,164 registered participants / 1,164 Hand-contact resolutions and 753 successful keeper claims, but **zero contested Hand-vs-Head arbitrations**. Issue #441 owns the remaining Head-reachability boundary: its frozen localization run `35887487201` recorded 1,811 committed HeaderIntents → 1,811 jump starts → 18 intents ever assigned a predicted contact frame → **0 prepared Head contacts / 0 executed headers**. This leaves W3's contest plumbing wired and test-proven while production Head participation is dormant upstream in Heading #10; #441 remains open to distinguish opportunity/geometry from prediction-to-contact realization. No gameplay `[GT]` change is implied.
>
...
**POST-#416 SIX-SEED REVALIDATION — landed September 20, 2026 (PR #429).** The preregistered six-seed corpus rechecks the historical settled-possession stall after the #416 perception/controlled-ball correction. Governing baseline run `35389373818` and result run `35389986678` are preserved by provenance; all six production seeds clear their frozen non-vacuity floors and strict possession-phase-share predicate. The disarmed control also clears the same predicate, so the result is deliberately bounded to **stall/non-vacuity revalidation**, not W2 efficacy, tackle-outcome calibration, or foul/card calibration. Durable repository record: `docs/tracking/w2-six-seed-evidence-preregistration.md` and `docs/tracking/evidence/w2-six-seed/{README.md,artifact-SHA256SUMS,baseline-counts-and-floors.tsv,provenance.txt,w2-six-seed-results.tsv}`. Per-arm TRX/detailed logs remain Actions-retained evidence only. No new measurement was required during reconciliation because current `main` introduced no overlapping change to the seven #429 final-diff paths.
...
### W3 — Keepers never claim crosses — ⚠️ **WIRED BUT DORMANT (landed via PR #439; Head reachability owned by #441)**
...
**Current branch state:** MatchEngine now publishes the read-only production `AGENT_BALL` fan-out,
commits real keeper `ClaimIntent` episodes, and composes #10 Head geometry with #11 Hand geometry.
Focused/composed tests prove both contest outcomes. The W6 Resolve-time Controlled-ball reattachment
...
A real, tested `OcclusionFilter` is live in the perception system —
`perception-system/PerceptionSystem.cs:396`, `BallPerceptionEvaluator.cs:71-74` — but at Stage 0 that
filter is **opponent-only** and explicitly skips the observer's team-mates (`OcclusionFilter.cs`, OQ-1).
That distinction matters for a goalkeeper because his own defenders are a primary source of screens.

> **⚠️ CORRECTED September 10, 2026 (v1.14) — read this before designing W4.** As filed, this entry
> closed *"The keeper is simply not on that path."* **That is false, and it is the kind of false that
> sends an implementer at the wrong seam.** The keeper **is** already processed by the perception
> heartbeat: `MatchEngine.cs:3074` runs `_perception.OnHeartbeat` over the full `SQUAD_SIZE`, keepers
> included (keeper index == team id, `MatchEngine.cs:922`), and `BallPerceptionEvaluator` computes a
> ball-visibility result for him every heartbeat. **No new keeper heartbeat/path is required.** PR #390
> review also caught that the first v1.14 wording overreached by calling W4 purely downstream: the
> existing occlusion primitive omits same-team screens, and the current goalkeeper reaction latch
> cannot be restarted by a same-episode deflection. W4 is therefore an integration problem spanning
> **perception semantics, downstream consumption, and tick/reaction ordering**. The five constraints
> below are the actual content of W4. No sequencing changed by this correction.

**Five constraints any W4 design must resolve explicitly.** None were visible while the entry read
"not on that path", and several make the obvious implementation a *regression* or an incomplete fix:

1. **Occlusion is not `BallVisible`.** `FilteredView.BallVisible` is `range AND FoV AND
   not-occluded` (`BallPerceptionEvaluator.cs:60-76`). The FoV term is taken against
   `AgentState.FacingDirection`, which for a keeper is `AUTO_ALIGN` — velocity direction, else the
   vector to his movement target, else unchanged when he is stopped
   (`AgentMovementSystem.cs:454-465`). A keeper is dispatched `MOVE_TO_POSITION` at the
   #12-composed GK slot (`MatchEngine.cs:4665-4670`); settled on that slot his velocity is ~0 and the
   fallback degenerates, so his facing is whatever it last was — **not** ball-directed. Base FoV
   half-angle is 80° (`PerceptionConstants.cs:57-65`, floor 60°). A keeper who last shuffled laterally
   has a shot from in front of goal at ~90° separation, i.e. **outside the cone**. Gating the save on
   `BallVisible` would therefore blind keepers to ordinary *unobstructed* shots while presenting as a
   working occlusion feature. W4 must either consume an appropriate occlusion term independently, or
   establish a keeper-facing contract first. **How often this bites is UNMEASURED** — it is a W12a
   question and one of the reasons W12a is worth taking first.
2. **Both call sites precede the perception heartbeat.** `SaveArmed` has **two** production callers,
   not one: `MatchEngine.cs:3311` (the DT-emitted-SAVE gate inside `RunMechanicsAI`, entered at
   `:3046`) and `MatchEngine.cs:4461` (the W1 rush exclusion inside `TryCommitRushIntents`, reached
   from `DriveGkHeadingTactical` at `:3072`). Both run **before** `_perception.OnHeartbeat` at
   `:3074`, so a view read at either site is the **previous** heartbeat — 100 ms stale at 10 Hz, the
...
4. **Friendly screens are absent upstream.** `OcclusionFilter.IsOccluded` explicitly skips an
   ...
   his centre-back is therefore *not* represented by today's perception result, even though the keeper
   is on the heartbeat. W4 must explicitly choose ownership: either a keeper-specific all-body
   line-of-sight/occlusion input, or a deliberate resolution of #7 OQ-1 that broadens perception
   semantics. It must **not** silently make every agent treat team-mates as occluders as a side effect
   of fixing goalkeeper visibility.
5. **A deflection needs an explicit new-threat/reaction-reset seam.** Ball perception has no
   recognition-latency episode (`BallPerceptionEvaluator` explicitly exempts the ball from `L_rec`).
   `GoalkeeperMechanics.OnThreatArmed` is a no-op while `_shotDetectedTickMs[gkIndex] > 0`, while
   `OnShotExecutedEvent` is the path that overwrites the stamp for a newly struck shot. Agent-ball
   deflections flow through `CollisionSystem.ProcessAgentBall` / `BallCollisionHandler` and do not
   produce that goalkeeper shot event. Therefore a body deflection during an already-live save episode
   cannot restart the reaction window today. W4 must pin the producer/event (or equivalent explicit
   signal), its ordering relative to collision/perception/save evaluation, the stamp overwrite/reset
...
but does not bank reaction time before reveal. No pending deflection latch, `CollisionEvent` ABI change,
...
**Resolved:** genuine open-play possession now enters `BallStateType.Controlled` through first-touch/interception, loose-ball pickup, tackle ball-won, and goalkeeper possession. `MatchEngine.DriveControlledBallToPossessor()` is the single attachment funnel: it runs after locomotion/keeper/heading Physics and, after the PR #439 correction, again immediately after Resolve-time Collision #3 agent-agent position correction. Outfield carriers use ball-rest height while goalkeeper control preserves claim/contact height. Non-kick releases leave Controlled explicitly; restart-taker designation remains a stationary placed ball and therefore is not treated as a physical tackleable carrier. The Resolve reconciliation is unconditional and therefore applies to the default engine and to keeper/outfield holders; it can change trajectories/digests wherever collision separation moves a physical carrier even though it adds no saved field or RNG draw.
...
**PR #439 post-closeout correction:** W3's larger live-claim population exposed a separate, older W6 phase-order hole: Physics attached the ball, then Resolve collision separation could move the still-controlling holder and leave the ball one frame behind. The fix reuses the same attachment funnel after collision writeback. It is not a W3 mechanism and is not feature-flag scoped. PR #439 therefore carries a mixed W3 + W6 trajectory change; its frozen-corpus evidence must describe that combined landing state rather than attributing every delta to W3.
...
- **C1 — ✅ FIXED August 8, 2026 (`ERR-012-011`).** #12 committed `InPoss` on 9.5% of final-third
  ...
  **The named creation lever remains C4.** Two new Class-A items fell out of this and are listed
  below.
  **The predicted cost is now QUANTIFIED (bisect, August 10, 2026; recorded August 17 —
  `close-chance-creation-design.md` §11).** Over 18 paired seeds × 90 minutes, C1's effect on the
  final-third dribble cosine is **−0.189 ± 0.038 (t = −5.05), 16 of 18 seeds down** — the shift
  arriving entirely at `e1480ee` and nowhere else along the chain, with the trees either side of it
...
> two wait ON arming; the third is the reason arming has not happened, so it does not belong beside
...
**Note (2026-08-09).** `close-chance-creation-design.md`'s "attack the ball"
...
|---|---|---|---|
| 1.32 | 2026-09-25 | — | **W8 decision-gate investigation.** Adds the pointer to `w8-goalkeeper-distribution-decision.md` and records that W8 has four unresolved pre-preregistration contracts: hand-only Law-12 authority versus the broader engine stall guard; producer/RNG policy versus the DT ordinal-8 ceiling; FR-GK-043/F-05 forced-release/receiver-validation defects; #11 §3.8.3's nonexistent `PassMechanics.ConsumePassIntent` surface; and a missing Pass Mechanics executor/pass-registration seam behind the currently event-only #11 distribution path. No gameplay, approved-spec, schema, RNG or `[GT]` change. |
...
| 1.9 | 2026-08-17 | — | **C1's close-chance cost QUANTIFIED, and the shot-lane chain refuted as its cause.** The C1 entry (§3) predicted the fix would push the deepest composed slot further from goal because the `InPoss` `PullFactor` column is less advanced than the `TransToAtk` column it replaced; an 18-seed paired bisect measures that prediction at **−0.189 ± 0.038 (t = −5.05), 16 of 18 seeds down**, arriving entirely at `e1480ee` with the trees either side bit-identical on all 18. The -021/-022/-023 shot-lane chain — the suspect the `close-chance-creation-design.md` Acceptance-3 row named and handed to KD-W1 — moves the same metric by **−0.027 ± 0.039 (t = −0.70, 8 up / 10 down)**, so it has no directional effect and that hand-off has nothing to act on. `sim_match_engine_close_chance` is therefore failing on a **mechanism**, not on a bound-tuning question — the same conclusion §10.9 of that document reached from the post-C1 side and the owner confirmed at v1.6, reached independently from the opposite direction. Measured August 10, 2026 on `claude/shot-lane-regression-bisect-vflxc2`, which never merged; ported here. **Currency: every tree in the sweep predates the W2 tackle wiring (v1.8 / `ERR-014-006`)** — the controlled attributions stand, the absolute figures are not today's `main`. Full measurement: `close-chance-creation-design.md` §11 (v2.3). No `[GT]` moved, no code changed — documentation only. |
