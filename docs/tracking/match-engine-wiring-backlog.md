...
> **UPDATED September 16, 2026 (v1.20):** W4, W5, W6, W7, and W12 are wired; four Class-A items remain: W3, W8, W9, W10. W2 remains built and shipping-disabled pending its separate activation change, but its post-W6 evidence obligation is now closed with an exact same-head/same-seed paired control. On production head `e4335f7f`, armed-at-`LooseBallPickupRadiusM` shares are `0.983/0.983` and `0.985/0.985` with gate/test exits `0/0`; the disarmed control is `0.530/0.530` and `0.979/0.979` with expected exits `1/1`. Run `35096793576` used measurement-driver head `682e7af9`; both exact artifact ZIPs, provenance, parameterized driver/workflow and SHA-256 manifests are archived under `docs/tracking/evidence/w2/`. The evidence supports removing the historical InPoss stall blocker at the intended current activation reach; it is not tackle-outcome calibration. The ten tackle-outcome `[GT]`s remain explicitly uncalibrated, foul/card calibration still follows activation, and W2 receives no T-DA-DET-005 credit. The W12 prerequisite-order deviation is recorded separately in `w12-preregistration-reconciliation.md`; the stricter 5 pp plus suppression-mass material-effect bar remains governing.
