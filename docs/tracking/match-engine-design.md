...
- **Phase G — Snapshot deserialize / restore path. 🚧 IN PROGRESS (opened July 20, 2026).** Phases
  ...
  - **G-Phase 1 (neutral-path reader + round-trip determinism).** ✅ **KD-8 writer half LANDED
    ...
    `MatchEngineSnapshotSchemaTests` v1.14 (pin 17 + `CardSeverityRngCursor` probe). 🚧 **Remaining:**
    the missing `RestoreState` counterparts (Pass/Shot executors, the four Mechanics-AI hysteresis
    surfaces, Perception, `RotationController`), `DeserializeWorldState`, the `RestoreFromSnapshot`
    factory, and the G3 round-trip determinism test (incl. the booking-before-snapshot regression).
  - **G-Phase 2 (distinct-squad re-projection).** The KD-3 `ISquadProvider` seam + roster
    re-projection keyed by `_activeBenchSlot` — the #27 T3 restore consumer (KD-T3-3).
