...
**six** of the review's **27 open findings**. #31 leaves this class with its PR #407 T0 landing and
discharges both deferred findings in code + spec + tests. The other **21** findings remain the workable
queue today.
...
| **D4** ◑ | **#40 minimal — T0 + T1a/T1b + T2a/T2b + T3a foundation LANDED.** The finance assembly and self-identifying save block are live; #27-backed bootstrap is active, and #30 owns one-time production initialization, staged season-boundary settlement, legacy-empty restore normalization, plus `SeasonLoop.FinanceView` / `AvailableTransferBudget` / `ApplyTransaction`. T3a adds the draw-free daily-revenue accounting primitive and current-season accumulator lifecycle. **REMAINDER:** T3b1 identity-zero daily invocation through #30, then T3b2 deterministic revenue formulas/`[GT]` values; T3c+ owns the first stochastic sponsorship-variance draw plus later FFP/board depth. | #40 §7 |
| **D5** ◑ | **#31 minimal — T0 IMPLEMENTED by PR #407.** New `src/transfers/` production/test assemblies provide integer valuation, always-on #27 positional scarcity, deterministic Accepted/CounterOffered/Rejected negotiation, contract/window/spend state and atomic manager `SubmitBid`. The two C6-deferred football-judgment findings are discharged. **REMAINDER:** T1 save composition; T2 #30 roster adapter/tick/genesis/boundary wiring (including roster re-key migration hygiene); T3 personality/CA/staff/wage/multi-day/rival-bid depth. No RNG stream/domain promotion at T0. | #31 §7 |
