...
| **B9b** | **Unity client P5b** — the UGUI binding: Main Menu, Tactics Setup, Match View (stats panel, speed controls, in-match tactical buttons onto the P2 command queue), Post-Match Report. Reads enablement from `MatchControlAvailability` and the speed set from `PlaybackSpeedLadder`; re-derives neither. **One open decision ahead of it** — where the four screens' `ScreenId` catalogue and navigation graph live, given FR-UI-010 forbids `ui-framework` and `ui-framework` sits above `match-client-core` (§6 item 2's question, in a new place). **Budget a cert-host run for this landing.** | §5-P5b | **host** |
