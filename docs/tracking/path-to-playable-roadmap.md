...
| **B9c** ✅ | **Unity client — the screen catalogue's home. LANDED August 7, 2026** (owner decision, resolving B9b's blocker). New gate-compiled assembly `src/client-app/` (`TacticalDirector.ClientApp`, references only `ui-framework`) on the `match-engine` composition-root precedent: FR-UI-010 makes a concrete screen set composition, not framework, and composition lives above what it wires. `ClientAppConstants` (four `[FIXED]` ids, 0 never allocated), `ClientScreens`, `ClientScreenFlow` (the five-edge graph as guarded moves over a privately-owned `NavigationShell` — the two Replace edges test-locked by where a later Pop lands; no abandon-match edge, FR-CS-049). 15 tests. §6 item 2's C3 screens inherit the answer by precedent. **GATE-VERIFIED later the same day** (SDK via apt — see v0.19): ClientApp.Tests 15/15 first execution; FR-UI-001 scan fired as designed, `client-app` sanctioned, UiFramework.Tests 50/50; the only red is two `MatchEngine.Tests` scenario locks verified pre-existing at origin/main (root OPEN ISSUES entry). | `interactive-unity-client-design.md` §5-P5a / v0.17–v0.18 | host-free |
