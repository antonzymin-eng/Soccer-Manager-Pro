...
| **B8** | **Unity client P4b** — the Unity binding. `MatchClientBehaviour : MonoBehaviour` + prefabs + scene: instantiate one primitive per `PitchMarking`, assign transforms and scales from the P4a draw states, map `TeamId` to a palette, drive the orthographic camera from `FollowBallCamera` through `PitchViewProjection`. No branch a test would want to reach. **Budget a cert-host run for this landing** (§7 / C2). | `interactive-unity-client-design.md` §5-P4b | **host** |
