...
| **D2** ◑ | **#29 minimal** — **T0 LANDED August 5, 2026** as the declared prerequisite of D3 (#41 §4.1 requires a `TacticalDirector.TrainingSystem` reference for `InjuryRiskContribution`, and the assembly did not exist). `src/training-system/`: the value types, the four §3.1–§3.4 entry points, the FR-TR-023 command, the Appendix A catalogue. Draw-free (FR-TR-008) — `_RESERVED_0x21_` / 83 stay reserved. **REMAINDER: T1 (`TrainingSaveCodec`) and T2 (the slot-1/slot-2 wiring, the `ProjectMatchEntryFatigue` match-boot seam, the FR-TR-025 roster handoff).** The assembly is inert until T2 — nothing constructs it. `ComputeTrainingInput` returns `TrainingInput.Neutral` on both branches because #28's type still has no fields; the deep branch is a marked seam, not an invented magnitude. **No gate run** (no .NET SDK in the landing environment). | #29 §7 |
| **D3** ◑ | **#41 minimal** — **T0 LANDED August 5, 2026.** `src/injuries-medical/`: the value types, §3.1's recovery-then-draw day step with the KD-6 entry-state gate, the keyed occurrence draw, §3.2's same-draw severity bucketing, §3.4's risk assembly, the Appendix A catalogue. `DOMAIN_TAG_INJURIES_MEDICAL = 0x2A` allocated in code (ERR-041-001 closed); **ordinal 92 deliberately NOT allocated** — #41 registers no cursor stream, so an ordinal would be FR-LW-031's zero-consumer phantom (the ERR-030-012 posture). Surfaced **ERR-041-002**: §2.2/§3.1's `rng.DrawKeyed(...)` names an API `DeterministicRngService` does not expose, and the only implementable alternative is cursor-positioned — which KD-1 forbids — so the draw is a local keyed SplitMix64 derivation and `AdvanceMedicalDay` takes `ulong worldSeed`. **REMAINDER: T1 (`MedicalSaveCodec`) and T2 (the #30 slot, the `IsAvailable` read into squad selection, the FR-MD-025 roster handoff).** Inert until then. **No gate run** (no .NET SDK in the landing environment). | #41 §7 |
