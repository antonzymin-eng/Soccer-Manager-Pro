...
| Engine | `GkRushTriggerTests` (dismissal, AR-2) | A keeper sent off mid-rush **stops dead** — the AR-1 fix itself, which shipped with nothing asserting it. Both keepers. |
| Engine | `GkRushTriggerTests` (substitute keeper, AR-2) | Red card, bench keeper on for an outfielder: the incoming keeper is **not** dragged along the run his predecessor committed, and starts at `Resting` with no intent armed. Both keepers. |
...
7. **`GoalkeeperMechanics.cs`'s `TacticalTick` comment claims `_attrs` is written only by
   `CommitSaveIntent` and `OnShotExecutedEvent`** (found at AR-2). W1 made that false —
   `CommitRushIntent` writes it too, and now has a production caller. The comment is load-bearing
   reasoning (it is *why* the wake predicates read `gkIndex` rather than `attrs.TeamId`), and its
   conclusion still holds; only its premise is stale.

8. **`CommitRushIntent` writes `_attrs` although nothing on the rush path reads it** (found at AR-2).
   `_attrs` is consumed only by dive kinematics and distribution accuracy. Four writers now share one
   slot across two independent episode lifetimes with no ownership rule. It cannot misfire today —
   commits are gated to `Set`/`Anticipate`, so no rush commit can land inside a live dive, and all
   four call sites project the same player with `fatigue: 0f` and therefore write identical values.
   The smallest fix is to delete the line; it is left alone because it is #11 surface outside W1's
   blast radius.

9. **`GoalkeeperStateMachine`'s `Anticipate` comment says "rush intent takes priority per #8 priority
   rules"** while the code evaluates the `→ Diving` row first (found at AR-2). The code order is the
   one W1 wants — `TryCommitRushIntents` already refuses to arm while `SaveArmed`, so the two cannot
   normally coexist — but the comment states the opposite of what the rows do.
