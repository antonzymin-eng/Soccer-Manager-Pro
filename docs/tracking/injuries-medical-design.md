...
| Version | Date | Change |
|---------|------|--------|
| v0.7 | August 7, 2026 | — | **THE BALANCE PASS LANDED — the dial is ARMED (FR-MD-027).** What the T2 record left open closes: **ERR-030-026's split** resolved WITHOUT splitting the step (ERR-030-027 — the fixture day's whole atomic day-step runs pre-round inside `AdvanceAndPlayNextRound`; recovery lands before selection, the draw on matchday morning; FR-MD-022/KD-6/the medical format survive verbatim); **ERR-041-010(b)** closed (the #30-owned appearance record — lazily-shifted bitmask, `APPR` v1 sub-blob, frame v4 — supplies FR-MD-010's `MatchLoad` through a window that never contains the current day); **the retune** (ERR-041-011: `OCCURRENCE_DRAW_DENOM` `[FIXED]` 1,000,000 decoupled from the `[GT]` ceiling so config edits stop re-rolling careers' draws; `BASELINE_DAILY_RISK` 4000 before-mitigation kills the injury-proof-forever default; `APPEARANCE_LOAD_WEIGHT` 5600) — measured by the new season-scale instrument over 8 seeds: league ~780 injuries/season (~39/club, E-1 band 30–55), starters 2.08, reserves 1.12, unavailability 9.4%, locked league-wide with perturbation-proof bands; **ERR-041-012** (§4.5's phantom registered stream rewritten to the keyed derivation; ordinal 92 deliberately unallocated); ERR-041-002/-003's deferred back-props discharged; ERR-029-007 corrects the #29 mirror text. The dial argument is REQUIRED (no default in either position); OFF stays supported and locked both ways, plus the restored-career-still-injures lock. |
| v0.6 | August 6, 2026 | **T2 AR passes 3–5 — 1H + 4M + 7L, all fixed.** The #41-side headline is **ERR-030-026**, and it is a football-correctness finding dressed as a wiring one. #30's KD-2 tick order pins nine day-slots and has **no slot for playing the round** — a round is a separate command — so where a fixture sits relative to slot 4 was specified nowhere and, in the code, fell out of `AdvanceToNextFixtureDay`'s loop condition: it stops on *reaching* the fixture day, so matchday's own medical step runs **after** the round. That is right for the occurrence draw (an injury sustained in a match must be drawn after it — and it is what makes FR-MD-010's `MatchLoad` term coherent once ERR-041-010(b) supplies an appearance record) and **wrong for the recovery countdown, which shares the same atomic `AdvanceMedicalDay` call**: a player whose `RecoveryRemaining` reaches 0 on matchday has his decrement applied after the round, so **every injury runs one matchday longer than its assigned tier.** Invisible today (the dial is off) and invisible to the suites either way, which is exactly why it needed filing rather than fixing quietly: the balance pass would have fitted `RecoveryDaysPerTickBase` and every tier-day constant straight through the bias and locked it in. The convention is adopted, documented at all three sites that determine it, and pinned by a test asserting both halves — `LastAdvancedWorldDay == fixtureDay − 1` at kickoff, and a one-day-outstanding player unavailable for that round and fit immediately after. **What is deferred, deliberately:** whether #41 should expose recovery and occurrence as separately callable halves so each lands on the correct side of the round. That is the football-right shape and a change to FR-MD-022's one-step-per-player-day contract, so it belongs with the balance pass and needs owner sign-off — at which point the tiers are fitted against a chosen convention rather than an emergent one. Also: `SeasonSaveManager.Load`'s v0.5 filter fix is now reachable in a second way — `FromBlocks` copies the state arrays, so the career it rebuilds cannot be written through the `SeasonSaveContents` the same call returns. |
