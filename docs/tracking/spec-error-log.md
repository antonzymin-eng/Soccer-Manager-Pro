...
**Version:** 1.54
**Updated:** July 31, 2026 (v1.54 — **ERR-029-004 filed + RESOLVED at the Training System #29 code AR-1 fix pass.** #29 §4.1's assembly-reference list names only #27/#28/#16 and omits `TacticalDirector.ProjectConstants` — which Code Standards #20 §3.2.3 (FR-CS-019) requires of every constant catalogue, since `[GT]` rows must be PascalCase `static readonly` reading `Config.GetInt/GetFloat` off `GameplayConfigHolder` (the June-30 migration across 17 catalogues) and a catalogue cannot do that without the reference. The shipped `TrainingSystemConstants` consequently declared every `[GT]` scalar as an ALL_CAPS `const`, which inlines into consumers and is structurally locked out of the config surface — the same defect class the #30 T0 pass 5 burned down against `SeasonLoopConstants`. `training-system/section-4.md` v0.4 adds the reference with its no-cycle rationale; the code migration landed in the same commit, behaviour-neutral (every fallback is the previous literal). No schema/RNG/draw-order change. Prior update below.)
...
| ERR-029-004 | Training System #29 back-prop: §4.1's assembly-reference list names only #27/#28/#16 and omits `TacticalDirector.ProjectConstants`, which the FR-CS-019 `[GT]` config surface requires of every constant catalogue. | Medium | 1 | ✅ RESOLVED July 31, 2026 — filed and landed atomically with the #29 code AR-1 fix pass (audit-report M-2). `training-system/section-4.md` v0.4 adds the fourth reference with its rationale: Code Standards #20 §3.2.3 makes ALL_CAPS `const` **[FIXED]-only** and requires `[GT]` rows to be PascalCase `static readonly` reading `Config.GetInt/GetFloat(section, key, fallback)` off `GameplayConfigHolder` (the June-30 migration across 17 catalogues), and a catalogue cannot do that without referencing `ProjectConstants` — so the shipped `TrainingSystemConstants` had every `[GT]` scalar as a `const`, which inlines into consumers and is structurally locked out of the config surface (the same finding the #30 T0 pass 5 recorded against `SeasonLoopConstants`). `ProjectConstants` references nothing and sits at the bottom of the graph, so the addition introduces no cycle and does not weaken FR-TR-013/024. Code landed in the same commit (`TrainingSystemConstants.cs` v1.1 + the asmdef); behaviour-neutral — every fallback is the previous literal, and no process binds a config today. |
...
*End of Spec Error Log v1.54 — July 31, 2026.*
