...
| ERR-008-021 | Decision Tree #8 §3.1.4.3 / §3.2.3.2 — the shot-lane occlusion check was attribute-blind (the follow-up deliberately deferred at the ERR-008-020 landing): every outfield blocker in the shot path occluded the same geometric arc regardless of who he is, so a Pace/Anticipation 1/1 defender walled off the goal exactly as hard as a 20/20 one in the identical spot, and no shooter attribute entered the read. Pattern (a) — continuous in position (no P1 cliff), blind to identity. | Moderate | 6 | ✅ **Resolved August 6, 2026** (spec §3.1.4.3 + §3.2.3.2 step 3a + `OptionGenerator.cs` v1.7, same commit; doctrine P2/P3/P5) — each **outfield** blocker's occluded arc scales by §3.1.3.3's `perceived_ability` (Anticipation/Pace → `INTERCEPTOR_ABILITY_MIN..MAX`, read through the **shooter's** Vision fidelity); **no new constants** — the ERR-008-020 `[GT]`s reused verbatim (one calibration lever, KD-W1). The **goalkeeper's** arc stays purely geometric: his shot-stopping quality is priced once, at the #11 save (P3 — no double-count), and `GK_BLOCKER_RADIUS` is an abstraction of coverage, not a body. Neutral/null-view ability = 1.0 ⇒ today's arcs exactly (P5 pivot). 6 `OptionGeneratorTests` locks incl. the away mirror and the GK-exclusion lock. Adjacent defect recorded, not fixed: §3.2.3.2's numerical example is in a legacy centre-origin frame and its blocker sits inside the GK heuristic yet uses the outfield radius (annotated in-place). **Gate NOT run — no .NET SDK in the authoring environment; CI on push is the gate.** |
...
- P5 pivot: a league-average blocker — and every blocker under a null attribute view (unwired
  host / legacy test) — multiplies by exactly 1.0, so today's arcs, and therefore today's SHOOT
  generation set, are reproduced bit-for-bit in the neutral case.
...
will move; same posture as ERR-008-020). Locked by 6 `OptionGeneratorTests` (v1.7): the
computed-league-average pivot equals the null-view arc exactly (the ERR-008-020 AR-1 M-1 lesson —
the pivot lock must drive the COMPUTED path, not the null-view guard), Vision-20 separates
elite/poor blockers while Vision-1 barely does, null-view neutrality, the goal-line keeper's arc
invariant under his attributes, and the discrimination case mirrored to the away side (the
home-team-only-example trap). **Gate NOT run — no .NET SDK in the authoring environment; CI on
push is the only compiler for this work.**
