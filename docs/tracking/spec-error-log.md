...
**Version:** 2.47
**Updated:** August 18, 2026, later still (v2.46 — **`ERR-020-006` and `ERR-020-007` filed and RESOLVED** (adversarial-review round 6, H6 + H7, both Code Standards #20, both spec-text only — no code changed). **`ERR-020-006`:** `[CROSS-PENDING]`, one of the SIX constant-source tags root `CLAUDE.md` mandates, was unknown to #20 anywhere — FR-CS-017 enumerated a closed list of five and §3.2.1's table, introduced as the root's verbatim reproduction, carried five rows against six. All **218** `[CROSS-PENDING]` occurrences in `docs/specs/` were therefore MUST-level violations of an APPROVED spec, and §9.4's own re-approval trigger 1 ("any tag addition… requires §3.2.1 and Appendix D to be updated") had not been honoured when the tag was added on August 10. **`ERR-020-007`:** §4.2's `ERR-020-004` carve-out, landed on this branch, certified `DisciplineConstants.CardKindYellow` "a compliant mirror" while FR-CS-022 and §3.2.3 forbid its `public const` storage class — 19 such symbol-referencing mirrors exist tree-wide. Resolved by carving out symbol-referencing initializers only (the compiler enforces the mirror), deliberately NOT extending it to the 6 literal-initialized `[CROSS]` consts, and deliberately NOT adopting "inherits the source's naming", which would have re-broken the very declaration the entry defends. No id moved; nothing reopened.)
**Updated:** August 18, 2026, later still (v2.47 — **`ERR-030-048` filed and RESOLVED**, adversarial-review
round 7, H4). #30 §3.4's serve pair and `fold?.Commit` were UNGATED inside the fixture loop while the comment
above them justified their null-safety by citing "the same condition this whole block already runs under" — a
condition that did not exist. Implemented verbatim it throws on the first fixture of any career without
discipline wired. It is the loud twin of `ERR-030-047`: same block, same omission, opposite failure mode — and
the false justification was written by -047's own fix pass, so the round that closed one half introduced the
other and offered a non-existent gate as its evidence. Resolved spec-text only; the code was already correct.

**Updated (prior):** August 18, 2026, later (v2.45 — **`ERR-030-047` filed and RESOLVED; `ERR-030-034` annotated in
place** (adversarial-review round 6, H1 + H2). **H1/`ERR-030-047`:** #30 §3.4's normative pseudocode was never
updated when `ERR-044-014` (August 16) changed the `OnClubFixturePlayed` call it specifies — the serve step
still read the two-argument `OnClubFixturePlayed(f.HomeClubId, homeXi)` form and the fallibility comment
still said "TWO guards" against the real three-argument signature with THREE guards — and the
unfiltered-roster precondition ERR-044-014 introduced (serving MUST read the UNFILTERED roster; every id
whose ban is being served is precisely an id the filter has just removed, so a filtered roster makes every
suspension unservable, silently) appeared NOWHERE in the owning spec — ERR-044-014's own recorded-not-fixed
list named #44 and `src/discipline` residuals but not #30 §3.4. Fixed at `section-3.md` v2.17 on BOTH
resolution paths; spec-text only, the code was already correct (verified: `DisciplineRules.cs:318-345`,
`SeasonLoop.cs:984-985`). **H2/`ERR-030-034`:** its Status cell — which IS the entry, no separate body exists
— still published the S7-condition-4 reading RETRACTED August 17 ("post-defensive-wiring … no player has
ever made a tackle"): `league-bootstrap-design.md` v1.5/v1.6 amended it to post-*arming*, W2 having landed
built-but-DISABLED at `TackleContactRadiusM = 0`, and the cell also omitted the August 17 **S9 owner
decision: HOLD KD-7a, neither adopt nor reject, decide after the post-W2-arming capture**. Annotated in
place, dated, original wording untouched. One id filed, one resolved, one row annotated; no id moved.)
**Updated (prior):** August 18, 2026 (v2.44 — **the version row this file had gone without through the adversarial-review rounds 1–3 over the owner-decision pass, plus this reviewed-findings pass's own corrections** (reviewed finding M-B: the file changed materially after the v2.43 note was written — 124 insertions / 17 deletions against the pre-review state (`git diff 71455b54..601a8117 --stat`) — with no new version row, while nine spec files in the same commits carefully dated themselves August 18). What changed, per commit: **(a)** AR round 1 (`12eba7d4`, August 17, 22:14 UTC) extended the v2.43 note below in place — the two `ERR-020-*` index-row repairs (six-cell rows GFM rendered as still-Open), `ERR-020-002`'s Files Affected 2 → 5, and the 19-of-31 annotations. **(b)** AR round 2 (`98662909`, August 18, 03:01 UTC) repaired six more over-split five-column index rows (`ERR-018-018` and `ERR-030-039` were actively misrendering their Status), re-escaping pipes so all 209 `ERR-` rows carry exactly 5 cells; corrected `ERR-020-002`'s Files Affected **5 → 8** and `ERR-020-003`'s 2 → 3 ("both renderings" → "all three"); and annotated the retracted 19-of-31 arithmetic at the entry heading AND Problem paragraph. AR round 3 did not touch this file. **(c)** This pass (August 18): the **8-vs-5 Files-Affected contradiction reconciled** by a dated addendum after `ERR-020-002`'s Files-changed table — the table keeps the adoption commit's 5; the addendum names the three remediation files beyond it (`README.md`, `tools/assembly-tier-check.py`, `docs/specs/code-standards/section-9-approval-checklist.md`'s FR-row regex widening), which the round-2 commit message had claimed the body already named when it named only two (M-B); the **105 + 38 verification quotes completed to "+ 5 sourced by the two out-of-band Infrastructure assemblies, 148 total"** at the v2.43 note below and at the entry's adoption-verification paragraph, re-derived by `tools/assembly-tier-check.py` — quoting 105 + 38 as the whole is the exact unreconcilable form #20 §3.5.2 warns against (M-D; `src/CLAUDE.md`'s taxonomy note corrected the same way in the same pass); and both `ERR-020-*` entries' `**Section:**` fields re-titled to §3.5.2's current heading, "Tier Order and Dependency Arrows" — section-3.md v1.3's rename had deferred exactly these two citations to this file's own pass (L-A). No entry filed, none resolved; no id moved.)
...
**Updated (prior):** August 16, 2026, round 5 (v2.42 — rows annotated, no new ids. The `ERR-030-046` row gains two
dated annotations: the round-4 pass's own six-site "self-heals" wording fix (carried in the row body,
never reflected in this header) and a round-5 pass over it (M1) correcting that round-4's own site
count was wrong — three sites in `AvailabilityComposition.cs`, not two, and a ninth site missed
entirely (`AvailabilityCompositionExtremisTests.cs`'s `CapFallbackExtremis` comment). Real
corrected-site total: nine. Annotated in place at every record of the undercount; no rule or guarantee
text changed.)
**Updated (prior):** August 16, 2026, later still (v2.41 — **`ERR-030-046` (High, ESCALATED) filed and RESOLVED**,
from a third adversarial-review pass over #30 §3.4's tier-2 within-tier rule. **The same defect has now
survived two successive fixes** — `ERR-030-044`'s roster-order key and `ERR-030-045`'s
ascending-`SquadRating.PlayerRating` re-key — so the no-third-identical-retry rule applied and the third
attempt was **ruled rather than iterated**. The rating is a **global scalar** while `LineupSelector`
selects **per position**: where the fit squad is thin in the globally *weakest* banned player's position,
the blind pass presses exactly him back, he starts, and the reinstated-suspended probe is then
unsatisfiable for every later candidate. **≥ 476 of 1920** generated mass-suspension fixtures had a clean
completing choice the algorithm missed. **Root-cause class: an element-wise greedy decision of a
set-valued constraint** — no per-player scalar can express a per-position property of a *set* of
reinstatements, which is why a third ordering key was refused outright. The rule becomes a **capped
exhaustive clean-completion search** over subsets of the eligible candidates, with a **theorem** in place
of a best effort: within the search bound the composed squad fields the MINIMUM achievable number of
reinstated-suspended players in its eleven — zero whenever any completing choice benches them all — so one
starts only in a probe-verified forced start. `ERR-030-044`'s two-case and `ERR-030-045`'s three-case
guarantee statements are **SUPERSEDED**; the residual is a two-item list (forced starts; the beyond-cap
corner) and is deliberately **not** an enumeration of routes into a forced start, that form having been
falsified twice. New `[FIXED] SeasonSaveConstants.EXTREMIS_SEARCH_CANDIDATE_CAP = 12` — an algorithmic
probe budget, not a `[GT]`, not subject to KD-W1. #44's `ERR-044-019` **EXTENDED in place, not re-filed**.
Code: `AvailabilityComposition.cs` v1.7, `SeasonSaveConstants.cs` v1.9 — no change to `SquadRating.cs`,
`LineupSelector.cs` or `MatchEngine.cs`. Locked by the mandatory `WeakPositionExtremis` mutant-killer,
observed failing at the pre-fix tree.)
**Updated (prior):** August 16, 2026, following the round-2 text-fix wave (v2.40 — **two rows annotated, no new
ids, from a reviewed-findings pass over five items (2 Medium, 3 Low).** **M-2:** `ERR-044-014`'s
"RECORDED, NOT FIXED" list was two-thirds stale — (a) the §6.2 cost-table residue landed at
`section-6.md` v0.5 (finding L10) and (c) the `DisciplineEntry.cs`/`DisciplineSaveCodec.cs` stale-division
justifications landed at those files' own v1.4/v1.3 (finding B) — both struck in place with their
landing citations; (b), the missing `section-5.md` test citations, was the one item still genuinely
open and is CLOSED in the same pass (`section-5.md` v0.9 cites the three `DisciplineRulesTests` roster
cases under T-DC-BAN-003 and §5.6's FR-DC-011 row). **L-C:** `ERR-044-015`'s row cited its finding by
`section-4.md:137-138`, a line citation that had already drifted (§4.5 now begins at line 190) —
re-cited by section instead, the same convention `section-2.md`/`appendices.md`/`section-6.md` adopted
this round. Neither annotation touches `section-4.md`, `section-6.md`, `DisciplineEntry.cs` or
`DisciplineSaveCodec.cs` themselves — those landings are cited, not redone.)
**Updated (prior):** August 16, 2026, latest of all (v2.39 — **two rows annotated, no new ids, from the final
text-fix wave of adversarial-review round 2.** **M16:** `ERR-044-019`'s row gains a dated annotation —
the correction it recorded stopped at the spec folder, and three further live sites (`CLAUDE.md`,
`open-issues.md`, one `SeasonLoopDisciplineTests.cs` assertion message) still asserted the refuted
sentence; all three swept to the row's own two-case form. **M19:** `ERR-044-020`'s row gains a dated
CLOSED annotation — the `section-4.md` §4.3/§4.5 sync it recorded as still owed landed at commit
`1683eb3`, and this file's own v2.36 header entry recorded that closure, but the row body itself was
left stale; a reader grepping the ERR id directly saw only the open note. Both annotations point at
where the fuller account already lives rather than restating it.)
**Updated (prior):** August 16, 2026, latest (v2.38 — **two ids filed and RESOLVED, both Medium, from a
reviewed findings pass over the M1/`ERR-044-021` landing.** **`ERR-044-022`:** `CardLedgerFold.
ApplySubstitution` still silently destroyed a player's occupancy whenever `SubstitutionEvent.Incoming`
named an OCCUPIED ON-PITCH agent id — the finding's own probe, seed `{5 -> 100, 6 -> 101, 22 -> 102}`,
`Sub(Outgoing=5, Incoming=6)`: no throw, player 100's mapping destroyed, a card at slot 5 misattributed
to player 101 — and the `CardLedgerFold.cs` v1.8 version-history row plus the M1 inline comment both
FALSELY claimed the vacated-slot clear alone made this "impossible rather than merely unlikely". Fixed
(owner-decided option (a)): the constructor gains a required `onPitchAgentIdCount` parameter
(`0 < onPitchAgentIdCount <= seed.Length`); `ApplySubstitution` refuses an `Incoming` below that
boundary and an `Outgoing` at or past it, both before the write. `SeasonLoop`'s construction site
passes `MatchEngineConstants.SQUAD_SIZE`. The false v1.8 claim is annotated in place (dated correction),
not rewritten. **`ERR-044-023`:** the one-to-one seed precondition `CardLedgerFold`'s constructor
enforces is violated by the ONLY documented seed producer — `MatchEngine.PlayerIdsByAgentId()` is not
one-to-one after any substitution (`SubstitutePlayer` never clears the bench-origin entry) — and
nothing anywhere bound the two sides' agreement: `MatchEngine.cs`'s XML doc was silent on the
post-substitution shape, and #44 §4.3 named no boot-time precondition at all. Fixed as doc + one new
cross-assembly test, no behaviour change: `MatchEngine.PlayerIdsByAgentId`'s XML doc now states the
array is one-to-one over non-sentinel entries AT BOOT ONLY; `CardLedgerFold`'s constructor doc states
the same precondition and where it comes from; new `SeasonLoopDisciplineTests.
PlayerIdsByAgentId_IsInjectiveAtBoot_ButNotAfterOneSubstitution` proves both halves against the real
engine, beside the existing `NO_PLAYER`-sentinel lock; #44 §4.3 now binds the precondition normatively.
Files: `CardLedgerFold.cs` v1.10, `CardLedgerFoldTests.cs` v1.8, `SeasonLoop.cs` v1.30,
`SeasonLoopDisciplineTests.cs` v1.11, `MatchEngine.cs` v1.72 (doc only);
`discipline-suspensions/section-2.md` v0.14, `section-3.md` v0.14, `section-4.md` v0.12.)
**Updated (prior):** August 16, 2026, later still again (v2.37 — **`ERR-030-045` (High) filed and RESOLVED**, from a second adversarial-review pass over the `ERR-030-044` landing (H2-continuation). `ERR-030-044` fixed tier 2's within-tier key for a single reinstatement and left pass 3 at earliest roster position, justifying it with "the outer loop asks again, so passes 1 and 2 still decide the reinstatement that finally closes the gap". **Fieldability is monotone in adding players**, so that is false: while the club is short by more than one NO candidate is fieldable, passes 1 and 2 are structurally unreachable, and pass 3 decides every reinstatement but the last — blindly. The blind pick then poisons the last pass too, because `AnyReinstatedSuspendedStarts` sees a previously reinstated starter and makes pass 1 unsatisfiable for every candidate. Net behaviour at `k >= 2` was exactly pre-`ERR-030-044`, in the population #44's own subject creates. Pass 3's key becomes **ascending selector rank** (new `SquadRating.PlayerRating` → `LineupSelector.MeanAttribute`, not a second rating formula), ties on earliest roster position — monotone, hence safe on precisely the passes that get no probe. The corrected claims are stated as a **minimisation, not a guarantee**: a `k >= 2` set in which every completing choice starts a suspended player still starts one, and positional forcing still exists. The `ERR-030-044` row below is annotated in place rather than rewritten; #44's `ERR-044-019` amended at both sections. Two Lows fixed with it (the undercounted `Cost` remark, the dead `candidate == null` branch).)
**Updated (prior):** August 16, 2026, later still (v2.36 — **the `section-4.md` half of `ERR-044-020` (M3)
...
the slot's own yellow count and `ApplyCardAndCheckSentOff` returns kind 2 only on a slot's SECOND
booking since reset. A kind-0 card now sits between the substitution and the kind-2, with the engine
precondition (`_yellowCards[slot] >= 1`) stated explicitly and the tallies recomputed — the two players
end the example in DIFFERENT states for the first time (one row drops at `(0,0)`, the other survives at
`(2,0)`), which the old example's symmetric outcome obscured. **`ERR-044-017`** (M7): the four `[GT]`
threshold/ban constants were ALL_CAPS in `appendices.md`/`section-2.md`/`section-3.md`/
`section-9-approval-checklist.md` while `DisciplineConstants.cs` and this spec's own §2.2/§5/§6 already
used the code's PascalCase names — renamed everywhere in active text, historical version-history quotes
left as-is per the `LEAGUE_COMPETITION_KEY`/`CardKind*` precedent. **`ERR-044-018`** (M8): §2.2's
`DisciplineState` block was a bare comment; declared the full landed API (`Count`/`EntryAt`/`EntryFor`/
`HasEntry`/`FromEntries`), cross-referencing `FromEntries`' refusals to F3/FR-DC-017 — the requirements
that rest on it at the file boundary. **`ERR-044-020`** (M3): `IDisciplineTickLedgerTap` was referenced
...
`AvailabilityTests.cs`'s header that was itself already stale when written (M10) — the header had
closed both retired-id mentions the same day, on the SAME file's own v1.3/v1.4; annotated with a dated
correction rather than rewritten, per this file's own convention. `section-5.md`'s matching residue
paragraph (which had independently re-chased the same stale claim through two more corrections of its
own, v0.5/v0.6) is deleted at the same commit and replaced with a one-line closure note; that file's own
v0.4/v0.5 version-history rows, published out of chronological order, are swapped into order in the same
pass (`tools/recurring-defect-lint.py`'s check).)
**Updated (prior):** August 16, 2026, later (v2.34 — **three ids filed and RESOLVED. `ERR-030-044` (High) + `ERR-044-019` (High), one defect cross-filed at both owners, from an adversarial review over the ERR-044-003 stage 1 landing (H2); plus `ERR-037-003` (Medium), landed here from the reviewed-findings pass that had already committed its spec and code halves.** **`ERR-030-044`:** #30 §3.4's extremis back-fill fires on a **bench** shortfall rather than an inability to field an eleven — the probe both #30 §3.4 and #44 §2.3 name is `SquadRating.CanFieldStartingEleven`, i.e. `LineupSelector`'s full selection walk, eleven starters PLUS the seven-slot bench — and tier 2's earliest-roster-position ordering key then put the reinstated suspended player into the pool the **rating-greedy** selector draws the starting XI from, so a club seventeen fit and one bench short fielded its best banned man in a fixture it could have played legally without him, after which ERR-044-003 stage 1's serving exemption stalled that ban indefinitely. §3.4's own rationale forbids exactly that outcome, which is why the ordering half is a defect correction and not an owner question; the trigger half is a clarification, with the eleven-vs-eighteen narrowing recorded as the OPEN owner question. The key becomes probe-qualified — the first candidate, in roster order, the selector would BENCH; earliest roster position only when no choice avoids the XI — entirely inside `AvailabilityComposition` (the reviewer's proposed engine-contract change was rejected). Digest invariance NOT claimed for seasons whose extremis tier fires. **`ERR-044-019`:** #44 §2.3 and §7.2 both asserted "a suspended player plays only when the alternative is a club that cannot take the field at all", false of the implementation on both halves; corrected to the two-case form (benched ⇒ ban advances; forced start ⇒ exempt and only then stalling), no FR row changed. **`ERR-037-003`:** #37 §3.2's `CardIssuedEvent` routing was a two-way test over a three-value ordinal, so every second-yellow dismissal counted as a plain yellow and `RedCards` stayed 0 — contradicting `MatchStatline.RedCards`'s own documented contract; the spec and code halves were committed at `2f4626f` and this row is the log entry they were owed.)
**Updated (prior):** August 16, 2026 (v2.33 — **two ids filed, `ERR-044-014` and `ERR-044-015`, both High, both
RESOLVED same commit, from an adversarial review over the ERR-044-003 stage 1 landing.**
**`ERR-044-014`** (spec + code): `DisciplineRules.OnClubFixturePlayed` decided club membership by
`PlayerId / CLUB_SQUAD_SIZE == clubId` while `Availability.MarkSuspended` — the removal half of the same
subsystem — walks the real squad. Two notions of membership, agreeing only while #27's packing holds,
validated by nothing; the precondition both the code comment and §3.3 cited (FR-DC-013's migration rule)
is not in force, since `MigratePlayerId`/`DropPlayer` have no production caller. The first disagreement —
a #31 transfer, or the id-space widening ERR-044-003 stage 3 already requires — removes a banned player
from every squad he is really in while his ban never decrements: suspended forever, silently. Closed by
DELETING the derivation: the roster ids come in as an argument, supplied by `SeasonLoop` from the
UNFILTERED squad at the same resolve→filter→configure site the fielded XIs are derived at, and membership
is presence in them. `Discipline.Tests` 118 → **121/121**, the disagreement lock executed against a
restored derivation and observed to fail. **`ERR-044-015`** (spec-only): §4.5's composition-root MUST told
the root to "run `FilterAvailable`" — the one method FR-DC-009, §2.2, §3.3 and §6.2 all say the root must
not call, and which has zero production call sites; a root built against it bypasses
`AvailabilityComposition`, so #30 §2.3 F9's back-fill never runs for suspensions and ERR-044-003's defect
is reinstated from APPROVED text. Rewritten to the landed contract, keeping the correct
FR-DC-010/ERR-044-002 both-paths scope.)
**Updated (prior):** August 15, 2026, later still (v2.32 — **reviewed-findings pass over four M-severity items
across the four files this session owns, three new ids filed and resolved (`ERR-030-043` extending
`ERR-030-035`, `ERR-017-006` extending `ERR-017-005`, `ERR-020-005` extending `ERR-020-004`), plus a
stale test-count correction in this file's own `ERR-044-006` row and in the root `CLAUDE.md`'s #44
entry, both re-derived by direct execution rather than trusted.** **`ERR-030-043`:**
`season-competition-loop/section-4.md` §4.3's `SeasonLoop` holdings-list prose — a different list from
the §2.2 data-structure list `ERR-030-035`'s own M21 citation already fixed — still named no #44 type;
§4.2's file layout was amended at `ERR-030-035` (v0.7) in the same landing but §4.3's architecture prose
was not, the THIRD recurrence of this section's own recorded omission class. New bullet naming
`_discipline`/`_disciplineRules`/`Discipline`/`_disciplineDriver`, verified against
`src/season-save/SeasonLoop.cs`. **`ERR-017-006`:** `event-system/section-3.md` §3.10's own
completeness preamble was false for two of the four codes its own `ERR-017-005` design-fixed-`[GT]`
note names by number — `ErrEvtUnregisteredOrdinal` (`0x1706`) and `ErrEvtOrdinalCollision` (`0x1707`)
had no catalogue row despite existing in code since v1.1/v1.2 (May 2026); two rows added.
**`ERR-020-005`:** `code-standards/section-4.md` §4.2's carve-out paragraph (`ERR-020-004`) sat 25 lines
above the subsection heading a reader looks under for the routing rule, which still stated the
pre-carve-out sentence unqualified; qualified in place. **The `ERR-044-006` stale figure:** that row's
own "measured today" claim — `Discipline.Tests` **105/105** — was already one commit's re-derivation
behind the branch it was written on (`45a29ae`, two commits before the filing commit, had already taken
the suite to 118 cases). Re-measured by direct execution — `dotnet test
src/discipline/tests/discipline-tests.gen.csproj -c Debug` (no `vstest` process was running;
`ps -o args -C dotnet | grep -c vstest` → 0) — **Passed! Failed: 0, Passed: 118, Skipped: 0, Total:
118**. Annotated in place per this file's own convention; root `CLAUDE.md`'s #44 entry's `81/81`
corrected the same way to the same measured figure. **Also checked, found correct, not changed:** the
#44 entry's `SEASON_SAVE_FORMAT_VERSION` "5 → 6" (code reads `6`, `SeasonSaveConstants.cs:81`),
`SNAPSHOT_SCHEMA_VERSION` "still 21" (code reads `21`, `MatchEngineConstants.cs:286`), and the
"35th production assembly" figure (a direct count of non-test `.asmdef` files under `src/` returns
**35**, matching the root `CLAUDE.md` header's own count) — the ordinal claim that discipline landed
*as* the 35th, specifically, was not independently re-derivable from a file count and is left as
recorded rather than re-verified.**)
**Updated (prior):** August 15, 2026, later still (v2.31 — **`ERR-030-042` filed and RESOLVED, spec only, no
code change: a reviewed High against APPROVED Season & Competition Loop #30.** §3.4's depleted-squad
rule states ONE ordering key and then claims "#44/#36 … inherit the rule unchanged when they join —
#44 has". #44 did not: `AvailabilityComposition.Reinstate` runs a two-tier rule (injured first,
suspended only in extremis) that #44 §2.3/§7.2 record as an owner decision and #30 — which OWNS the
rule — never stated. The mechanism is what makes it a High rather than a doc nit: the ordering key is
written only by #41, so a suspended-but-uninjured player keeps the `int` default 0 and a verbatim
implementation of §3.4 presses BANNED players back ahead of every injured one, silently, with the
never-worse-than-unfiltered invariant intact throughout. It also falsifies `ERR-044-003` stage 1's
cost argument under such an implementation, since a reinstated banned player is exempt from serving.
Fixed in the owning text: tiers stated, "inherit the rule unchanged" requalified to the INVARIANT and
explicitly not the ordering, the zero-default trap carried as a MUST NOT.) Prior: August 15, 2026, later (v2.30 — **`ERR-017-005` and `ERR-020-004` filed and resolved**, both owed by #44's AR round 5 and both filed here by the orchestrator because their fixers were scoped out of this file. `ERR-017-005`: two restatements of facts `ERR-017-004` had already corrected survived in files that fix did not touch — the grep-boundary class again. `ERR-020-004`: §4.2's routing rule had no branch for a many-consumers/one-owner constant, which is why the code carried a false "only consuming assembly" justification; owner chose the carve-out. **`ERR-017-004`'s own body text made the identical false claim and is corrected in place** rather than left to contradict the catalogue it governs.) Prior: August 15, 2026, yet later still (v2.29 — two ids filed against Season & Competition Loop
#30, both RESOLVED same commit, from a reviewed-findings pass over the ERR-044-003 stage 1 landing.
**`ERR-030-040`** (M1): §3.4's `OnClubFixturePlayed` comment (both the spec pseudocode and its
`SeasonLoop.cs` mirror) went stale the moment ERR-044-003 stage 1 landed a second guard beside it —
the same defect shape L9 fixed for this exact passage two days earlier, recurring at the next thing
that touched it. **`ERR-030-041`** (M7): §3.5 step (f)'s idempotency rationale was simply wrong — the
code comment at the same site had the right reason all along, and nobody had checked the two against
each other. Filed as two ids rather than one because they are different defects in different sections,
found together but independent — the same posture ERR-030-037/-038/-039 used for the same landing.
**Updated (prior):** August 15, 2026, yet later (v2.28 — **six ids filed, `ERR-044-007` through `ERR-044-012`,
all RESOLVED same commit except `ERR-044-010` (recorded, not fixed) — a reviewed-findings pass over the
#44 C1/C2 landing, spec text only, no code change on five of six; the sixth (`ERR-044-012`) is a
back-prop owed by a code change (`DisciplineConstants.cs` v1.3, M26) already landed in a concurrent
commit.** **`ERR-044-007`** (Medium, §2.2/§2.3/§4.5): §2.2's `DisciplineRules.OnClubFixturePlayed`
signature still showed the pre-ERR-044-003-stage-1 `(int clubId)` form — the code has taken
`(int clubId, int[] fieldedPlayerIds)` since that same-day landing, and v0.7's own amendment reached
FR-DC-011/§3.3/§4.5 but never this code block — and the block was missing three landed members
(`DisciplineRules.State`, `CardLedgerFold.PendingCardCount`, the public static
`CardLedgerFold.RequireCommittableConfig()`, the round-level `[GT]` pre-check §3.1's pseudocode calls
and `SeasonLoop.PlayNextRound` enforces in production with no §4.5 MUST naming it). F2 (§2.3) extended
to state the null-`fieldedPlayerIds` refusal §3.3's own pseudocode already attributed to it
(`REQUIRE fieldedPlayerIds is not null  # F2`) but the table never named. Grouped under one id: all
three are the same defect class (§2's API surface not tracking a landed member or refusal), found in
the same pass, in the same two sections. **`ERR-044-008`** (Low, §1.2/§4.3/§7.3/§8.1): four sites
(the §1.2 instance found on a full-file sweep after fixing the other three) stated "one tap feeds
both [#37 and #44] when both are built" as a FUTURE condition; both assemblies have
existed since July 27 (#37) and August 13 (#44), and `src/discipline/IDisciplineTickLedgerTap.cs`'s own
remarks record the prediction as checked and found FALSE — §4.1's reference rule makes #37's identical
tap interface unreachable from either #44 or the composition root that owns the engine, so #44 declares
its own interface and always will. **`ERR-044-009`** (Low, §1.4 KD-2): KD-2's closing sentence still
read "the `Incoming`-id semantics are absorbed by the fold either way and re-verified at T-phase" — the
T-phase re-verification ran and FAILED (`ERR-044-001`, Appendix C's "slot 19" example was
unimplementable), and `appendices.md` v0.3 deleted the sibling hedge in Appendix C while this row, which
stated the assumption first, was left standing through a later, unrelated KD-2 edit (v0.2, ERR-044-003
stage 1). Restated onto the verified synthetic bench-id formula. **`ERR-044-010`** (Low, RECORDED NOT
FIXED — FR-DC-011/§3.3/§4.5): the ERR-044-003 stage 1 exemption's `fieldedPlayerIds` is, today,
`SeasonLoop.FieldedXi` — the STARTING eleven, derived from the same pre-kickoff `LineupSelector` walk
`ConfigureSquads` consumes, not a record of who actually played. Correct only because no
`MatchEngine.SubstitutePlayer` call site exists on the season path (`CardLedgerFold.cs`'s own recorded
gap); #44 is otherwise scrupulously substitution-correct for card ATTRIBUTION (the occupancy fold
tracks every `SubstitutionEvent`), and this exemption is not. The moment a substitution seam gets a
production caller, a suspended player fielded as a SUBSTITUTE rather than a starter will not appear in
the array, reopening ERR-044-003's free-appearance defect at the substitution boundary. Recorded at all
three sites; the fix belongs at the #30-owned derivation site once a substitution seam lands.
**`ERR-044-011`** (Low, §6.2): the first bullet named `OnTapRecord`, a member that has never existed
(`ObserveTick` is the real one, and the pre-M27 pseudocode name §3.1 itself carried before its own
v0.10 rewrite); `OnClubFixturePlayed`'s complexity was stated as "O(active-ban entries for the club),
typically 0–2" when `DisciplineRules.cs:265`'s descending loop walks the WHOLE state across every club,
with an additional O(11) `WasFielded` scan per active ban for the stage-1 exemption. **`ERR-044-012`**
(Low, Appendix A — back-prop owed, not spec-original): `LEAGUE_COMPETITION_KEY` was tagged `[FIXED]`
while being a verbatim copy of APPROVED Competition Structure #43's `LEAGUE_COMPETITION_ID`
(`docs/specs/competition-structure/appendices.md`, FR-CP-004) — the identical `[CROSS]` argument
already applied two rows below to `CardKindYellow`/`Red`/`SecondYellow` (L17), missed for this
constant; `DisciplineConstants.cs` v1.3 (a concurrent commit, M26) already re-tagged the code side and
explicitly recorded the back-prop as owed. Re-tagged to match; literal `0` unchanged, since #43 has no
`src/` assembly to bind a real mirror to. Files: `discipline-suspensions/section-1.md` v0.4,
`section-2.md` v0.8, `section-3.md` v0.11, `section-4.md` v0.8, `section-6.md` v0.3, `section-7.md`
v0.7, `section-8.md` v0.3, `appendices.md` v0.7. Prior entry below.)
**Updated (prior):** August 15, 2026, later (v2.27 — **`ERR-044-006` filed and RESOLVED same commit** (#44
Discipline & Suspensions adversarial-review round 5, the round's High — spec text only, no code
change): **#44 §5's traceability table named two tests that do not exist, and §9 ratified three gates
on them.** **T-DC-VIEW-001** ("exercising every #44 path leaves the #27 canonical squads
byte-identical") had exactly one test, `AvailabilityTests.FilterAvailable_LeavesTheSourceSquad-
Untouched`, **deliberately deleted at the C1/C2 adversarial review round 1 as tautological** — the
deletion is recorded in that file's own version history (v1.1, L4(a): *"`Squad` is immutable and
exposes no mutator, so no implementation could ever fail that assertion; it had no real failure
mode"*) — and no replacement was ever written, in that suite or any other; `SeasonLoopDisciplineTests`
listed the same id in its `Spec:` header, so **two files claimed it and neither implemented it**.
**T-DC-INT-001** ("every field integer; no float (static/reflection assertion); #44 registers no RNG
stream") was never implemented in either half — `grep -n "typeof\|GetFields\|Reflection"
src/discipline/tests/*.cs` returns nothing. **G6 ✅ cited T-DC-VIEW-001, G13 ✅ cited T-DC-INT-001, and
G14 ✅ ("FR-DC-001..022 each traceable to a T-DC-* test") cited §5.6, which asserted total coverage** —
this project's own founding trap (root `CLAUDE.md`: *"Fabricated checklist values … Approval Checklist
claimed sections existed that were never written"*). **The aggravating fact is that the August-15
round-4 pass existed specifically to fix §5's staleness**: it verified only the four rows it ADDED,
left these untouched, and then re-certified G14 "against the corrected table" — a partial verification
presented as a complete one, which is why this entry's fix is a per-FR map a reader can re-derive by
grep rather than a summary sentence. **Three further defective rows found by verifying every
surviving `T-DC-*` row against the two test assemblies**, the first of them worse than either named
test: **T-DC-VIEW-002 required "a pass-through filter returns an equal (but DISTINCT-COPY) squad"** —
the opposite of FR-DC-009's *"with no active ban it MUST pass the squad through unchanged"*, of the
enforced `Assert.That(result, Is.SameAs(squad))` contract, and of FR-DC-018's identity floor. That is
not absent evidence but **wrong instruction**: an implementer conforming to §5 as written returns an
equal-but-distinct `Squad` on every clean fixture, passes every other row in the table, and silently
moves **every** fixture's digest. **T-DC-FOLD-001** described a scripted kind-{0,0,2,1} sequence test
that has never existed (the FR-DC-006 property is locked, per kind, by three other tests), and
**T-DC-DET-001** claimed "and identical filtered squads", for which no test exists and one would be a
tautology of the L4(a) class. **Two requirements, FR-DC-002 and FR-DC-022, were named by no row at
all** while G14 certified total traceability. **Resolution:** both dead rows WITHDRAWN in place under
the `T-DC-BAN-004` precedent (kept as records, not deleted), each stating what establishes the
requirement instead; T-DC-VIEW-002 corrected; §5.6 replaced by a per-FR **Test / Construction /
Deferral** disposition map; G14's wording widened to that third disposition — recorded as a widening,
with its reason (a structural negative like FR-DC-019 has no observable to assert, which is **G3's
already-approved posture**) and with its one honest gap named rather than absorbed: **FR-DC-020's
integer posture rests on audit, not enforcement**, and a reflection lock — unlike the two tautologies
withdrawn here, it guards a change that can actually be made — is filed as a §9.2 follow-up instead of
being claimed. All three gates stay ✅ on re-derived evidence: every property they certify was verified
to hold. Prior entry below.)
**Updated (prior):** August 15, 2026 (v2.26 — **`ERR-017-004` filed and RESOLVED same commit** (#44 Discipline
...
two-match red cost a depleted club nothing. The owner's decision is **exemption**:
`DisciplineRules.OnClubFixturePlayed` now takes a required `int[] fieldedPlayerIds` and skips anyone
in it, because a ban is served by the club playing **without** the banned player. Fixed at the serving
site rather than in `AvailabilityComposition` — the rule is about who PLAYED — so the reinstatement
tier order is untouched; the eleven is the array the FR-MD-010 appearance record already derived at
the filter+configure site, with `SeasonLoop.FieldedXi` widening that derivation's gate to the union of
its two consumers. Behaviour-identical on every non-extremis fixture by construction. **Stages 2 and 3
of the owner's staged answer are agreed and NOT implemented** — youth call-ups (blocked: #42 has no
`src/` assembly) then generated low-attribute cover (blocked: the `clubId × CLUB_SQUAD_SIZE + local`
id space is fully packed at 25) — see the row for the full account. Also recorded this pass: the
**foul/card calibration order is settled** — arm the W2 tackle challenge FIRST, then calibrate once
against the complete contact stream, rather than fitting `FoulCallProbability` to today's pre-tackle
population; that is KD-W1 applied to the open foul/card entry, and it is a sequencing decision, not a
finding. Prior: v2.24 — **The FIFTH adversarial-review pass over the
...
leaving the public nine-argument form to forward `disciplineWired: true` unconditionally — so the guard
was live on one write entry point and bypassed on the one every external caller reaches, and `new
DisciplineState()` (the value the `discipline` parameter's own doc sanctions for "no cards recorded
yet") emptied a populated tally with no refusal. Fixed by promoting the flag onto the public long form
and deleting the forwarding overload. The same pass fixed two Mediums, recorded in that row rather than
as new ids: **M17** (`SeasonLoop.PlayNextRound` runs the four #44 `[GT]` guards once, before the fixture
loop, because M6's after-the-mark commit placement makes a config throw there strand the round
PERMANENTLY — both the code comment and #30 §3.4, which claimed "nothing is lost by running the pair
last", corrected to state that cost) and **M16** (`SeasonLoop.TestOnly_AfterHomeClubServed`, process-
static mutable state on a production class invoked from the production round loop, deleted in favour of
a constructor-injected `IFixtureDisciplineDriver` seam — FR-CS-051..054). Prior entry below.)
**Updated (prior):** August 13, 2026, later still (v2.22 — **`ERR-030-037` EXTENDED (L9), a third
adversarial-review pass over the #44 C1/C2 landing: the row's own resolution text asserted
`OnClubFixturePlayed` was config-fallible alongside `fold.Commit`, which is false — corrected in both
`SeasonLoop.cs` and `season-competition-loop/section-3.md`.** The same pass found six further Medium/Low
findings, all fixed and recorded in the extended row rather than as new ids: **M12** (a test-only hook
locking the M6 serve+commit block's POSITION, which no existing test actually covered — two
version-history rows had wrongly credited that coverage to a test that locks a different property,
M7's ORDER, corrected in place in both `SeasonLoop.cs` and `SeasonLoopDisciplineTests.cs`), **M13**
(`CardLedgerFold.Commit` made atomic), **M14** (`PlayerCareerStates.MarkUnavailable` and
`Availability.MarkSuspended` given one shared mask contract), **L10** (a `RollToNextSeason` comment's
idempotence reasoning corrected), **L11** (a non-discriminating unit test replaced via a new internal
`DisciplineRules.ApplySecondYellow` seam), **L12** (three `discipline-suspensions` spec files brought
back in line with the landed surface), **L13** (`discipline-suspensions` §2.3 gains **F6** for the two
`[GT]` fail-loud guards, with the matching guard calls added to §3.2's pseudocode). Prior entry below.)
**Updated (prior):** August 13, 2026, later still (v2.21 — **`ERR-030-038`, a High from the third adversarial-review
pass over the #44 C1/C2 landing, filed and resolved in the same commit — and it is a defect IN
`ERR-030-036`'s own fix, in both its code and its spec text.** That fix's new
`RequireDestinationCarriesNoDiscipline` was invoked on `discipline.Count == 0`, treating an empty tally
as proof the tally was dropped. Sound for the two sibling guards — a career roster never legitimately
empties — and **false for #44**: FR-DC-017 makes the empty tally the *canonical clean state*, dropping a
row the instant it reaches `(0, 0)`, mid-season the moment a ban is served out with no residual yellows
and again for every yellows-only row at the boundary sweep. So a **correctly wired, correctly resumed**
loop whose last row cleared could never save over its own file again, and stayed unsaveable until fresh
cards accrued — with the thrown message telling the operator to resume with
`SeasonSaveContents.Discipline`, which is exactly what they had done. **Demonstrated by execution
against the built assemblies** (save `(player 100, yellows 0, ban 1)` → OK; `OnClubFixturePlayed(4)`
serves it and FR-DC-017 drops the row; saving the same live tally to the same path throws). ERR-030-036's
four locks miss it **by construction** — fresh path, already-empty destination, and two resume cases
carrying a populated tally throughout; nothing exercised a tally that **drains**. Fixed by re-keying the
guard from "the tally is empty" to "**no #44 subsystem is wired behind this save**", the predicate that
actually distinguishes the loss and the one the save root's `?? new DisciplineState()` destroys: a new
internal `Save` overload takes a **required** `bool disciplineWired` (never defaulted — the reason those
five block parameters reject null), the public long form forwards `true` because a caller that hands over
a tally drives #44 by construction, and `Save(SeasonLoop, …)` passes `loop.Discipline != null`. Chosen
over `null`-meaning-unwired so `null` never acquires a second meaning in a file whose doctrine is that it
has none. **Recorded, not fixed:** a loop wired with a *fresh* tally rather than the file's is
indistinguishable here from a legitimate drain; `Restore`'s `disciplineOrNull` closes that, no
destination predicate can. **Back-prop:** Appendix B.1's rule was itself over-broad — it said "refuse to
overwrite a populated block with an **empty** one, per block family" and extended that to #44 — and now
keys on the writing composition, with the explicit qualifier that emptiness is evidence of loss only for
a family with **no legitimate empty state**, #44 being the counter-example (`appendices.md` v1.4).
**Mutation-verified: restoring the `Count == 0` predicate fails both new locks and nothing else.** Prior
entry below.)
**Updated (prior):** August 13, 2026, later still (v2.20 — **Two Medium findings owed by the #44 Discipline
C1/C2 adversarial review, `ERR-044-004` and `ERR-044-005`, filed and resolved in the same commit —
spec back-props only, the code fixes having already landed.** **ERR-044-004 (§2.3 F2):** F2's
fail-loud covered only the CLUB half of "a club/player outside the resolvable universe" — a negative
`PlayerId` was constructible at four entry points (`DisciplineEntry`'s own constructor, which gated
`Yellows`/`BanMatchesRemaining` but not `PlayerId`; `AddYellow`/`AddBan`/`ApplyCard`;
`DisciplineSaveCodec.Decode`; `MigratePlayerId`'s target id), and C#'s truncating integer division
sends every id in `[-24, -1]` to club 0 in `OnClubFixturePlayed` — probe-verified: `new
DisciplineEntry(-5,0,0,2)` constructed; `AddBan(-5,0,3)` wrote a row; `OnClubFixturePlayed(0)`
decremented it 3→2; Encode/Decode round-tripped it; `MigratePlayerId(100,-7)` succeeded — a player
who does not exist, served, decremented, saved and migrated throughout. `CardLedgerFold`'s
constructor already refused a negative seed id, so the assembly disagreed with itself at one of four
entry points. The ERR-041-019 shape one spec over: a derivation safe only by accident of today's
single id allocator, stated nowhere and checked nowhere, and the `OnClubFixturePlayed` doc paragraph
had argued the hazard away ("no id divides to a negative club") rather than checking it. **ERR-044-005
(FR-DC-009 / §3.3):** FR-DC-009's "reduced value copy" requirement is stated as total, but
`FilterAvailable` was partial — an all-suspended squad drove `new Squad(clubId, new
PlayerRecord[0])` straight into `Squad`'s own zero-player refusal, throwing a cross-assembly
`ArgumentException` naming neither discipline nor the cause. The spec never states the all-removed
case and `Squad` cannot represent it, so the requirement was unsatisfiable as written — and
`FilterAvailable` has **no production caller** (#30's composed seam consumes `MarkSuspended`'s mask
directly, correctly returning `null` for the zero case through `AvailabilityComposition`'s own
private helper), so the gap was exercised only by FR-DC-009's own unit tests. **Resolution (already
in the tree, both):** `DisciplineEntry`'s constructor and `DisciplineSaveCodec.Decode` both now
refuse `playerId < 0` (the #29/#41 AR pass 6 both-boundaries rule — the file boundary refuses what
the composition boundary refuses), locked by `Decode_NegativePlayerId_Throws`; `FilterAvailable`
returns `null` for a fully-suspended squad, mirroring the composed seam's private helper, with its
XML doc recording that this method is FR-DC-009's own surface rather than #44's production path,
locked by `FilterAvailable_EveryPlayerSuspended_ReturnsNull`. **Back-props:** `discipline-suspensions/
section-2.md` §2.3 F2 gains the player half explicitly and FR-DC-009 gains the null-return case;
`section-3.md` §3.3's `FilterAvailable` pseudocode comment gains the same null case and names
`MarkSuspended`'s mask as the actual production path. No code, `.asmdef` or `.meta` file touched by
this pass — the fixes were already green (`Discipline.Tests` 92/92, `SeasonSave.Tests` 425/0/3).
Prior entry below.)
**Updated (prior):** August 13, 2026, later still (v2.19 — **A second adversarial-review pass over the #44
C1/C2 landing (round 2): six findings, all fixed — one Medium filed as `ERR-030-037`, the rest
code/doc-only with no spec text to have sourced them.** **M6 (no id):** `SeasonLoop.PlayNextRound`
ran the FR-DC-011 serve decrement and `fold?.Commit` BEFORE `MarkFixturePlayed`, and both calls are
FALLIBLE under a bound config (`DisciplineRules.AddYellow` throws below
`YellowAccumulationThreshold < 1`; the ban-length guard throws on a negative `[GT]`) — a throw there
left the fixture UNPLAYED with both clubs' bans already decremented once, and a caller retrying
`AdvanceAndPlayNextRound` would replay the SAME fixture (the unplayed-index filter would not exclude
it) and decrement every outstanding ban in the league a SECOND time, silently. The neighbouring
comment claiming "it is the only fallible call in this block, and a throw after `MarkFixturePlayed`
would strand the round" predated the discipline calls joining the block and was false the moment
they did. Fixed by moving the serve+commit pair to AFTER `MarkFixturePlayed`, preserving the
serve-before-commit ORDER between the two calls — the off-by-one contract itself — untouched
(`SeasonLoop.cs` v1.22). **M7 (no id):** that within-fixture ordering had NO lock at all — every
existing test either pre-seeds a ban before kickoff (blind to commit order) or runs `QuickSimAll`
(no fold is ever built), so swapping the two calls would have shipped green. Fixed with new
`SeasonLoopDisciplineTests.ANewBanEarnedThisFixtureIsNotServedByThisSameFixture`: an independent
deterministic replay of the SAME fixture (both tallies start empty, so nobody is
suspension-filtered and the two engine runs are byte-identical) commits its fold to a fresh,
unrelated `DisciplineRules` with no serving anywhere near it, giving the ground-truth tally the
production run's own commit must reproduce exactly if serving does not touch a card the fixture
just earned. **Verified by execution**: reverting M6 turns this test red; restoring it turns the
suite green again. Also landed as normative text at `discipline-suspensions/section-3.md` §3.3 v0.5.
**M8 (`ERR-030-037`, row below):** `season-competition-loop/section-3.md` §3.4/§3.5's normative
pseudocode never gained the #44 loop at all — patched to the POST-M6 ordering. **M9:**
`season-save-tests.asmdef` used `TacticalDirector.Discipline` types (three files) without declaring
the reference — compiled only because MSBuild's transitive `ProjectReference` papered over it; the
reference is now explicit and `discipline.gen.csproj` confirmed present in the generated test
project. **M11:** `SeasonLoop.RollToNextSeason`'s roster-sync site gains a comment naming FR-DC-013's
two unwired methods (`DisciplineRules.MigratePlayerId`/`DropPlayer`, zero production callers) and the
id-reuse-inherits-the-ban hazard a #28 boundary regen would hit (player ids are
`clubId * CLUB_SQUAD_SIZE + local`); mirrored into `discipline-suspensions/section-7.md` §7.2.
**L1:** deleted a `SeasonLoop.cs` comment sentence claiming serving "runs AFTER the fold committed
this fixture's own cards (inside ResolveFixture)" — backwards on both counts. **L7:** extends the
existing `ERR-044-003` row (not a new id) — the extremis back-fill fields a suspended player AND
that fixture's serving then discharges his ban regardless, the ERR-044-002 hazard reintroduced one
tier over by ERR-044-003's own compromise; recorded in `AvailabilityComposition`'s type remarks as an
owner call, not decided. **L8:** `PlayerCareerStates.SelectAvailable` imported `#44`'s catalogue to
name a `competitionId` argument `Compose` provably never reads on its `discipline: null` path — now
`competitionId: 0` with a comment, and the `using TacticalDirector.Discipline;` line dropped.
**L6:** a stale-future-tense sweep of `discipline-suspensions/section-4.md`, `section-7.md`,
`section-8.md` and `section-9-approval-checklist.md` — four "null seam until #44 T2" / "at the
T-phase" mentions corrected to LIVE/landed, now that T2 has shipped; `file-manifest.md`'s
...
**Updated:** August 12, 2026 (v1.98 — **ERR-014-006 filed + RESOLVED at wiring-backlog W2: the tackle.** #14 KD-6 delegated the tackle OUTCOME to #8 and #3, and neither can accept — #8's `ActionType` ordinals are exhausted by the 3-bit composure-noise field and it has no tackle model; #3 defers slide tackles to Stage 2 and its own KR-3 says ball-first-vs-player-first is undecidable at 60 Hz. The `ERR-011-010` shape, and the cost was ten weeks of unreachable code plus an engine in which **no path anywhere dispossessed a controlled carrier**. New #14 §3.6.5 takes it back on the W1 precedent, as an abstract attribute duel with **four** outcomes — the fourth, `BALL_LOOSE`, at owner direction. `SNAPSHOT_SCHEMA_VERSION` 20 → 21; card-severity draw order moves by design; no digest invariance claimed. Prior entry below.)
...
| ERR-014-006 | Defensive AI #14 **KD-6 delegated the tackle outcome to two delegates that structurally cannot accept it.** KD-6 read *"#14 produces `TackleIntentRequest` intent; #8 mediates dispatch; #3 owns contact physics"* — but #8's `ActionType` ordinal space is exhausted (`SAVE = 7` is the last value that fits the 3-bit composure-noise field in `ActionSelector.ComputeOptionNoise`; an eighth forces a digest rebaseline, which is why the DT-emitted HEADER is deferred as wiring-backlog W9) and #8 has no tackle model, while #3 §7.2.1 defers slide-tackle collision to **Stage 2** and its KR-3 records that ball-first-versus-player-first is undecidable at 60 Hz discrete detection. So the outcome belonged to nobody, and the consequence was not a gap in a design but **ten weeks of unreachable code**: `GetTackleIntentRequests` was populated every tick and read by nobody, both engine collision adapters hardcoded `GetAndClearTackleFlag => false`, and #5 §3.8.5's tackle-interrupt branch was dead. Exactly the `ERR-011-010` shape. **Also in this entry:** #14's tackle intent has no ball model at all — `MarkAssigner` contains zero ball references while `DefensiveAgentSnapshot.HasBall` is supplied to it and read by nothing — and `TackleIntentRequest` is absent from the engine's per-tick digest while #14 §4.6 / XC-014-020 / XC-014-024 declare it load-bearing. | **High** | 10 | ✅ **Resolved August 12, 2026** (wiring backlog W2) — new **#14 §3.6.5 Tackle Outcome Resolution** takes the decision back into the spec that owns the players, on the W1 precedent (#11 §3.7.0 took back the keeper's rush-commit distance because *"how far he comes out is a property of the keeper"*; whether a defender wins a challenge is the same sentence). A Stage-0 tackle **cannot** be a physics contact, because the physics that would adjudicate it is a Stage-2 deliverable — so it is an abstract duel between two players' abilities. **Four** outcomes, not three: `MISSED` / `BALL_WON` / **`BALL_LOOSE`** / `FOUL`, the third at owner direction, because won-or-missed has no way to express the commonest result of getting a foot in — the ball going somewhere neither player controls — and folding it into `BALL_WON` makes every successful challenge a clean turnover. One draw, three nested inverse transforms (the KD-F2 shape), continuous in every input per football-judgment §6 P1. Ten new `[GT]` + one `[FIXED]` ceiling (`[FIXED]` because it is a numerical guarantee that the second transform never divides by zero, not a football judgment); every `[GT]` un-calibrated per KD-W1. Composition root keeps only the trigger geometry; #3 retains contact physics for Stage 2+, at which point §3.6.5 becomes the fallback rather than a parallel authority. **`Tackling` gains its first consumer anywhere in the tree** (it and `Marking` were canonical #27 attributes read by no formula); `Marking` still has none. `DOMAIN_TAG_DEFENSIVE_AI` (0x1A) gains its first draw site, un-blocking #14's own T-DA-DET-005. Nine #14 files back-propagated; `SNAPSHOT_SCHEMA_VERSION` 20 → 21. **Draw order on `match-flow.card-severity` MOVES** (the foul branch draws on ticks that previously had none) — intended, and no digest invariance is claimed anywhere in the landing. See `docs/tracking/tackle-wiring-design.md`. |
...
| ERR-017-005 | Event System #17: two restatements of facts `ERR-017-004` had already corrected were missed in files that fix did not touch — Appendix A.1's note still read "design-fixed `[GT]`" against §3.10's same-day `[FIXED]` retag, and Appendix A row 0x06's payload list still read `foulOrdinal: byte`, six weeks stale against the AR-5 `ushort` widening (the engine publishes `0xFFFF`, a value the declared type cannot hold) | Medium | 2 | ✅ **RESOLVED August 15, 2026, same commit** (#44 AR round 5, M2/M3). Appendix A.1's note corrected to `[FIXED]`; row 0x06 corrected to `foulOrdinal: ushort` with `Current version` left at 1 — verified against code that no shipped consumer ever decoded the byte form, so this is a documentation correction, not an Appendix C migration. §3.10's `[GT]`-sub-class note reworded to carve out wire-format encodings as `[FIXED]` explicitly and to retract its claim that `[FIXED]` is physics-only. **Recorded, checked but NOT resolved:** the remaining design-fixed-`[GT]` rows (`EVENT_TYPE_ORDINAL_WIDTH`, `PAYLOAD_VERSION_WIDTH`, the `ERR_EVT_*` codes) were verified to be `public const` too — the same shape that failed `CARD_KIND_*`'s test — so the same tagging question is open for them. Root cause is the grep-boundary class this project keeps re-filing: a correction landed in one file of a multi-file spec without sweeping for restatements elsewhere. Documentation-only; the code was already correct. |
...
| ERR-020-003 | Code Standards #20 §3.5.2 draws the layer rule as `Physics ──► Mechanics ──► AI ──► UI`, while the root `CLAUDE.md` states the same rule as **AI → Mechanics → Physics, never the reverse**. The arrows point opposite ways. Both are defensible readings of their own notation (#20's arrow = "is available to"; CLAUDE.md's = "may reference"), and neither states which it means, so the two authoritative statements of the project's single most load-bearing architectural rule read as contradictory. | Low | 3 | ✅ **RESOLVED August 17, 2026 — all three renderings now label their arrow.** (As filed, 🟡 August 2, 2026.) Proposed fix: label the arrow in #20 §3.5.2 explicitly (`──► reads "may be referenced by"`) and add the reference-direction sentence verbatim beneath the diagram, so the two files state the rule in the same words. No behaviour change; the rule itself is not in dispute. |
| ERR-020-004 | Code Standards #20 §4.2's `[CROSS]` routing rule states only a two-way split (≥ 2 consumers → `ProjectConstants.cs`; exactly 1 → the source spec's catalogue), with no branch for a constant that has MANY consumers but ONE owning catalogue. `DisciplineConstants.CardKindYellow` therefore justified its (correct) direct mirror with a false "#44 is the only consuming assembly" claim — `MatchEngineConstants` and `MatchAnalyticsConstants` mirror the same encoding, so there are three consuming catalogues rather than one | Medium | 4 | ✅ **RESOLVED August 15, 2026, same commit** (#44 AR round 5, M4; owner decision: carve-out, no code movement). §4.2 and `src/CLAUDE.md`'s `[CROSS]` mirrors section both gain an **owning-catalogue carve-out** — a constant whose owning spec has its own catalogue mirrors from THAT catalogue regardless of consumer count; `ProjectConstants.cs` routing is for constants with no single owning catalogue. The argument, not merely the rule: routing a spec-owned encoding through a shared bucket adds a hop without adding an authority. The false sentence in `DisciplineConstants.cs` is deleted and replaced with the real reason, and `ERR-017-004`'s own body text — which made the identical false claim — is corrected in place. No code moved. |
| ERR-030-043 | Season & Competition Loop #30 §4.3, found in a **reviewed-findings pass** (August 15, 2026): the `SeasonLoop` holdings-list prose — this section's own architecture description, distinct from `section-2.md` §2.2's data-structure list that `ERR-030-035`'s M21 citation already fixed — still named no #44 type, three landings after `SeasonLoop` started holding `_discipline`/`_disciplineRules`, exposing `Discipline`, and taking `disciplineOrNull` on both the constructor and `Restore`. §4.2's file layout WAS amended at `ERR-030-035` (v0.7, the `DisciplineBlock.cs` row) but §4.3's holdings prose was not touched in the same commit — the THIRD recurrence of this section's own recorded omission class (v0.4 "had no career pair", v0.6 "no equivalent entry for `_progression`"). | Low | 1 | ✅ **Resolved August 15, 2026, same commit, docs-only.** New bullet added to §4.3, naming the unpaired optional `DisciplineState _discipline`, the `DisciplineRules _disciplineRules` view, the read-only `Discipline` property, and the internal `IFixtureDisciplineDriver _disciplineDriver` collaborator — verified against `src/season-save/SeasonLoop.cs` (fields, the constructor's and `Restore`'s `disciplineOrNull` parameters, the property, and the driver's construction-time companion-state refusal). `section-4.md` → **v0.8**. |
| ERR-017-006 | Event System #17 §3.10, found in a **reviewed-findings pass** (August 15, 2026): the constants-catalogue preamble states "every numeric and identifier constant declared in this spec appears here with its source tag", but `ErrEvtUnregisteredOrdinal = 0x1706` and `ErrEvtOrdinalCollision = 0x1707` — both named explicitly by `ERR-017-005`'s own design-fixed-`[GT]` sub-class note as members of the reserved `0x17NN` block — carried no catalogue row; `grep -rn "0x1706\|0x1707" docs/specs/event-system/` returned only that note. Both codes have existed in `EventSystemConstants.cs` since v1.1 (`0x1706`, May 30, 2026) and v1.2 (`0x1707`, May 31, 2026) — `ERR-017-004`'s exact defect (a #17-owned constant with no catalogue home) recurring on the two codes the correcting note itself named by number. | Low | 1 | ✅ **Resolved August 15, 2026, same commit, docs-only.** Two rows added to §3.10 — `ERR_EVT_UNREGISTERED_ORDINAL` (`0x1706`) and `ERR_EVT_ORDINAL_COLLISION` (`0x1707`), both `[GT]` per the existing design-fixed sub-class, citing `EventSystemConstants.cs`'s own doc comments (FR-EVT-020 / AR-2 M-1 for the first; `CosmeticChannel.Subscribe` / AR-4 L for the second). `section-3.md` → **v1.0.6**. |
| ERR-020-005 | Code Standards #20 §4.2, found in a **reviewed-findings pass** (August 15, 2026): the paragraph that introduced `ERR-020-004`'s carve-out (`… and have no single owning catalogue — see the carve-out below`) sits 25 lines above the subsection a reader looks under for the routing rule — "`ProjectConstants.cs` — Cross-Spec Source of Truth" — whose own opening sentence still read, unqualified: "The primary declaration for any constant that multiple specs consume lives in `ProjectConstants.cs`." Under that sentence as written, the three `CardKind*` mirrors the carve-out exists to legitimise were all non-compliant. | Low | 1 | ✅ **Resolved August 15, 2026, same commit, docs-only.** The subsection's opening sentence qualified in place to state the carve-out (a singly-owned constant's primary declaration is its owning spec's own catalogue) and narrow `ProjectConstants.cs` to the remaining case — a constant with no single owning catalogue — with the section's own `PHYSICS_TICK_HZ` worked example now cited as that case. `section-4.md` → **v1.0.3**. |
| ERR-020-006 | Code Standards #20 **§2.2.2 (FR-CS-017)**, **§3.2.1**, **§3.2.3**, **§4.2** and **Appendix E**, found at **adversarial-review round 6**: `[CROSS-PENDING]` — one of the **six** mandatory constant-source tags root `CLAUDE.md` defines — is unknown to #20 anywhere. FR-CS-017 enumerates a **closed** list of five ("`[GT]`, `[EST]`, `[FIXED]`, `[DERIVED]`, or `[CROSS]`"), and §3.2.1's table is introduced as root `CLAUDE.md`'s reproduced "verbatim as a citation" while carrying five rows against the root's six. `grep -rn 'CROSS-PENDING' docs/specs/code-standards/` returned **zero**; `git grep -c 'CROSS-PENDING' 9b841d1^ -- docs/specs \| awk -F: '{s+=$NF} END {print s}'` returned **218** CITING LINES immediately before this fix (⚠️ the figure is annotated August 18, 2026, on two counts, both found by `tools/doc-claim-check.py` — the tool written because this project kept publishing verification commands nobody re-ran. (1) The live `grep -rn 'CROSS-PENDING' docs/specs/ \| wc -l` no longer returns 218 and must not be offered as proof of a historical figure: it returns 251 today, because this fix and the passes after it keep adding citations of the tag itself — the same trap §3.2.1 and §2's v1.4 row were corrected for. (2) 218 counts LINES, not occurrences; occurrences at that commit were 235, so "every one of those 218" understated the violation set by 17.), and `.github/workflows/ci.yml`'s tag lint accepts it. So every one of those 235 occurrences was a MUST-level violation of an APPROVED spec — the tag the project uses for an unresolved cross-spec dependency was the one tag its own code standard did not permit. Aggravating: #20's **§9.4 re-approval trigger 1** requires exactly this update ("any tag addition… requires §3.2.1 and Appendix D to be updated and re-reviewed") and was not honoured when the tag was added to root `CLAUDE.md` on August 10, 2026. | High | 8 | ✅ **Resolved August 18, 2026, same commit, spec-text only.** §3.2.1's table takes the `[CROSS-PENDING]` row verbatim from root `CLAUDE.md` (source-note corrected with it); FR-CS-017's enumeration widened to all six; §3.2.3 gains the storage-class row; §4.2's `#region` ordering gains the `CrossPending` slot (five regions → six); Appendix E gains the glossary entry, and Appendix D was reviewed under §9.4 trigger 1 with no row owed — recorded rather than assumed. Five consequential five-tag enumerations corrected in the same pass (`section-1.md`, `section-5.md` §5.4 items 2/6/8, `section-8.md`, `section-9` Q-01). The **73-FR count and the FR-CS numbering are untouched** — FR-CS-017's list widened in place. `section-2.md` **v1.4**, `section-3.md` **v1.6**, `section-4.md` **v1.1**, `appendices.md` **v1.2**, `section-1.md` **v1.0.3**, `section-5.md` **v1.1**, `section-8.md` **v1.2**, `section-9-approval-checklist.md` **v1.1.1**. |
| ERR-020-007 | Code Standards #20 **§4.2**, **§3.2.3** and **FR-CS-022**, found at **adversarial-review round 6**: §4.2's `ERR-020-004` carve-out — text landed on this branch — cites `DisciplineConstants.CardKindYellow` as "a compliant mirror", but that declaration is `public const byte` (`src/discipline/DisciplineConstants.cs:147`) while FR-CS-022 and §3.2.3 both require a `[CROSS]` constant to be `public static readonly` PascalCase. **#20 certified as compliant a declaration its own MUST forbids**, and the pattern is tree-wide: **19** symbol-referencing `const` mirrors across `discipline`, `match-engine`, `match-analytics` and `player-progression` (10 PascalCase / 9 ALL_CAPS). | Medium | 3 | ✅ **Resolved August 18, 2026, same commit, spec-text only — the rule was wrong, not the tree.** A `[CROSS]` mirror whose initializer **references the owning catalogue's symbol** MAY itself be `public const`: the compiler enforces the mirror, so the divergence risk FR-CS-022 exists to prevent cannot arise. **Bounded deliberately, on measured evidence, in two ways the proposed wording did not have.** (1) The carve-out covers symbol-referencing initializers ONLY — **6 literal-initialized `[CROSS]` consts** exist (5 `TODO: mirror from ProjectConstants` tick-rates plus `DisciplineConstants.LeagueCompetitionKey`) where nothing is enforced; those stay non-conformant and are recorded as a code-owning follow-up rather than absorbed. (2) "Inherits the source's naming" was **NOT** adopted — `CardKindYellow` is PascalCase against an ALL_CAPS source, so that rule would have re-broken the very declaration this entry defends; naming is MAY-either and both forms are live. `section-2.md` **v1.4**, `section-3.md` **v1.6**, `section-4.md` **v1.1**. |
...
| ERR-030-034 | Season & Competition Loop #30 §3.4.1 (FR-SN-013a) / league-bootstrap **KD-7 (model shape)**, found by **executing A4a's corpus run** (August 12, 2026): KD-7 resolves a fixture as two independent **Poisson** draws, and a Poisson variable's variance equals its mean *by definition* — the shape has no spare parameter for spread. The engine's scorelines are **over-dispersed**: across the 198-match corpus the dispersion index `var/mean` averages **1.395** over 22 bucket-sides, **19 of 22 are above 1**, and the pooled test is `chi2 = 521.7` on 374 dof — **z = +5.40**. So the engine produces more blowouts and more shut-outs than any Poisson with the same means can, and correspondingly **fewer draws**: at the `dSquad ≈ 0` acceptance bucket the corpus draws at **19.2%** against the fitted model's **26.8%** — a **7.6 pp** gap, which is the whole of the W/D/L bar's miss. That figure is measured at **n = 198**: the acceptance bucket was deliberately deepened past the grid's 18 (four parallel sample windows, 180 extra matches) precisely because ERR-030-033 makes the bar unresolvable at 18, and the deepening moved the draw share from 11.1% to 19.2% — so the grid-depth reading would have overstated this defect by more than a factor of two while still, correctly, detecting it. The gap is 2.7σ against the deepened corpus's own 2.8 pp standard error. **No choice of the three fitted parameters closes this** — `BaseGoals`, `GoalRatingSlope` and `HomeAdvantageRating` all move the two *means*, and the discrepancy is in the second moment. It is a statement about the model's FAMILY, not its coefficients, which is why it is filed against KD-7's shape rather than against the fit. Consequence if left: a quick-sim league table will show systematically **more draws and fewer decisive results** than the same fixtures played through the engine — the precise "league tables feel wrong" failure roadmap risk row 1 names, surviving a fit that did its job. | High | 2 (`league-bootstrap-design.md` KD-7, `round-resolution-corpus.md`) | ◑ **Recorded, NOT fixed — but the successor is now PRE-DECIDED and gated, August 12, 2026: `league-bootstrap-design.md` **KD-7a**.** The section pins what the successor would be with KD-7's own specificity (S1 NB2 marginal as `var = μ(1+αμ)`, **not** a constant ratio, because dispersion rises with the mean; S2 `NegativeBinomialInverseCdf` by inversion, pinned by name, **one uniform per side with the existing sub-streams unchanged**, so the keyed order-independent fixed-budget contract survives exactly; S3 a new `[GT] QuickSimDispersion` whose **zero case routes to `PoissonInverseCdf` verbatim** — an explicit branch, not a limit, so `α = 0` is bit-identical to today rather than identical-in-the-limit), and states four things that stop it being adopted prematurely. **S4 — α is NOT determined by this corpus**: 0.0773 weighted vs 0.1552 unweighted, a factor of 2.01, with **one 18-sample cell carrying 36% of the weighted fit** (a variance estimate at n=18 has ~33% relative error and the weights go as `1/var²`), so adopting on it would be fitting noise; the fitter now emits both estimators, the max single-cell leverage and a determined/not-determined verdict every run. **S5 — NB2 does not fix the draw deficit and must not be adopted expecting it to**: measured 26.5% draws vs Poisson's 26.8% and the engine's 19.2%, i.e. ~0.3 pp of a 7.6 pp gap. **S6 — the draw deficit gets no pinned successor**, because its mechanism is unestablished: the shared-swing family that would cut draws implies negative home/away correlation and the corpus refutes that (**+0.044 ± 0.073**, n=198, ~3σ from the ≈ −0.20 it predicts); a Dixon–Coles `ρ` remains the candidate but needs the joint scoreline histogram at depth, which is why the raw rows are now committed under `docs/tracking/corpus-data/`. **S7 — the adoption tripwire**: dispersion still z > 3, α determined, the draw gap still beyond 2·se under KD-8's A5, and the capture taken **post-defensive-wiring** — the corpus is produced by an engine in which no player has ever made a tackle (wiring backlog W2), and the second moment of scorelines is exactly what that wiring moves. **S8 corrects this row's own cost claim** (see the ⚠️ above): there is no save-format bump. **Both remaining findings inside this ERR are now separately stated** — marginal over-dispersion (real, family-inexpressible, successor pinned) and the draw deficit (real, mechanism unestablished, no successor) — which the original single causal sentence had conflated. **[ANNOTATED August 18, 2026 (adversarial-review round 6, H2) — two later records added; this Status cell IS the entry (no separate body exists below), and the original wording above is left as written.]** **(a) S7 condition 4 reads post-*ARMING*, not post-*building*.** `league-bootstrap-design.md` amended exactly the clause this cell quotes, on August 17, 2026 (v1.5; moved to sit at condition 4 itself at v1.6/M19): W2 landed August 12, 2026 — hours after this row was written (`ERR-014-006`) — so "no player has ever made a tackle" is now FALSE about the code, but the challenge SHIPS DISABLED at `TackleContactRadiusM = 0` (`src/match-engine/MatchEngineConstants.cs`, config default 0.0f), so it stays TRUE of every shipped match. Satisfying condition 4 therefore requires a capture from an engine running a NON-ZERO `TackleContactRadiusM`; read the other way — treating W2's landing as satisfying it — the tripwire would fire against a corpus statistically identical to the one it exists to reject. **(b) S9, owner decision, August 17, 2026: HOLD KD-7a — neither adopt nor reject; decide after the post-W2-arming calibration capture, exactly as S7 says** (`league-bootstrap-design.md` S9 / v1.5 row; recorded the same day at root `CLAUDE.md`, `open-issues.md` and `CHANGELOG.md`). The ERR itself stays ◑ open on unchanged grounds. |
| ERR-044-001 | Discipline & Suspensions #44, found at **the C1/C2 landing's own §7.1 T2 verification obligation** ("the `Incoming`-id semantics verified against the live engine"): two defects. **(a)** Appendix B specified the discipline sub-blob **version-first with no magic** — the FOURTH instance of the defect ERR-029-005/ERR-041-009 turned into a MUST at #29 §4.4 and #41 §4.4 (ERR-028-004 the third) — acute here because the season frame now carries SEVEN identically `version \| count \| i32 fields`-shaped sub-blobs (progression/training/medical/appearance/discipline plus the season and match blocks), so a transposed `byte[]` among `SeasonSaveCodec.Encode`'s payloads would decode cleanly, completely and silently as the wrong subsystem's state. **(b)** Appendix C's worked fold example placed a bench player at "slot 19" with `SubstitutionEvent{Outgoing: 7, Incoming: 19}`, hedging "(or 19 → 191, absorbed either way)" — the hedge was the spec declining to verify a fact it could have checked. `MatchEngineConstants.SQUAD_SIZE = 22` makes 19 an **on-pitch slot**, not a bench identity; `MatchEngine.SubstitutePlayer` derives a bench player's `Incoming` id as the **synthetic** `SQUAD_SIZE + teamId * SUBSTITUTES_PER_TEAM + benchIndex` ∈ [22, 36) — never an on-pitch index. Implemented as written, every post-substitution card would have been misattributed and F1 would never have fired. | High | 5 | ✅ **Resolved August 13, 2026, spec + code same commit** — `DisciplineConstants.DISCIPLINE_SAVE_MAGIC = 0x44495343` ('D','I','S','C') leads the block and is checked BEFORE the version (`src/discipline/DisciplineSaveCodec.cs`); the compile-time half is the typed `src/season-save/DisciplineBlock.cs`. Locked by `DisciplineSaveCodecTests` (a foreign-magic blob is refused; the magic check is mutation-killed). Appendix C re-derived onto the real synthetic bench-id formula (agent id 22 = bench slot 0, club 7 local 16 = PlayerId 191), the hedge deleted. `appendices.md` v0.3, `section-4.md` v0.3. |
| ERR-044-002 | Discipline & Suspensions #44, found at **the C1/C2 landing's own verification**: FR-DC-010 scoped the availability filter to "**the engine-resolved fixture**" while FR-DC-011 one row below served bans "regardless of resolution path", and #30 §3.4 (APPROVED, later than #44) has the seam LIVE, applied to both clubs of every fixture on both resolution paths. The spec contradicted itself and the narrower half was wrong: had #44's wording shipped, a quick-sim fixture would decrement a ban the banned player had just played through — and since nearly every fixture of a career is quick-simmed, a ban would have cost essentially nothing. | Medium | 4 | ✅ **Resolved August 13, 2026, spec + code same commit** — the filter sits at #30's single `SeasonLoop.SelectAvailable` site, reached by both `ResolveFixture` (quick-sim) and `BootFixtureEngine` (engine). Locked by `SeasonLoopDisciplineTests.QuickSimPath_MakesASuspensionCostSomething` — an OUTCOME assertion (a club whose first-choice eleven is suspended all season must finish with fewer points, not a squad-shape check, so it fails when the call is removed) — plus two both-clubs engine-path cases mirrored home AND away (ERR-008-002's asymmetry class). FR-DC-010 (§2), §3.3, §4.5 amended. |
| ERR-044-003 | Discipline & Suspensions #44, found at **the C1/C2 landing's own verification**: #44 §2.3 **F5** required `FilterAvailable` to fail loud when it reduces a squad below the eighteen `ConfigureSquads` consumes. #30 §2.3 **F9** / §3.4 (ERR-030-029, approved AFTER #44) settles the identical event the opposite way — the seam back-fills, probing `SquadRating.CanFieldStartingEleven`, failing loud only if the WHOLE squad cannot field the formation — and states outright that "the rule is #30's … #44/#36 contribute removals only and inherit the rule unchanged when they join." Two viability rules of opposite posture on one shared method; implementing #44's would also have wedged a career permanently mid-save on a mass-suspension season, reachable at the engine's measured card rate. | Medium | 6 | ✅ **Resolved August 13, 2026, spec + code same commit** — #30 wins: F5's fail-loud WITHDRAWN and restated as removals-only; `src/discipline/Availability.cs` implements no viability gate at all. The removal/back-fill responsibility is SPLIT — `PlayerCareerStates.MarkUnavailable` contributes a removal set, and the new `src/season-save/AvailabilityComposition.cs` owns the one intersection and the one back-fill, preserving #30 §3.4's order-independence-because-both-are-removals property structurally rather than by convention. **RECORDED, NOT FIXED (owner decision):** preserving #30 §3.4's "never worse than unfiltered" invariant means a suspended player IS reinstatable in extremis, which the Laws of the Game do not allow; implemented as a stricter reinstatement TIER instead (every injured player pressed back before any suspended one, a banned man playing only when the alternative is a club that cannot field a team at all). #44 §7.2's deferral queue is recorded as the alternative if the owner would rather refuse the fixture. Locked by `SeasonLoopDisciplineTests.TheBackFillPressesAnInjuredPlayerBackBeforeASuspendedOne`. §2.3 F5, §3.3, §7.2 amended. **EXTENDED August 13, 2026 (a second adversarial-review pass, L7):** the extremis tier's own consequence went unrecorded — `AvailabilityComposition.Reinstate`'s suspended-tier branch presses a banned player onto the pitch, and `SeasonLoop.PlayNextRound`'s `OnClubFixturePlayed` then decrements HIS ban for that same fixture regardless, since serving is driven purely by "did the club play", not by who was actually selected. That is the identical "decrement a ban the banned player had just played through" hazard this very row's ERR-044-002 fix exists to prevent, reintroduced one tier over by this row's own back-fill compromise. **RECORDED, NOT FIXED (owner call, unchanged posture):** whether an extremis appearance should count as a match served (today's behaviour, unchanged by this note) or should be exempted so the ban still costs a full fixture is not decided here. `AvailabilityComposition.cs`'s type remarks now state the hazard (v1.1); no code or test changed. **✅ DECIDED AND FIXED August 15, 2026 — "ERR-044-003 stage 1", spec + code same commit.** The owner's call is exemption: an extremis appearance does NOT count as a match served. `DisciplineRules.OnClubFixturePlayed` gains a **required** `int[] fieldedPlayerIds` and skips the decrement for any banned player in it, because the rule is not "the club played" but "the club played **without** him" — a match a suspended man appears in is not a match of his ban. Required rather than optional-with-a-default: an omitting call site would silently restore the free appearance, which is this same landing's own H1/H4 shape. Fixed at the **serving** site, not in `AvailabilityComposition` — the football rule is about who PLAYED, so the reinstatement tier order is untouched and the type's back-fill is unchanged. The eleven passed is the array `ERR-041-010(b)`'s appearance record **already** derived at the filter+configure site, so no second selection walk appears (AR pass 2's parallel-surface finding); `SeasonLoop.FieldedXi` widens that derivation's gate from `_career` alone to the union of its two consumers, keyed on `_disciplineDriver` so a substituted driver cannot be handed a null eleven the production path would have filled. **Behaviour-identical on every fixture that does not reach the extremis tier**, by construction: the filter removes every suspended player before selection, so nothing but the back-fill can put a banned id in the eleven. FR-DC-011 amended (it read "per played fixture of the player's club" full stop, correct only while a banned player can never take the field); §3.3 pseudocode, §7.2 and `AvailabilityComposition`'s remarks amended. **STAGES 2 AND 3 AGREED AND NOT IMPLEMENTED, with their blockers named:** the fuller answer is the Football Manager posture — youth call-ups ahead of any suspended player, then generated low-attribute cover ahead of that, under which a banned man never takes the field and the suspended tier becomes unreachable rather than merely expensive. Stage 2 is blocked on **#42 Youth having no `src/` assembly**; stage 3 on the `PlayerId = clubId × CLUB_SQUAD_SIZE + local` id space, which is **fully packed at 25** — a 26th player for club N collides with club N+1's first — so generated cover needs that formula widened, touching #27 FR-SQ-010 (as amended by ERR-027-004), every save, and the ERR-041-019 uniqueness guard. Until then the suspended tier stays, and now costs. **Mutation-verified under control** — the fix→mutate→restore cycle was re-run with the source state `grep`-checked and printed before each measurement, after an uncontrolled first attempt had a delegated agent mutating the tree mid-run (its numbers were discarded, not reported). Fix intact: `Discipline.Tests` **105/0**, the extremis wiring lock **1/1**, `SeasonSave.Tests` **432/0/3**. `WasFielded` skip deleted: `Discipline.Tests` **104/1**, wiring lock **fails**. The single failing unit test is `OnClubFixturePlayed_ExemptsAFieldedBannedPlayer_ButStillServesAnUnfieldedTeammate` — the other three new unit tests pass under the mutant **by design**, locking adjacent properties (null-throws, no-row-invented, the FR-DC-017 drop) rather than the exemption, and are not claimed as locks on it. `SeasonLoopDisciplineTests.ASuspendedPlayerPressedInByTheExtremisBackFill_...` carries a HARD positive-control assertion that a suspended player really is in the fielded eleven; without it the case is green even when the back-fill never reaches the extremis tier, which is ERR-030-014's shape and this project's most-refiled test defect. |
| ERR-030-035 | Season & Competition Loop #30, found at **the #44 C1/C2 landing (roadmap C1, T1 save codec)**: the season frame gains a SEVENTH mandatory sub-blob (`DISC`), so `SEASON_SAVE_FORMAT_VERSION` must bump 5 → 6, and #30's own Appendix A/B, FR-SN-021's `Save` signature and §4's file layout all still described the prior six-blob frame. Also stale by the time #44 actually joined: §3.4 and `SeasonLoop.BootFixtureEngine`'s comment both still read "#44 suspensions and #36 call-ups join the same seam at their own T-phases" / "#44's suspension view joins at its T2" after #44 had already joined — future tense describing a past event. | Low | 9 | ✅ **Resolved August 13, 2026, spec + code same commit** — Appendix A's version row and Appendix B's frame gain the mandatory `DISC` block (between `progression` and the optional match block; "four mandatory career sub-blobs" → five); FR-SN-021's `Save` signature gains the ninth, required, null-rejecting `discipline` argument and `Load`'s return gains the reconstructed `Discipline`; §4's file layout gains `DisciplineBlock.cs`. §3.4 and the `BootFixtureEngine` comment corrected to say #44's suspension view HAS joined the seam (citing ERR-044-002 for the filter re-scope and ERR-044-003 for the depleted-squad rule #44 now inherits) — only #36 call-ups remain a future joiner. `appendices.md` v1.2, `section-2.md` v1.7, `section-3.md` v2.0, `section-4.md` v0.7. |
| ERR-030-036 | Season & Competition Loop #30, found at **the adversarial review over the #44 C1/C2 landing (H1)**: a resume-and-save cycle silently forgave every outstanding suspension. `SeasonLoop.Restore` took six parameters and none of them was the tally, so a loop rebuilt through the documented restore path could not carry `SeasonSaveContents.Discipline` **even deliberately** — `_discipline` was null whatever the file held — and `Save(SeasonLoop, match, path)` then wrote `loop.Discipline ?? new DisciplineState()`, a well-formed ZERO-ENTRY `DISC` block, over the live one. Both existing destination guards are keyed on OTHER block families (`RequireDestinationCarriesNoRoster` fires only when `progression.ClubCount == 0`, `RequireDestinationCarriesNoCareer` only when all three career blocks are empty), so a resumed career with a populated store AND populated career blocks — the ordinary case — sailed through both: every ban and every yellow in the destination gone, frame still v6, `Load` still succeeding, and **FR-DC-014 retains no ledgers**, so the cards exist nowhere else and nothing downstream can recompute them. **ERR-028-008's measured shape in the third block family, for the third time** — and the hazard was stated verbatim in `SeasonSaveManager.cs`'s own comment ("a save that silently wrote an empty block would forgive every outstanding suspension") one line above the call that caused it. Even the constructor path relied on a caller remembering an optional seventh argument, which is the null-meaning-empty posture `Save`'s own required `discipline` parameter exists to refuse. | High | 6 | ✅ **Resolved August 13, 2026, spec + code same commit** — two halves, both mirroring the existing precedent. (a) `SeasonLoop.Restore` gains `disciplineOrNull` and forwards it to the constructor, so the correct resume is expressible at all (`SeasonLoop.cs` v1.21). (b) New `SeasonSaveManager.RequireDestinationCarriesNoDiscipline`, the third sibling of the two guards above, invoked when `discipline.Count == 0`: it decodes the destination's `DisciplineBlob` and refuses when it carries rows, keyed on the tally's own emptiness because #44's state is `(PlayerId, CompetitionId)`-keyed and shares no dimension with the block families the siblings watch (`SeasonSaveManager.cs` v1.23). Unreadable/foreign/pre-v6 destinations are passed through, as both siblings already do. Locked by six cases in `SeasonSaveManagerTests` (v1.18): the forwarding half, the refusal with BOTH sibling guards' subjects deliberately populated so only the new one can fire, its two carve-outs (new path, already-empty destination), and the two end-to-end resume cases. **Mutation-verified**: dropping the `Restore` forwarding fails 2 locks; disabling the guard call fails 2 others; neither mutant fails anything else. Back-prop: #30 Appendix B.1 states the cross-blob refusal rule as a THIRD rule beside F8/F10 (`appendices.md` v1.3). |
| ERR-030-037 | Season & Competition Loop #30 §3.4/§3.5, found at **a second adversarial-review pass over the #44 C1/C2 landing (M8)**: the normative `AdvanceAndPlayNextRound`/`PlayThroughEngine`/`RollToNextSeason` pseudocode never gained the #44 discipline loop the code has carried since T2 — no `OnClubFixturePlayed`, no `CardLedgerFold` construction or per-tick pump, no `fold.Commit`, no FR-DC-017 boundary sweep anywhere in §3.4/§3.5's blocks. An implementer following §3.4/§3.5 verbatim would rebuild the season loop with no discipline wiring at all — missing not just the feature but the one ORDERING that is discipline's entire off-by-one contract (M6/M7, recorded in the header entry above rather than as their own ids — both are code/test-only findings with no spec text to have sourced them), since a pseudocode block that never mentions serving or committing cannot state their relative order either. | Medium | 1 (`section-3.md`) | ✅ **Resolved August 13, 2026, spec + code same commit** — §3.4's pseudocode gains `PlayThroughEngine`'s `CardLedgerFold` construction (seeded from `engine.PlayerIdsByAgentId()`) and the per-tick `fold.ObserveTick(tap)` pump inside the `RunTick` loop, returning `fold` UNCOMMITTED; `AdvanceAndPlayNextRound` gains the `OnClubFixturePlayed(home)`/`OnClubFixturePlayed(away)`/`fold?.Commit(...)` triple, positioned to match the code AFTER `f.Played := true` (the M6 fix, described in the header entry above — the pseudocode is patched to the POST-fix ordering, not the pre-fix one it would otherwise have silently re-specified). §3.5's `RollToNextSeason` gains step (f), `DisciplineRules?.RollToNextSeason()`, after (e)'s commits. `section-3.md` v2.1. **EXTENDED August 13, 2026 (a third adversarial-review pass, L9):** the M6 comment this row's resolution text quotes — reproduced in both `src/season-save/SeasonLoop.cs` and `section-3.md`'s patched §3.4 pseudocode — claimed `OnClubFixturePlayed` and `fold.Commit` are "BOTH fallible under a bound config". False for `OnClubFixturePlayed`: it reads no `[GT]` at all and its only guard is `clubId < 0` (F2), a caller-contract bug no fixture drawn from a real round can trigger — only `fold.Commit` (via `DisciplineRules.AddYellow`/`RequireBanLength`) is config-fallible. The placement argument (running the pair after `f.Played := true`) is UNAFFECTED, since it survives on `fold.Commit` alone. Both copies corrected in place; `SeasonLoop.cs` v1.23, `section-3.md` v2.2. Also filed at this pass and resolved separately: **M12** (`SeasonLoop.cs`/`SeasonLoopDisciplineTests.cs`, no spec text sourced it — the M6 POSITION claim had no test that failed on revert; a new `TestOnly_AfterHomeClubServed` hook and lock added), **M13** (`CardLedgerFold.Commit` made atomic via the new `CommitWithExplicitConfig`), **M14** (`PlayerCareerStates.MarkUnavailable` given an OWNING mask contract to match `Availability.MarkSuspended`'s, so `AvailabilityComposition.Compose` allocates one mask per contributor rather than sharing one), **L10** (the `RollToNextSeason` boundary-sweep comment's idempotence claim corrected), **L11** (`DisciplineRulesTests`' non-discriminating kind-2 order test replaced, via the new internal `DisciplineRules.ApplySecondYellow` seam), **L12** (three `discipline-suspensions` files brought back in line with the landed surface — `appendices.md`'s stale F5-filter-floor catalogue row removed, `section-4.md`'s file table gains the three files it omitted, `section-2.md` §2.2's API block corrected to the real signatures and gains `MarkSuspended`/`DisciplineRules`/`DisciplineEntry`), and **L13** (`section-2.md` gains **F6** for the two `[GT]` fail-loud guards, and `section-3.md` §3.2's `AddYellow` pseudocode gains the matching guard calls). None of M12-M14/L10-L13 required their own `ERR-` id — all are code/test/doc-only findings with no spec DEFECT of their own to log (L9/M8 share `ERR-030-037` because both are the same spec passage; the discipline spec's own three doc fixes are recorded in `discipline-suspensions/appendices.md`, `section-2.md` and `section-4.md`'s own version histories rather than re-stated here). |
| ERR-030-038 | Season & Competition Loop #30 **Appendix B.1**, found at **the third adversarial-review pass over the #44 C1/C2 landing (H3)** — **a defect introduced by `ERR-030-036`'s own fix, in both its code and its spec text**: `SeasonSaveManager`'s new `RequireDestinationCarriesNoDiscipline` was invoked on `discipline.Count == 0` and treated an empty tally as proof the tally had been **dropped**. That inference is sound for the two sibling guards — #28's block IS the roster (KD-4) and a career roster never legitimately empties — and it is **false for #44**: **FR-DC-017 makes the empty tally the canonical clean state**, requiring an entry that reaches `(Yellows = 0, BanMatchesRemaining = 0)` to be dropped **immediately**, mid-season the moment a ban is served out with no residual yellows (`OnClubFixturePlayed`) and again for every yellows-only row at the boundary sweep (`RollToNextSeason`). So a **correctly wired, correctly resumed** loop whose last discipline row cleared could never save over its own file again, and stayed unsaveable until fresh cards accrued, because the destination keeps the old rows forever — while the thrown message instructed the operator to "resume the loop with the tally from `SeasonSaveContents.Discipline`", which is exactly what they had done. **Demonstrated by execution against the built assemblies:** save a career carrying `(player 100, yellows 0, ban 1)` → OK; `OnClubFixturePlayed(4)` serves the ban and FR-DC-017 drops the row → `Count == 0`; save the *same live tally* to the *same path* → `InvalidOperationException: Refusing to overwrite … it carries #44 discipline state for 1 player(s) and this save has none.` The four locks landed with ERR-030-036 miss it **by construction**: two use a fresh path or an already-empty destination, and both resume cases carry a populated tally throughout — nothing exercised a tally that **drains**. The spec text carries the same defect: Appendix B.1 stated the rule as "MUST refuse to overwrite a populated block with an **empty** one, per block family" and explicitly extended it to #44, so the **generalisation was over-broad**, not merely the code. | High | 3 | ✅ **Resolved August 13, 2026, spec + code same commit.** The guard is re-keyed from "the tally is empty" to "**no #44 subsystem is wired behind this save**" — the predicate that actually distinguishes the loss, and one that exists at the call site (`SeasonLoop.Discipline` is null or it is not) but is destroyed by the `?? new DisciplineState()` the save root applies. A new **internal** `SeasonSaveManager.Save` overload takes a **required** `bool disciplineWired` (never defaulted, for the reason the block parameters reject null: a caller that can stay silent about wiring is a caller whose silence is indistinguishable from the loss); the public long form forwards `disciplineWired: true`, because a caller that constructs and hands over a `DisciplineState` drives #44 by construction and cannot omit it (null is refused there); `Save(SeasonLoop, …)` passes `loop.Discipline != null` alongside the `??`. **Chosen over `null`-meaning-unwired on the internal overload** so that `null` never acquires a second meaning anywhere in the file — the null-is-not-the-empty-set doctrine those five required parameters exist to state — and because a same-signature internal overload is not expressible in C# anyway. `SeasonSaveManager.cs` v1.24. **Recorded, not fixed:** a loop wired with a *fresh* `DisciplineState` rather than the file's is wired, so the guard passes it, and it is indistinguishable at this boundary from a legitimate drain (both present as "wired, zero rows"); `SeasonLoop.Restore`'s `disciplineOrNull` is what closes that case, and no destination predicate can. **Locked by two new cases** in `SeasonSaveManagerTests` (v1.19) — `AWiredTallyThatDrainsToZero_SavesOverItsOwnPopulatedFile` (FR-DC-017's mid-season drop) and `ASeasonRollThatClearsEveryYellow_LeavesTheCareerSaveable` (the boundary sweep) — each saving a wired loop over its own populated file and asserting the drained tally reaches disk. **Mutation-verified: restoring the `discipline.Count == 0` predicate fails both, and nothing else.** The three ERR-030-036 locks that exercise the guard now drive it through the internal overload (renamed `Save_AnUnwired*`), since the public long form can no longer express "no discipline wired" and its empty tally is no longer a refusal. **Back-prop:** Appendix B.1 now states the rule's trigger as a predicate on the **writing composition**, carried to the write rather than re-derived from the block, with the explicit qualifier that **emptiness is evidence of loss only for a family with no legitimate empty state** and that #44 is the counter-example; `appendices.md` v1.4. |
| ERR-030-039 | Season & Competition Loop #30 **Appendix B.1**, found at **the fourth adversarial-review pass over the #44 C1/C2 landing (H4)** — **a defect introduced by `ERR-030-038`'s own fix, in both its code and its spec text, exactly as `-038` was a defect in `-036`'s**: the `disciplineWired` flag `-038` introduced was placed on an **internal** `SeasonSaveManager.Save` overload, and the **public** nine-argument long form forwarded `disciplineWired: true` unconditionally. The stated reasoning — "a caller of THIS form hands over a tally it constructed and cannot omit it (null is refused below), so it drives #44 by construction" — **excludes only `null`**. The `discipline` parameter's own documentation sanctions `new DisciplineState()` as the way to say "no cards recorded yet", which is precisely what an unwired caller passes, so the public form asserted wiring from the very value that denies it and `RequireDestinationCarriesNoDiscipline` — the whole of `ERR-030-036`'s fix — was unreachable from the only long form an external caller can call. **Demonstrated by execution against the built HEAD assemblies**, two identical public `Save(world, season, null, path, …, ProgressionEngine.Empty, tally \| new DisciplineState())` calls: `after save 1, file tally rows = 1` / `save 2 SUCCEEDED (no refusal)` / `after save 2, file tally rows = 0`. FR-DC-014 retains no ledgers, so the bans and yellows are unrecoverable — `ERR-030-036`'s own measured shape, reopened one round later by the fix for it. It also **contradicted approved text**: #30 Appendix B.1 v1.4 states the wiring fact "MUST therefore be **passed to the write, not re-derived from the block**", and the public form re-derived it from nothing at all. The false premise was written down in the suite too — `Save_AnUnwiredDiscipline_OverAPopulatedTally_IsRefused`'s own comment read "The public long form cannot express this case at all", sitting in the test that exists to lock this. | High | 3 | ✅ **Resolved August 13, 2026, spec + code same commit.** `disciplineWired` is **promoted onto the public long form** as a required, never-defaulted parameter and the forwarding overload is **deleted**, so exactly one long form exists and no entry point can answer for its caller (`SeasonSaveManager.cs` v1.25). Chosen over two explicitly-named public methods because this file already states the convention the fix needs: five block parameters that reject `null` rather than defaulting, on the stated ground that "a caller that can stay silent is a caller whose silence is indistinguishable from the loss" — the wiring fact is the same kind of fact and now travels the same way. **Every call site updated** (56 in three test files; the sole production long-form caller, `Save(SeasonLoop, …)`, already passed `loop.Discipline != null`). **Locked by** `SeasonSaveManagerTests.Save_ThePublicLongForm_DrivingNoDiscipline_CannotEmptyAPopulatedTally` (v1.20), which drives the PUBLIC form on both saves. **Mutation-verified**: reinstating the hardcode by assigning `disciplineWired = true` at the top of the promoted method fails that lock and nothing else. **Back-prop**: Appendix B.1 now requires the fact on **every public write entry point**, and records why non-nullness cannot stand in for it — the empty value each such parameter documents as its own "tracks none" spelling is exactly what an unwired caller passes, and a wiring predicate is only as strong as the weakest signature that reaches the write (`appendices.md` v1.5). **Two Mediums from the same pass are recorded here rather than as new ids.** **M17:** `SeasonLoop.PlayNextRound` now calls the new `CardLedgerFold.RequireCommittableConfig()` once, with its other guards, before the fixture loop. M6 (`ERR-030-037`) placed the fallible `fold.Commit` AFTER `MarkFixturePlayed`, so a config-driven throw there leaves the fixture marked played — the retry's unplayed-index filter skips it, its whole card list is lost (M13 made the commit all-or-nothing), and once every fixture of a round has been marked this way `PlayNextRound` throws F5 forever, the cursor never advances and `RollToNextSeason` refuses: a career that saves and reloads cleanly and can never progress. Both the M6 comment ("nothing is lost by running the pair last") and #30 §3.4 stated only the benefit, while the same method's appearance-record comment ~25 lines above names this exact outcome as the reason a fallible call must precede the mark — two opposite rules for one hazard in one method, now corrected in both places (`section-3.md` v2.3). The ordering is unchanged (serve strictly before commit, both after the mark); the config CAUSE is removed ahead of it, a bad `[GT]` being a property of the config and identical for every fixture in the round. `CardLedgerFold.cs` v1.3 shares the four guards between the new fold-free entry point and `CommitWithExplicitConfig`, so the pre-check cannot drift from the guard it front-runs. **M16:** `SeasonLoop.TestOnly_AfterHomeClubServed` — process-static mutable state on a production class, invoked from the production round loop mid-way through a persisted-state mutation, whose own doc conceded "a test MUST clear it in a `finally`" — is **deleted**. `src/CLAUDE.md` bans the static-mutable-singleton pattern in game-state assemblies (FR-CS-051..054) and names constructor injection as the required alternative, which is what landed: an `internal IFixtureDisciplineDriver` seam (`OnClubFixturePlayed` + `CommitFixtureCards`), bound once in a new internal constructor, production binding a thin adapter over the loop's own `DisciplineRules` and the M12 position lock supplying a failing implementation instead. A driver without a tally is refused at composition. Re-verified by execution: the M12 lock still fails when the serve+commit block is moved back above `MarkFixturePlayed`. |
| ERR-030-040 | Season & Competition Loop #30 §3.4, found at **a reviewed-findings pass over the ERR-044-003 stage 1 landing (M1)**: the M6 comment's claim that `OnClubFixturePlayed` "is NOT [fallible] (L9): it reads no `[GT]` and its only guard is `clubId < 0` (F2)" — mirrored verbatim in both `src/season-save/SeasonLoop.cs` and this section's `AdvanceAndPlayNextRound` pseudocode — went stale the moment `ERR-044-003` stage 1 landed the same day: `DisciplineRules.OnClubFixturePlayed` (`src/discipline/DisciplineRules.cs`) gained a second mandatory guard, `fieldedPlayerIds == null` → `ArgumentNullException`, inside the exact post-`MarkFixturePlayed` block whose safety this comment establishes. It cannot fire today, but only because of a DISTANT invariant the paragraph never named: `FieldedXi`'s own gate (`SeasonLoop.cs` ~1620) returns non-null exactly when `_career != null \|\| _disciplineDriver != null` — the same condition the whole `if (_disciplineDriver != null)` block already runs under. Narrow that gate, add a third `FieldedXi` consumer, or add a resolution mode whose XI derivation differs, and a post-mark `ArgumentNullException` strands the round permanently, exactly as M17 describes for `fold.Commit` two paragraphs below — with the comment the next author consults asserting that cannot happen. The identical defect shape L9 fixed for this same passage two days earlier (stale-guard-count claim, one guard added since), now recurring at the very next thing that touched it. | Medium | 2 (`SeasonLoop.cs`, `section-3.md`) | ✅ **Resolved August 15, 2026, spec + code same commit.** Both copies corrected: "its only guard is `clubId < 0`" → both guards named, plus the structural reason the null case cannot fire on this path (FieldedXi's gate is the union `_career != null \|\| _disciplineDriver != null`, identical to this block's own guard). `SeasonLoop.cs` v1.27, `section-3.md` v2.5. Not mutation-verified (a doc-accuracy fix with no behaviour change to test against) — the claim rests on reading `FieldedXi`'s implementation directly, cited in the corrected comment. |
| ERR-030-041 | Season & Competition Loop #30 §3.5, found at **the same reviewed-findings pass (M7)**: step (f)'s pseudocode comment stated the season-boundary discipline sweep is placed after (e)'s commits "for the same reason the roster sync at (d′) is: a discipline state swept for a season that never began is a half-rolled state, and the sweep is NOT idempotent — running it twice on a refused-then-retried roll would silently forgive a second season's yellows." The idempotency claim is FALSE: `DisciplineRules.RollToNextSeason()` sets `Yellows := 0` and leaves bans untouched, so a second run finds every row already at 0 and writes nothing — it is a no-op, not a double-forgiveness. The code comment for the identical call site (`src/season-save/SeasonLoop.cs` ~1093-1098) already states this correctly ("RollToNextSeason IS idempotent … but that only helps a SECOND run") and gives the actual reason: the hazard is a FIRST run firing against a roll that is then REFUSED, sweeping a discipline state for a season that never actually began. The spec text and the code comment at the same site had drifted onto opposite, mutually exclusive justifications for the same (correct) placement, and nobody had checked one against the other. | Low | 1 (`section-3.md`) | ✅ **Resolved August 15, 2026, spec + code same commit.** §3.5's step (f) comment's idempotency clause replaced with the real reason, taken from `SeasonLoop.cs`'s own comment at the mirrored call site (unchanged — it was already correct). The placement itself is unaffected; only the stated justification changes. `section-3.md` v2.5. |
| ERR-044-004 | Discipline & Suspensions #44 §2.3 **F2**, found at **the adversarial review owed by the #44 C1/C2 landing**: F2 requires a fail-loud when `OnClubFixturePlayed`/`FilterAvailable` names "a club/player outside the resolvable universe", but the implementation guarded only the CLUB half. A negative `PlayerId` was constructible at four entry points — `DisciplineEntry`'s own constructor (which gated `Yellows`/`BanMatchesRemaining` but not `PlayerId`), `AddYellow`/`AddBan`/`ApplyCard`, `DisciplineSaveCodec.Decode`, and `MigratePlayerId`'s target id — and `PlayerId / CLUB_SQUAD_SIZE` truncates toward zero in C#, so every id in `[-24, -1]` derives to club 0 and was silently served, decremented and migrated as one of club 0's players. Demonstrated by executing a probe against the built assembly: `new DisciplineEntry(-5,0,0,2)` constructed; `AddBan(-5,0,3)` wrote a row; `OnClubFixturePlayed(0)` decremented it 3→2; Encode/Decode round-tripped it; `MigratePlayerId(100,-7)` succeeded. `CardLedgerFold`'s constructor already refused a negative seed id, so the assembly agreed negative ids were invalid at one of four entry points and not the others — the ERR-041-019 shape, a derivation safe only by accident of today's single id allocator, stated nowhere and checked nowhere, and the `OnClubFixturePlayed` doc comment argued the mirror hazard away ("No id divides to a negative club, so this would otherwise be a silent no-op") rather than checking it. | Medium | 2 | ✅ **Resolved August 13, 2026, spec + code same commit (code fix already landed)** — `DisciplineEntry`'s constructor refuses `playerId < 0` citing §2.3 F2, the same posture as its two existing gates; `DisciplineSaveCodec.Decode` gained the matching F3 gate before constructing a `DisciplineEntry`, so the file boundary refuses what the composition boundary refuses (the #29/#41 AR pass 6 both-boundaries rule). The misleading `OnClubFixturePlayed` doc paragraph is corrected. Locked by `Decode_NegativePlayerId_Throws` and the constructor gate's own cases. **Back-prop:** §2.3's F2 row now states the player half explicitly — a negative or otherwise unresolvable `PlayerId` is refused at construction and at decode — rather than leaving "club/player" readable as club-only. `discipline-suspensions/section-2.md` v0.5. |
| ERR-044-005 | Discipline & Suspensions #44 **FR-DC-009** (§2.1) and §3.3, found at **the adversarial review owed by the #44 C1/C2 landing**: FR-DC-009 requires `FilterAvailable(in Squad) → Squad` to "return a reduced value copy (available players only)" — the requirement is TOTAL as written, and the implementation was PARTIAL. With every player suspended, the reduced copy has zero players and `new Squad(clubId, new PlayerRecord[0])` hits `Squad`'s own `players.Length == 0` refusal, so the method threw `ArgumentException: Squad must have between 1 and 25 players; got 0. (Parameter 'players')` — a cross-assembly message naming neither discipline nor the cause. Demonstrated by execution. The spec never says what the all-removed case returns, and `Squad` cannot represent it, so the requirement is unsatisfiable as worded. Compounding it: `FilterAvailable` has **no production caller** — #30's composed seam consumes `MarkSuspended`'s mask and `AvailabilityComposition`'s own private compose, which correctly returns `null` for the zero case — so FR-DC-009's surface is exercised only by its own unit tests. | Medium | 2 | ✅ **Resolved August 13, 2026, spec + code same commit (code fix already landed)** — `FilterAvailable` returns `null` for a fully-suspended squad, mirroring `AvailabilityComposition`'s private helper, and its XML doc records both that `null` is the all-removed representation and that this method is FR-DC-009's surface rather than #44's production path. Locked by `FilterAvailable_EveryPlayerSuspended_ReturnsNull`. **Back-prop:** FR-DC-009 in `section-2.md` now states the all-removed case (returns `null`, because `Squad` cannot represent an empty roster); §3.3's `FilterAvailable` pseudocode comment in `section-3.md` likewise, and now names the `MarkSuspended` mask consumed by #30's composed seam as the production path. `discipline-suspensions/section-2.md` v0.5, `section-3.md` v0.6. |
| ERR-044-006 | Discipline & Suspensions #44 **§5** (Test Plan) and **§9** (Approval Checklist), found at **the round-5 adversarial review over the C1/C2 landing**: §5's traceability table named **two tests that do not exist**, and §9 ratified **three gates** on them. **T-DC-VIEW-001** ("exercising every #44 path leaves the #27 canonical squads byte-identical") had one test — `AvailabilityTests.FilterAvailable_LeavesTheSourceSquadUntouched` — **deleted at C1/C2 AR round 1 as tautological**, recorded in that file's v1.1 L4(a) (*"`Squad` is immutable and exposes no mutator, so no implementation could ever fail that assertion"*), with **no replacement anywhere**; `src/season-save/tests/SeasonLoopDisciplineTests.cs` listed the same id in its `Spec:` header, so two files claimed it and neither implemented it. **T-DC-INT-001** ("every field integer; no float (static/reflection assertion, FR-DC-020); #44 registers no RNG stream (FR-DC-019)") was never implemented in either half — `grep -n "typeof\|GetFields\|Reflection" src/discipline/tests/*.cs` returns nothing. **G6 ✅** cited T-DC-VIEW-001; **G13 ✅** cited T-DC-INT-001; **G14 ✅** ("FR-DC-001..022 each traceable to a T-DC-* test") cited §5.6, which asserted total coverage — the founding trap in this project's own root `CLAUDE.md` ("Fabricated checklist values"). Aggravating: **the August-15 round-4 pass existed specifically to fix §5's staleness**, verified only the four rows it ADDED, left these untouched, and re-certified G14 "against the corrected table" — a partial verification presented as a complete one. **Verifying every surviving row against `src/discipline/tests/` and `src/season-save/tests/` found three more defective rows**, the first worse than either missing test: **T-DC-VIEW-002 required "a pass-through filter returns an equal (but DISTINCT-COPY) squad"**, the opposite of FR-DC-009 ("MUST pass the squad through unchanged"), of the enforced `Is.SameAs` contract, and of FR-DC-018's identity floor — **wrong instruction rather than absent evidence**: a conformant implementation returns an equal-but-distinct `Squad` on every clean fixture, passes every other row, and silently moves every fixture's digest. **T-DC-FOLD-001** described a scripted kind-{0,0,2,1} test that has never existed; **T-DC-DET-001** claimed "and identical filtered squads", untested and a tautology if written. **FR-DC-002 and FR-DC-022 were traced by no row at all** while G14 certified total traceability. | High | 3 | ✅ **Resolved August 15, 2026, spec-only, same commit — no code change** (the properties all hold; only the evidence was wrong). **T-DC-VIEW-001 and T-DC-INT-001 WITHDRAWN in place** under the `T-DC-BAN-004` precedent (kept as records, not deleted), each naming what establishes the requirement instead: FR-DC-001's #27 half by `Squad`'s construction — **re-verified against `src/player-database/Squad.cs` rather than taken from the deletion note**: `sealed class`, `private readonly PlayerRecord[]` deep-copied in the constructor, `GetPlayer` returning `PlayerRecord` **by value**, no mutator — and FR-DC-019 as a structural negative with no observable to assert (zero `DOMAIN_TAG`/`SubsystemOrdinal`/`SplitMix`/RNG-service occurrences in `src/discipline/**`; `discipline.asmdef` references `DeterministicSim` for `CanonicalSerializer` alone). **T-DC-VIEW-002 corrected** to the same-instance pass-through contract with both tests cited; **T-DC-FOLD-001 and T-DC-DET-001 re-cited to what exists**. **§5.6 replaced by a per-FR Test / Construction / Deferral disposition map**, so G14 is re-derivable by grep instead of asserted. **G14's wording widened to that third disposition — recorded as a widening, with its reason**: FR-DC-001's #27 half, FR-DC-019 and FR-DC-022 are structural negatives, which is **G3's already-approved posture for FR-DC-019**, so this aligns G14 with an approved gate rather than lowering a bar to fit a result. **One gap named rather than absorbed: FR-DC-020's integer posture rests on audit, not enforcement** (no `float`/`double`/`decimal` declared in `src/discipline/**`, grep-verified — but nothing would stop a future one), and a reflection lock **would** have a real failure mode, so it is filed as a **§9.2 follow-up** with what to write and under a new id (T-DC-INT-001 stays withdrawn) instead of being claimed as coverage. G6/G13/G14 stay ✅ on re-derived evidence; **R-05's sign-off annotated in place** (the §9.3 L6 precedent) where it cites the same withdrawn lock, its substance unaffected and the decision not reopened. `SeasonLoopDisciplineTests.cs`'s `Spec:` header drops the withdrawn id (header comment only — no test logic touched; `Discipline.Tests` re-run **105/105, 0 failed, 0 skipped** — note this is the suite's *measured* count today, not the 81/81 the root `CLAUDE.md`'s #44 entry still records from the C1/C2 landing, and `src/season-save/tests/` rebuilt 0 errors to prove the header edit compiles; the whole-tree gate was deliberately NOT run for a spec-text pass). **⚠️ CORRECTED August 15, 2026, later still (reviewed-findings pass): the "105/105" figure above was already stale the moment it was written** — it is the count as of `5f17308`, and `45a29ae` (two commits *before* the commit that filed this very entry) had already taken the suite to 118 cases, so a claim presented as "measured today" was one commit's re-derivation behind the branch it was committed on. Re-measured now by direct execution — `dotnet test src/discipline/tests/discipline-tests.gen.csproj -c Debug` → **Passed! Failed: 0, Passed: 118, Skipped: 0, Total: 118**. Annotated in place rather than silently edited, per this file's own convention; the root `CLAUDE.md` #44 entry's `81/81` is corrected the same way in the same pass. **RECORDED, NOT FIXED — one residual stale claim, outside this pass's owned file set:** `src/discipline/tests/AvailabilityTests.cs`'s own `Spec:` header (line 7) still lists `§5 T-DC-VIEW-001/002` — the **third** file to claim the withdrawn id, and the one whose v1.1 deleted its test in the first place. It is a header comment with no behavioural effect and the same one-word edit as the `SeasonLoopDisciplineTests.cs` fix above; left only because this pass's owned surface was spec text plus that one header. Its v1.0 version-history row also names the deleted test, and that row is **correct as history** and must not be edited. `discipline-suspensions/section-5.md` v0.5, `section-9-approval-checklist.md` v0.5, `src/season-save/tests/SeasonLoopDisciplineTests.cs` v1.8. **[ANNOTATED August 16, 2026, M10 — final fixer pass; dated correction, row otherwise left unrewritten per this file's own convention.]** The "RECORDED, NOT FIXED" claim immediately above ("line 7 still lists `§5 T-DC-VIEW-001/002`") was ITSELF wrong at the moment it was written: `AvailabilityTests.cs`'s own v1.3 (landed the same day) had already dropped `T-DC-VIEW-001` from its `Spec:` line, and its own v1.4 (L6, also the same day) had already dropped `T-DC-BAN-004` too — so by the time this row was filed the header read `§5 T-DC-VIEW-002, T-DC-BAN-005`, naming neither retired id, and there was nothing left to fix. `section-5.md`'s §5.4 carried the identical stale claim through two further corrections of its own (v0.6 restated it against the wrong id; v0.7 finally verified the header directly and closed the paragraph) before the chain closed — see `section-5.md` v0.7's version-history row for the full account. No `AvailabilityTests.cs` change is or was needed. |
| ERR-044-007 | Discipline & Suspensions #44 §2.2 (Data structures) / §2.3 F2 / §4.5 (root contract), found at **a reviewed-findings pass over the #44 C1/C2 landing**: three drifts between §2.2's landed-API listing and `src/discipline/`, grouped under one id because all three are the same defect class (§2's API surface not tracking a landed member or a landed refusal) found in the same pass, in the same two sections. **(a)** `DisciplineRules.OnClubFixturePlayed`'s §2.2 signature still read `(int clubId)` — the pre-ERR-044-003-stage-1 form — while the real method (`DisciplineRules.cs:245`) has taken `(int clubId, int[] fieldedPlayerIds)` since that same-day landing; v0.7's own amendment reached FR-DC-011, §3.3 and §4.5 but never this code block. **(b)** §2.2 was missing three landed public members: `DisciplineRules.State` (`DisciplineRules.cs:49`), `CardLedgerFold.PendingCardCount` (`CardLedgerFold.cs:127`), and the public static `CardLedgerFold.RequireCommittableConfig()` (`CardLedgerFold.cs:276`) — the round-level `[GT]` pre-check §3.1's own pseudocode calls and `SeasonLoop.PlayNextRound` enforces in production, with no §2.2 declaration and no §4.5 composition-root MUST naming it anywhere. **(c)** §2.3's F2 row covered only "a club/player outside the resolvable universe"; §3.3's pseudocode already reads `REQUIRE fieldedPlayerIds is not null  # F2 — see the note below`, and `DisciplineRules.cs:254-261` throws `ArgumentNullException` on a null eleven, but F2's own text never named that refusal. | Medium | 2 | ✅ **Resolved August 15, 2026, spec-only, same commit — no code change** (every fact was already true in `src/discipline/`; only the spec text lagged). §2.2's `OnClubFixturePlayed` signature corrected; `State`, `PendingCardCount` and `RequireCommittableConfig()` added to the API block. F2 (§2.3) extended to state the null-`fieldedPlayerIds` refusal explicitly, the identical caller-contract posture as an unresolvable id. §4.5 gains a composition-root MUST to call `RequireCommittableConfig()` once per round, before the first fixture of the round is resolved — a bad `[GT]` discovered only at a per-fixture `Commit` strands the round permanently, since the fixture is already marked played by then. `discipline-suspensions/section-2.md` v0.8, `section-4.md` v0.8. |
| ERR-044-008 | Discipline & Suspensions #44 §1.2, §4.3, §7.3 and §8.1 XC-044-002, found at **a reviewed-findings pass over the #44 C1/C2 landing**: all four sites state "one tap feeds both [#37 and #44] when both are built" as a FUTURE condition (the §1.2 instance found on a full-file sweep after fixing the other three). Both #37 (`match-analytics`, since July 27, 2026) and #44 (`discipline`, since August 13, 2026) now HAVE `src/` assemblies, so the condition has already occurred — and `src/discipline/IDisciplineTickLedgerTap.cs`'s own remarks (written the same day as T2) record the prediction as checked and found FALSE, not merely pending: §4.1 forbids `discipline` to reference the match engine, and `season-save` (the composition root that owns the engine) does not reference `match-analytics` either, so #37's identically-shaped `ITickLedgerTap` is unreachable from either end and #44 declares its own interface instead. Four APPROVED-spec sites asserted a claim the implementing assembly's own doc comments had already disproven. | Low | 4 | ✅ **Resolved August 15, 2026, spec-only, same commit — no code change** (the interface split is already correct and tested; only the four claims about why were wrong). §1.2 and §4.3 rewritten to state #44 declares its own `IDisciplineTickLedgerTap` and why (§4.1's reference rule), with the two-reads-not-two-behaviours cost named; §7.3's `#37` bullet and XC-044-002's Contract cell restated to match. `discipline-suspensions/section-1.md` v0.4, `section-4.md` v0.8, `section-7.md` v0.7, `section-8.md` v0.3. |
| ERR-044-009 | Discipline & Suspensions #44 §1.4 KD-2, found at **a reviewed-findings pass over the #44 C1/C2 landing**: KD-2's closing sentence read "The `Incoming`-id semantics are absorbed by the fold either way and re-verified at T-phase" — an assumption `ERR-044-001` already disproved at T2. Appendix C's original worked example placed a bench player at "slot 19", hedged "(absorbed either way)", and was verified unimplementable against the live engine: `MatchEngineConstants.SQUAD_SIZE = 22` makes 19 an on-pitch slot, and `MatchEngine.SubstitutePlayer` derives a bench player's agent id as the synthetic `SQUAD_SIZE + teamId * SUBSTITUTES_PER_TEAM + benchIndex`. `appendices.md` v0.3 (August 13, 2026) deleted the sibling hedge in Appendix C's own worked example, calling doing otherwise "the spec declining to verify a fact it could have checked" — but §1.4's KD-2, the row that stated the assumption first, was edited again on August 15 (v0.2, ERR-044-003 stage 1) for an unrelated clause and left this sentence standing. | Low | 1 | ✅ **Resolved August 15, 2026, spec-only, same commit — no code change** (the correct derivation has been implemented and tested since ERR-044-001). KD-2's closing sentence restated onto the verified synthetic bench-id formula, with the occupancy-seed coverage requirement (`[0, SQUAD_SIZE + 2*SUBSTITUTES_PER_TEAM)`, F1 on any gap) made explicit rather than left implicit in code comments alone. `discipline-suspensions/section-1.md` v0.3. |
| ERR-044-010 | Discipline & Suspensions #44 FR-DC-011 (§2.1) and §3.3 / §4.5, found at **a reviewed-findings pass over the #44 C1/C2 landing**: ERR-044-003 stage 1 made `OnClubFixturePlayed`'s ban-serving exemption depend on `fieldedPlayerIds`, but neither FR-DC-011 nor §3.3's comment block on that exemption records what the array is derived from or its dependency on Stage 0's fixed-XI posture. `SeasonLoop.FieldedXi` (`src/season-save/SeasonLoop.cs:1620`) derives it from the SAME pre-kickoff `LineupSelector` walk `ConfigureSquads` consumes — the STARTING eleven, not a record of who took the field. #44 is otherwise scrupulously substitution-correct for card ATTRIBUTION: `CardLedgerFold`'s occupancy tracking follows every `SubstitutionEvent` and reattributes cards to whoever actually occupies a slot at the card's tick, including synthetic bench identities. The serving exemption does not share that care — it is correct today only because `CardLedgerFold.cs`'s own remarks record that `SeasonLoop` never calls `MatchEngine.SubstitutePlayer` ("Stage 0 fields a fixed eleven"), so starting XI and played XI are identical by construction. The moment a substitution seam gets a production caller, a suspended player fielded by the extremis back-fill as a SUBSTITUTE (rather than a starter) will not appear in the starting-eleven array, and his ban will decrement for a fixture he actually played — reopening ERR-044-003 stage 1's free-appearance defect one boundary over, silently. | Low | 3 | ◑ **Recorded, not fixed — spec-only, same commit, no code change** (the hazard is dormant: no `SubstitutePlayer` call site exists on the season path today, so `fieldedPlayerIds` and the played eleven are identical by construction and no test can distinguish them). FR-DC-011 gains a note stating the required eleven is who played, not who started, and that `SeasonLoop.FieldedXi` satisfies it only in the absence of substitutions; §3.3's `OnClubFixturePlayed` comment block gains a SUBSTITUTION DEPENDENCY paragraph naming the exact failure mode and pointing the eventual fix at the #30-owned derivation site; §4.5's fielded-eleven bullet gains the matching composition-root MUST for when a substitution seam lands. `discipline-suspensions/section-2.md` v0.8, `section-3.md` v0.11, `section-4.md` v0.8. |
| ERR-044-011 | Discipline & Suspensions #44 §6.2, found at **a reviewed-findings pass over the #44 C1/C2 landing**: two defects in the per-operation cost table. **(a)** The first bullet named a member, `OnTapRecord`, that has never existed in `src/discipline/` — verified by `grep -rn OnTapRecord src/discipline/`, zero hits; the real member, `CardLedgerFold.ObserveTick`, is the pre-M27 pseudocode name §3.1 itself carried before its own v0.10 rewrite, and §6.2 was never updated to match. **(b)** `OnClubFixturePlayed`'s complexity was stated as "O(active-ban entries for the club), typically 0–2", but `DisciplineRules.cs:265`'s `for (int i = _state.Count - 1; i >= 0; i--)` loop walks the ENTIRE `DisciplineState` — every club's rows, not just the calling club's — filtering by `clubId` and `BanMatchesRemaining > 0` inline; the true bound is O(total discipline entries across every club). The row also omitted the O(11) `WasFielded` linear scan of `fieldedPlayerIds` the ERR-044-003 stage 1 exemption runs for each active ban found. | Low | 1 | ✅ **Resolved August 15, 2026, spec-only, same commit — no code change.** §6.2's first bullet renamed `OnTapRecord` → `ObserveTick` and restated to describe buffering (nothing applies to `DisciplineState` until `Commit`); `OnClubFixturePlayed`'s complexity corrected to O(total entries across every club) with the O(11) fielded-eleven scan named. `discipline-suspensions/section-6.md` v0.3. |
| ERR-044-012 | Discipline & Suspensions #44 Appendix A, back-prop owed by `src/discipline/DisciplineConstants.cs` v1.3 (M26, August 15, 2026 — that commit's own header states the back-prop is owed and not filed there): `LEAGUE_COMPETITION_KEY` was tagged `[FIXED]` in Appendix A, but it is a verbatim copy of APPROVED Competition Structure #43's `LEAGUE_COMPETITION_ID` (`docs/specs/competition-structure/appendices.md:15`, value 0, FR-CP-004: "instance 0 — the #30 league binding") — verified by reading both catalogues directly. The root `CLAUDE.md` tag table makes a value defined in another APPROVED spec and consumed read-only `[CROSS]`, never `[FIXED]`; that is the identical argument this same Appendix A already applies to the `CardKindYellow`/`CardKindRed`/`CardKindSecondYellow` rows two rows below (L17, August 13, 2026) — `LEAGUE_COMPETITION_KEY` was simply missed by that pass. | Low | 1 | ✅ **Resolved August 15, 2026, spec-only, same commit (code fix already landed in `DisciplineConstants.cs` v1.3)** — Appendix A's `LEAGUE_COMPETITION_KEY` row re-tagged `[FIXED]` → `[CROSS]`, citing #43's `LEAGUE_COMPETITION_ID` and recording that the literal `0` stays a literal (not a compiler-checked mirror) because #43 has no `src/` assembly yet to bind one to. `discipline-suspensions/appendices.md` v0.7. |
| ERR-044-013 | Discipline & Suspensions #44 §2.2 + Appendix A carried **no declaration of `CardLedgerFold.NO_PLAYER`** (`src/discipline/CardLedgerFold.cs:66`, value `-1`) despite it being caller-facing — the constructor THROWS on any negative occupancy value that is not this sentinel — and used normatively by Appendix C's worked example ("every other bench id `NO_PLAYER`"). It also carried no tag in code, violating FR-CS-060/061, while §9 **G1 cites Appendix A as the evidence that every #44 constant carries exactly one tag** | Medium | 3 | ✅ **RESOLVED August 15, 2026, same commit** (#44 AR round 6). Tagged `[FIXED]` in `DisciplineConstants`-adjacent code, declared in §2.2's `CardLedgerFold` block, and given its Appendix A row. Same omission class §2.2 was extended for one round earlier on `DisciplineRules.State`, `CardLedgerFold.PendingCardCount` and `RequireCommittableConfig()` (ERR-044-007) — the members were added and the sentinel beside them was not, which is why this is filed rather than folded into that id. |
| ERR-030-042 | Season & Competition Loop #30 §3.4 (and §2.3 **F9**), found at **a reviewed-findings pass over the #44 C1/C2 / ERR-044-003 stage 1 landings**: §3.4's depleted-squad rule (ERR-030-029) is the OWNING normative text for the back-fill, and it states exactly ONE ordering key — back-fill by "pressing the least-injured players back in one at a time — ascending remaining recovery, ties broken by earliest roster position" — and then asserts that "#44/#36 contribute removals only and **inherit the rule unchanged** when they join — **#44 has**". **#44 did not inherit it unchanged.** `src/season-save/AvailabilityComposition.Reinstate` has implemented a **two-tier** rule since the C1/C2 landing: its first pass skips `suspended[i]` outright (`if (!removed[i] \|\| suspended[i]) continue;`) and only falls through to a suspended player once no merely-injured one remains. #44 §2.3 and §7.2 record that tier as an owner decision — "every injured player is pressed back before any suspended one, and a suspended player plays only when the alternative is a club that cannot take the field at all" — but **#30's spec set, which owns the rule, never stated it anywhere**. This is a High rather than a doc nit because of the failure mechanism: `recoveryRemaining` is written ONLY by `PlayerCareerStates.MarkUnavailable` (`recoveryRemaining[i] = isInjured ? injury[index].RecoveryRemaining : 0`), so a **suspended-but-uninjured player is never written into it and keeps the `int` default 0** — an implementer following §3.4 verbatim, storing one key for both kinds of removal, sorts him to the FRONT of the ascending order and presses **banned players back ahead of every injured one**, the exact inversion of the owner's decision. It fails **silently**: the never-worse-than-unfiltered invariant still holds, `SquadRating.CanFieldStartingEleven` still returns a fieldable eleven, F9 never fires and the season plays on. **Consequence beyond the ordering:** ERR-044-003 stage 1 exempts anyone in the fielded eleven from serving (`OnClubFixturePlayed(clubId, fieldedPlayerIds)`), so under a §3.4-conformant implementation the reinstated banned player plays AND does not serve — that entry's cost argument ("a two-match red still costs a depleted club two full fixtures rather than one") is false there: the ban costs nothing and never advances while the club stays depleted. The argument is sound only under the tiered rule the code actually implements, which is why the tier belongs in #30's text and not only in #44's. | High | 2 (`section-3.md`, `section-2.md`) | ✅ **Resolved August 15, 2026, spec only — NO code change; the code was already correct and the spec was wrong.** §3.4 now states reinstatement as explicit **tiers** (tier 1 the merely injured, ascending remaining recovery, ties by earliest roster position; tier 2 the suspended, eligible only once no tier-1 player remains and the club still cannot field the formation, by earliest roster position), quoting #44 §2.3's own wording rather than inventing a second phrasing. "Inherit the rule unchanged" is **requalified**: what a joining contributor inherits is the **invariant** — viability adjudicated here, the selector as the probe, the unfiltered squad as the limit, F9 as the terminal case — and explicitly **not the ordering**; a contributor whose removals are not interchangeable with #41's adds a TIER, whose position is stated in #30 §3.4, and #36 (the one remaining future joiner) MUST state its own rather than assume tier 1. The **zero-default trap is written into the spec as a MUST NOT** (a block quote in §3.4): the tier must not be derived from the ordering key, because the key is written only by #41 and a suspended player's zero default inverts the order — judged worth stating because the trap is invisible (no failure mode, no invariant breach, no test signal) and because the natural implementation of the rule as previously written walks straight into it. The ERR-044-003 stage-1 interaction is recorded in §3.4 as the reason the tier order is load-bearing rather than cosmetic. §2.3 F9's parenthetical ("least-injured first, selector-probed") mirrors the tiers and points at §3.4. `section-3.md` v2.6, `section-2.md` v2.0. **Incidental, recorded rather than quietly fixed:** `section-3.md`'s v2.5 (ERR-030-040/-041) shipped with a header entry but no version-history row and no `Version:` field bump — the FR-CS-057 class again; the missing v2.5 row was added retroactively in this commit, annotated as such, and the `Version:` field taken 2.4 → 2.6. |
| ERR-044-014 | Discipline & Suspensions #44 **FR-DC-011** (§2.1), §2.2, §2.3 **F2**, §3.3 and §4.5, found at **an adversarial review over the ERR-044-003 stage 1 landing (H1)**: `OnClubFixturePlayed` derived club membership from #27's packed id formula — `entry.PlayerId / CLUB_SQUAD_SIZE == clubId` (`src/discipline/DisciplineRules.cs:275`) — while the REMOVAL half of the same subsystem, `Availability.MarkSuspended`, walks the real squad and reads `PlayerRecord.PlayerId`. **Two notions of "is this player at this club", agreeing only while the packing holds, with nothing anywhere checking that it does.** On the first disagreement — a #31 transfer, or the id-space widening §7.2 / `ERR-044-003` stage 3 already requires — a banned player is removed from every squad he is really in AND his ban is never decremented: **suspended forever, silently — no throw, no log, and no test able to observe it.** Both the code comment (`DisciplineRules.cs:225-231`, *"FR-DC-013's migration rule keeps a transferred player's id current — so no roster read is needed"*) and the APPROVED spec text (`section-3.md:133-135`) asserted the precondition as satisfied. **It is not:** `MigratePlayerId` and `DropPlayer` have **zero production callers**, recorded at `src/season-save/SeasonLoop.cs:1095-1104` in the landing that introduced the reliance. This is `ERR-041-019`'s defect one subsystem over — there rated High and closed by enforcing the id contract at every entry point; here nothing enforced it anywhere. | High | 6 | ✅ **Resolved August 16, 2026, spec + code, same commit.** The derivation is DELETED, not guarded: `OnClubFixturePlayed(int clubId, int[] clubPlayerIds, int[] fieldedPlayerIds)` matches membership by PRESENCE in the roster the caller resolved, the same question asked the same way as the removal half — one notion of membership, so there is nothing left to disagree. `clubId` is kept for the F2 caller-contract gate and identity only, takes part in no matching, and is deliberately NOT cross-checked against the roster (any such check re-derives a club from an id). `clubPlayerIds` is required and null-refused on `ERR-044-007`'s posture — both silent readings of the unknown case are wrong, "serve everybody" being the removed derivation's failure and "serve nobody" a permanently suspended squad. `src/season-save/SeasonLoop.cs` supplies it from the **UNFILTERED** squad at the resolve→filter→configure site the fielded XIs already come from (new `RosterIds` helper mirroring `FieldedXi`, gated on `_disciplineDriver` alone; `ResolveFixture`/`PlayThroughEngine`/`BootFixtureEngine` thread two more out parameters rather than a second provider read in `PlayNextRound`, per AR pass 2's parallel-surface rule). **The unfiltered squad is load-bearing:** every id whose ban is being served is precisely an id the filter has just removed, so serving off the filtered squad would make every suspension unservable — the same permanently-suspended outcome by the opposite route. **Behaviour preserved on today's ids and proven both ways:** a new lock asserts the roster match and the retired derivation select the same set over a full packed roster (probed at both ends of a club's range and one past it), and a second lock puts BOTH its players' ids in the wrong club for the roster they are on — **executed against a restored derivation and observed to fail** (1 failed / 12 in that filter), so it is a lock on the fix rather than on its result. `Discipline.Tests` **121/121, 0 failed, 0 skipped** (118 → 121: the two membership locks plus the null-roster refusal). `discipline-suspensions/section-2.md` v0.10, `section-3.md` v0.12, `section-4.md` v0.10, `appendices.md` v0.9; `src/discipline/DisciplineRules.cs` v1.7, `src/discipline/tests/DisciplineRulesTests.cs` v1.8, `src/season-save/SeasonLoop.cs` v1.29, `src/season-save/tests/SeasonLoopDisciplineTests.cs` v1.10, `src/season-save/tests/SeasonSaveManagerTests.cs` v1.24. **RECORDED, NOT FIXED — outside this pass's owned file set:** §6.2's per-operation cost table names the `O(11)` `WasFielded` scan per active ban but not the roster scan beside it (at most one squad's worth of ids, same call, same condition); `section-6.md` is another fixer's file this session, and the complexity class of the method — O(total entries across every club), corrected at `ERR-044-011` — is unchanged. §5's traceability table likewise does not yet cite the three new `DisciplineRulesTests` cases; they strengthen FR-DC-011's existing rows rather than replacing any, so nothing in §5 became false, but `section-5.md` is outside this pass's owned set and the citation is owed. Finally, `src/discipline/DisciplineEntry.cs` and `src/discipline/DisciplineSaveCodec.cs` still justify their negative-`PlayerId` refusals by the division that no longer happens (`ERR-044-004`'s own text). **The refusals themselves remain correct and are not weakened** — a negative id is an unresolvable identity, `MigratePlayerId` refuses one, and §2.3 F2 is annotated accordingly — but their stated reason is now historical; both files are outside this pass's owned set. **[ANNOTATED August 16, 2026, M-2 — dated correction, row otherwise left unrewritten per this file's own convention.]** Two of the three items above are now stale. **(a)** landed at `section-6.md` v0.5 (finding L10, same day): the `OnClubFixturePlayed` bullet now names the `O(\|clubPlayerIds\|)` `IsOnClubRoster` roster scan this row's own derivation-deletion added, beside the already-priced `O(\|fieldedPlayerIds\|)` `WasFielded` scan. **(c)** landed at `src/discipline/DisciplineEntry.cs` v1.4 and `src/discipline/DisciplineSaveCodec.cs` v1.3 (both "finding B", doc/message only, same day): neither file's negative-`PlayerId` refusal doc or runtime message cites the packed-id division this row deleted any longer. **(b)** was the one item genuinely still open, and is CLOSED in this same pass: `section-5.md` v0.9 adds the three missing citations — `OnClubFixturePlayed_ServesByRosterMembership_NotByThePackedIdDerivation`, `..._OverAPackedRoster_ServesExactlyWhatTheRetiredDerivationServed`, `..._NullClubPlayerIds_Throws` — under T-DC-BAN-003 and §5.6's FR-DC-011 row. |
| ERR-044-015 | Discipline & Suspensions #44 **§4.5** (the composition-root MUST bullet — *corrected August 16, 2026, L-C: cited as `section-4.md:137-138` at filing; §4.5 has since moved to begin at line 190, so the line citation had already drifted. Cited by section instead, per the not-a-stable-citation rule `section-2.md`/`appendices.md`/`section-6.md` adopted this round (their L6/L10 fixes)*), found at **an adversarial review over the ERR-044-003 stage 1 landing (H3)**: the composition-root MUST instructed the root to *"run `FilterAvailable` at the resolve→configure seam on both clubs' resolved squads"* — naming **the one method this spec elsewhere says the root must NOT call**. Four other sites already state the production path is `MarkSuspended`'s removal mask consumed by #30's composed availability seam: FR-DC-009 (`section-2.md:59`), §2.2's `Availability` block comment (*"FR-DC-009's OWN surface, not #44's production path"*), §3.3's pseudocode (*"#44's production path is `MarkSuspended`'s removal mask … which never calls `FilterAvailable`"*) and §6.2 v0.4. `FilterAvailable` has **zero production call sites** — verified in `src/`. A root built against §4.5 verbatim would filter with #44's own reduced value copy and never reach `AvailabilityComposition`, so the intersection with #41's removals and the **#30 §2.3 F9 back-fill would not run for suspensions at all** — reinstating exactly the defect `ERR-044-003` was filed and staged to close, from APPROVED text, in the section an implementer of the root reads first. | High | 1 | ✅ **Resolved August 16, 2026, spec-only, same commit — no code change** (the landed root already does the right thing; only the instruction was wrong). §4.5's clause rewritten to the landed contract: gather #44's removals through `Availability.MarkSuspended` into the composed seam (`AvailabilityComposition`), which owns the intersection and the #30 §2.3 F9 back-fill, with an explicit **MUST NOT** on `FilterAvailable` and the reason named, so the next reader cannot restore it by preferring the shorter call. The FR-DC-010 / `ERR-044-002` scope half of the same sentence — both clubs, every fixture, both resolution paths — is CORRECT and is kept verbatim. `discipline-suspensions/section-4.md` v0.10. |
| ERR-037-003 | Match Analytics & Statistics #37 §3.2, found at the **reviewed-findings pass (M4)**, August 16, 2026: the `CardIssuedEvent` routing row gave the formula `CardKind==red ? RedCards[team]++ : YellowCards[team]++` — a two-way test over `CardKind`, which is #17's **three**-value domain ordinal (0=Yellow, 1=Red, 2=SecondYellow — Event System #17 Appendix A row 0x06 / §3.10, `EventSystemConstants.CARD_KIND_SECOND_YELLOW`). The formula has no branch for the third value at all, and `MatchAnalyticsAggregator.RouteRecord` implemented it literally: `evt.CardKind == MatchAnalyticsConstants.CardKindRed ? _redCards[team]++ : _yellowCards[team]++`, with `MatchAnalyticsConstants` itself carrying no `CardKindSecondYellow` row to test against even if the code had wanted to. Every second-yellow dismissal therefore fell into the `else` and counted as a plain yellow, and `RedCards` stayed 0 for that team no matter how many second-yellow dismissals the match had — directly contradicting `MatchStatline.RedCards`'s own XML doc, in the same file's sibling struct, which states the field is "Red cards received (**including second-yellow dismissals**)." Reachable on ordinary play (any two-caution dismissal), not a corner case. | Medium | 1 | ✅ Resolved August 16, 2026 at the reviewed-findings pass — **`MatchStatline.RedCards`'s documented contract wins; the §3.2 formula is the defect.** `MatchAnalyticsConstants` gains `CardKindYellow` and `CardKindSecondYellow` `[CROSS]` mirrors of `EventSystemConstants.CARD_KIND_YELLOW`/`CARD_KIND_SECOND_YELLOW` (the ERR-020-004 owning-catalogue carve-out, alongside the existing `CardKindRed`), and `MatchAnalyticsAggregator.RouteRecord`'s `CardIssuedEvent` branch becomes an explicit three-way mapping: `Yellow ⇒ YellowCards++`; `Red ⇒ RedCards++`; `SecondYellow ⇒ YellowCards++ AND RedCards++`. The double-increment on `SecondYellow` is deliberate, not a bug: the producer (`MatchEngine.ApplyCardAndCheckSentOff`) emits the second caution and the resulting dismissal as **one** event, never a yellow-then-red pair, so nothing else will ever increment a yellow for that caution — the single event must cover both tallies for the box score to read as the two-yellows-plus-one-red a real match report shows (the player's *first* caution earlier in the match already published its own separate kind-0 event and is counted there). This mirrors the converged #44 Discipline precedent exactly: `DisciplineRules.ApplyCard`'s kind-2 branch is documented as "one yellow AND one dismissal ban" for the identical producer-contract reason. `section-3.md` §3.2's routing row rewritten to the explicit three-way mapping (v0.2 → v0.3); `MatchStatline.YellowCards`'s XML doc gains a matching cross-reference to `RedCards` recording that a second-yellow's caution counts toward both (doc only, no field change). Locked by `MatchAnalyticsAggregatorTests.Cards_SecondYellowDismissal_CountsAsBothAYellowAndARed` (a kind-2 event on a recipient who already has a kind-0 yellow takes YellowCards 1→2 and RedCards 0→1; fails against the pre-fix aggregator, which leaves RedCards at 0). No FR text change, no format-version change, no `DETERMINISM_DIGEST_VERSION` bump — `MatchAnalyticsConstants`/`MatchAnalyticsAggregator`/`MatchStatline`/`MatchAnalyticsAggregatorTests`/`section-3.md` all bumped their own version rows in the same change. |
| ERR-030-044 | Season & Competition Loop #30 §3.4 (the depleted-squad back-fill), found at **an adversarial review over the ERR-044-003 stage 1 landing (H2)**: the extremis back-fill fires on a **bench** shortfall, not on an inability to field an eleven, and its within-tier ordering key then started the banned player it pressed back. Two halves. **(a) Trigger.** §3.4's own probe is `SquadRating.CanFieldStartingEleven`, which is `LineupSelector`'s FULL selection walk — eleven position-matched starters PLUS the seven-slot bench (`PLAYERS_PER_TEAM + SUBSTITUTES_PER_TEAM` = 18) — while §3.4's prose has read "unable to field the formation" / "a fieldable eleven" since ERR-030-029 and #44 §2.3 reads "cannot take the field at all". A club with **seventeen** fit, position-complete players can field a perfectly legal XI and still reach tier 2. **(b) Within-tier ordering.** Tier 2 (ERR-030-042) ordered by earliest roster position, full stop. Because the reinstated player joins the pool the **rating-greedy** selector draws the starting eleven from, that key handed the selector whichever suspended player happened to sit first on the roster and **started** him — and ERR-044-003 stage 1 then exempts anyone in the fielded eleven from serving, so his ban stalled for as long as the club stayed depleted. §3.4's own rationale states that reinstating a suspended player into play "stalls the suspension itself", so the stated key produced exactly the outcome the same rule's rationale forbids. Probed: 8 bans, 17 fit, zero injuries → 1 suspended player in the starting XI, ban unmoved. | High | 1 | ✅ Resolved August 16, 2026, spec + code same commit. **(a)** is a CLARIFICATION, not a decision change: the named mechanism is authoritative and §3.4 now says "cannot field WHAT SELECTION REQUIRES", with the eleven-vs-eighteen **narrowing** (a short-bench posture) recorded as an OPEN owner question — **CLOSED August 17, 2026 by owner decision: keep the eighteen; the short-bench posture is NOT built** (#30 §3.4 v2.12), because the honest fix for a depleted club is cover rather than a shorter bench and `ERR-044-003`'s stages 2–3 retire the question instead of answering it. This entry's open half is therefore discharged; the closure is recorded here as well as in the spec because a decision that reaches only the owning section leaves this log — the remediation backlog — reading as though it were still pending — it changes what a fixture is, and narrowing the probe without narrowing selection recreates the two-walks divergence `LineupSelector.TrySelect` was collapsed to one walk to prevent (#41 T2 AR pass 1 H3). **(b)** is a DEFECT CORRECTION, filed as an ERR rather than raised as an owner question precisely because an ordering key that contradicts its own rule's stated purpose is this log's class. Tier 2's key becomes "the first candidate, in roster order, the selector would BENCH; earliest roster position only when no candidate choice keeps every reinstated-suspended player out of the XI", implemented at `AvailabilityComposition.ChooseSuspendedCandidate` as three passes over the still-removed (necessarily suspended) candidates in roster order: (1) first candidate whose materialised squad is fieldable AND whose XI contains no reinstated-suspended id — the set being `{i : suspended[i] && !removed[i]} ∪ {c}`, compared by `PlayerId` since materialisation renumbers indices; (2) first candidate fieldable at all — the **forced-start** residual, canonically a club's sole goalkeeper; (3) earliest roster position, verbatim today's behaviour, for the multi-reinstatement case where no single candidate reaches fieldability, after which the outer loop asks again. Tier 1's key, the outer loop, the `availableCount == total` §2.3 F9 terminal throw, the one-reinstatement-per-pass bound and FR-DC-018's reference-identity fast path are **untouched**; `DisciplineRules` needed no change, being already correct on both sides (bench-reinstated ⇒ absent from `fieldedPlayerIds` ⇒ ban advances; forced start ⇒ exempt). The reviewer's proposed engine-contract change was REJECTED — the fix is confined to `AvailabilityComposition`. No RNG, no draw site, no `SNAPSHOT_SCHEMA_VERSION`/`SEASON_SAVE_FORMAT_VERSION` movement, no `ISquadProvider` change; **digest invariance is NOT claimed** for a season whose extremis tier fires, the reinstatee being intentionally different. Locked by the new `AvailabilityCompositionExtremisTests` (4 cases): the bench-depth mutant-killer, the ban-advances-from-the-bench consequence driven through a really played round, the forced-start residual asserted as the RECORDED compromise (reinstated, IS in the XI, ban does NOT advance), and the double-shortfall pass-3 fallback with repeat-call determinism. The first two were **observed to fail at the pre-fix tree** and pass after. Spec: `season-competition-loop/section-3.md` v2.7. Code: `src/season-save/AvailabilityComposition.cs` v1.5. Cross-filed at #44 as **ERR-044-019**. **⚠️ SUPERSEDED IN PART, August 16, 2026, later — see `ERR-030-045` below.** Pass (3) of the key landed here, and the claim above that "the outer loop asks again" leaves passes 1 and 2 deciding the reinstatement that closes the gap is **FALSE**: fieldability is monotone in adding players, so passes 1 and 2 are structurally unreachable on every reinstatement but the last, and the blind pass-3 pick then poisons the last one too — `k >= 2` reproduced this row's own pre-fix behaviour exactly. The 4-case lock count above also stands as landing-time history: the double-shortfall case asserted the pre-fix key on a roster where the choice provably could not matter. Pass 3's key is now ascending selector rank (`ERR-030-045`, `AvailabilityComposition.cs` v1.6). Passes 1 and 2, the trigger clarification and the OPEN eleven-vs-eighteen owner question are unaffected. |
| ERR-030-045 | Season & Competition Loop #30 §3.4 (the depleted-squad back-fill's within-tier key, **pass 3**), found at **a second adversarial-review pass over the `ERR-030-044` landing (H2-continuation)**: **`ERR-030-044`'s defect survives intact whenever the club is short by more than one player — and code, spec and suite all assert that it does not.** Passes 1 and 2 of `ChooseSuspendedCandidate` both gate on `SquadRating.CanFieldStartingEleven` over a squad of `availableCount + 1`. Fieldability is **monotone in adding players**, so on any reinstatement that is not the last no candidate is fieldable: passes 1 and 2 are **structurally unreachable** and pass 3 — earliest roster position, the pre-`ERR-030-044` key — decides alone. It compounds: `AnyReinstatedSuspendedStarts` also inspects the players earlier passes pressed back, so if a blindly chosen one starts, pass 1 is unsatisfiable for **every** candidate on the final pass and pass 2's forced-start fallback takes it. Net behaviour at `k >= 2` is exactly pre-`ERR-030-044`, and a mass-suspension club — the population #44's own subject creates — reaches `k >= 2` more readily than `k = 1`. Demonstrated by probe: a nineteen-man roster with sixteen fit and the roster-earliest banned player made the club's best forward composed an XI **containing** him, while the alternative pair completes the squad and benches every banned player. Three claims were consequently false and are corrected rather than deleted: #30 §3.4's "so passes 1 and 2 still decide the reinstatement that finally closes the gap" and "The stall is now confined to the forced-start case"; `AvailabilityComposition`'s "two cases" type remark; and `MultiReinstatementExtremis_FallsBackToRosterOrder_AndTerminatesDeterministically`, which asserted `Contains(composed, banned[0])` — the **pre-fix key** — on a roster where the choice provably could not matter. | High | 1 | ✅ Resolved August 16, 2026, later, spec + code same commit. **Pass 3's key becomes "the candidate the selector ranks LAST"** — ascending `SquadRating.PlayerRating`, ties broken by earliest roster position. The property that earns it is that it is **safe without a probe**, which is exactly what the blind passes need: the weakest banned players are pressed back first, so the club's best is reached only when nothing weaker closes the gap, and the final pass's pass-1 test still finds a benchable candidate whenever one exists. The rating read is a **new straight delegation** — `SquadRating.PlayerRating(in PlayerAttributes)` → `LineupSelector.MeanAttribute`, the selector's own per-player key — rather than a mean re-derived in `season-save`, which would be the parallel-surface trap `SquadRating` exists to prevent (league-bootstrap KD-7 / AR-4 M-1, and #41 T2 AR pass 1's H3). **The claims are corrected to the property that actually holds, not to a stronger one.** This is a best-effort **minimisation**, NOT a guarantee: a multi-reinstatement set can still start a suspended player when every completing choice does, and positional constraints can force it outright (the weakest banned man being the club's only goalkeeper). §3.4's two-case statement becomes three — benched whenever any choice permits it; forced start when none does, reached both by single-reinstatement positional forcing AND by a `k >= 2` set in which every completing choice starts someone; the ban stalls only for a reinstatee who is actually fielded. Tier 1's key, passes 1 and 2, the outer loop, the §2.3 F9 terminal throw and FR-DC-018's identity fast path are **untouched**; no RNG, no draw site, no `SNAPSHOT_SCHEMA_VERSION` / `SEASON_SAVE_FORMAT_VERSION` movement, no `src/discipline/` change; digest invariance still not claimed for a season whose extremis tier fires. Two Lows fixed in the same file: the `Cost` remark undercounted (a **fieldable** candidate costs TWO full `TrySelect` walks, the second inside `AnyReinstatedSuspendedStarts`), and the dead `candidate == null` branch at the probe site — `MaterializeSquad` returns null only at `availableCount == 0` and this site passes `availableCount + 1 >= 1` — is replaced by a comment stating why it cannot fire. Locked by `AvailabilityCompositionExtremisTests` (4 → 5 cases): the reshaped multi-reinstatement case now asserts termination, repeat-call determinism and the new key's effect (weakest pressed back first, best banned player never reached) on a roster where the choice matters, and a new `k >= 2` case asserts that **no** banned player is in `StartingElevenPlayerIds(composed)` when an alternative completing choice exists. **Both were observed failing at the pre-fix tree** (`Expected: not some item equal to 16 / But was: < 0, 2, 3, 4, 5, 7, 8, 9, 10, 16, 12 >`) and green after. Spec: `season-competition-loop/section-3.md` v2.8. Code: `src/season-save/AvailabilityComposition.cs` v1.6, `src/match-engine/SquadRating.cs` v1.5. Back-propagated to #44 as an amendment to **ERR-044-019** (`section-2.md` v0.13, `section-7.md` v0.9), whose two-case statement pinned the forced-start case to positional forcing alone. |
| ERR-044-019 | Discipline & Suspensions #44 §2.3 (the ERR-044-003 note) and §7.2, found at **the same adversarial review (H2)** that filed `ERR-030-044`: both sections assert that "a suspended player plays only when the alternative is a club that cannot take the field at all". That was **false of the pre-fix implementation** on both halves of the sentence — the back-fill's trigger is #30 §3.4's probe, the full selection walk (eleven starters PLUS the seven-slot bench), so it fires for **bench depth** at a club that could field a legal XI; and tier 2's earliest-roster-position key then let the rating-greedy selector **start** the reinstated man. So the sentence described neither when the reinstatement happens nor what it costs. It is a description defect rather than a requirement defect: FR-DC-011 already reads "per played fixture of the player's club **that the player did not appear in**", which is exactly right in both cases. | High | 2 | ✅ Resolved August 16, 2026, same commit as `ERR-030-044` (which owns the rule and the code fix). Both sections corrected to the **two-case** form the amended within-tier key produces: **benched** — the common case, and what the amended key now prefers — he is not in `fieldedPlayerIds`, FR-DC-011's decrement is not exempted, and **his ban advances normally**, so the suspension costs what the Laws say even though the club needed him in its eighteen; **forced to start** — no candidate choice keeps a reinstated-suspended player out of the XI, the sole-goalkeeper case — he plays, ERR-044-003 stage 1's exemption fires, and **only then does the ban stall**. The stall is stated as a property of being *forced* onto the pitch, never of being reinstated, and is named as the residual §7.2's two unbuilt tiers (youth call-ups, then generated cover — blocked on #42 having no `src/` assembly and on the packed `PlayerId` id space needing widening) exist to delete. No FR row changed, no format-version change, no code change in `src/discipline/`. `section-2.md` v0.11, `section-7.md` v0.8. **ANNOTATED August 16, 2026 (reviewed-findings pass, M16): the correction stopped at the spec folder — three further live sites still asserted the refuted sentence and are now swept.** `CLAUDE.md` (root, the #44 OPEN ISSUES index entry), `docs/tracking/open-issues.md` (the same entry's live copy), and `src/season-save/tests/SeasonLoopDisciplineTests.cs`'s `TheBackFillPressesAnInjuredPlayerBackBeforeASuspendedOne` assertion message all still read "a banned man plays only when the alternative is a club that cannot take the field at all" — corrected to the two-case form this row's own fix already states (benched whenever any choice permits it, his ban advancing normally; forced into the XI only when no completing choice avoids it — positional forcing or a `k≥2` shortfall, `ERR-030-045` — and only then does the stage-1 exemption stall his ban). No FR row, format-version, or code-logic change; one test's assertion **message** only. |
| ERR-044-016 | Discipline & Suspensions #44 Appendix C (worked fold example), found at **the final fixer pass over the reviewed-findings round (M6)**: the example's kind-2 `CardIssuedEvent` at tick 12 000 arrived as agent slot 7's FIRST card since the tick-9 000 substitution moved player 191 onto it — unproducible by the live engine. `MatchEngine.SubstitutePlayer` resets `_yellowCards[outSlot] = 0` on every substitution, and `ApplyCardAndCheckSentOff` returns kind 2 only when the RECIPIENT SLOT's own count is `>= 2` **after** the increment — so a slot's first booking since a reset can never legitimately be emitted as a kind-2; it is always kind 0. Had this example been implemented as written, the tick-12 000 card either could not exist as described or the fold's kind-2 branch (one yellow AND one dismissal ban, KD-5) would have been exercised by a card the engine cannot actually produce at that point in the sequence — the reverse of ERR-044-001's defect in the same appendix (a hedge the spec declined to verify), this time a claim the spec COULD have checked against `ApplyCardAndCheckSentOff`'s own return contract and did not. | Medium | 1 | ✅ **Resolved August 16, 2026, spec-only, same commit — no code change** (the engine's kind-emission contract is unchanged; only the example was unproducible). A kind-0 `CardIssuedEvent{Recipient: 7, Kind: 0}` for slot 7 (occupant 191) now sits at a new tick 10 500, between the substitution and the kind-2 card, with the engine precondition stated explicitly: `_yellowCards[7] >= 1` before tick 12 000's booking, satisfied by the new tick-10 500 card incrementing it from its post-substitution 0 to 1. Ticks and tallies recomputed: player 191 ends the fixture at `Yellows = 2, BanMatchesRemaining = 1` (not `1, 1` as before); after `OnClubFixturePlayed` serves both outstanding bans to 0, player 183's row reaches `(0, 0)` and is dropped per FR-DC-017, while player 191's row survives at `(2, 0)`, carrying his two accumulated yellows toward the next accumulation ban — a real, nonzero row that FR-DC-017's canonical-minimality rule does NOT drop. The id arithmetic verified at `ERR-044-001` (`183 / 25 = 191 / 25 = 7`, the synthetic bench id 22) is unchanged. `discipline-suspensions/appendices.md` v0.10. |
| ERR-044-017 | Discipline & Suspensions #44 `appendices.md` (Appendix A), `section-2.md` (FR-DC-006/007), `section-3.md` (§3.1/§3.2 pseudocode), `section-9-approval-checklist.md` (§9.2), found at **the final fixer pass over the reviewed-findings round (M7)**: the four `[GT]` threshold/ban constants — `YELLOW_ACCUMULATION_THRESHOLD`, `ACCUM_BAN_MATCHES`, `SECOND_YELLOW_BAN_MATCHES`, `STRAIGHT_RED_BAN_MATCHES` — were ALL_CAPS in every one of those four files, while `DisciplineConstants.cs` declares them PascalCase (`YellowAccumulationThreshold` etc., v1.0 onward) and `src/CLAUDE.md`'s naming table reserves ALL_CAPS for `[FIXED]` constants only — `[GT]`/`[EST]`/`[DERIVED]`/`[CROSS]` are PascalCase. §2.2 F6, §3.2's own `RequireYellowThreshold`/`RequireBanLength` calls, §5 and §6 of this same spec had already used PascalCase for these identical constants, so the spec was internally inconsistent about its own four `[GT]`s, not merely out of step with the code. | Medium | 4 | ✅ **Resolved August 16, 2026, spec-only, same commit — no code change** (the code was already correct; only the spec text lagged it). All four constants renamed ALL_CAPS → PascalCase everywhere they appear in ACTIVE spec text across the four files named above. Genuine `[FIXED]` ALL_CAPS constants (`DISCIPLINE_SAVE_MAGIC`, `DISCIPLINE_SAVE_FORMAT_VERSION`, `NO_PLAYER`) are untouched, per the tag table's own rule. **Historical version-history rows that quote the pre-rename pseudocode by name are deliberately left ALL_CAPS** (`section-3.md`'s v0.7/v0.8 rows, `docs/tracking/discipline-suspensions-design.md`, and this log's own M20 entry above) — they describe the text as it stood at that landing, the same precedent `DisciplineConstants.cs` v1.5's L4 and `appendices.md` v0.5's L3 already established for `LEAGUE_COMPETITION_KEY`/`CARD_KIND_*`. `discipline-suspensions/section-2.md` v0.12, `section-3.md` v0.13, `section-9-approval-checklist.md` v0.6, `appendices.md` v0.10. Grep-verified tree-wide afterward: no ALL_CAPS occurrence of any of the four names survives in `docs/` or `src/` outside the preserved historical quotes named above. |
| ERR-044-018 | Discipline & Suspensions #44 §2.2 (data structures), found at **the final fixer pass over the reviewed-findings round (M8)**: the `DisciplineState` block was a bare `{ /* map (int PlayerId, int CompetitionId) -> DisciplineEntry; NO RNG state */ }` comment — the landed type's entire public API (`Count`, `EntryAt(int)`, `EntryFor(int,int)`, `HasEntry(int,int)`, and the static restore-door `FromEntries(DisciplineEntry[])`) was undeclared. This mattered beyond completeness: `FromEntries`' strictly-ascending-keys and no-all-zero-rows refusals are what FR-DC-015 (F3) and FR-DC-017 rest on AT THE FILE BOUNDARY — `DisciplineSaveCodec.Decode` returns through it — and neither requirement's text pointed at the method that actually enforces them for a decoded state. `EntryFor`'s negative-`playerId` posture (returns the clean zero row rather than throwing, `DisciplineState.cs` v1.1, L1) was likewise undocumented here, leaving §2.2 silent on a caller-visible behaviour change from an earlier landing. | Medium | 1 | ✅ **Resolved August 16, 2026, spec-only, same commit — no code change.** §2.2's `DisciplineState` block now declares the full public surface, with `FromEntries` cross-referenced to F3 (§2.3) and FR-DC-017, and `EntryFor` annotated with its current negative-key posture. `discipline-suspensions/section-2.md` v0.12. |
| ERR-044-020 | Discipline & Suspensions #44 §2.2 (interface declaration) and §3.1 (§4.3's normative tap-read text, out of this pass's owned file set), found at **the final fixer pass over the reviewed-findings round (M3)**: `IDisciplineTickLedgerTap` was referenced by §2.2's `CardLedgerFold.ObserveTick(IDisciplineTickLedgerTap tap)` signature but never itself declared anywhere in the section set, and neither §2.2 nor §3.1's `ObserveTick` pseudocode said anything about lossless-consumption enforcement — `CardLedgerFold.cs` v1.8 (a same-day reviewed-findings code pass, M3) gave the interface a `CurrentTick` member and gave `ObserveTick` a consecutive-tick refusal plus a partial-application poison latch, mirroring #37 `MatchAnalyticsAggregator`'s F6 guard, with nothing in the spec text describing either. Not a contradiction of existing text — the spec was simply silent on enforcement the sibling subsystem it names as its own precedent (#37) is code-enforced for. | Medium | 2 | ✅ **Resolved August 16, 2026 (spec-side, §2.2/§3.1 only — §4.3 is `section-4.md`, outside this pass's owned file set and RECORDED, NOT FIXED here).** §2.2 gains the `IDisciplineTickLedgerTap` interface declaration (`CurrentTick`, `RecordCount`, `OrdinalAt`, `RecordAt<T>`), with `CurrentTick` documented as what `CardLedgerFold.ObserveTick` uses to enforce the lossless pump; `CardLedgerFold`'s `ObserveTick` declaration gains a note describing both refusal semantics. §3.1's `ObserveTick` pseudocode gains `faulted`/`lastObservedTick` state and the matching two REQUIREs, exactly mirroring `CardLedgerFold.cs` v1.8. **`section-4.md` §4.3/§4.5 still need the identical sync** (the tap-read section and the composition-root MUST) — owed against this ERR id and left open in substance for whichever pass next owns that file. `discipline-suspensions/section-2.md` v0.12, `section-3.md` v0.13. **ANNOTATED August 16, 2026 (reviewed-findings pass, M19): CLOSED.** The `section-4.md` sync landed at commit `1683eb3` ("AR round 1, wave 3d: the ERR-044-020 section-4 sync carry") — §4.3 now declares `IDisciplineTickLedgerTap`'s `CurrentTick` member and describes `CardLedgerFold.ObserveTick`'s consecutive-tick refusal and partial-application poison latch, and §4.5's root-contract bullet states that losslessness is enforced by the tap contract itself; `section-4.md` v0.10 → v0.11. This closure was previously recorded only in this file's own v2.36 header entry above, not in this row's body — a reader grepping this ERR id directly saw only the stale open note. See the v2.36 entry for the fuller account. |
| ERR-044-021 | Discipline & Suspensions #44 §3.1 (the `CardLedgerFold` pseudocode), found at **the final fixer pass over the reviewed-findings round (M1)**: the normative pseudocode stated the substitution swap as `occupancy.ApplySub(record.Outgoing, record.Incoming)` with no instruction to clear `record.Incoming`'s own slot afterward, and the constructor line's seed contract read `occupancy := lineup # seeded by the root (F1 on any gap)` with no one-to-one requirement. Both are the EXACT pre-fix shapes a same-day reviewed-findings code pass (M1) closed in `CardLedgerFold.cs` v1.8: an implementer following either verbatim reproduces the dormant occupancy-injectivity hole — after a substitution, the player who came on would occupy TWO agent ids at once (the new on-pitch slot and the stale bench id), so a later malformed record naming the stale id (Appendix C's own "slot 19" shape, `ERR-044-001`) would silently misattribute a second card to him instead of failing loud (F1); and a doubly-mapped construction seed was silently accepted with no diagnostic, an implementer of which would attribute one player's cards to whichever of his two ids a record happened to name while the seed's OTHER intended occupant of one of those ids received no mapping at all. Unlike `ERR-044-020`, this is a genuine contradiction of the pre-fix text with the fix, not mere silence — the pseudocode actively described the unguarded operation. | Medium | 1 | ✅ **Resolved August 16, 2026, spec-only, same commit — no code change** (the code fix landed same-day, ahead of this spec sync). §3.1's constructor line gains the one-to-one seed requirement alongside "F1 on any gap"; the `SubstitutionEvent` branch's `ApplySub` call gains an inline comment stating it clears `record.Incoming`'s vacated slot to `NO_PLAYER` and refuses `Outgoing == Incoming` (the write and the clear would otherwise target the same index, the clear erasing the write an instant after it landed). Matches `CardLedgerFold.ApplySubstitution`/constructor, v1.8, exactly. `discipline-suspensions/section-3.md` v0.13. |
| ERR-044-022 | Discipline & Suspensions #44 `src/discipline/CardLedgerFold.cs` (`ApplySubstitution`), found at **a reviewed findings pass over the M1/`ERR-044-021` landing**: `ApplySubstitution` still silently destroyed a player's occupancy whenever `SubstitutionEvent.Incoming` named an OCCUPIED ON-PITCH agent id, and the `CardLedgerFold.cs` v1.8 version-history row and the M1 inline comment ("Clearing makes that impossible rather than merely unlikely") both FALSELY claimed the vacated-slot clear alone closed this. Probe: seed `{5 -> 100, 6 -> 101, 22 -> 102}`, record `Sub(Outgoing=5, Incoming=6)` — no throw; `_occupancy[5] <- 101`, `_occupancy[6] <- NO_PLAYER`; player 100 (slot 5's prior occupant) loses his mapping entirely and a card at slot 5 lands on player 101 (slot 6's real occupant) — the Appendix C "slot 19" family (`ERR-044-001`) one layer deeper than M1's fix reached. Unreachable through today's `MatchEngine.SubstitutePlayer` (its derived `Incoming` is always `>= SQUAD_SIZE`), so a latent hole plus a falsified verification claim, not a live defect. Root cause: the M1 seed-injectivity check runs once, at construction, over player ids alone — it has no way to learn which agent ids are on-pitch versus bench, so `ApplySubstitution` could not tell a malformed `Incoming` (an on-pitch id) from a legitimate one (a bench id). | Medium | 1 | ✅ **Resolved August 16, 2026, spec + code same commit (owner-decided option (a)).** `CardLedgerFold`'s constructor gains a required `int onPitchAgentIdCount` parameter, validated `0 < onPitchAgentIdCount <= occupancyByAgentId.Length`; `ApplySubstitution` now refuses `Incoming < onPitchAgentIdCount` ("an on-pitch agent id cannot come on") and `Outgoing >= onPitchAgentIdCount` ("only an on-pitch agent id can go off"), both naming the offending id and BEFORE either the write or the M1 clear runs. `SeasonLoop`'s construction site (`PlayThroughEngine`) passes `MatchEngineConstants.SQUAD_SIZE` — the same cross-assembly crossing `NO_PLAYER_ID` already makes, now also exercised by `SeasonLoopDisciplineTests`'s existing sentinel-agreement lock's neighbour (see `ERR-044-023`). Every `CardLedgerFoldTests` construction updated for the new parameter; two new locks — the finding's own exact probe now throws and leaves player 100's mapping untouched, and `Outgoing >= onPitchAgentIdCount` throws, isolated with a bench-vs-bench pair so the Incoming guard cannot mask it. **The false `CardLedgerFold.cs` v1.8 version-history claim is annotated in place, not rewritten** — a dated correction stating the claim was false when written and is true only as of this fix; the M1 inline "impossible rather than merely unlikely" comment is corrected the same way, in place, as part of this code change. `CardLedgerFold.cs` v1.10, `CardLedgerFoldTests.cs` v1.8, `SeasonLoop.cs` v1.30, `SeasonLoopDisciplineTests.cs` v1.11. Spec sync: `discipline-suspensions/section-2.md` v0.14 (§2.2), `section-3.md` v0.14 (§3.1), `section-4.md` v0.12 (§4.5). |
| ERR-044-023 | Discipline & Suspensions #44 §4.3 (the tap read — no binding of `MatchEngine.PlayerIdsByAgentId`'s own one-to-one precondition existed anywhere in this spec), found at **the same reviewed findings pass as `ERR-044-022`**: the ONLY documented producer of `CardLedgerFold`'s occupancy seed, `MatchEngine.PlayerIdsByAgentId()`, violates the constructor's one-to-one precondition the moment a substitution has occurred — `MatchEngine.SubstitutePlayer` writes `_slotPlayerIds[outSlot] = _benchPlayerIds[team][bench]` and never clears the bench entry (`ReprojectSubstitutions`, the restore path, does the same), so after one substitution the incoming player occupies TWO agent ids at once. Nothing anywhere asserted or bound the agreement both sides need: `MatchEngine.cs`'s own `PlayerIdsByAgentId` XML doc was silent on the post-substitution shape, and `CardLedgerFold`'s constructor doc named only the seed's OWN one-to-one requirement, not where a caller must take the snapshot to satisfy it. Today's only production caller (`SeasonLoop.PlayThroughEngine`) constructs the fold at boot, before any tick can call `SubstitutePlayer`, so this cannot fire in production yet — but the mid-fixture restore seam (`MatchSession.TickOnce`/`CaptureSave`/`RestoreFrom`, one assembly over) is exactly the kind of caller that would rebuild a fold from a running, possibly post-substitution engine, and would hit this precondition with no spec text anywhere naming it. | Medium | 3 | ✅ **Resolved August 16, 2026, spec + doc-only code, same commit.** No behaviour change — the code fix for `ERR-044-022` above happens to make a post-substitution seed loudly refused (via the new `onPitchAgentIdCount` guards plus the pre-existing M1 one-to-one check) rather than silently wrong, but the underlying array itself is unchanged: `MatchEngine.SubstitutePlayer`'s bench entry is deliberately left uncleared (out of scope for a Medium — that array feeds other consumers) and the fix here is entirely doc + one new cross-assembly test. `MatchEngine.PlayerIdsByAgentId`'s XML doc now states explicitly that the array is one-to-one over non-sentinel entries **AT BOOT ONLY**, and names why (the uncleared bench entry). `CardLedgerFold`'s constructor doc and type remarks state the same boot-time precondition and where it comes from. New `SeasonLoopDisciplineTests.PlayerIdsByAgentId_IsInjectiveAtBoot_ButNotAfterOneSubstitution` proves both halves directly against the real engine — injective before any substitution, NOT injective after one — beside the existing `TheEngineAndTheFoldAgreeOnTheNoPlayerSentinel` lock, for the identical reason (neither `match-engine` nor `discipline` can see the other's code, so `season-save` is the one place both are visible). §4.3 now binds the precondition normatively, with the boot-only qualifier, and names §4.5's existing "before kickoff" phrasing as this precondition stated in full rather than incidental wording. `MatchEngine.cs` v1.72 (XML doc only), `CardLedgerFold.cs` v1.10, `SeasonLoopDisciplineTests.cs` v1.11. Spec sync: `discipline-suspensions/section-4.md` v0.12 (§4.3, normative), `section-2.md` v0.14, `section-3.md` v0.14 (constructor doc cross-references). |
| ERR-030-046 | Season & Competition Loop #30 §3.4 (the depleted-squad back-fill's tier-2 within-tier rule), found at **a third adversarial-review pass over the same rule** and **ESCALATED** under the no-third-identical-retry rule: **the SAME defect has now survived two successive fixes**, so the third attempt was ruled rather than iterated. `ERR-030-044` keyed tier 2 on earliest roster position with a probe; `ERR-030-045` re-keyed its blind fallback to ascending `SquadRating.PlayerRating`. **That rating is a GLOBAL SCALAR and `LineupSelector` selects PER POSITION.** Where the fit squad is thin in the globally *weakest* banned player's position — five fit midfielders rated 5–7 against a banned midfielder rated 8, the other banned players defenders whom five fit defenders outrank — the blind pass presses exactly him back, into a slot no fit player can contest; he **starts**; `AnyReinstatedSuspendedStarts` then sees a reinstated starter in every later candidate's eleven, so pass 1 is unsatisfiable for the final pick and pass 2 takes it in roster order. Net: a club with a clean completing choice fields a banned man and `ERR-044-003` stage 1's exemption stalls his ban. **Measured on generated mass-suspension fixtures: ≥ 476 of 1920 had a clean completing choice the algorithm missed.** **ROOT-CAUSE CLASS, and the reason a third ordering key was refused: an element-wise greedy decision of a SET-VALUED constraint.** A depleted club is completed by a *set* of reinstatements; whether that set puts a banned player in the eleven is a property *of the set*; and the property is decided per position. No per-player scalar — roster order, rating, or any successor — can express it, which is why two keys in two days produced the same outcome by different routes. | High | 1 | ✅ Resolved August 16, 2026, later still, spec + code same commit. **The rule becomes a CAPPED EXHAUSTIVE CLEAN-COMPLETION SEARCH** at `AvailabilityComposition.ChooseSuspendedCandidate`, which keeps its signature, its call site, its single-`int` return and its purity. Still-removed candidates are ordered ascending by `SquadRating.PlayerRating`, ties on earliest roster position — a **total** order that now decides nothing on its own and exists only to make the enumeration canonical. Let `m` be the candidate count. **Cap:** `m > [FIXED] SeasonSaveConstants.EXTREMIS_SEARCH_CANDIDATE_CAP` (**12**, new — a `2^13` probe bound, an *algorithmic budget* explicitly NOT a `[GT]` and NOT subject to KD-W1) ⇒ commit the weakest candidate with **no** cleanliness claim; self-healing, since each pass commits one candidate and the exact search resumes as `m` falls. **Search:** enumerate every subset `R`, sizes ascending and lexicographic within a size; materialise `F ∪ R` (the tentative mask flip generalised from one index to a subset, restored before continuing) and probe `SquadRating.CanFieldStartingEleven`; for a completing subset compute `dirty(R)` = how many of `StartingElevenPlayerIds` belong to a reinstated-suspended player (`suspended[i]` and either `!removed[i]` or `i ∈ R`) — `AnyReinstatedSuspendedStarts` generalised from a predicate to the subset-parameterised **count** `CountReinstatedSuspendedInXi`, because minimising requires telling "starts one" from "starts three". **Preference:** min `dirty`, then the smallest subset, then first canonically; the first `dirty == 0` subset is globally optimal and stops the search. **Commit rule** (which single member of `R*` is returned): a one-member `R*` ⇒ that member; a larger `R*` in which some member's singleton was probed NOT fieldable ⇒ the canonical-first such member (the outer loop cannot exit this pass, so the search reruns next pass); a larger `R*` in which every member singly completes ⇒ re-choose among the size-1 completing sets by the same preference (deterministic; the exit XI is one just probed). **GUARANTEE, in theorem form, because two weaker claims were falsified within two days:** with `m` inside the cap the composed squad fields the **minimum achievable** number of reinstated-suspended players in its starting eleven, minimised over every completing subset of the still-removed candidates — **zero whenever any completing choice benches them all** — so a reinstated-suspended player starts only in a **probe-verified forced start**. Induction step: committing a non-completing member `c` of `R*` cannot raise the optimum, because `R* \ {c}` remains available and `F ∪ {c} ∪ (R* \ {c})` is byte-identical to the probed `F ∪ R*` — the count reaches `c` through `suspended[c] && !removed[c]` exactly as through `c ∈ R`. Termination unchanged: `availableCount` strictly increases per pass, §2.3 **F9** at the bound, no repair loop. **RESIDUAL, stated exhaustively and deliberately NOT as an enumeration of routes into a forced start — that FORM has now been falsified twice:** **(i)** forced starts, where the minimum is positive (sole-GK forcings and their `k ≥ 2` generalisations), and there the stage-1 exemption stalls exactly the fielded bans and no fewer; **(ii)** the beyond-cap corner (> 12 concurrent suspended candidates, unreachable at measured card rates), degrading to ascending-rank greedy with no minimality claim, self-healing as commits reduce `m`. **`ERR-030-044`'s two-case and `ERR-030-045`'s three-case guarantee statements are SUPERSEDED**, and the claims they rested on ("the final pick's pass 1 still has a benchable candidate whenever one exists"; the two-routes-into-forced-start remark) are **deleted** rather than re-worded. Everything else in `Compose` is untouched: the fresh-mask contract, tier-1 precedence, the one-reinstatement-per-pass outer loop, the F9 terminal throw, FR-DC-018's identity fast paths. **Cost:** worst case `m·2^(m+1)` `TrySelect` walks = **98,304** at the cap; reachable case `m ≤ 4` ⇒ **≤ 128** walks, single-digit ms; extremis-only, season cadence. No RNG, no draw site, no `SNAPSHOT_SCHEMA_VERSION` / `SEASON_SAVE_FORMAT_VERSION` movement, **no change to `SquadRating.cs` / `LineupSelector.cs` / `MatchEngine.cs`**; digest invariance still not claimed for a season whose extremis tier fires. Code: `src/season-save/AvailabilityComposition.cs` v1.7, `src/season-save/SeasonSaveConstants.cs` v1.9. Spec: `season-competition-loop/section-3.md` v2.9. **Back-props:** #44 `ERR-044-019` **EXTENDED, not re-filed** (`discipline-suspensions/section-2.md` v0.17, `section-7.md` v0.10) — the compromise's statement becomes "a reinstated-suspended player starts only in a probe-verified forced start; every completing choice within the search bound starts at least as many", with the residual restated as the two-item list above. Locked by `AvailabilityCompositionExtremisTests` (5 → 8 cases): **`WeakPositionExtremis`**, the mandatory mutant-killer, **observed FAILING at the pre-fix tree** (`Expected: not some item equal to 16 / But was: < 0, 2, 3, 4, 5, 16, 7, 8, 9, 12, 13 >` — the banned midfielder in the composed XI) and green after; **`MinimalStallExtremis`**, the minimisation half (no clean set exists; XI dirty count asserted **exactly 1**, the benchable second banned player benched, and through a really played `AdvanceAndPlayNextRound` the forced keeper's ban stalls while the benched one's decrements); and **`CapFallbackExtremis`** (13 candidates — termination, fieldability and repeat-call determinism only, since the beyond-cap branch makes no minimality claim). The five existing cases keep every assertion and needed only their pass-structure prose updated. **ANNOTATED August 16, 2026, even later still — a round-4 reviewed-findings pass closed six further findings against this same landing, all Medium/Low, all fixed, none reopening the resolution above.** The commit rule's third branch (`\|R*\| ≥ 2` with every member singly completing) was unreachable, unproven, and would have violated the guarantee above if it were ever live; a **monotonicity lemma** is now stated — `LineupSelector`'s per-position top-`k` selection makes `dirty(R)` monotone non-decreasing under adding candidates, so a completing singleton is already the global minimum and a `\|R*\| ≥ 2` winner whose every member singly completes cannot arise (verified: `thirdBranchReachable = 0` over 6,858 oracle cases; the collapsed `order[bestSubset[0]]` form is behaviour-identical over 3,966 further cases and 11/11 tests) — and the third branch is now a fail-loud `InvalidOperationException` naming the lemma, its re-choice machinery deleted rather than kept dead. A **fourth Commit case** is now bound for "no subset completes at all" (⇒ commit the weakest candidate; the outer loop's own `availableCount == total` guard reports §2.3 F9), with a hoisted full-candidate-set probe (same monotonicity premise) skipping the `m·2^m` enumeration on that branch. Both "self-heals" residual statements (the Cap bullet and residual (ii)) conflated the SEARCH resuming beyond the cap with the GUARANTEE resuming; corrected here and at `AvailabilityComposition.cs` (two sites), `SeasonSaveConstants.cs`, and the mirrored residual at `discipline-suspensions/section-2.md` v0.18 / `section-7.md` v0.11. Code: `src/season-save/AvailabilityComposition.cs` v1.8, `SeasonSaveConstants.cs` v1.10, `src/match-engine/SquadRating.cs` v1.6 (doc only — the falsified ERR-030-045 rationale in its `PlayerRating` doc annotated, not deleted). Spec: `section-3.md` v2.10. Tests: `AvailabilityCompositionExtremisTests.cs` v1.3, +2 mutant-killers (`InCapBoundaryExtremis` at `m == CAP`, `TiedForcedStartExtremis` at a tied positive dirty count), each verified by actually applying its named mutant and observing the failure before reverting. **ANNOTATED August 16, 2026, round 5 — a round-5 reviewed-findings pass over the round-4 annotation above found its own "two sites" count wrong (M1).** The round-4 diff corrected THREE sites in `AvailabilityComposition.cs` (~152, ~421, and the inline comment inside `ChooseSuspendedCandidate`'s Cap branch at ~528), not two, and missed a NINTH site entirely: `AvailabilityCompositionExtremisTests.cs`'s own `CapFallbackExtremis` comment still carried the pre-fix "it self-heals … the exact search resumes for every later pass" sentence, in a file that same round-4 commit had otherwise edited. The real corrected-site total across the whole sweep is **nine** — #30 `section-3.md` §3.4's Cap bullet and residual (ii); the three `AvailabilityComposition.cs` sites; `SeasonSaveConstants.cs`; #44 `section-2.md`; `section-7.md`; and this test comment. Fixed at the test file (`AvailabilityCompositionExtremisTests.cs` v1.4) and annotated in place, not rewritten, at every other record of the undercount: this row, `section-3.md` v2.11, `section-2.md` v0.19, `section-7.md` v0.12, `AvailabilityComposition.cs` v1.9, `SeasonSaveConstants.cs` v1.11. No behaviour, guarantee, or normative rule changed by the correction — it is a count, not a claim about the algorithm. |
...
(owning-catalogue mirror routing per the ERR-020-004 carve-out — **CORRECTED 2026-08-15: this row originally read "discipline is the only consumer", which is false; `MatchEngineConstants` and `MatchAnalyticsConstants` mirror the same encoding, so there are three. The routing is right for a different reason — #17 owns the encoding and has its own catalogue — and §4.2 had no branch saying so until ERR-020-004 added one.** It mirrors #17's catalogue
...
from the 31 folders it was drafted against to the 35 now in `src/`. See "Adoption" at the end of this
entry for what changed, what was verified at adoption, and what was deliberately left alone.
...
infrastructure. `src/` now holds **31 assembly folders**. Twelve are placed nowhere: *[Annotation, August 17, 2026 (reviewed-findings pass): 14 + 12 ≠ 31 — see the heading annotation. The box named **14**, so **17** were unplaced, not 12; the five this list omits are `project-constants`, `deterministic-sim`, `event-system`, `performance-optimization` and `testing-strategy`, which §3.5.2 never named and which this entry silently treated as placed.]*
...
### Adoption — August 17, 2026 (owner decision)

The proposal is adopted. Recorded here rather than in the proposal text above, so the
draft stays readable as what was offered and this section carries what was actually done.

**Extended before adopting.** The proposal covered 31 folders; `src/` now holds **35**. Four
assemblies landed after it was drafted and are placed with it, from their `.asmdef` references,
not their names:

| Assembly | Tier | Why |
|---|---|---|
| `training-system` (#29) | 7 Management | References `player-progression` (tier 7) and `player-database` (tier 5). Long-horizon career state above a match; the reference to a Management sibling is intra-tier. |
| `injuries-medical` (#41) | 7 Management | References `training-system` — intra-tier, acyclic, and the direction #41 T1 established. |
| `discipline` (#44) | 7 Management | Suspensions carry across fixtures, so it is Management by nature. Its highest reference is `player-database` (tier 5), so tier 6 would also have been legal; Management is the honest description and nothing references it from below. |
| `client-app` | 9 Client | References `ui-framework` only — intra-tier, and the reason the assembly exists (FR-UI-010 forbids the framework hard-coding screens). |
...
**Two rules added that the proposal did not contain**, both surfaced by the four new placements:

1. **A tier is a ceiling, not a licence.** `discipline` sits in Management, but #44's own FRs forbid
   it to reference `match-engine` or `season-save` — the composition root mediates. Without this
   sentence the adopted order would read as *widening* a constraint #44 deliberately took on. Where a
   spec is stricter than its tier, the spec wins.
2. **Test assemblies are not members of the order.** `event-system.Tests` references `decision-tree`
   — tier 0 referencing tier 4, upward and entirely legitimate. FR-CS-046 binds production assemblies
   only, and the check above excludes `src/*/[Tt]ests/` for that reason. This was true before adoption
   and unstated; an adopted order that did not say so would have made every test assembly a violation.

**Files changed:**

| File | Change |
|---|---|
| `docs/specs/code-standards/section-2.md` v1.0.1 → **v1.1** | FR-CS-046 restated against the tier order and its published double negative repaired ("No assembly **MUST NOT** reference…"); **FR-CS-046a** registered as a sub-numbered clause, so the FR-CS-046…055 span and the 73-FR count are unchanged |
| `docs/specs/code-standards/section-3.md` v1.0.1 → **v1.1** | §3.5.2 replaced: ten-tier table, labelled arrows, FR-CS-046a, the ceiling rule, the test exclusion, the verification record |
...
history, against a SPEC_INDEX status of APPROVED (May 11, 2026) — the FR-CS-056/057 class.

**Deliberately not changed, so a later sweep does not have to rediscover the list.**
*(This paragraph was itself wrong when first published on August 17, 2026, and is corrected here the
same day: it named three outline files, one of which — `outline.md` — carries no layer chain in any
form; and it claimed the two greps hit nowhere else in `docs/specs/`, which the very commit
publishing the claim had already falsified. The actual output, re-run rather than recalled:)*

```
$ git grep -n 'Physics → Mechanics → AI → UI' -- docs/specs
docs/specs/code-standards/outline-detailed.md:235
docs/specs/code-standards/outline-detailed.md:397
docs/specs/code-standards/outline-mid.md:228
docs/specs/code-standards/section-2.md:327

$ git grep -n 'Physics ──► Mechanics' -- docs/specs
(no hits)
```

`outline-mid.md` and `outline-detailed.md` are pre-authoring artifacts describing what was to be
written, not normative text, and this project's convention is that the section files carry the
contract — so they stay. `section-2.md:327` is a **version-history row quoting the retired chain** to
say it was retired; it is intentionally retained and is not a live rule. Pattern B returns nothing at
all, because the arrow form only ever existed in §3.5.2 itself, which this adoption replaced.

**A second correction, about the sweep's METHOD rather than its output.** Grepping for the arrow
strings can only find renderings that spell the chain that way. It could not, and did not, find
`README.md`'s assembly tree, which grouped the same taxonomy under bare `Physics:` / `Mechanics:` /
`AI:` labels, claimed 34 assemblies, listed 30 and omitted five — the project's front-door document,
missed by a sweep that reported itself complete. Renderings outside `docs/specs/` must be swept by
**enumerating assembly names** against `src/`, not by matching the arrow prose. `README.md` was
corrected on August 17, 2026, and `tools/assembly-tier-check.py` now makes the enumeration
mechanical for the authority table itself.
...
proposed resolution was taken as written: #20 §3.5.2 now labels its arrow (`──►` reads "is available
to") and restates the rule in the root `CLAUDE.md`'s own words beneath the diagram; `src/CLAUDE.md`'s
`### Reference Direction` labels its own `←` and names the other convention explicitly, so a reader
checks the label rather than the arrowhead. The diagram was not reversed or renumbered, per the
proposal.

*(Corrected August 17, 2026, same day: this resolution first claimed "all **three** renderings agree
in vocabulary". Two did. The third — root `CLAUDE.md`'s one-line `AI → Mechanics → Physics, never the
reverse` — was unlabelled, and root `CLAUDE.md` was absent from this entry's Files Affected table
below, so the claim covered a file the fix had never been scoped to. That rendering now carries its
label too — an arrow reading "may reference", pointing consumer→provider — and the table names the
file. The unlabelled one was the rendering in the document every agent is ordered to read first and
completely, which is why this was worth correcting rather than narrowing.)*
...
| `CLAUDE.md` (root) | the reference-direction rule in the rules list | Label its arrow ("may reference", consumer→provider) so the third rendering states the convention it uses |
