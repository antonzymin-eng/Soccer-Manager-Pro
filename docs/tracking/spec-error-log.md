...
a tier is a ceiling, not a licence, and test assemblies are not members of the order.)
...
`outline.md`, `outline-mid.md` and `outline-detailed.md` in the same spec folder still describe the
three-layer chain. They are pre-authoring artifacts describing what was to be written, not normative
text, and this project's convention is that the section files carry the contract. Every *normative*
site was swept: `git grep -n 'Physics → Mechanics → AI → UI'` and `'Physics ──► Mechanics'` return
hits in those three outline files and nowhere else in `docs/specs/`.
...
`### Reference Direction` labels its own `←` and names the other convention explicitly, so all three
renderings agree in vocabulary and a reader checks the label rather than the arrowhead. The diagram
was not reversed or renumbered, per the proposal.
