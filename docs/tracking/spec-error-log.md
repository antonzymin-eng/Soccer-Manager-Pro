...
| ERR-008-021 | Decision Tree #8 §3.1.4.3 / §3.2.3.2 — the shot-lane occlusion check was attribute-blind (the follow-up deliberately deferred at the ERR-008-020 landing): every blocker in the shot path occluded the same geometric arc regardless of who he is, so a Pace/Anticipation 1/1 defender walled off the goal exactly as hard as a 20/20 one in the identical spot, and no shooter attribute entered the read. Pattern (a) — continuous in position (no P1 cliff), blind to identity. | Moderate | 5 | ✅ **Resolved August 6, 2026** (spec §3.1.4.3 + §3.2.3.2 step 3a + `OptionGenerator.cs`, same commit; doctrine P2/P3/P5; **AR-1 same day: 1 High, 7 Medium, 5 Low — all fixed**) — each blocker's occluded arc scales by §3.1.3.3's `perceived_ability` (Anticipation/Pace → `INTERCEPTOR_ABILITY_MIN..MAX`, read through the **shooter's** Vision fidelity); **no new constants** — the ERR-008-020 `[GT]`s reused verbatim (one calibration lever, KD-W1). Exactly ONE opponent — the **single GK candidate**, goal-line-nearest within `GK_PROXIMITY_TO_GOAL` (**AR-1 H-1**: the landed form exempted the whole 6 m band, leaving every near-goal defender unweighted precisely where shots are blocked) — stays purely geometric: the keeper's quality is priced once, at the #11 save (P3). Ability = 1.0 exactly at the midpoint (`mean01` 0.5, raw 10/11) or under a null view; the all-default 10/10 profile reads ≈ 0.979 (**AR-1 M-2** corrected the original "today's arcs exactly" overclaim). 7 `OptionGeneratorTests` locks incl. the away mirror, the GK-candidate exemption, and the in-band-defender-is-weighted lock. Adjacent defects recorded: §3.2.3.2's numerical example is in a legacy centre-origin frame with a blocker inside the GK heuristic priced at the outfield radius (annotated in-place); the Known-limitation paragraph's inverted radius consequence corrected (AR-1 M-7). §3.1.4.1 gate (4) boundary aligned to the spec's strict ">" (AR-1 L-5). **Gate NOT run — no .NET SDK in the authoring environment; CI on push is the gate.** |
