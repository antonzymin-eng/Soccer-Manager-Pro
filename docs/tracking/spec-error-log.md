...
| ERR-018-001 | Performance Optimization #18 `outline-detailed.md` cites Deterministic Simulation #16 sections by stale numbers / non-existent name (`#16 §7 regression scenarios`, `#16 §5 canonical save format`, `#16 §8 trace channels`) | Medium | 1 | Open — filed May 13, 2026 with #18 `outline-detailed.md` v1.0. Same drift pattern #19 v0.2 corrected May 12, 2026: #16 regression suite actually at §5; #16 canonical schema actually at §3.2.4.1; **no "§8 trace channels" section exists in #16** (§8 is "References & Citation Audit"). Outline-level inaccuracy is bounded by #18 KD-3 `TBD-NORMATIVE` tag policy which already mandates `grep` of `deterministic-sim/section-*.md` at section-file draft time. Resolution at section-file authoring: (a) re-cite §5 / §3.2.4.1 with verified subsection numbers; (b) for "trace channels", determine whether #16 will publish a trace-channel section, whether KD-3 needs re-anchoring to a different #16 section, or whether Spec #18 owns the trace pipeline from scratch (in which case the KD-3 boundary statement must be inverted). |
