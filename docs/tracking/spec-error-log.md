...
The gate is not simply removed — it has a real job (stopping teammates converging on a ball their own
carrier is standing over, since a carried ball is also slow). It is re-expressed as *a slow ball is
intercept-eligible only while LOOSE*, and the loose case is routed to a dedicated collect that skips the
look-ahead geometry (meaningless at v ≈ 0: every projected point is the ball's own position) and carries
feasibility 1.0, because for a stationary ball being the designated player IS the feasibility.
...
**Resolution.** `decision-tree/section-3-1.md` (§3.1.9 loose-ball collect) and `section-3-7.md` (§3.7.2
...
*End of Spec Error Log v1.45 — July 26, 2026.*
