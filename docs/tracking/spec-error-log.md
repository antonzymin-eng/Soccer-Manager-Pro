...
**Version:** 1.17
**Updated:** May 17, 2026 (ERR-013-001 through ERR-013-005 filed from Pressing AI #13 section-files v0.1 draft — back-prop tracking only; consuming specs not yet patched)
...
| ERR-013-001 | Pressing AI #13 requires a back-prop into Decision Tree #8 §3.1.8.2 (or §2.2.6) to read #13's per-agent `PressAssignment` during PRESS utility scoring. Two candidate mechanisms preserved at section-file draft (OI-001 / KD-3): Option A read-only accessor `PressingAI.GetAssignment(EntityId)`; Option B `TacticalContext.PressDirective` field extension via #8 §2.2.6 amendment. Section-file draft does NOT pre-decide — non-binding recommendation toward Option B (cleaner #8 hot path; aligns with #12 freeze-then-amend pattern; #13's Stage-1 binding does not violate the Stage-0 freeze argument). | Medium | 2 | Open — filed May 17, 2026 from #13 section-files v0.1 / `outline-detailed.md` v1.0. Gates #13 §9 lead-developer sign-off and #13 Stage-1 runtime activation (FR-PR-044). Consuming spec (#8) NOT yet patched. |
