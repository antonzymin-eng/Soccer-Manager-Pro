...
**Version:** 1.18
**Updated:** May 17, 2026 (ERR-013-007 and ERR-013-008 filed from Pressing AI #13 section-files v0.2 PASS-1 fix pass — #12 `GetPhase`/`GetLine` Stage 1 accessor back-prop requests; ERR-013-001 through ERR-013-005 previously filed from v0.1 draft)
...
| ERR-013-001 | Pressing AI #13 requires a back-prop into Decision Tree #8 §3.1.8.2 (or §2.2.6) to read #13's per-agent `PressAssignment` during PRESS utility scoring. | Medium | 2 | Open — filed May 17, 2026 from #13 section-files v0.1. Gates #13 §9 lead-developer sign-off and #13 Stage-1 runtime activation (FR-PR-044). Consuming spec (#8) NOT yet patched. |
...
| ERR-013-004 | Stale "Fatigue System #13" reference at `decision-tree/section-3-1.md` L753 — but #13 is **Pressing AI**, not the Fatigue System (which is a separate Stage-1 spec, not yet allocated a number). One-token patch request: "Fatigue System #13" → "Fatigue System (Stage 1; spec number TBA)" or equivalent. Verified present at section-file draft via `grep -n "Fatigue System #13" docs/specs/decision-tree/section-3-1.md`. | Minor | 1 | Open — filed May 17, 2026 from #13 section-files v0.1. Consuming spec (#8) NOT yet patched. |
| ERR-013-005 | `DOMAIN_TAG_PRESSING_AI = 0x19` allocation needed in Deterministic Simulation #16 §3.4 domain-tag registry. Inherits the ERR-012-001 Phase B/C block proposal (originally `0x16…0x1B`, shifted to `0x17…0x1C` on May 16, 2026 after #10 took `0x16` via ERR-010-001). Current ordering: #11 → `0x17` (or `0x18` depending on first-to-APPROVED), #12 → `0x17`/`0x18`, #13 → `0x19`. Whichever spec in the block reaches `APPROVED` first claims the next-available slot. | Medium | 1 | Open — filed May 17, 2026 from #13 section-files v0.1. Gates `[CROSS-PENDING] → [CROSS]` promotion in #13 §6.1 / FR-PR-005. Tracked alongside ERR-012-001. |
| ERR-013-007 | Pressing AI #13 requires `GetPhase(TeamId)` to be published as a Stage 1 accessible on Positioning AI #12. Currently #12 §4.4.3 declares `GetFormationSlot` as the sole Stage 0 accessor; `GetPhase` is internal-only (not in the published accessor list). #13 §3.11 / §4.4.3 KD-11 phase gate reads `GetPhase` to suppress pressing when the team is in possession. Back-prop into #12 §4 (or §4.5 Stage 1+ extension table) to publish `GetPhase(TeamId)` at Stage 1. | Medium | 2 | Open — filed May 17, 2026 from #13 section-files v0.2 PASS-1 fix pass (AR-S1-H4). Non-blocking for Stage 0 spec text; gates #13 Stage-1 runtime activation. Consuming spec (#12) NOT yet patched. |
| ERR-013-008 | Pressing AI #13 requires `GetLine(EntityId)` to be published as a Stage 1 accessor on Positioning AI #12. Currently #12 §4.5.1 declares `GetLine` as Stage 1+ only (NOT exposed at Stage 0 per CLAUDE.md "Interface Design Principle"). #13 §3.9 invariant (2) KD-16 backline floor reads line membership. Back-prop into #12 §4 to promote `GetLine` to Stage 1 availability (aligned with #13's own Stage 1 runtime). | Medium | 2 | Open — filed May 17, 2026 from #13 section-files v0.2 PASS-1 fix pass (AR-S1-H4). Non-blocking for Stage 0 spec text; gates #13 Stage-1 runtime activation (KD-16 invariant enforcement). Consuming spec (#12) NOT yet patched. |
