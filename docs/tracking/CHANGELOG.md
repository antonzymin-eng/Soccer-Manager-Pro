...
> **Last Updated:** September 26, 2026 — **W8 ERR-011-017 clock correction + #11 §3.8.3 hand-episode teardown (owner-directed separate pre-B landing).**
>
> `DriveGkHeadingTactical` now passes `_clock.CurrentTacticalTick` to #11 `TacticalTick`. It passed the raw 60 Hz frame, so after frame 72 the 60-tick hold limit, `releaseTickEarliest` and the recovery cooldown all matured on the first tactical pass after any claim. Correcting the clock alone would have regressed keepers: `HandsOnBall` has no exit but `Distributing`, and the broken clock was what cleaned up after every keeper pass — with a true 10 Hz tick a keeper who had already passed would stay in hands for up to 6 s, unable to rush, claim or dive (all three commit only from `Set`/`Anticipate`). The approved #11 §3.8.3 possession-change teardown therefore lands with the clock fix, ahead of the rest of B: `SetPossessingAgent` ends the outgoing keeper's live hand episode on a real change of holder, and `ApplyRestart` ends any live hand episode even when the same keeper is awarded the restart. The keeper enters `Recovering` with the ordinary cooldown. Every loss is an ordinary one in this landing; B adds the typed cause for its own distribution CONTACT. The B producer, #5 goalkeeper request, Decision Tree hand-episode suppression and B schema bump are **not** in this landing, so ERR-011-015/016 remain code-pending.
>
> Determinism: intentional trajectory/digest change against the frozen Stage A corpus. No `SNAPSHOT_SCHEMA_VERSION` change — every #11 field written was already serialized and already stored in the tactical-tick domain; no new RNG stream, domain tag, draw site or draw-order change; no `[GT]` change. Locked by `GoalkeeperClockCorrectionTests` (17 cases: 16 home/away-mirrored plus one stride invariant): its first-tactical-pass assertion fails against the pre-fix call, and the loss cases prove a live, non-default distribution intent is cleared from both `HandsOnBall` and `Distributing`. **Not yet measured:** the same-six-seed comparison against Stage A with the W8 instrument has not run; it is required before merge and must report the keeper self-reclaim loop separately from the clock effect.
>
> Blast radius: composed scenarios with keeper-sensitive predicates may move, because a keeper's #11 hand state now lasts until the ball leaves him. **The recovery cooldown is NOT made effective by this landing**: its arithmetic is now in tactical ticks, but the composition root feeds `UpdateBaselineSlot` the keeper's own position, so `Recovering → Set`'s at-baseline test is always true and the keeper returns to `Set` on the first tactical pass exactly as before. That pre-existing defect (first recorded in `gk-rush-trigger-design.md` §7 item 5, never filed) is now an `open-issues.md` entry; active 31 → 32. The cooldown contract itself is locked at #11 unit level instead: new `GoalkeeperMechanicsTests` T-5.1.1-I2 proves an off-baseline keeper stays `Recovering` before the boundary and returns to `Set` at it. Checked: no ordinary test pins a gameplay digest, and the rush/claim unit tests drive at frame 0, where both clocks agree. Gate: **dotnet gate not runnable in this environment** (SDK download blocked by the network policy); compilation and tests are left to the PR functional gate and are not claimed here.
>
> **Last Updated (prior):** September 26, 2026 — **W8 B #461 owner approval recorded; wiring authorized after merge.**
>
...
>
> The separate #462 follow-up is merged as `a4056ea6371f6dcf72dcaa6ef51980f7bbeaabc7`: `PossessionChangeSeamTests` now rejects `ldflda` address-taking of `_possessingAgentId` in addition to unlisted direct `stfld` stores; `MatchEngine.cs` v1.90 records #460's previously omitted helper history; and `CHANGELOG-src.md` v2.149 records #458/#460 plus the evidence boundary that 6/6 digest equality proves only the assignment-only seam. Reflection/unsafe mutation remains explicitly outside that IL guard. No B hand-episode teardown is implemented here.
>
> Tracking catch-up records the previously omitted #458/#459/#460 W8 Stage A/helper files and the canonical measurement-catalog modification in `file-manifest.md`. This closeout deliberately does **not** land #461's pending B policy/spec choices; those remain in the separate draft/owner-approval gate. No gameplay, approved spec, snapshot schema, RNG draw/order, calibration result or gameplay `[GT]` changes.
>
> **Last Updated (prior):** September 26, 2026 — **W8 Stage A / pre-B review correction before B wiring.**
>
> Review of merged #459/#460 verified the frozen A run and helper equivalence but exposed two evidence limits and three tracking/structural omissions. The Stage A record now names the claim→pass→claim loop as a known A→B confound (862 hand claims; 501/861 hand-origin passes interrupted by a later hand claim; 13/861 committed receivers controlled; 32.086-frame mean hold). Supplemental pinned-tree run `36251412673` closes the exact 501-outcome split at **501 same-keeper / 0 opponent-keeper**, reproducing all six frozen terminal digests; artifact `10909441933`, ZIP SHA-256 `0e076292accbb43eb3b5bda7a0b27b9df37447764d9778f19055e9a707ad1ed6`. It also records that all observed A policies were `SlowDown`: the frozen six-seed A→B rerun is therefore SlowDown evidence only, and B must prove all six #21 policies in deterministic composed fixtures. The stale helper-next-step text and missing Created date are corrected.
>
> #461's B contract is tightened before review: the initial distance-blind `lerp(vMin,vMax,power)` speed is replaced by #5's existing distance-sensitive distance/power shape and the exact Roll/Throw/Kick launch rules are pinned with worked examples; `TargetPosition` becomes the commit-time fallback while a valid receiver is re-aimed at his live CONTACT-frame position; the error hash reserves fixed discriminator `0x47`; Rejected/Cancelled/Completed feedback distinguishes retained possession from actual loss; `Distributing` means an accepted #5 request only; fallback geometry derives through own-goal/attack-direction helpers; and the inherited-guard proof is explicit as `210 + 84 + 1 = 295 < 355 < 360`. ERR-011-017's mechanism is corrected from 'roughly six times too fast' to the actual first-tactical-pass maturation after frame 72. #21 gives each new numeric exactly one source tag/range and records the 35 m LongKick fallback as an explicit owner-approval realism choice. #5/#11 remain DRAFT, so merge is not treated as approval. The SlowDown-only corpus coverage gap remains a normative all-six-policy B test obligation. ERR-011-015/016/017 remain spec-first/code-pending.
>
> Tracking catch-up registers the previously omitted #458/#459/#460 Stage A/helper files in `file-manifest.md` and advances its two maintained spec-error-log pointers v2.65 → v2.66. A separate behavior-neutral follow-up (#462) hardens the possession writer guard against `ldflda` address-taking and records the omitted `MatchEngine.cs` history; B production wiring remains blocked on the spec/evidence gates. No B production code, snapshot-schema bump, RNG draw or calibration result is introduced here.
>
>
> **Last Updated (prior):** September 25, 2026 — **W8 goalkeeper-distribution architecture recorded; staged work authorized.**
>
> Draft PR #456 adds `docs/tracking/w8-goalkeeper-distribution-decision.md` at production anchor `c50726e6…`. Investigation finds four contracts that must be decided before preregistration: (1) #11's hand-claim clock versus Match Engine's broader keeper-possession stall guard; (2) the producer/RNG contract from #21 `GkDistributionPolicy`, whose approved text still lacks concrete defaults, receiver selection and voluntary-release timing while a DT action would consume W9's ordinal-8 boundary; (3) FR-GK-043's missing forced ROLL/event plus its undefined empty-penalty-area fallback and the live `agentRosterContains: true` F-05 stub; and (4) the missing executor — current #11 code publishes `DistributionExecutedEvent` but never initiates Pass Mechanics, releases possession through the canonical seam, arms W5's pass-in-flight feed or reaches `Ball.ApplyKick`; approved #11 §3.8.3–§3.8.4 actually names a phantom #5 contract (`PassIntent`, `ConsumePassIntent`, `PassMechanics.DeliveryKind`, `LowDriven`, `GroundRoll`), while live `PassRequest` cannot carry #11's source-point/power/spin/delivery payload without semantic loss; the event type's comment also falsely claims `Ball.ApplyKick` already precedes publication. The packet also records that current IFAB Law 12 (2026/27) retains the eight-second opponent-corner sanction introduced in 2025/26, whereas the pre-2025/26 six-second law awarded an opponent indirect free kick; the repository's forced ROLL is its own policy, not either law's sanction. Owner review must name the edition or an explicit house rule before timeout code. It records producer-neutral candidate measurements and falsifier classes, RNG/draw-order/source-tag obligations and an instrument-first A→B→C sequence: distribution under the unchanged ground-drop guard is measured before a separate current-law/offence-specific corner landing. The inherited keeper-at-feet guard remains a non-law safeguard pending measurement; the proposed faithful #5 extension and Match Engine adapter remain owner decisions. v0.8 verifies Law 17 §17.1's keeper-position placement, makes actual CONTACT (not commit) the release deadline, budgets B's single #11-derived windup before the old guard, and requires C's exact serialized 60 Hz claim frame with pre-CONTACT >8 s sanction. Existing #11 hand/distribution states distinguish the feet-only guard. Near-zero B→C corpus movement is expected; forced boundary fixtures prove the rare sanction. v0.9 also isolates #11's rounded 10 Hz no-intent timeout from the later engine guard, budgets B against both, and locates C's hand-offence check before the C3 pass executors while leaving the feet safeguard after first touch. Its decision summary is advisory; accepted rules must enter owning approved specs with matching code. v0.10 proposes a total receiver-or-fixed-zone selector for every live hand claim under each #21 policy, including no eligible receiver and F-05 disappearance; receiverless #5 CONTACT must launch once without a ghost W5 receiver latch. B/C no-receiver fixtures guard against artificial C corners, and OD-W8-3 has no separate fallback after OD-W8-2 owns it. v0.11 defines the proposed F-05 safety exception when F-09 clamps a point to the keeper's own goal line, marks the old no-intent clock fixture injected/defensive, and separates receiverless-zone pass outcomes from receiver-targeted completion denominators before A→B. v0.12 incorporates the native integrity/evidence council: keeper Decision Tree on-ball competition, the live-controlled-possession predicate and #11 hand-episode/intent teardown, serialized B windup/cancel state with its own schema bump, and architecture-only owner approval with approved-spec amendments paired to code. It records the Resolve-event/Tier-A, #5-owned type and single windup/error obligations; A measures hold exits and selector fallback, while B→C uses exact gameplay-stream non-regression and forced law fixtures. `open-issues.md` now explicitly records the unresolved FR-GK-043 forced-ROLL code/spec divergence without assigning an ERR id. v0.13 records the owner sequence: merge #456; merge A preregistration and nonserialized instrument before inspecting six-seed outputs; isolate a six-writer behavior-neutral possession helper with inventory/guard test and exact digest parity; put actual policy delays, delivery ranges, tie-break and punt-zone geometry in #11/#5/#21/Match Engine specs and file #11 ERRs before B code; then wire B and compare the same corpus. Startup, restore and test-only writers stay direct. Review on the ready PR exposed an additional live #11 clock-unit mismatch (60 Hz caller argument against 10 Hz `_claimTick`): A records it unchanged, B spec/code correct it, and the earlier five-frame-skew assertion is withdrawn. C limits the eight-second corner to hand control inside the keeper's own penalty area and specifies outside-area handling separately. C remains separate. No production source, approved spec, schema, digest, RNG behavior, gameplay `[GT]`, calibration result or ERR change.
>
> **Last Updated (prior):** September 25, 2026 — **PR #455 hardens the PR #439 evidence gate for durable post-delete operation.**
>
...
>
> **Last Updated (prior):** September 24, 2026 — **#441 header-reachability census and W3 artifact member hashes carried into `pr439-w3/` from closed #446; evidence only.**
>
> Branch `archive/pr439-evidence-20260923` (closed #446, superseded by #447) held two evidence files with no copy on `main`. Both now land byte-for-byte in `docs/tracking/evidence/pr439-w3/`. `header-reachability-441-results.tsv` is the per-seed #441 funnel from run `35887487201`, which uploaded no artifact. All 16 counters for all six seeds, and the aggregate, were checked against the six live job logs on September 24, 2026; the source lines are kept in `header-reachability-441-run-log-lines.transcribed.txt`. The aggregate (1,811 commits → 18 ever-predicted → 0 prepared → 0 executed; 1,793 `PositionedPoorly`, 16 early) matches the W3 design record. `authoritative-artifact-member-hashes.tsv` records per-file SHA-256 inside the artifacts of runs `35925236129` and `35814050060`; its zip digests match the Actions API and its TSV hash matches `final-w3-results.tsv`, but its other member hashes were not re-derived (artifact download blocked in the archiving environment) and it does not cover the pre-W3 run `35815761065`. `pr439-w3/README.md` and `SHA256SUMS` are updated. `run-heads.tsv`, `pr439-ref-archive/history/`, the 15-run PR #439 citation ledger and all 16 `evidence/pr439-*` refs are untouched; no deletion is authorized. No code, test, spec, schema or RNG change. Tooling unit suite, evidence integrity contracts, the PR #439 archive verifier (`--mode all`, without its GitHub-API citation step, which needs a token), `doc-consistency-check.py` and `check_drift.sh` pass locally; dotnet gate not run (no C# or build input changed).
>
> **Last Updated (prior):** September 23, 2026 — **PR #447 durably archives W3/#439 evidence; no evidence refs deleted.**
>
> The exact six-seed pre-W3 structural-comparator table and final landed-W3 table are committed under `docs/tracking/evidence/pr439-w3/` with full SHA-256 coverage. The archive records the original post-W3 run/artifact/production SHA as well as the final rerun; direct artifact verification in this work confirms the two post-W3 `report.txt`, `results.tsv` and `results.json` files are byte-identical, while logs/TRX are not asserted identical. Interpretation remains narrower than a raw table diff: the pre-W3 W3 counters were structural literal zeros, `headerAttempts` is contacts+failures rather than HeaderIntent commits, and the measured landing state also includes the W6 reattachment correction, #442/#443 merge and `ERR-010-004`/`ERR-011-014`, so trajectory/foul/aerial deltas are not attributed to W3 alone.
>
> `docs/tracking/evidence/pr439-ref-archive/` now preserves **39 exact branch-history blob states** across all **16/16** live `evidence/pr439-*` refs, plus exact ref heads, explicit dispositions and **15 cited workflow-run heads**. `check_pr439_evidence_refs.py` verifies committed blob identity and, in live mode, requires the current remote ref set/heads and introduced branch-history blob states to match the archive. A dedicated workflow fetches all PR439 evidence refs and runs that proof. All 16 refs are classified `deletable` but remain `delete_now=false`; no cleanup is performed or authorized by this landing. Evidence/governance only; no gameplay `[GT]` or production behavior change.
> Supporting evidence was added before this archive lands: the uncited pre-W6 six-seed arm (`35815667045`, measured `f69aaef0…`) is retained from artifact `10732311509` with the API digest verified, byte-exact report/TSV/JSON/runsettings, the raw instrument stream and TRX retained losslessly, the artifact's original SHA-256 ledger committed, and the redundant full-console `measurement.txt` explicitly hash-accounted; the artifact API expiry is `2026-12-22T03:46:31Z`. A separate transcribed supporting file preserves timestamped result lines for the W6 pre-fix discriminator (`39.75` expected vs `40.0` observed), the 5/0 focused W6 post-fix proof, and the 2/0 SeasonSave focused proof. This does not change the 15-run PR #439 citation ledger, authorize ref deletion, or make a W3-vs-W6 attribution claim.
>
>
> **Last Updated (prior):** September 23, 2026 — **PR #439 lands W3 (shared `AGENT_BALL` fan-out + goalkeeper cross-claim feed) as WIRED BUT DORMANT, with the W6 Resolve-time Controlled-ball reattachment correction it surfaced and the review-closure fixes `ERR-010-004` / `ERR-011-014`.**
>
> Production now publishes a read-only same-frame `AGENT_BALL` observation feed after Physics movement, commits bounded keeper `ClaimIntent` episodes (snapshot schema v22 → v23), and routes contested contacts through canonical, fail-closed mixed Hand/Head arbitration. Collision #3 is candidate discovery only; Heading #10 owns Head geometry and Goalkeeper #11 owns live Hand reach (`ERR-011-011`, `-012`, `-013`). PR #439's review-closure commits `16233d1`/`bab4cd4` add `ERR-010-004` (Heading #10 contact buffer default/floor raised from 16 to `MAX_AGENTS` so W3's 22-agent same-frame fan-out fits; overflow still fails closed) and `ERR-011-014` (possession acquisition and restart awards hard-cancel live `ClaimIntent` state immediately). The W3 corpus was rerun after those gameplay changes: Frozen six-match corpus rerun `35925236129` on production head `bab4cd41…` (identical instrument and seeds) reproduces run `35814050060` at `f40f0853…` exactly — all 264 per-seed and aggregate counters — 215,083 fan-out events, 1,297 claim-eligibility episodes, 1,164 single-keeper Hand resolutions, 753 successful claims and **zero contested Hand-vs-Head arbitrations**. Heading #10 never supplies a production Head participant (#441 localization run `35887487201`: 1,811 HeaderIntent commits → 18 ever-predicted → 0 prepared Head contacts). The owner classified W3 **wired but dormant**; #441 owns Head reachability, and #440 (BallLoose same-tick re-pick) is deferred to a separate gameplay PR. The unconditional W6 reattachment after Collision #3 writeback applies to keeper and outfield holders and can move default-engine trajectories, so the corpus is evidence for the combined landing state, not W3 alone. `ShotExecutor` FM-03 containment debt is filed in `open-issues.md`.
>
> #439's W2 gate had been deterministically red (`Won=0`); the owner-approved #442 contract landed separately via #443 (merge `6876c5c1…`) and `main` was merged into #439 without rebasing (merge `9668643c…`, via merge-only PR #444), so every cited evidence SHA remains in history. The six-seed W2 corpus reproduces the pinned evidence arm exactly — `Won=3 Loose=12 Foul=2 Missed=73 dispossessions=3` — at both `9ba53481…` and `bab4cd41…` (the gate console does not print pooled counts; they were read from same-commit local reruns of the pooled fixture); `ATackleFoulIsPublishedAsASlideTackle` passes and only the foul-share lock stays skipped (`connected=17`). The post-merge MatchEngine total reads 536 instead of 535 because the test console leaves `Assume`-skipped tests out of its total and the publication lock moved from skipped to passed; the test set is unchanged. Close-out also: fixes the red Spec hygiene job on `bab4cd4` (two `spec-error-log.md` pointers left at v2.64 after its v2.65 bump, and a double-escaped regex that disabled the parser's new fail-closed W3 check and failed its unit test); adds six omitted source version-history rows and renumbers two rows into date order (comment-only); repairs a literal `\n` that had merged two `match-engine-wiring-backlog.md` table rows and advances it to v1.28 with its maintained pointers; demotes a second current `**Updated:**` label in `spec-error-log.md`'s header chain to `(prior)`; indexes FM-03 and refreshes the stale wiring-backlog and GK/Heading index titles in `project-reference.md` (active 27 → 28); and closes out `w3-agent-ball-fanout-design.md` v0.15. No gameplay `[GT]` change. Gate: PR run `35923780245` on production head `bab4cd41…` passed the functional gate (MatchEngine 526 passed / 0 failed / 12 skipped; the +2 over `9ba53481…` are the mirrored `W3_PossessionHardCancelsClaim_BeforeNextTacticalPass` cases) but failed Spec hygiene (fixed in this close-out) and the Markdown link check (external `https://unity.com/` returned no response from a Shot Mechanics spec this PR does not touch); this close-out head needs fresh CI before merge.
>
> **Last Updated (prior):** September 23, 2026 — **#442/#443 W2 composed-tackle regression corpus sizing; test-only contract, no gameplay change.**
>
> Owner-approved #442 splits `MatchEngineTackleTests` v1.3 into a six-seed pooled `CorpusSeeds` using the already-frozen #435 seed set and a two-seed `RestoreSeeds` retaining the original save/restore inputs. The per-seed horizon remains 150,000 ticks; assertions, Assume guards, thresholds and upper bounds are unchanged. Result-bearing run `35902447061` used the same frozen contract on base/main and pinned #439 production: base pooled `Won=5 Loose=9 Foul=0 Missed=64 dispossessions=5`, head pooled `Won=3 Loose=12 Foul=2 Missed=73 dispossessions=3`. On head, `ATackleFoulIsPublishedAsASlideTackle` actually ran and PASSED at `foul=2 / slideTackleFouls=2`; the foul-share lock remained SKIPPED at `connected=17`. On base both foul-specific locks are SKIPPED (`connected=14`, `foul=0`), which is recorded as a coverage gap rather than treated as exercised coverage. The clean-win margin is also deliberately recorded as thin (5 base / 3 head); it satisfies the frozen contract and is not a post-result reason to resize it.
>
> PR CI run `35902016795` completed green on contract commit `e8680a1d…`: MatchEngine **502 passed / 0 failed / 12 skipped / 514 total** and the gate reported PASSED. Executed-test artifact `10771553111` directly proves both unchanged `SaveAndRestoreCarryTheTackleLatches` cases PASS on seeds `0xD1A6D05E` and `0x0F1E2D3C4B5A6978`; the same TRX records the two skipped foul tests with `won=5 loose=9 foul=0 missed=64 dispossessions=5`. Focused same-runner measurement is 166.51 s for the old two-seed pooled fixture versus 490.95 s for six seeds, **+324.44 s (~5.4 min)**. The one full-gate comparison is noisier: main run `35783692588` completed in ~36m46s with MatchEngine 35m30s / SeasonSave 15m15s, while #443 took ~55m56s with MatchEngine 54m22s / SeasonSave 21m21s; SeasonSave's unrelated ~40% slowdown shows the observed +19m full-run delta is not attributable to #443 alone. The focused delta is ~15% of the 36.8-minute main gate and remains an explicit owner runtime-review item. No production source, approved spec, assembly edge, snapshot/save schema, digest, RNG stream/domain/draw site/order or gameplay `[GT]` changes. `src/CLAUDE.md` remains unchanged under its compact-history split; source history is recorded in `CHANGELOG-src.md`.
>
> **Last Updated (prior):** September 22, 2026 — **Frozen #435 six-seed foul/card characterization durably closed out; no calibration or gameplay tuning.**
>
> Run `35765635130` / job `106874328764` measured exact landed #436 `main` SHA `c56e5e1adff4e240b54c9abbb5ebea9c0290f65e` on the six preregistered full-match seeds. Live production produced **50 fouls = 8.33/90, 5 cautions = 0.83/90 and 1 total dismissal = 0.167/90**, split **42 collision FROM_BEHIND + 8 W2 SLIDE_TACKLE**; the collision funnel reconciles and all **42 priced-candidate identity checks match with 0 mismatches**. Both preregistered tackle/cooldown counters are zero, but with only eight tackle fouls that observation is explicitly non-dispositive: the owner decision on the existing W2 cooldown bypass remains open.
>
> New durable evidence under `docs/tracking/evidence/foul-card-six-seed/` retains the exact result-producing harness, runsettings, lossless compressed TRX, verbatim report, machine-derived TSV/JSON, provenance, the first failed attempt as non-result-bearing, and a full `SHA256SUMS` contract. New `tools/dotnet-ci/parse_foul_card_characterization.py` regenerates the structured rows and fails closed unless the exact six frozen seed identities/order are present, every additive aggregate counter equals the six seed rows, the shipped per-90 rate record is present, and all structural identities hold; `tools/tests/test_foul_card_characterization_parser.py` locks those failure modes. `check_evidence_manifests.py` registers the directory. The harness gate built the full generated tree but tested only the foul diagnostic filter, so this closeout does **not** claim the full suite ran. #435 §5 remains the future final-calibration envelope; the present result is descriptive characterization only. **No production gameplay file, gameplay `[GT]`, approved spec, schema, digest or RNG behavior changes. W3/W8/W9/W10 remain before the complete-engine fit.**
>
...
>
> W3/W9 are preregistered as trajectory/contact-stream invalidators on the same corpus. W3 must first define the shared multi-agent `AGENT_BALL` feed owner/slices/ordering/evidence/estimate; W9 retains its ActionType ordinal-8/3-bit rebaseline problem. Their before/after reports carry cross/header/contact metrics and the complete foul/card census. `match-engine-wiring-backlog.md` advances **v1.23 → v1.24** and `foul-discipline-balance-design.md` **v1.2 → v1.3**; v1.3 also corrects KD-F5's dismissal-vs-straight-red derivation. Review adds mandatory **collision-cooldown suppression / tackle-during-cooldown bypass / same-tick displacement** counters and records the live `FoulCallProbability = 0.030` starting point/§5.Z.13 emission-granularity precedent. Codex review corrected an interaction premise before results: W2 decided tackle fouls re-arm but do **not** consult the 180-tick foul cooldown, so the bypass is an explicit production fact and a pre-final-fit owner decision/invalidation point rather than a fictitious tackle-suppression bucket. Claude follow-up then made the rare-dismissal rule count-exact for the six-match corpus: **0–2 PASS / 3 INCONCLUSIVE / 4+ FAIL**; the exactly-three tie cannot trigger source localization or a red-probability fit and instead requires a separately preregistered larger rare-event corpus. The new `Cautions / fouls` ratio is explicitly separate from the legacy double-counting plausibility predicate. `MatchEngineConstants.cs` v1.38/v1.39 receives matching **comment-only** corrections to the `RedCardProbability` and `FoulCooldownTicks` rationales, recorded in `CHANGELOG-src.md` v2.144/v2.145; values remain 0.011 and 180. **No source/test behavior, gameplay `[GT]` value, calibration result, acceptance result, spec, SPEC_INDEX row, assembly edge, snapshot/save schema, digest, RNG stream/domain/draw site/order or ERR changes.** The preregistration must land before the source-complete characterization is run.
>
...
>
> The repository-wide claim is intentionally narrow: this is **repository-wide integrity-contract registration**, not a claim that all evidence bytes are SHA-manifested. W12 remains governed by `check_w12_evidence.py`; the PR #416 ref archive remains governed by `check_pr416_evidence_refs.py`; `artifact-SHA256SUMS` covers only its named payloads. Review hardening makes `*SHA256SUMS*` detection case-insensitive, rejects stale registry entries and symlinks in full-manifest scope, and fails closed on Git worktree/`ls-files` errors instead of silently reverting to filesystem scanning. README documentation was rebuilt from current `main` after review exposed a malformed fenced command/duplicated-tail edit; it now carries one section chain, one Version History table, a September 21 governance row, and the strict regeneration command.
>
> New `check_branch_ancestry.py` still adds a useful fail-closed local guard and distinct exit 3 for shallow/other guard errors. Codex review further hardened its graph semantics: every Git invocation disables replacement refs, legacy `info/grafts` state fails closed, and remote deletion documentation now requires a freshly fetched remote-tracking descendant or authoritative remote OID/API result rather than local `main`. The guard remains procedural rather than mechanically enforced on branch deletion. The shallow-clone issue is therefore restored to `open-issues.md` as **NARROWED, NOT CLOSED** instead of being archived. The PR #424 manifest issue alone moves to the archive. Active/archive counts advance **28 / 57 → 27 / 58**. The historical owner-held pinned-main leakage anomaly also remains open. **Blast radius:** tooling/tests/tracking only; no `src/`, APPROVED spec, SPEC_INDEX, gameplay behavior, save/snapshot schema, RNG surface, `[GT]`, calibration threshold or ERR change. Current-head CI is the landing gate.
>
...
>
> Review corrected four substantive defects before merge. **(1)** A dedicated path-filtered W12 workflow duplicated the checker already exercised by `tools/tests/test_w12_evidence.py` inside the required `Spec hygiene checks` context; it was removed rather than creating a second, potentially unreporting required status. **(2)** `w12-preregistration-reconciliation.md` now records that the census is the one useful branch-only record recovered by #432 and that enforcement remains in the required existing gate. **(3)** the checker now requires the static-sweep member map and exact ZIP namelist instead of treating missing `sweep_members` as empty/acceptable, and malformed metadata returns controlled `ERROR:` diagnostics rather than relying on a traceback. **(4)** negative tests now prove those failure modes, row/provenance corruption rejection, and future exit-key preservation.
>
> The checker additionally validates the recorded pre/post `instrument-output.txt` and `measurement.txt` hashes against the committed ZIP members, cross-checks those extracted-hash rows in the W12 evidence manifest, identifies the first differing seed/team/field for census-row failures, and derives aggregate phase/exit keys from the parsed records so a future exit kind cannot disappear silently. Final review also closes the remaining schema/consumer holes: the census root key set plus `generated_from` / `salvaged_from` provenance text are exact, the declared consumer must exist, and all seven locked baseline table rows plus the non-`InPossession` population are mechanically recomputed from the raw pre-#398 aggregate. This matters because `main` currently carries `w6-controlled-ball-preregistration.md` pointing at this census path while the file itself is absent; #432 therefore repairs a dangling evidence pointer, not merely an archival convenience.
>
> **Tracking/blast radius:** new `docs/tracking/evidence/w12/w12-gate-firing-census.json`; modified checker, W12 tests, evidence README, pre-registration reconciliation, this changelog, and `file-manifest.md`. `open-issues.md` is deliberately unchanged because no live blocker or owner decision is created by the salvage. No production/source behavior, APPROVED spec, assembly edge, save/snapshot schema, RNG behavior, gameplay `[GT]`, threshold, or ERR changes. Fresh current-head CI remains the merge bar.
>
> **Last Updated (prior):** September 21, 2026 — **PR #406 reconciled onto current main; UX S0 Gate A COMPLETE, Gate B NEXT.**
>
...
>
> T0 remains deliberately narrow: a Tier-7 `TacticalDirector.Transfers` production/test assembly; exact integer attributes+age valuation with fractional mean retained through currency calculation; always-on #27 coarse-position scarcity measured prospectively with the negotiated player excluded; deterministic synchronous `Accepted` / `CounterOffered` / `Rejected` negotiation; typed `InsufficientBudget` / `SquadFull` command outcomes; contract/window/committed-spend state; and manager `SubmitBid` with #40 mutation staged on a value copy and published only after the roster preflight/commit contract succeeds. T1 save composition, T2 production #30 roster/season/genesis/boundary wiring and re-key migration, and T3 personality/CA/staff/wage/multi-day/rival-bid depth remain deferred.
>
> Intervening-main reconciliation corrects two stale live-status surfaces rather than carrying them forward: Code Standards §3 v1.15 now recognizes `club-finances` as an already-live #27/`PlayerDatabase` consumer from #40 T2a, and #31 §7 v0.7 finalizes T0 as implemented by PR #407 rather than “pending merge.” It also records the current-main T2 finance-composition obligation: production finance lives behind #30 `SeasonLoop`, so T2 must preflight/stage against the exact held #40 state and publish through the canonical transaction path after an infallible roster commit, rather than expose/reconstruct a raw mutable `ClubFinances` value. Both original Codex P2 findings remain closed. No match snapshot/save schema, season-save format, RNG stream/domain/draw site/order, or gameplay constant retune is introduced.
>
> **Verification posture:** re-run assembly-tier, documentation consistency, governance/spec hygiene, Transfers tests, relevant finance integration tests, and tooling tests on the reconciled head. Unity 6000.4.9f1 remains the governing compiler and a host compile is required before merge; Linux shim/C# format success is supplementary, not compiler certification.
>
> **Last Updated (prior):** September 21, 2026 — **PR #415 / `ERR-013-012` Pressing AI §2 reconciliation closeout; documentation/specification only.** The stale branch was reconciled onto current `main` without dropping its original three-commit lineage, and the final normative correction brings §2 FR-PR-010 into the same contract as §3.1.2 / `ERR-013-009` / `ERR-013-011`: `BACKWARD_PASS` uses the possessing-team frame (`-attackingDirection`), rejects pressing-team passers, and uses EventBus-stamped `PassAttemptEvent.Tick` — never payload `FrameNumber` — to decide whether a pass may **start** dwell in `[N-AI_PHASE_STRIDE,N)`. A qualifying discrete pass then remains logically raw-true only for its already-started bounded dwell; reaching `TRIGGER_DWELL_TICKS` commits the trigger per §3.2, and the stale retained event cannot restart it. Review corrections are incorporated before landing: the original `FrameNumber` clock-domain mistake, the overcorrection that would have killed second-heartbeat commit, FR-PR-010's missing sufficiency clause, and F7's coverage attribution (`T-U-010 / T-U-010a`). `docs/specs/pressing-ai/section-2.md` advances **v0.3 → v0.4**; `docs/tracking/spec-error-log.md` advances **v2.59 → v2.60** with v2.59 preserved verbatim as prior history and a dated residual annotation on `ERR-013-009`; `docs/tracking/file-manifest.md` advances only its maintained current-version pointers and adds this landing record; this CHANGELOG gains the required closeout entry. No `open-issues.md` entry is created because the defect is filed and resolved in the same landing and leaves no live blocker. No runtime/source, serialized state, snapshot/save schema, digest, RNG stream/domain/draw site/order, gameplay constant, assembly edge, or test behavior changes. Pre-closeout head `567a3526…` passed CI run `35561104128` (10 successful checks, Unity matrix placeholder skipped); the final ledger/spec closeout commit is documentation-only and receives its own CI run.
>
> **Last Updated (prior):** September 20, 2026 — **Owner retires `sim_match_engine_close_chance` from owner-held RED; policy/tooling/tracking only.** The September 20 owner call supersedes the August 11 “hold red” disposition without moving the scenario's seeds, predicates, floors, gameplay code, or any `[GT]`. Exact retirement evidence run `35344056248` measured **2,494** final-third dribbles, `meanCosine=-0.038` and `goalwardShare=0.496` against unchanged floors **−0.16 / 0.42**; the evidence ref's only scenario-source delta is an observation-only `Console.WriteLine` after both values are computed. `tools/dotnet-ci/owner-held-red.txt` is now comments-only, activating the already-unit-tested terminal state: ordinary sweep unfiltered, dedicated owner-held stage skipped. Expected unchanged-corpus transition is **3,730 ordinary / 1 dedicated → 3,731 / 0** and MatchEngine **501 passed / 12 skipped / 513 total → 502 / 12 / 514**. The latter becomes legitimate ordinary cardinality after retirement and therefore does **not** resolve the historical pinned-main 514-case anomaly, which remains open because it occurred while the exclusion was still configured. The ~0.122 cosine margin is only about **1.1×** the scenario's documented 0.110 two-seed resampling sd; a future trajectory-moving failure is therefore a sensitivity to investigate, not automatic permission to rebaseline. `close-chance-creation-design.md` advances **v2.3 → v2.4**, `match-engine-wiring-backlog.md` **v1.22 → v1.23**, project-architecture integration plan **v0.45 → v0.46**, and localization implementation plan **v2.5 → v2.6**. Current operational wording is synchronized across the gate README/runner, agent skills/guides, A3.3 correction, ERR-019 current-status record, architecture-governance plan, localization plan, live gate-integrity issue, and file manifest. The old steady-red reason against requiring the non-certifying functional CI context is now historical; **retirement does not itself authorize promoting that job to required status**. Active/archive issue counts remain **28 / 57**. Historical evidence/version rows, the APPROVED Decision Tree spec, and the identical `−0.165 → PASS` historical idiom in `OptionGenerator.cs` remain untouched. **CI pre-merge correction:** the owner-held conformance fixture now supplies its own temporary populated ledger instead of depending on the live ledger having a row; this keeps generic exact-name/exclusion policy coverage valid while the live ledger is intentionally empty. Spec hygiene also required the archived football-judgment record's maintained `now` pointer to advance from close-chance v2.3 to v2.4; its dated findings remain unchanged.
>
> **Last Updated (prior):** September 20, 2026 — **W2 post-#416 six-seed reconciliation closeout; documentation/tracking only.** PR #429 merged to `main` as `e3f34194acf785ee3cb68905b7a98c7aa3c3e584` with parents `896df122…` and reconciled W2 head `24fea90b…`; the original nine W2 evidence commits, governing baseline/result heads, production base, closeout commit `f5296a81…`, and frozen preregistration remain reachable from `main`. **Landing-order deviation:** #429 omitted the required tracking-ledger synchronization; this follow-up closes that gap separately rather than rewriting the already-merged evidence commit. The owning `match-engine-wiring-backlog.md` advances v1.21 → v1.22 in the same closeout commit. The #429 landing adds six durable paths: `docs/tracking/w2-six-seed-evidence-preregistration.md` plus `docs/tracking/evidence/w2-six-seed/{README.md,artifact-SHA256SUMS,baseline-counts-and-floors.tsv,provenance.txt,w2-six-seed-results.tsv}`; `docs/tracking/evidence/w2/README.md` was modified in place. Post-merge push run `35535380578` has all six live branch-protection contexts green. Its overall workflow result remains red only because the non-certifying functional job reproduces the existing owner-held `sim_match_engine_close_chance` unexpected-green policy condition; that disposition is independent of W2 reconciliation and is not changed here. The six-seed record closes only the post-#416 stall/non-vacuity revalidation: production is healthy on all six frozen seeds, but the disarmed control is healthy too, so the corpus does not prove W2 efficacy and does not authorize tackle-outcome, cooldown, or foul/card calibration. No result-bearing rerun was needed for reconciliation because current `main` introduced no overlapping change to the seven #429 final-diff paths. Per-arm TRX/detailed logs remain time-bounded Actions artifacts (90-day retention, expected around 2026-12-17); aggregate rows, hashes, provenance and interpretation are durable in-repo. Active/archive issue counts stay **28 / 57**. No production/source semantics, APPROVED spec, assembly edge, save/snapshot schema, RNG behavior, owner-held row/bound, or `[GT]` value changed.
>
> **Last Updated (prior):** September 19, 2026 — **Post-#420 owner-held gate lifecycle correction, durable isolation payload, and cleanup reconciliation; tooling/tests/tracking only.** PR #420's exact-head evidence is now durable in repository bytes, not only prose: `docs/tracking/evidence/pr420-owner-held-isolation.md` pins run `35420947427`, head `2e12fa3e…`, source artifact `10578621101`, source size 1,106,934, and source SHA-256 `8a441096…71570`. The source ZIP is **not** committed. Its exact 70 TRX members are preserved in a normal-Git 472,056-byte `tar.xz` (SHA-256 `f8a37e7c…bd1c`), a 70-row relative-path/member digest ledger `TRX-SHA256SUMS` (SHA-256 `484a9217…9c67`), the byte-stable normalized `executed-results.tsv` (SHA-256 `846d72d7…cea7`), and `SHA256SUMS`. `tools/dotnet-ci/pr420-evidence.py` separates bootstrap from verification: bootstrap accepts only the pinned source size/hash before creating the member ledger; verify checks committed payload digests, exact member paths/hashes, and regenerates/byte-compares the TSV without requiring compressed-byte regeneration. Later validation-run artifacts are explicitly inadmissible substitutes. The archived payload verifies **70 TRX files, 3,730 ordinary result records / 1 dedicated result, and exact `sim_match_engine_close_chance` multiplicity 0 ordinary / 1 dedicated**. The gate-integrity issue remains narrowed rather than closed because the historical pinned-main leakage mechanism is still unidentified. The #420 lifecycle path is repaired without weakening policy: an empty existing owner-held ledger means **no exception exists**, so no owner-held exclusion is applied and the ordinary suite owns the result; a missing ledger file still fails closed. The verifier's success-only isolation line derives observed multiplicities rather than hardcoding `0/1`; failing leak/duplicate multiplicities stay on their earlier error paths. `run-gate.sh` now merges verifier stderr into stdout at the call site (`2>&1`), and exact-head run `35458191366` on predecessor `f9be2529…` proved real Actions ordering `TRX → ISOLATION → unexpected-green ERROR`; the ordering regression tests the same single-FD condition. **Step 5 cleanup is also closed:** PR #427 merged as `c470eace…`, its head `d1d79b6…` is an ancestor of current `main`, the landed checker enforces the completed 21-deleted / 3-policy-retained topology, and the former live cleanup issue moves verbatim to the archive. That move is **28 active / 56 archived → 27 / 57**. The same pass files a new live tooling/process issue after a shallow agent clone falsely reported fully merged branch `Copy-Main-All-Specs-From-Claude` as unrelated to `main`; authoritative remote comparison confirms branch head `caaf5cf0…` is the merge base and `main` is 2,497 commits ahead / 0 behind from that base. Ref-only fetches do not unshallow a clone, and current-tree blob absence is not reachability evidence. Final counts are therefore **28 active / 57 archived**. The three PR416 policy-retained refs remain retained; `close-chance-retirement` is not deleted or reclassified. The current `sim_match_engine_close_chance` row and recorded diagnostics/bounds are unchanged. The functional job's expected unexpected-green RED is a separate owner-held disposition, not a failure of lifecycle/topology validation. `.gitattributes` marks `*.xz` binary only; no PR420 evidence path is routed through Git LFS. No production/source semantics, APPROVED spec, assembly edge, save/snapshot schema, RNG behavior, owner-held bound, or `[GT]` value changed.
>
> **Last Updated (prior):** September 18, 2026, later — **PR #416 Step 5 deletion authorization and post-delete gate hardening; tooling/tracking only.** The durable PR #416 archive remains 100 snapshot rows / 24 dispositions / 31 run mappings. PR #425 already proved the live deletion set is complete: **266/266 branch-exclusive blob instances covered, 234 candidate instances, 0 delete-only blob objects, 0 uncovered, 0 violations**. This revision therefore authorizes the **21 `deletable` refs atomically** (`delete_now=true`) while leaving the three policy-retained refs false. The checker now accepts only two complete live topologies: all 24 refs before deletion, or exactly the three retained refs afterward; a partial deletion fails closed, and CI requires authorization so `true → false` rollback is not a valid state. The 21 disposable refs were subsequently deleted outside this PR while review was still in progress. Revised exact-head Gate A now passes the live post-delete state with exactly **3 refs / 21 deleted / 3 policy / authorized=True**, and those three refs remain at their recorded heads. **This landing does not claim to have performed the deletion.** The cleanup issue remains active only until this post-delete-capable gate and synchronized durable record reach `main`. Counts remain **28 active / 56 archived**. The current owner-held `sim_match_engine_close_chance` unexpected-green failure is unchanged from this PR's base and is not rebaselined here. No production/source semantics, APPROVED spec, assembly edge, snapshot/save schema, RNG behavior, owner-held bound, or `[GT]` value changed.
>
> **Last Updated (prior):** September 18, 2026, later — **PR #424 SHA-256 evidence-manifest enforcement remainder recorded; documentation only.** `docs/tracking/evidence/v210-rolling/SHA256SUMS` currently covers all seven non-manifest evidence files and verifies **7/7**, but no repository workflow/tool enforces either those digests or coverage. Because `README.md` is itself hashed, later documentation edits can silently stale the manifest. The live issue explicitly leaves the repository-wide SHA-256 gate to a separate follow-up PR, so PR #424 remains documentation/evidence only. Active issues **27 → 28**; archive remains **56**. No production/source, test semantics, APPROVED spec, assembly edge, durable gameplay field, snapshot/save schema, RNG stream/domain/draw site/order, owner-held bound, or `[GT]` value changed.
>
> **Last Updated (prior):** September 18, 2026 — **Step 3.3 Ball Physics v2.10 elevated-`Rolling` characterization closed; documentation/evidence only.** Six preregistered full-match seeds compare shipped v2.10 against the preserved `bb501a2…` narrow counterfactual. The supplementary same-tick census measures **1,302–1,603 moving Rolling height-crosses per v2.10 match**, all reclassified to Airborne on that update; the tested narrow arm instead accumulates **61,053–71,768 elevated-Rolling boundary ticks per match** and materially resamples every trajectory fingerprint. The conclusion is deliberately bounded: the tested narrow semantics is non-viable, while transitions into Rolling were not counted, so this instrument does not rule out all possible v2.10 defect classes or future narrowings. Exact original Actions aggregate ZIP, original and supplementary rows, observer-neutrality hashes, deltas and provenance are durable under `docs/tracking/evidence/v210-rolling/`. The temporary instrumentation/workflow is absent from the landing diff. The live residual moves verbatim to the resolved archive. Current-main reconciliation retains PR #422's cleanup issue and PR #423's Step 5 evidence-ref archive corrections and yields **27 active / 56 archived**. No production/source, test semantics, APPROVED spec, assembly edge, durable gameplay field, snapshot/save schema, RNG stream/domain/draw site/order, or `[GT]` value changed.
>
> **Last Updated (prior):** September 18, 2026 — **PR #416 Step 5 evidence-ref archive corrected for intermediate history; documentation/evidence only.** The initial tip inventory was exact but insufficient for deletion authority because branch-exclusive blobs can exist only on intermediate commits. The corrected archive now contains **100 quarantined snapshot paths**: 72 current-tip files across 24/24 refs, 13 run-time files, and **15 intermediate-history paths representing 13 previously unarchived blob identities**. Those recovered blobs include two historical `MatchEngineTackleTests.cs` states, one `ShotExecutorStateTests.cs` state, historical workflow revisions, and the ApplyKick ablation generator/workflow. `MANIFEST.tsv` now covers current/run/history snapshots and preserves each selected source commit's subject + committer date; `run-heads.tsv` records authoritative branch/head/subject/date/event/result metadata for all **31 Actions run ids cited** across the durable provenance and W6 diagnosis. The deletion model is corrected: historical completeness must be proven against the **live refs before deletion**; post-merge `main`-only verification proves archive integrity and interpretive reconstruction only, without claiming full Git author/parent/co-change reconstruction. The disposition remains **21 `deletable` / 0 `retain` / 3 `policy-retained`**, but all 24 rows remain `delete_now=false`; no ref is deletion-authorized. Step 4 remains closed; active/archive issue counts remain **28 / 55**. No production/source, test semantics, APPROVED spec, assembly edge, durable field, snapshot/save schema, RNG stream/domain/draw site/order, owner-held bound, or `[GT]` value changed.
>
> **Last Updated (prior):** September 18, 2026, post-#421 — **PR #416 evidence-cleanup remainder recorded; Step 4 remains closed; documentation only.** `docs/tracking/open-issues.md` now records that the 24 surviving `evidence/pr416-*` refs are not cleanup-authorized merely because durable provenance landed. The merged `main` tree contains no `.github/workflows/pr416-*` or `.github/evidence/*` files, so arm-specific workflow/helper/TRX-enforcement material remains branch-only. The cleanup criterion therefore requires durable archival and a fresh per-ref uniqueness review before deletion. The record also preserves the source-ground distinction: candidate blobs `e739a306…`, `ce945b57…`, and `4511b1cb…` identify narrow/state-only candidate forms not embedded on `main`, so harness archival alone cannot make those retained refs deletable. PR #421's CI provenance is stated precisely: no functional-gate verdict existed for any head at merge time; `905a2ee…` and `f246efe…` runs were cancelled and final-head run `35393162486` was still running post-merge. That landing basis is acceptable only for the Markdown-only #421 scope and is not precedent for source changes. Active issues **27 → 28**; archive remains **55**. No production/source, test semantics, APPROVED spec, assembly edge, durable field, snapshot/save schema, RNG stream/domain/draw site/order, owner-held bound, or `[GT]` value changed.
>
> **Last Updated (prior):** September 18, 2026, post-#416 — **durable PR #416 investigation provenance recorded after merge; documentation only.** New `docs/tracking/pr416-evidence-provenance.md` pins the merged PR (`e8207f4c…`), the pinned-main / diagnostic-control / final-code anchors, every valid production-delta arm's evidence branch, exact head or code SHA, reference SHA and workflow run, and the materially significant Ball Physics deltas themselves. It also records measurement/candidate arms with independently verified branch↔run↔blob mappings, failed/void/non-certifying attempts, all 24 surviving `evidence/pr416-*` refs with exact current heads, and the Actions-log rule without asserting a repository-specific retention period. The owning `w6-elevated-stationary-ball-fix.md` diagnosis now links back to the provenance record so future cleanup reaches it from the investigation entry point. The three deliberately preserved refs — `state-only-preforce-candidate`, `narrow-rolling-candidate`, and `close-chance-retirement` — remain explicitly non-deletable by this record. No production/source, test semantics, APPROVED spec, open-issue count, assembly edge, durable field, snapshot/save schema, RNG stream/domain/draw site/order, owner-held bound, or `[GT]` value changed. No source gate is owed for this Markdown-only landing; applicable PR documentation checks own the landing verdict.
>
> **Last Updated (prior):** September 18, 2026, final documentation correction — **owner-held isolation closure is defined on executed results, not discovery/filter intent.** The gate-integrity remainder now uses stable refs for the no-config-drift proof (`1bad655f…` → validated code head `b6a21d2d…`, zero changed paths under `.github/` or `tools/`) and requires the actual executed test-name multiset from durable TRX/equivalent results: the held test must appear zero times in the ordinary sweep and exactly once in the dedicated owner-held invocation. Discovery output, filter text and cardinality remain supporting context only. The PR body separately drops all "current documentation head" language and names only stable validated code evidence, so future doc-only commits cannot make the body false. No issue-count change (27 active / 55 archived) and no production/source, test semantics, APPROVED spec, owner-held bound, `[GT]`, RNG, or schema change.
>
> **Last Updated (prior):** September 18, 2026, later still — **PR #416 residual tightened from cardinality accounting to owner-held gate integrity; documentation only.** Follow-up review supplied a second independent fact: direct compare `1bad655f…` → `dded4c70…` has **zero `.github/` or `tools/` changes**, so the same repository workflow/tooling configuration produced the pinned-main 514-case ordinary sweep and #416's expected 513-case excluded sweep. The open issue is therefore reframed: strict owner-held RED isolation is unverified because one run logged `Name!=sim_match_engine_close_chance` yet behaved cardinally as if the held test remained in the ordinary pass. The investigation is scoped to execution ordering, runner/environment state, adapter/filter handling, or another runtime-selection effect, with durable TRX/executed-name proof required before closure. The `c61c7d78…` `mergeable_state=unstable` observation is also explicitly marked historical/non-terminal and superseded by the final PR head's own completed result, rather than being readable as #416's terminal state. No issue-count change (27 active / 55 archived) and no production/source, test semantics, APPROVED spec, owner-held bound, `[GT]`, RNG, or schema change.
>
> **Last Updated (prior):** September 18, 2026, later — **PR #416 closeout precision after review; documentation only.** GitHub state is recorded without conflating conflict status and check status: reviewed pre-correction head `c61c7d78…` had `mergeable=true` but `mergeable_state=unstable` because the non-required functional verifier was red on the unchanged owner-held close-chance unexpected-green policy. The status is pinned to that exact head rather than presented as timeless current state, since later documentation commits re-trigger evaluation. The **501 passed / 0 failed / 12 skipped** ordinary MatchEngine result remains correctly cited to run `35307891032` on code/spec/test head `b6a21d2…`, not to a later documentation-only run. The apparent **514 pinned-main vs 513 #416** ordinary-sweep delta is also narrowed: project-scoped discovery sets are identical, while pinned-main run `35133104970` logged the same `Name!=sim_match_engine_close_chance` exclusion yet still executed 514 cases before running that owner-held test separately. That is recorded as a Testing Strategy/tooling selection-accounting remainder rather than repaired by changing #416 tests. A second open remainder records Ball Physics v2.10 elevated-`Rolling` trajectory sensitivity without misclassifying it as the regression root or rolling back the contract-correct fix. Active issues **25 → 27**; archive remains **55**. No production/source, test semantics, APPROVED spec, assembly edge, snapshot/save schema, RNG stream/domain/draw site/order, owner-held bound, or `[GT]` value changed.
>
> **Last Updated (prior):** September 18, 2026 — **PR #416 final CI disposition: correctness landing complete; held-red close-chance policy unchanged.** Final exact-head CI run `35307891032` on `b6a21d2…` has all six recorded branch-protection contexts green. Its ordinary MatchEngine sweep is **501 passed / 0 failed / 12 skipped**; the non-certifying functional job is red only because the separately verified owner-held `sim_match_engine_close_chance` unexpectedly passed. Evidence-only run `35344056248` measured that pass at **2,494 dribbles, meanCosine -0.038, goalwardShare 0.496** against unchanged `-0.16 / 0.42` floors. #416 changes perception correctness, not the DRIBBLE direction scorer; this is the same trajectory/population class already recorded at W6 PR #412, so the August 11 owner call remains **hold red; do not rebaseline a third time**. No owner-held ledger/bound/`[GT]` change and no KD-W1 calibration. GitHub reports #416 mergeable; the non-required Linux functional verifier may remain red solely on its unexpected-green policy.
>
> **Last Updated (prior):** September 17, 2026 — **PR #416 live-head regression closure (`ERR-007-004`).** Live head `a7f2b77…` already contained the minimal production correction: Perception #7 treats an exactly co-located ball as having no FoV bearing, preventing a Controlled carrier from losing sight of its attached ball because `atan2(0,0)` invents world-East. Exact-head validation run `35305911122` restores all four main-relative regression targets; the foul-share lock remains `NotExecuted`, matching pinned main. The preregistered three-seed full-match census reports longest Controlled spells **1,096 / 1,236 / 1,256 ticks** and **5 wins / 5 dispossessions**, eliminating the prior 97,682-tick seed-A HOLD pathology. The normal-CI save/restore red was teardown-only: both latch seeds pass all state/replay assertions when unrelated composed-play `ShotExecutor FM-03` logging is excluded, so that single test now scopes log suppression to its actual oracle. Perception §3.5.1 v1.5 + BP-007 + code citation close `ERR-007-004`; no Ball Physics v2.11 change lands. Post-fix corrected-baseline run `35286928656` measured 15,830 / 16,423 per-seed samples at 0.970815 / 0.964379 mirrored shares; the preregistered 80% floors are frozen at **12,664 / 13,138** and the temporary capture workflow is retired. No snapshot/save schema, RNG stream/domain/draw site/order, or `[GT]` change. Normal CI rerun owns the final merge verdict.
>
> **Last Updated (prior):** September 16, 2026 — **W2 tackle resolution is ACTIVE IN PRODUCTION.** `TackleContactRadiusM` now defaults relationally to `LooseBallPickupRadiusM` rather than `0`; the production contract is `> 0` and `<= LooseBallPickupRadiusM`, with both violations fail-loud on the production catalogue path. Explicit zero remains only as a test/measurement negative control, so the current 1.0 m fallback is not an eternal ceiling. Composed W2 locks now run against the shipping default. Pre-final focused activation validation run `35122576991` and non-certifying Linux capstone run `35123092565` succeeded before the final lower-bound hardening; final-head PR CI owns the merge verdict. No tackle-outcome `[GT]`, `TackleCooldownStrides`, discipline `[GT]`, RNG stream/domain/draw site, durable field, snapshot schema, or save format changed. **T-DA-DET-005 remains deferred and receives no W2 credit.** The ten tackle-outcome `[GT]` values remain accepted uncalibrated debt, foul/card calibration remains a separate next pass, and the owner-held `sim_match_engine_close_chance` RED is unchanged.
>
> **Last Updated (prior):** September 16, 2026 — **W2 post-W6 paired-control closeout is durable and activation may proceed as a separate change.** Actions run `35096793576` measured exact production head `e4335f7ff059deb483b1aaa534f2190fd3762008` from measurement-driver head `682e7af9ed8cf5eed64d78defe7358e9e7bd81a6`, using the same two governed seeds and the same reporting/assertion transform in both matrix legs. Armed at current `LooseBallPickupRadiusM` (1.0 m), the seeds read **0.983 / 0.983** and **0.985 / 0.985**, with gate/test exits **0 / 0**. Disarmed, they read **0.530 / 0.530** and **0.979 / 0.979**, with expected exits **1 / 1**; the corrected harness validates that negative control rather than incorrectly demanding armed acceptance from it. Exact ZIPs are checked into `docs/tracking/evidence/w2/` with archive SHA-256 digests `5772b980…dcb00` and `3b6dcd10…5e9e9`; both include the parameterized driver/workflow and internal SHA-256 manifests. This closes the pre-activation evidence obligation only: production `TackleContactRadiusM` is still 0, the durable activation lock is `> 0` and `<= LooseBallPickupRadiusM` rather than a permanent literal 1.0 m ceiling, the ten tackle-outcome `[GT]`s remain explicitly uncalibrated, and foul/card calibration still follows activation. **T-DA-DET-005 remains inactive and receives no W2 credit** because its deferred test requires `DefensiveAITick`'s own `DeterministicRngService` path. The earlier W12 procedure-order deviation is already recorded pre-interpretation in `w12-preregistration-reconciliation.md`; no historical preregistration was rewritten.
>
> **Last Updated (prior):** September 15, 2026 — **Post-W6 W2 P-W6-1 measurement closes the historical possession-stall blocker; production activation remains separate.** Actions run `35047291977` / job `104639783769` checked out exact W6 merge head `e4335f7ff059deb483b1aaa534f2190fd3762008` and armed W2 only through `TestOnly_ArmTackleChallenge(LooseBallPickupRadiusM)`. Governed seed `0x0F1E2D3C4B5A6978` produced 14,751 samples at **0.983 / 0.983** mirrored InPoss share; `0x1A2B3C4D5E6F7081` produced 14,203 at **0.985 / 0.985**. All satisfy the unchanged strict `> 0.70` per-seed predicate. The strongest like-for-like historical comparator is the pre-W6 **same 1.0 m radius** result `0.369 / 0.963`, so the known tackle-created stall no longer reproduces under W6. This clears the **P-W6-1 blocker only**: `TackleContactRadiusM` remains 0 on production `main`; the ten tackle-outcome `[GT]`s remain intentionally un-calibrated; foul/card calibration still follows activation under the standing **arm first, calibrate once** owner sequence. Before activation, a same-head/same-seed disarmed control is being captured with the parameterized measurement driver. Two conflicting W12 preregistration thresholds were also found before canonical post-W6 W12 interpretation; `w12-preregistration-reconciliation.md` preserves both historical files unchanged and requires the stricter 5 pp + suppression-mass conditions for any material-causation claim.
>
> **Last Updated (prior):** September 15, 2026 — **W6 Controlled Ball wiring / PR #412 review closure.** `BallStateType.Controlled` now represents genuine open-play physical possession: first-touch/interception, loose pickup, tackle ball-won and goalkeeper possession enter control; the ball is driven with its holder; non-kick release exits control explicitly; restart-taker designation remains a stationary placed ball. The recovered tackle-cooldown P2 is closed. A W6-exposed keeper-carry defect (review corpus 2/17 held claims crossing the keeper's own goal line) is closed at `MatchEngine.DriveControlledBallToPossessor()` by constraining only a Controlled goalkeeper at its defended goal plane while preserving global movement buffers and legitimate loose/kicked-ball goal adjudication. Focused W6 locks are 9/9 green and the composed keeper-claim scenario is 1/1 green. No new assembly edge, durable field, snapshot/save schema, RNG stream/domain/draw site/order, or `[GT]`. `w6-controlled-ball-preregistration.md` remains byte-for-byte historical. Close-chance is deliberately **not** rebaselined: merge-candidate measurement moved the exact held pair from main `-0.165 / 0.407` to W6 `+0.125 / 0.598` and the six-seed pool from `-0.180 / 0.381` to `+0.061 / 0.583`; W6 changes trajectory population but not the DRIBBLE scorer, so the August 11 owner call remains **hold red; do not rebaseline a third time**. The non-certifying Linux functional job may remain red solely on that unexpected-green policy; the six live branch-protection contexts remain the merge bar. W2 stays disabled pending the separately pre-registered post-W6 measurement.
>
> **Last Updated (prior):** September 12, 2026 — **#40 Club Finances T3a: the deep-tier daily revenue accounting primitive lands, draw-free and deliberately unconsumed (PR #400, `ERR-040-003`).**
>
> `src/club-finances/` only. `FinanceStep.cs` **v1.8** adds `AccrueDailyRevenue(in ClubFinances prior, long sponsorshipRevenue, long matchdayRevenue, bool deepRevenueEnabled)` — a pure accounting transform that, with its gate enabled, adds the day's non-negative sponsorship + matchday total to `Balance` **and** `SeasonRevenueAccrued` under `checked` arithmetic at all three addition sites, and with its gate disabled returns a coherent `prior` field-identically without interpreting the deep-only amounts. `SettleFinances` additionally closes `SeasonRevenueAccrued` to zero for the new season while carrying `WageBillAggregate` and `FfpBalanceWindow`. `ClubFinances.cs` **v1.4** makes `SeasonRevenueAccrued >= 0` an F1 coherence invariant. `ClubFinancesCritiqueTests` **v1.9** and `ClubFinancesSaveCodecTests` **v1.4** add 10 locks. **No assembly edge, no `.asmdef` change, no RNG stream / domain tag / draw site / draw order, no `SNAPSHOT_SCHEMA_VERSION`, and `FINANCE_SAVE_FORMAT_VERSION` stays 1** — the persisted six-field record already carried the accumulator, so T3a mutates fields that were already serialized and adds no cursor or ordinal. `_RESERVED_0x29_` / `SubsystemOrdinals.ClubFinances = 91` stay RESERVED per `ERR-040-001`.
>
> **`ERR-040-003` filed and RESOLVED, spec + code same landing** (`spec-error-log.md` **v2.58**) — same PR, not same commit: the code half and the §2/§4/§5/§7 back-props landed across this PR's earlier commits, and `1869af4` carried the §1/§3 corrections with the register row. Nothing deferred past the PR; no single commit holds both halves. T3a's autonomous accrual contradicted an unqualified MUST carried by **three** APPROVED #40 section files — FR-FN-003, §3.2's prose, and §1.2's scope line — all of which made `ApplyTransaction` the sole between-boundary mutation path. FR-FN-003 (§2 **v0.7**) now scopes that rule to the **externally-commanded** ledger and bounds #40-owned autonomous accrual to `Balance` + `SeasonRevenueAccrued`, forbidding it from touching the budget ceilings, `WageBillAggregate`, or creating a second caller-command ledger; §3 (**v0.9**) and §1 (**v0.3**) carry the same qualifier. Two further review defects fold into the same id: the newly-normative non-negative accumulator was absent from `ValidateCoherence`, so `ClubFinancesSaveCodec.Decode` — which does validate each entry — admitted a negative value from a corrupt save; and the requirement that a future FFP term read the completed season's revenue *before* the settlement reset existed only as prose.
>
> **Three things this landing got wrong first and is recording rather than smoothing.** **(1) The §1.2 half of the mutation-contract correction was missed** when §2 and §3 were corrected, and was found by the repository's Codex reviewer on PR #400, not by the authoring pass — a contract change has to be swept across every section file, not only the one that states the FR. This is the second time in two weeks a #40 correction landed in the FR and left a sibling section stale. **(2) The FFP-ordering fix was first implemented as a `CloseCompletedSeasonRevenue(long)` helper** (`FinanceStep.cs` v1.7) that discarded its only argument and returned a literal `0`; the call site then read as if the completed revenue were being transformed, which is precisely the invitation the FFP slice must not receive. Reverted at v1.8 to a direct `result.SeasonRevenueAccrued = 0` with the insertion point pinned above it. **(3) §3.1's pseudocode lagged that revert by one commit**, leaving an APPROVED spec instructing the next implementer to rebuild the construct the code had just deleted — realigned here. Each of the three was found by review, none by a mechanical check; no check exists for any of them.
>
> **Recorded, not fixed: `AccrueDailyRevenue` has no production caller.** §7.1 (**v1.5**) bounds it explicitly — **T3b1** is an identity-zero #30 daily-slot wiring slice that must close the prerequisite *before* any economic tuning, and **T3b2** then defines the sponsorship/matchday formulas and `[GT]` catalogue against an already-proven invocation path. This is the wire-first posture (KD-W1) applied to finance: the alternative — formulas and wiring in one slice — would calibrate magnitudes against a path nothing has exercised. The single `deepRevenueEnabled` producer is assigned to #40/config ownership, explicitly **not** #30. Open at `open-issues.md`; active count 24 → **25**.
>
> **Unity compile evidence is NOT supplied by this landing, and under the PR #404 rule that is a real gap, not a formality.** `src/CLAUDE.md` now states that Unity 6000.4.9f1 governs the build and a green Linux gate is **not sufficient on its own**; this landing changes four `src/club-finances/` C# files, so it is exactly the case that rule covers. What was verified here is narrow and should be read as such: the three *known* Unity/NUnit-3.5 incompatibility classes (`Assert.Multiple`, the non-string `Does.Not.Contain` overload, and an `.asmdef` missing a directly-referenced assembly) were each checked against all four files and none is present. That is a check for known traps, **not** a compile. The repository cannot close the gap from CI either: `ci.yml`'s `unity-tests` job is gated on `needs.unity-license-check.outputs.configured == 'true'`, `UNITY_LICENSE` is unset, and the job has reported `skipped` on every run of this PR. A clean editor compile on the pinned host is therefore **owner-held evidence**, in the same posture as `cert-run-runbook.md`, and is owed before merge rather than after.
>
> **Gate:** `PR functional gate (Linux shim, non-certifying)` **PASSED** on branch head `15494e4` (run `34674879501`, September 12, 2026 05:42Z), all 11 checks green — that run is evidence for the **code**, which this close-out does not touch. `main` then advanced through PRs #379 and #396, both tracking-document landings, and the merge was taken here rather than left for the merge queue: it conflicted in `CHANGELOG.md` and `spec-error-log.md`, both additively, and both sides are kept. **The merge cost this entry one real correction, recorded because the class recurs:** `spec-error-log.md` **v2.56 was allocated twice** — by PR #379's `ERR-051-001` index-row repair on `main`, and independently by this branch — so this landing renumbers to **v2.57** and demotes #379's entry to `(prior)`; `file-manifest.md`'s two maintained pointers follow. The `ERR-040-003` **id itself** was re-verified free against `main` at `9eff053` after the merge and did not collide, which is the `ERR-030-025` branch-vs-main rule doing exactly what it is for: the id was checked, the **document version** was not, and only one of the two was protected. Index cardinality 237 → **238**.
>
> **The class then recurred within the same PR, which is why it is written up rather than just fixed.** `main` advanced again through **PR #404** (Unity editor compile fixes), a second merge was taken, and `CHANGELOG-src.md` **v2.134 turned out to be allocated twice** — by #404 on `main` and independently by this branch — so this landing's `src/` entry renumbers to **v2.135**. Two document-version collisions in two days, on two different chains, both caught only by a human-eye read of the conflict. `check_drift.sh` verifies that each chain exists, carries exactly one bare `**Last Updated:**` label, and that the open-issues counts reconcile; **it does not verify that a chain's version numbers are unique or monotonic**, and neither does `doc-consistency-check.py`, which cross-checks citations rather than the chain's own sequence. A duplicate-version chain passes every check this repository has. Recorded against the existing `check_drift.sh` blind-spot entry in `open-issues.md` rather than as a new one, since it is the same script and the same append-only-record property.
>
> **What #404 changed that this PR had to be checked against:** Unity now governs the build, and a green Linux gate is explicitly no longer sufficient — Unity bundles NUnit 3.5, so `Assert.Multiple` and the non-string `Does.Not.Contain(x)` overload do not compile there, and every referenced assembly must be listed directly in the `.asmdef`. Verified against this PR's four `src/club-finances/` files rather than assumed: **no `Assert.Multiple`, no `Does.Not.Contain`, and no new cross-assembly type reference**, so `club-finances-tests.asmdef` is unchanged and correct. #404 touched no file this PR touches.
>
...
>
> **Seventh base advance: the version collision recurred a fourth time, and the Unity compile evidence is now supplied.** `main` moved through **PR #413** (W12 evidence repair). It changed only `docs/`, `tools/` and `.gitattributes` — no file under `src/`, `Packages/`, `Assets/` or `ProjectSettings/`. It also took **`CHANGELOG-src.md` v2.137**, the number this branch had used, so this landing's `src/` entry renumbers to **v2.138**. `CHANGELOG.md` and `CHANGELOG-src.md` conflicted additively and both sides are kept. `file-manifest.md` conflicted on the same maintained-pointer line as the sixth merge and was merged line-wise again: this branch's `spec-error-log.md` **v2.58** kept, `main`'s `match-engine-wiring-backlog.md` **v1.17** adopted. **The owner-held Unity evidence recorded above as owed before merge was taken before this merge commit.** On September 14, 2026 the Unity **6000.4.9f1** editor on the pinned Windows host compiled branch head **`3fb0889`** in batch mode (`-batchmode -quit -nographics`, interactive editor closed): `Tundra build success`, exit code 0, **0 `error CS`**. This is a compile, not a test run; no Unity test was executed. Two things were checked rather than assumed. The build was not stale: the compiler response files list the files new since the editor's previous build (`PressingTickDiagnostics.cs`, `W12GateFiringDiagnosticTests.cs`), and every `ClubFinances`, `PressingAI`, `MatchEngine` and `MatchClientCore` assembly and test assembly in `Library/ScriptAssemblies` was written by this run. The working copy was not pristine: it carried the owner's uncommitted `com.unity.ai.assistant` and `com.unity.ai.inference` entries in `Packages/manifest.json` and five Unity-expanded `.meta` files with unchanged GUIDs, none of them C#. Because #413 changed nothing Unity compiles, this merge's `src/`, `Packages/`, `Assets/` and `ProjectSettings/` trees are identical to `3fb0889`'s, so the evidence carries to the merged head without a second compile.
>
> **Last Updated (prior):** September 14, 2026 — **W12 evidence repair closes the post-#398 measurement gate with durable, mechanically reconciled evidence.** The exact Actions archives from runs `34803051927`, `34844425733`, and `34847990460` are committed byte-for-byte with GitHub ZIP digests plus per-extracted-file SHA-256s. The corrected matched three-seed comparison proves W5 producer→consumer wiring: `latestPass` 0 → 141,491 team-heartbeats and BACKWARD_PASS 0 → 1,827 raw / 2,770 committed, while Active 140, InvariantRejected 139,309, NoPrimaryPresser 84, Disengaged 1,890, Cooldown 22,671, and all three scorelines are unchanged. `primaryAssigned +3` / WeakReceiver `−3` is selection-path wiring evidence, not a claimed gate-outcome change. A CI-discovered checker now reconstructs the census from the committed archives, verifies whole-table aggregates and heartbeat accounting, and prevents a transcription-only recurrence. Post-W6 measurement is pre-registered as normalized exit shares with an explicit falsifier. PR #412 remains separate; its MatchEngine `_tackleCooldown` P2 is not the W12 Pressing AI `Cooldown` bucket.
>
...
> **Last Updated (prior):** September 13, 2026 — **The Unity match client runs on the pinned host for the first time: `Scene.unity` is wired to prefabs built to the binding's contract. Roadmap B8 host verification is PARTIAL.**
>
...
>
> - **Renderer.** `GraphicsSettings.asset` and both `QualitySettings.asset` levels referenced three URP asset GUIDs that no tracked `.meta` defines, and no URP package was declared. The editor was already rendering with Built-in, because every reference resolved to null. The references are cleared and the orphaned `URPProjectSettings.asset` is removed. Unity also upgraded both quality levels to serializedVersion 5 on save.
> - **Packages.** `manifest.json` asked for test-framework 1.1.33 while the editor installs 1.6.0 as a built-in package. The committed lock described yet another set, including AI Inference, Multiplayer Center and extra depth-0 modules. The manifest now names what actually resolves: ai.assistant 2.19.0-pre.2, ide.rider 3.0.31, ide.visualstudio 2.0.22 and test-framework 1.6.0. Unity regenerated the lock from it, giving exactly those four at depth 0 and 16 entries in all. AI Assistant stays because it provides the Unity MCP relay the editor workflow uses. AI Inference had no consumer and was the only package pulling in `com.unity.dt.app-ui`.
> - **Input.** Active Input Handling stays at the committed `0`, "Input Manager (Old)", which is what `MatchClientBehaviour`'s legacy `UnityEngine.Input` calls need. A local-only switch to "Both" was reverted, and the Input System package is not added.
> - **CI.** The `unity-tests` job never created the untracked `Assets/Scripts` link that `nightly.yml` creates, so once `UNITY_LICENSE` is configured it would have imported none of `src/`. It now runs `ln -s ../src Assets/Scripts`, and the EditMode leg fails if its results contain no `TacticalDirector.` test. The check is EditMode-only because no project test assembly targets PlayMode. **Neither step has executed:** the job is still skipped without the licence secret. The workflow YAML was not parsed locally (no PyYAML), so GitHub accepting the workflow on push is the check.
> - **Also:** Unity's expanded stub metas for five `src/` files (GUIDs unchanged); the AI Assistant project-settings file and Unity's `SceneTemplateSettings.json`; and the `match-client-unity` README no longer describes the pipeline as unsettled.
>
> Verified in the editor after its restart: `defaultRenderPipeline` and both quality levels report Built-in, `activeInputHandler` is 0, the direct packages are exactly the four above, 71 project assemblies compile through `Assets/Scripts`, and the console shows no errors. `check-meta-integrity.sh` and `check-binaries.sh` pass. No `SNAPSHOT_SCHEMA_VERSION` change; no new RNG stream, domain tag or draw site, and no draw-order change. No source changed, so no scenario tick window or perf baseline can move. **Linux PR gate not runnable on this host:** `tools/run-tests-local.sh --pr` ran its auditors (0 blocking), meta integrity (OK) and project generation (70 csproj, 35 test), then stopped at Restore because only the .NET runtime host is installed, not an SDK. No C# or asmdef changed; CI's `dotnet-compile-test` job runs the gate on push. Unity 6000.4.9f1 editor compile on the pinned host: 0 errors.
...
>
> **The packet defines no new rule.** §1 states that where it and `ux-detailed-plan.md` disagree, the detailed plan wins and the packet is the defect. Verified against the plan rather than taken on report: the two-participant floor, the removed provisional bypass to Gate H/I, the one-round cap and its two exceptions, and the four severity bands all match F4.2/F4.4/F4.5 in substance.
>
> **F4 remains OPEN, and the packet says so in its own §11.** Three human-assignment facts are unfilled and none is invented here: the `ux-detailed-plan.md` §10.1 UX-workstream accountable owner, S0-P1, and S0-P2. `open-issues.md`'s September 9 owner-acceptance record already states those three are F4 outputs and still unassigned; this landing does not move them, does not advance the detailed plan's status line, and does not claim Gate E, F or G. Gate G is unavailable until both participant rows close.
>
> **Twelve review findings were raised across six rounds and all twelve are fixed (v0.1 → v0.5).** **(1)** The §5 matrix silently dropped two of F4.6's nineteen binding test-data profiles — `no save` and `save/load failure where relevant` — behind a generic error row, while §11 claimed the walkthrough was READY; §6.3's Gate-A audit names the current save/new-game limitation explicitly, so the gap was not hypothetical. §5 now covers all nineteen in F4.6's own order. **(2)** No task exercised the Tactics Setup cancel/back edge that §6.4 names and Gate F requires prototyping; **S0-T7** now does, and is wired into §7.2, the §9 record and the keyboard-only/mouse-only rows. **(3)** The file declared `Version: 0.1` with no version history; §12 now carries one. **(4)** S0-T1 assumed a working New Game path and is narrowed so the current start limitation is recorded as a dependency rather than scored as participant failure. **(5) The v0.2 fix for (1) introduced a narrowing of its own:** it *replaced* the original generic error row with `Save/load failure where relevant` instead of adding alongside it, so §5 no longer covered F4.3's wider `disabled/error states` condition. v0.3 restores a general `Error/failure state` row and makes the non-substitution note symmetric in both directions. **(6)** Color-independent meaning was coupled to the `Many status indicators` stress row, so marking that density profile `N/A` could silently waive Gate E's requirement; v0.4 splits it into a standalone condition. **(7)** An `honestly testable` qualifier let a prescribed task disappear from Gate-G scoring; an untestable prescribed task now fails Gate F/G and returns the prototype to Gate F, consistently at all three sites (§3.1, §6.3, §9), citing §5 Gate F as the authority it operationalizes rather than legislating a new criterion. **(8)** Gate E had no per-condition record, so a skipped row was invisible; every condition now carries `PASS`/`FAIL`/justified-`N/A`, prototype/version and evidence, with blank fields explicitly preventing a Gate-E pass. **(9)** Gate E had no contrast check, although `ux-shared-system.md` **F3-001** states that mockup tokens are references, not compliance evidence, and that Gate E must prove contrast; a standalone contrast row now demands its own verification evidence and says tokens do not count as it. **(10)** §7.3's ledger carried five columns against the eight fields `ux-detailed-plan.md` §12 requires; journey, gate, release condition and retest result are restored, with §12 named as the field authority. **(11)** That restored `Disposition` column cited §12 without carrying §12's **closed five-value vocabulary** — `FIX NOW`, `ACCEPT FOR CURRENT GATE`, `DEFER TO P2/P3`, `BLOCKED BY DOMAIN/CLIENT IMPLEMENTATION`, `INVALID / NOT REPRODUCED` — leaving a runner to invent free text; §8 now states all five and that no free-text value is valid. **(12)** The packet was S0-only although F4 governs S0 **and** S1; **S1-P1/S1-P2** slots, **S1-T1–T8** mapped one-to-one onto the plan's §7.6 task list, S1 evidence rows and a journey-parameterized Gate-G record are now defined. §11 scopes the S1 participant slots as S1-Gate-F prerequisites, **not** new F4 exit blockers — F4's own exit criterion names S0 only.
>
> **Findings (5), (11) and (12) are the ones worth carrying forward, because each was created by the fix for an earlier finding.** (5) narrowed §5 while repairing §5; (11) added a field without its vocabulary while restoring the field set; (12) became visible only once the S0 half was complete enough to show the asymmetry. A fix that lands one round and is refuted the next is the normal shape of this document's convergence, not an anomaly — which is why the round count is published rather than smoothed into "reviewed and corrected".
>
> **A near-miss on this landing's own tracking half is recorded as a new open issue rather than quietly repaired.** While syncing these findings, an automated edit replaced `CHANGELOG.md` wholesale: commit `85911f7` wrote 9 lines and **deleted 5,669**, destroying this entire header chain — 234 prior entries, every measured result and recorded null finding since July 31, 2026. `d462dd4` reverted it to a **byte-identical** blob (`6a1af4dc280004092c04b249b8b3ce8d0f16b50b`, verified by blob hash against `f4f6533`, not by line count), so nothing was lost — but only because the pre-write state was already committed. **No existing check would have caught it:** `check_drift.sh` verifies chain existence, exactly one bare `**Last Updated:**` label, no chain in root `CLAUDE.md`/`README.md`, and the open-issues counts; a 9-line file with one bare label passes all four, and `doc-consistency-check.py` cross-checks version citations and cardinalities, not chain length. The chain's own length and prefix-preservation are unguarded, which is exactly the property an append-only record needs. Filed to `open-issues.md` (active **23 → 24**) with the guard shape and the read-modify-write rule for append-only tracking files. **Deliberately not fixed here:** the guard is a `.claude/skills/` tooling change and this PR's stated product boundary is documentation under `docs/design/`.
>
> **A pre-existing manifest gap is corrected in the same commit rather than widened.** `file-manifest.md` carried **zero** `docs/design/ux-*.md` rows, although `open-issues.md`'s September 7 updates twice state that `ux-baseline-evidence.md` and `ux-experience-architecture.md` are "inventoried in `file-manifest.md`". They were not, and neither were the high-level plan, the detailed plan, the shared system, or the superseded foundation stub. Landing a sixth uninventoried UX document under a claim that the fifth was already inventoried is the drift `9af9626` exists to clean up, so a **UX Workstream Documents** section now lists all six with their current versions. Those five rows record files that landed earlier; only `ux-validation-protocol.md` is new here.
>
> **Deliberately not done.** The UX entry in `open-issues.md` is **not** advanced — its status (F0–F3 complete, F4 next) is unchanged by this landing, and the three F4 human assignments stay open; the only `open-issues.md` movement is the new tracking-toolchain entry above, which takes the `project-reference.md` count to **24 active / 54 archived**. `src/CLAUDE.md` and `CHANGELOG-src.md` are untouched: no `src/` file changed. `README.md`'s status snapshot is untouched — no production-assembly count, assembly-less figure, Unity target or governance stage moved. No `SPEC_INDEX.md` row, no `ERR` filed (the packet contradicts no APPROVED spec; it is a design document under `docs/design/`, not a spec).
>
...
>
> **The correction is the load-bearing half.** `match-engine-wiring-backlog.md` **v1.13 → v1.14**. Since v1.0 the W4 entry had closed *"The keeper is simply not on that path."* Direct source read refutes it: `MatchEngine.cs:3074` runs `_perception.OnHeartbeat` over the full `SQUAD_SIZE` including both keepers (keeper index == team id, `:922`), and `BallPerceptionEvaluator` already computes an occluded ball visibility for each of them every heartbeat. **Nothing needs putting on the perception path.** W4 is a *downstream-consumption and tick-order* problem, and the old framing pointed an implementer at a seam that does not exist. Three constraints are now recorded, and **the first two make the obvious implementation a regression rather than a fidelity gain**: (1) `FilteredView.BallVisible` is `range AND FoV AND not-occluded`, and keeper facing is `AUTO_ALIGN` — velocity direction, else the movement-target vector, else unchanged when stopped (`AgentMovementSystem.cs:454-465`) — so against an 80° base half-angle (`PerceptionConstants.cs:57-65`) a keeper settled on his #12-composed slot has a shot from in front of goal *outside* his cone, and gating the save on `BallVisible` would blind keepers to ordinary **unobstructed** shots while presenting as a working occlusion feature; **how often is UNMEASURED**. (2) `SaveArmed` has **two** production callers, not one — `MatchEngine.cs:3311` (the DT-emitted-SAVE gate in `RunMechanicsAI`, entered at `:3046`) and `:4461` (the W1 rush exclusion via `DriveGkHeadingTactical` at `:3072`) — and **both precede** the `:3074` heartbeat, so a view read at either site is the *previous* heartbeat: 100 ms stale at 10 Hz, the same order as the reaction latency W4 exists to model. (3) `:4457-4460` states the invariant that the two sites share **one** predicate, or the save/rush exclusion drifts into the ERR-011-007 shape. **No ERR filed**: the backlog is an AUDIT document, not an APPROVED spec, so the correction has no spec to back-propagate into. **The §5 wire-order table is deliberately untouched** — a revision (W12 split into a targeted runtime census **W12a** and the static written-but-never-read sweep **W12b**; W6 ahead of W4) is under discussion and **not adopted**.
>
> **The lane, and the defect review found in it.** New `.github/workflows/measure.yml` (`workflow_dispatch` only, `permissions: contents: read`), `tools/dotnet-ci/measurement_catalog.py`, `tools/tests/test_measurement_lane.py`. **The first cut of this workflow was wrong in a way worth recording, because it is the same failure class the whole wiring backlog is about: a surface that reports success without doing the work.** It took an env switch and a free-text VSTest filter as *independent* operator inputs, checked only that the filter was non-blank, and treated `run-gate.sh` exit 0 as success. Two paths therefore produced a green run, a `Gate PASSED` line and an uploaded artifact containing **no measurement at all**: a filter matching zero tests (VSTest returns success by default), and a mismatched env/filter pair, where the instrument `Assert.Ignore`s itself because its own gate is absent. `run-gate.sh:268` compounds it — `Gate PASSED` is an unconditional `echo` with no executed-test or output postcondition. **Closed by making evidence the success criterion rather than exit status:** the operator now picks a **canonical instrument id**, not two independent values; the catalog resolves it to one env/source/project/filter/sentinel tuple and `validate` re-checks every tuple against tracked source so a renamed test or reworded report fails loudly; `TreatNoTestsAsError=true` is applied **scoped to the owning project**, never solution-wide, where it would fail every unrelated project with zero filter matches; the run records TRX beside detailed console; and `verify-run` requires a `Passed` TRX outcome **and** the catalogued report sentinel in the captured output. The whole-tree build gate runs first with the diagnostic env **removed**, so it is structural evidence and cannot double-execute the expensive instrument.
>
> **Blast radius: checked, nothing moved.** No `src/` file, no scenario, no tick window, no per-90 band, no `[GT]`. No `SNAPSHOT_SCHEMA_VERSION` change, no new RNG stream, domain tag or draw site, and no draw-order change. No per-tick work added, so the FR-PO-052 perf baseline is untouched. No calibration invalidated. **`run-gate.sh` is not modified by this branch at all — it is byte-identical to `main`.** An earlier cut added a `--test-logger` flag to it; the hardened workflow then moved the measurement leg to a direct project-scoped `dotnet test` with two loggers, which left that flag with **zero production callers**, and it was removed at review rather than kept. Recorded because the temptation to keep it was real and the argument against it is the whole point of this landing: a surface with no caller is exactly what `match-engine-wiring-backlog.md` exists to catalogue, and shipping a new one in the same commit that corrects that backlog's own W4 entry would have been the joke writing itself.
>
> **A standing environment claim is corrected.** The W1 landing recorded *"the authoring environment has no .NET SDK and the agent proxy denies the installer"* as its reason for shipping unmeasured. That is **too broad**. The proxy does deny `dot.net` / `builds.dotnet.microsoft.com` (`connect_rejected`, re-verified), but the **Ubuntu archive route works**: `apt-get update && apt-get install -y dotnet-sdk-8.0` yields a working **8.0.131**. `tools/dotnet-ci/README.md`'s manifest row had already scoped the finding correctly and was not read. **The trap: `apt-get install` alone 404s against a stale index and reads exactly like the package not existing** — `apt-get update` first is the entire fix. Corrected in place in `open-issues.md`, the original left standing as the record of why W1 shipped unmeasured. **Consequence:** the still-owed W1 close-range **conversion** pre/post pair on identical seeds (`gk-rush-trigger-design.md` §6 — the release condition for the parked `pointQuality` decision, KD-CC6a) is runnable now and is not waiting on tooling.
>
> **Gate: PASSED, and the lane's guarantee is proven by three controls on the real path, not by unit tests alone.** Whole-tree `generate → build` 0 errors. **Positive:** `TD_CONVERSION_DIAGNOSTIC=1`, instrument's own project, `TreatNoTestsAsError` runsettings, console+TRX loggers → `dotnet` exit 0 and **`verify-run` PASS** (passing test + report sentinel). **Negative A — the env/filter mismatch class:** same command with the env gate removed → the instrument `Assert.Ignore`s, **`dotnet test` still exits 0**, and `verify-run` **rejects it (exit 2)**: *"did not execute a passing test; ignored/not-executed-only runs are not evidence"*. **Negative C — measurement lost after a passing test:** the positive run's own TRX with the sentinel line stripped from the captured output → `verify-run` **rejects it (exit 2)**: *"measurement test passed but expected report sentinel was absent"*. So the guard catches exactly what `dotnet test`'s exit status does not. Also green: **270 tooling tests + 32 subtests** (`python3 -m pytest tools/tests/`, +9 over the pre-hardening 261), `measurement_catalog.py validate` — **11 instruments valid** against tracked sources, `python3 tools/doc-consistency-check.py --repo .` **PASS**, `recurring-defect-lint.py` clean, `yamllint` under CI's own config clean. **Not yet run: full CI**, which requires a PR — `ci.yml` triggers on `pull_request` to `main` only. **Known constraint: GitHub offers `workflow_dispatch` only for workflows on the default branch, so `measure.yml` is not dispatchable until it is on `main`.** No measurement is claimed from a dispatched run yet.
...
> **A fixture the correction landed red, now repaired.** `SeasonSaveManagerTests`' `TwoClubFinances` helper described clubs `{3, 7}` while every season it was saved against carried `{10, 11, 12, 13}` — legal under `ERR-030-049`, refused the moment `SeasonFinanceCoherence` made the finance set's club universe a checked property. The functional correction therefore reached CI with `Save_Load_RoundTripsTheFinanceBlockThroughTheFile` and `Save_CarryingNoFinances_CannotEmptyAPopulatedFinanceBlock` already failing, and the earlier "green everywhere completed so far" reading of that head had simply not reached the functional gate yet. The fixture is now `SeasonClubFinances`, four entries over `SeasonClubs`, still handed over in descending ClubId order and still carrying negative fields, with the ascending result asserted position by position. Test-only; no production C# changed by the repair.
> **Evidence discipline.** The whole-tree gate at `899bd9e` predates the corrected functional commits and remains historical evidence only. It is not claimed for the corrected head; fresh checks are recorded only from commands actually run after this documentation patch.
>
> **Last Updated (prior):** September 10, 2026 — **#40 Club Finances T1b: the finance sub-blob joins the season-save envelope, and #30's frame goes v6 → v7 (`ERR-030-049`).**
> **What landed.** #40's persistence stops being standalone. `SeasonSaveCodec` now frames a mandatory `FNCE` sub-blob between the #44 discipline block and the optional match block; `SEASON_SAVE_FORMAT_VERSION` bumps **6 → 7** with no migration (F3 refuses every older frame, as it has at every prior bump); `SeasonSaveManager.Save` takes a required `ClubFinanceEntry[]` on its public long form and `Load` returns the restored set on `SeasonSaveContents.Finances`. `season-save.asmdef` gains `TacticalDirector.ClubFinances` — an intra-Tier-7 edge, permitted by Code Standards #20 §3.5.2, leaving #40's own reference set untouched and its #27 edge still deferred to T2. This is the first phase in which the cross-assembly save/restore acceptance cases can actually run, which is what §7.1 said T1b was for.
> **A guard landed ahead of its producer, deliberately.** `RequireDestinationCarriesNoFinances` is the fourth of #30 Appendix B.1's destination guards, and the first written at its block's landing rather than after a measured loss — its three siblings (`ERR-028-008`, the career-triple guard, `ERR-030-036`) were each written after a career's state had already been deleted with every gate green. It keys on **emptiness** rather than on a wiring flag, and the reason is a real distinction rather than a shortcut: FR-FN-025 makes a club's finance entry permanent from the moment `CreateInitial` creates it, so a destination that carried entries and now carries none has lost them. `ERR-030-038` had to re-key #44's guard precisely because FR-DC-017 makes #44's empty tally canonical; #40 is on the other side of that line.
> **Two of Appendix B.1's own rules needed repair to state that correctly.** Its emptiness-vs-wiring key was phrased as a predicate on *this save's* set, which puts #40 on the wrong side — a finance-less save is not merely legal, it is the only thing a save can carry until T2 bootstraps the entries. The property that matters is a predicate on the **destination**. And its `SeasonLoop.Restore` expressibility MUST, written from `ERR-030-036` where #44 had a live driver mutating its tally, read literally as condemning a frame-only landing with no producer at all — where no `Restore` argument could be passed that is not already the default. Both are amended in `appendices.md` v1.6 under the same ERR.
> **A stale checklist corrected.** #40 §9.2 had published `TacticalDirector.ClubFinances` and `ClubFinancesSaveCodec` as **NOT STARTED** since PR #363 merged on September 6 — six days of a spec contradicting shipped code. Rewritten with a per-row verifying source anchor rather than a bare checkbox, per root `CLAUDE.md`'s never-fabricate rule.
> **Deliberately not done.** T2's `SettleFinances` at step (b′), `CreateInitial` at league bootstrap, the `SeasonLoop.Restore` `financesOrNull` seam, and the `PlayerDatabase` edge — all still deferred, and `Save(SeasonLoop, …)` passes `Array.Empty` until they land. No RNG stream is registered, no draw site added, no snapshot schema or tick order touched: #40 draws nothing at the minimal tier (KD-2), so `_RESERVED_0x29_` / ordinal 91 stay reserved.
> **Blast radius.** `src/season-save/` production files and their tests; `src/club-finances/` is byte-unchanged. Save-file compatibility: any v6 file is refused fail-loud on load, which is the standing Stage-0 posture.
>
> **Last Updated (prior):** September 10, 2026 — **Localization T0 scaffold governance repair (PR #389). PR #380 merged `codex/localization-infrastructure-t0` even though the subsequently-landed authoritative `localization-implementation-plan.md` v2.5 records that scaffold as non-authoritative, says it must not be merged, and requires L1 to be built fresh. PR #389 exactly reverts #380 after proving all 22 affected paths were untouched by the 19 intervening commits: the competing `localization-t0-implementation-plan.md` and `src/localization/` scaffold are removed, Code Standards §3.5.2 returns to its pre-#380 state, and the README production-assembly count returns 37 -> 36. No later work is overwritten; historical tracking entries are not rewritten. PR #388 was closed unmerged because it built additional unauthorized behavior on the reverted scaffold. Localization resumes from the v2.5 authority at L1.**
>
> **Last Updated (prior):** September 9, 2026 — **The localization execution plan reaches `main` on PR #382 as v2.5, closing the discoverability gap the L3A landing recorded but could not close. The `<L3A_MERGE_COMMIT>` placeholder is resolved and two further pre-T0 spec defects are recorded. Documentation only; a stray header label from the PR #370 close-out is repaired.**
> **What this closes.** The PR #370 close-out stated plainly that `docs/tracking/localization-implementation-plan.md` — the execution authority for #49 — lived only on the unmerged `codex/localization-planning-v2` branch and was therefore unreachable from `main`, and that restating its slice cut in a `main`-side document would have created a competing execution authority. This lands the real document instead of a paraphrase of it: 803 lines defining §4 L3A, §4.4 the three mandatory mutation classes, §5 L1 T0 core, §6 L2 renderer, §7 L3B retrofit, §11 the slice order **L0R -> L3A -> L1 -> L2 -> L3B -> L4+**, §12 verification, and §14 the implementation-ready exit criteria.
> **The frozen-oracle acceptance rule is now runnable rather than a template.** v2.2 could only write `<L3A_MERGE_COMMIT>` because L3A had not merged. It has: PR #370 merged into `main` on September 9, 2026 as **`67f2343c34e767ba02a4dc13816c91090b3bf3d9`**. §7.7 and §12 now carry that literal, plus the explicit warning that the merged branch tip `79543a5` is *not* the value to use — a distinction the plan itself insisted on ("use the actual merged L3A commit, not a branch tip") and which a reader would otherwise have had to resolve by guessing. The L3B gate is `git diff --quiet 67f2343… HEAD -- src/living-world/Tests/InteractionTextOracleExpectations.cs`, two-dot, non-zero blocks L3B. Verified at authoring time that the file is byte-identical at `67f2343`, at `79543a5` and on `origin/main` (blob `0907c88b312e43e07363abb847fc3fae6a475b05`); that blob hash is recorded as informational corroboration, deliberately **not** as a second gate, since adding an acceptance criterion is not this landing's business.
> **v2.2's hold on L1 is discharged, with the evidence named.** v2.2 held L1 until L3A had both positive CI and negative-control evidence. Both are complete: PR #370's head `79543a5` passed the canonical gate (run `34308477489`, `Gate PASSED`, quarantine empty, the single failure the recorded owner-held RED `sim_match_engine_close_chance`, inherited and not rebaselined), and disposable negative-control PR #371 — closed unmerged — produced 9 `LivingWorld.Tests` failures in run `34263162049` across all three §4.4 mutation classes. The narrowed claim is carried forward rather than smoothed over: `Generator_EveryCitationClause_IsFrozenEndToEnd` diverged at index 12 on the row-text mutation, so that one end-to-end assertion evidences the row-text class only; the clause-punctuation class rests on `Corpus_EveryCitationClause_IsFrozen` (`EventKind` ordinal 1, index 50). The classes are all covered — no single assertion covers all of them.
> **A defect from the previous landing is repaired here.** The PR #370 close-out commit `35932ba` prepended a header entry to `file-manifest.md` without relabelling the previous newest entry, leaving **two** bare `**Last Updated:**` labels in that file — the recurring header-chain defect this project has now fixed several times. It shipped because `check_drift.sh`'s duplicate-label class covers `CHANGELOG.md` and `CHANGELOG-src.md` only and does not police `file-manifest.md`; the check reported exit 0 on a genuinely broken chain. The contentless duplicate of the September 6 `#40 Club Finances` line is removed — that entry survives intact, with its content, as a `(prior)` entry further down, and no historical entry is rewritten. **The tooling gap is recorded, not fixed**: extending the checker to the manifest is a `.claude/skills/` change and does not belong in a documentation-only planning PR.
> **Blast radius: none.** No `src/` file, `.asmdef`, asset, snapshot/save schema, RNG stream, domain tag, draw site, draw order, `SPEC_INDEX.md` row, or spec status changed, so `CHANGELOG-src.md` and `src/CLAUDE.md` are deliberately untouched. `open-issues.md` and the `project-reference.md` count are untouched — this landing opens no new blocker and closes none. Spec #49 remains APPROVED-but-unimplemented: no localization assembly exists in `src/`, and `codex/localization-infrastructure-t0` remains non-authoritative and must not be merged, which landing this plan does not change.
> **Two further pre-T0 defects in APPROVED #49 are recorded here, not fixed (v2.4).** Review of the v2.3 draft raised two findings against §6, both verified against the spec rather than taken on assertion. **(1) §3.3, proposed `ERR-049-004`:** FR-LC-009 mandates a bounded plural/gender category selector, while §2.2 defines `NamedSlotSet` as a name→string map whose values are "ALREADY formatted to a string" and gives `LocalizedTextRequest` no selector operand — a CLDR plural category needs a numeric operand plus locale rules, so the requirement has no input. **(2) §3.4, proposed `ERR-049-005`:** FR-LC-011 says a missing key renders "the base-locale identity" and must not crash, but FR-LC-008a/§2.3 F5's loud construction coverage is enumerated over the *procedural rosters only*; for a **static** `LocalizationKey` absent from both catalogues the instruction is circular and the production terminal result is undefined. Base English declares no plural categories and nothing before L2's synthetic catalogues exercises the selector, which is why both survived approval.
> **The disposition, and a correction to the one first proposed.** The initial reading — leave this plan unchanged and let PR #372 absorb both — was wrong: **C6 requires known defects in an assembly-less spec to be recorded**, and recording a finding with a named discharge stage is not the pre-T0 hardening C6 forbids. Landing the authority with L0R still scoped to two defects when four are known would have made it stale at birth. L0R therefore grows from two entries to four across §3.5, §3.6, §11 row 0 and §14 criterion 6. **`ERR-049-004` is assigned to L1, not L2** — a locale-neutral typed operand must live on `LocalizedTextRequest` or `NamedSlotSet`, both in §2.2's CORE block, and §5 is the commit that freezes them; deferring would land an incomplete T0 seam and then change it. `ERR-049-005` discharges in L2, conditional on a signature question that §5.1 and §5.5 force to be settled **before** L1 freezes `ILocalizer`. Executable exit evidence is named for both (§5.5, §6.6) so neither closes by assertion. **No normative fix is chosen and no FR or KD wording is altered** — that is the discharging slice's work. No ERR id is allocated here; L0R allocates and re-verifies each is free at filing, per the `ERR-030-025` precedent.
> **A second review round (v2.5) caught two follow-through failures in v2.4, both now fixed.** Recording the defects was not enough on its own. **(1)** §5.2 still listed the L1 public surface as "exactly `Resolve`/`Render`", `NamedSlotSet` as name→already-formatted-string and `LocalizedTextRequest` with only the old fields — the very shapes §3.3/§3.4 require L1 to change. Landing it would have frozen the defect the same commit is meant to discharge; those three bullets are now marked **subject to** the recorded L1 decisions. **(2)** §5.5 asked L1 to prove plural selection with a synthetic non-English catalogue, but the renderer, catalogue and selector are all L2 work, so L1 could not have run that proof without pulling L2 forward. The evidence is split at the slice boundary: **L1** proves the operand *exists* with the required properties (§5.3) and closes `ERR-049-004` there, since the recorded defect is that the seam supplies no operand; **L2** proves it *drives* `one/few/many/other` selection (§6.6), recorded as FR-LC-009 conformance rather than as the ERR's discharge. §2 H1 and the §3 preamble, which still said two known defects and "file both entries", now say four.
> **What is still open.** This lands a plan, not an implementation. L0R (PR #372) and then L1 are the next slices; **PR #372 currently records only `ERR-049-002/003` and needs the two new entries before it lands.** L3B's dependency `L2 + merged L3A` is now half-satisfied. Where this plan and APPROVED #49 differ, the spec wins.
>
> **Last Updated (prior):** September 8, 2026 — **Localization L3A landed on PR #370: the pre-migration English oracle is frozen and PROVEN non-vacuous. Test-only; no production code changed.**
> **What this is for.** L3A is the oracle slice of the #49 localization work: capture the current authored-English surface byte-exactly *before* anything moves, so the later retrofit can be shown not to have altered player-visible text rather than reviewed and hoped. New: `InteractionTextOracleExpectations.cs` (frozen data), `InteractionTextOracleTests.cs` (the replaceable harness), `InteractionTextOracleRosterTests.cs` (the enum-roster lock), plus `.meta` files. `LivingWorld.Tests` **119 -> 129** (+10), measured by direct execution on both `main` and the branch.
> **The expectations file is deliberately migration-independent** — it names no `InteractionTextCorpus`, `InteractionTextGenerator`, `InteractionIntent`, `EventKind`, or future localization type, so the retrofit may repoint the harness but must not edit the frozen data. That rule is not this landing's invention and is not restated as a new one here: it is already a **mechanical acceptance rule in the governing plan** (see the governing-document note below), which requires a two-dot `git diff --quiet` from the merged L3A commit proving the file unchanged.
> **Non-vacuity is proven by execution, not asserted.** Disposable negative-control PR #371 (`codex/localization-l3a-mutation-probe`, head `ed6bcd2`, **closed unmerged**) mutated all three classes the plan's non-vacuity clause names — a template row changing, a row reordered/deleted, and clause punctuation/spacing changing — and run `34263162049` failed on all three: `LivingWorld.Tests` 120 passed / **9 failed** / 129, every other assembly green. Row text by `Corpus_ExactRowCountAndOrder_IsFrozen(1)` (index 12, `press` -> `quiz`); row order by `Corpus_ExactRowCountAndOrder_IsFrozen(2)` (the two `PlayerQuestionsMinutes` variants swapped); clause punctuation by `Corpus_EveryCitationClause_IsFrozen` (`EventKind` ordinal 1, index 50, `.` -> `!`). **Recorded against the evidence rather than the claim:** `Generator_EveryCitationClause_IsFrozenEndToEnd` also failed, but at index 12 on the *text* mutation, so it evidences the row-text class only — that run does **not** demonstrate end-to-end sensitivity to the punctuation class, and the PR body's flat "all three classes" reading is narrowed here. The plan requires the mutation evidence be preserved outside the disposable branch before it closes; it is, on PR #370, with the full failure-to-class map and verbatim assertions.
> **Governing document, and a discoverability gap this landing does NOT close.** The execution authority for this work is `docs/tracking/localization-implementation-plan.md` (**v2.2**, `codex/localization-planning-v2`; a `Last Updated: September 4, 2026` revision also sits on `codex/localization-planning`). Its §4 defines the L3A slice and §4.5 its exit criteria, §4.4 the non-vacuity classes, §7 the L3B living-world retrofit, and §11 row 4 the frozen-file zero-diff acceptance rule. **That plan is not on `main`** — it lives only on those unmerged planning branches, so nothing reachable from `main` describes the slice order (L0R -> L3A -> L1 -> L2 -> L3B -> L4+, with the Wave-8 content/a11y tier a separate deferred lane) that this landing is one step of. **No parallel plan is written here on purpose:** restating the slice cut in a `main`-side document would create a second, competing execution authority, which is worse than the gap. Closing it is the planning branches' disposition to settle, not this landing's.
> **Blast radius — one real item, and it is deliberate.** `ProducerEnumRosters_AreFrozenUntilL3B` pins `InteractionIntent` at 4 and `EventKind` at 7 members, so **appending to either enum before the retrofit lands will now fail the suite by design** — that is the point (content added between capture and migration would otherwise escape the oracle), but it is a live constraint on production, not merely on tests. Checked and nothing else moved: no scenario tick window or per-90 rate band is touched (this is `living-world` text, not the match engine), no A4a round-resolution refit is invalidated, and no per-tick work is added so the `FR-PO-052` perf baseline is unaffected.
> **Close-out surfaces, including the two deliberately skipped.** `src/CLAUDE.md` is **not** edited: the compact restructure left it carrying no inline version marker, so its version bump exists only as the `CHANGELOG-src.md` v2.128 -> **v2.129** entry and VERSION HISTORY row, which are here. `localization-seam-template-design.md` is **not** touched either — L3A changed no design decision, seam, or scope boundary in it, and the landing narrative belongs in this chain rather than in a design document.
> **Determinism declaration:** `SNAPSHOT_SCHEMA_VERSION` unmoved; no new RNG stream, domain tag, draw site, or draw-order change; no save-format or assembly-graph change; no spec status or `SPEC_INDEX.md` row moved. Spec #49 remains APPROVED-but-unimplemented — there is still no localization assembly in `src/` and no `ILocalizer`/`LocalizationKey` anywhere in code.
> **Gate: PASSED on the code head `3955fae` — CI run `34263115953`, whole tree, quarantine empty.** 36 suites, 3,337 passed / 217 skipped / **1 failed**, that one being the recorded owner-held red `sim_match_engine_close_chance` (`OWNER-HELD RED MATCHES RECORDED BASELINE`), not rebaselined here. `LivingWorld.Tests` **129 / 0 / 0**; `MatchEngine.Tests` 472 / 0 / 11 (1 h 13 m); `SeasonSave.Tests` 447 / 0 / 3. Reproduced locally at `LivingWorld.Tests` 129 / 0 / 0 against the same head. **This close-out commit is documentation-only and does not re-run the gate**; the numbers above belong to the code head, and CI re-runs on the doc commit.
>
> **Last Updated (prior):** September 7, 2026 — **Audio G0 accepted by owner and all maintained acceptance surfaces synchronized; still zero audio code and zero audio assets.**
...
> The audio workstream now has governance and nothing else. New: `docs/planning/audio-implementation-plan.md` **v1.4** (seven gates G0–G6 plus two external dependency gates D48/D49, production lifecycle, rights/provenance, provisional-asset blocking, PR boundaries) and `docs/planning/path-to-playable-audio-parallel-amendment.md` **v0.4**. **G0 is OPEN pending owner acceptance** — this landing authorizes no implementation.
> **The recorded defect, and why recording it is the whole point.** #51 §2.2's `CueEntry` names two **#51-owned** types — `AssetRef` and `CueParams` — that **§4.2's file inventory never declares and the spec never shapes**; and because both are missing from §4.2's CS0104 pre-check enumeration, that pre-check, which asserts every #51 name *"was checked against every name that could be in scope with it"*, has a **two-name hole**. Filed as **`ERR-051-001`** (`spec-error-log.md` **v2.50 → v2.51**), status **RECORDED — OPEN**. Path-to-Playable **C6** governs the rest: #51 has no `src/audio/` assembly, so the normative §2/§4 correction, the concrete types, the collision re-scan and a regression proof that fails when reverted all land **with #51 T0, in one commit**. Writing two invented shapes into approved prose now would be the `ERR-008-021`/`-022` class again — edited text with no compiler and no test behind it. C6's other half **is** discharged here: `audio-sound-design/section-7.md` **v0.1 → v0.2** carries risk **R-7** and a new **§7.5 Pre-T0 recorded findings** table, so a T0 implementer reading only the spec cannot miss the finding. The id was **re-verified free against `main` at `4c07393` and re-verified free again at `9fbd753` after `main` advanced (PR #375), which is the `ERR-030-025` case arriving live rather than in principle — the log moved underneath an open branch, so the check was re-run rather than trusted; it must be re-run once more at the actual merge head**, and per **`ERR-030-025`** — four id collisions in one day, one of them between a branch and main — it must be re-verified again at merge; a verified id is not a reservation.
> **Two design decisions worth the record.** **(a) Variant selection is host-owned.** An earlier revision put a seeded display PRNG inside `TacticalDirector.Audio`; that would have placed mutable state in a leaf assembly whose entire discipline is construction-time refusal over immutable value types, and introduced a type §4.2 does not declare — the same defect class as `ERR-051-001`, deliberately, one tier later. `AssetRef` now exposes the variant **set**, the Unity host binding picks the member with a client-local non-serialized display PRNG, and **#51 declares no PRNG, seed, cursor or selection state at all**. Consequence added to the T0 discharge scope: `AssetRef` must carry a set, and the regression proof must cover the multi-variant case. FR-AU-033 is satisfied identically. **(b) `AudioMixer` is a live CS0104 hazard** — #51 declares `TacticalDirector.Audio.AudioMixer`, Unity declares `UnityEngine.Audio.AudioMixer` — harmless inside the Unity-free leaf, real in the host binding, where the plan pins fully-qualify-from-first-draft, the discipline §4.2 already specifies for the two `CueParams`.
> **Roadmap.** §6's *"deliberately deferred past PM-3"* line (`path-to-playable-roadmap.md` **v0.23 → v0.24**) now points at the amendment. The deferral **stands for the critical path**; what changed is that the line never distinguished "off the critical path" from "must not be worked concurrently", and the September 4, 2026 owner directive authorizing parallel audio / UI / UX / localization / art-pipeline work made that gap load-bearing. **#48 is authorized by none of this:** it has no `src/` assembly, so #51 T2 and the G3 audible slice are hard-blocked on `D48`, captions on `D49` (#49 likewise has no production assembly), and the amendment explicitly forbids inventing or stubbing #48 inside #51 or the shell to unblock audio.
> **Determinism.** No `SNAPSHOT_SCHEMA_VERSION` change, no new RNG stream, domain tag, draw site or draw-order change, no save-format change — and none is possible from this landing, because **no `.cs`, no `.asmdef` and no asset exists in it**. The host-side display PRNG decided above is client-local and non-serialized by construction; the P4A observer-neutrality lock (byte-identical digest chain, audio adapter enabled vs no-op sink) is scheduled at the first wired landing, ahead of any audible output. `SPEC_INDEX.md` is untouched — #51 stays APPROVED — and the build stays silent (FR-AU-038).
> **Blast radius: checked, nothing moved.** No scenario tick window, per-90 rate band, calibration corpus or `FR-PO-052` perf baseline is touched by a documentation-only landing that adds no per-tick work.
> **Gate: not run, and not applicable — no compilable change exists in this landing** (no `.cs`, no `.asmdef`, no project file). Restating a previous landing's green would be dishonest. What was run instead: `python3 tools/recurring-defect-lint.py --repo .` → **0 ERROR** tree-wide (123 WARN / 27 INFO, all pre-existing), and `.claude/skills/landing-close-out/scripts/check_drift.sh` → chains single-labelled and open-issue counts agreeing at **22 active / 53 archived** after this entry's own filing. The `dotnet` gate becomes meaningful at #51 T0, which is where G2 requires it.
> **Deliberately not done:** no bulk audio production before G3; no middleware adopted without G3 host evidence (native Unity is the default through Early Access); no prose-only repair of `ERR-051-001`; no #48 or #49 work pulled forward. *(Noted, not fixed: the roadmap's version table carries a duplicated version number — two separate rows share one identifier — and `spec-error-log.md`'s header chain carried two bare `**Updated:**` labels — the stray August 18 one is relabelled `(prior)` here, a label correction only, its text preserved verbatim.)*
> **Last Updated (prior):** September 6, 2026 — **#40 Club Finances T0 + T1a landed on PR #363; external-review correction cycle closed.**
...
> Claude's external review found that the branch had already crossed from T0 into persistence while the spec/PR still denied T1 work. #40 §7 now defines **T1a standalone codec** and **T1b #30 composition + outer format bump**. The unused `PlayerDatabase` asmdef edge is removed until T2's first `Squad.ClubId` consumer. Added locks cover exact current references, no RNG cursor/action ordinal, the upper budget clamp, non-positive board-modifier failure, decode ordering, and short/truncated framing. The shared `GameplayConfig.GetInt` Int32 tuning ceiling for long finance fields is documented rather than hidden.
...
>
> **Last Updated (prior):** September 5, 2026 — **Art-pipeline planning PR #360 grounded and closed out for owner acceptance; documentation only; G0 remains CLOSED.**
> **The plan now fits the project it governs.** New `docs/planning/art-pipeline-foundation.md` **v0.5** inherits the July 25 `touchline` choice in `docs/design/ui-mockups/README.md` instead of reopening visual direction, treats APPROVED UI / Client Framework #38 as the architectural authority, and names the actual client surfaces (`src/ui-framework/`, `src/match-client-core/`, `src/match-client-unity/`, `src/match-client-web/`, `src/client-app/`). The first draft's premature `Assets/GameArt/` / `art-source/` companion files were removed from the effective diff rather than legitimized ahead of G0. `.art.json` is the one initial production/provenance format, routine `_v001`-style revision copies are prohibited, typography work is narrowed to the existing candidates plus redistribution/offline-shipping/script/fallback validation, and Stage-1 professional visual quality is grounded in `master-development-plan.md`.
> **The follow-up review closed the remaining enforcement gap before it could repeat the first draft's `.meta` mistake.** Both `tools/unity-ci/check-meta-integrity.sh` and `tools/unity-ci/generate-missing-metas.sh` currently enumerate only `src/`. v0.5 therefore makes widening those tracked-path universes to include `Assets/GameArt/` the **first AP-01 action**, requires a mutation/firing proof before any real GameArt path is created, and leaves AP-06 for evidence-driven hardening rather than first-line integrity. The integrated-asset identity rule also drops the `where practical` hedge; necessary move/rename exceptions are explicit under §6.4 and require GUID/reference verification.
> **Tracking drift corrected in the same landing.** `docs/tracking/file-manifest.md` now inventories the new planning document and corrects its UI design-reference row from `stadium` / `touchline`, “neither chosen yet” to the actual July 25 disposition: **`touchline` chosen, `stadium` rejected**. That stale sentence survived because it is plain descriptive prose, not a maintained version citation or a machine-derivable count/cardinality; `doc-consistency-check.py` structurally had no oracle for it.
> **CI classification, not a runtime repair.** PR #360 pre-closeout CI run **#832** completed with the Linux shim gate red only in `TacticalDirector.MatchEngine.Tests.dll`: **472 passed / 1 failed / 11 skipped**; the failure is `sim_match_engine_close_chance`, explicitly owner-held RED on `main` by the steward contract and existing tracking record. `SeasonSave.Tests` passed its executed tests, Markdown link passed, and every other completed job was green. This PR changes no `.cs`, `.asmdef`, or dotnet-gate file, so there is no art-planning runtime fix or rebaseline to make here.
> **Determinism/runtime declaration:** no snapshot/serialization schema change; no new RNG stream, domain tag, draw site, or draw-order change; no runtime behavior, assembly graph, workflow contract, required status, `SPEC_INDEX.md` row, or spec status changed. **G0 remains CLOSED until the owner explicitly accepts the plan.** Merging the planning record does not itself authorize AP-01 or asset production.
>
> **Last Updated (prior):** September 4, 2026, latest — **The `System XI` rename landed (PR #359): 94 files, +140/−138, audited behaviour-safe. Its spec version-history obligation is NOT discharged, and is filed as an owner decision rather than papered over.**
> **What the rename changed.** `Tactical Director` and `Soccer Manager Pro` → **System XI** across human-readable surfaces: `README.md`, both `CLAUDE.md` guides, 59 files under `docs/specs/`, the planning and agent-guide documents, 15 mockup/design HTML pages, `tokens.css`'s banner comment, `chat-review.py`'s module docstring, `.gitattributes`'s header comment, and five `.claude` agent/skill `description:` fields. `docs/design/ui-mockups/Soccer Manager Pro - Design System.html` is renamed to `System XI - Design System.html`, its `file-manifest.md` row moves with it, and no tracked file still cites the old path.
> **The `src/` half is four display strings in three files, and nothing else.** `MatchClientPage.cs` (`<title>` and `<h1>`), `HtmlReplayExporter.cs` (`<title>`), `LiveMatchServer.cs` (`<title>`) — generated-HTML branding. No method, namespace, type, field, serialization identifier, route, constant or control-flow change; no `.asmdef`, `.meta`, `.unity`, `.prefab`, JSON/YAML, project or binary file appears in the diff at all. **The deterministic-serializer S-03 golden vector is deliberately untouched** — `SerializeCanonicalCorpusTests.cs:170` still writes the literal `"Tactical Director"`, because its bytes and SHA-256 are a pinned canonical vector rather than branding, and `docs/specs/deterministic-sim/golden-vectors/` is absent from the diff. The superseded `SMP-RNG-MATCH` text in the HKDF KAT is preserved for the same reason.
> **Two over-broad replacements were caught in review and reverted before this landing.** `docs/design/ui-mockups/assets/app.js` had changed the persisted `localStorage` key `smp-direction` → `sxi-direction`, which would have silently reset every saved UI-direction preference; and `squad.js`'s `SMP Sports` is mock player-agent data, not a product-brand identifier. Both files are byte-identical to `main`, verified by their absence from the diff. **A third, milder class was classified rather than reverted:** `chat-review.html`'s lesson identifiers `SMP-1 … SMP-6` → `SXI-1 … SXI-6` are identifier mutations, not display strings; safe because a tree-wide search finds no surviving reference to either form.
> **The governance half is the part worth reading.** Root `CLAUDE.md` requires a version-history row on **every** modified spec file. **47 of the 59 modified spec files carry a Version History section and none received a row**, and `Spec hygiene checks` passed green regardless — the rule is documentation-enforced only. Appending 47 rows for a product rename was rejected as churn that drags every maintained version pointer with it; **amending the rule inside the PR that would otherwise fail it was rejected as the circular, fail-open shape `ERR-019-001` and `ERR-019-003` refused one commit earlier.** A new `open-issues.md` entry carries the three owner decisions it implies, including that any exemption adopted must carry a **mechanical** test — *re-applying the declared substitution set to the base tree reproduces the branch tree exactly* — rather than the prose judgement "zero normative change". **This landing is a recorded deviation, not a precedent.**
> **The count moved with it:** `open-issues.md` 20 → **21 active**, archive unchanged at **53**, with `project-reference.md`'s claim and index bullet moved in the same commit; `check_drift.sh` exit 0 and clean.
> **README's H1 is now `System XI`**, with *Football Management Simulation* — the title `4e758d9` set two commits ago — retained as the descriptor line beneath it. Only the title area moved; the README's dated status snapshot is untouched, because this landing falsifies none of its stated facts.
> **Verified:** the rename's own effective diff against `main` at `d12709a` — this tracking close-out commit aside — is **94 files, +140/−138**; `app.js`, `squad.js` and the golden-vector corpus confirmed absent from it by direct check; `check_drift.sh` exit 0. CI on the branch head: YAML lint, Markdown lint, Markdown link check, C# format, spec hygiene, Unity `.meta` integrity, Unity asset/binary hygiene and file-manifest sanity all green. **No gate result is claimed for this landing** — `Compile + test (Linux shim gate, non-certifying)` was still running when this entry was written. No schema, executable semantics, RNG, draw-order, format-version, workflow, required status, `SPEC_INDEX.md` row or spec status changed.
>
> **Last Updated (prior):** September 4, 2026, later still — **`ERR-019-003`: the `ERR-019-002` fix published a reached, unmet Stage 0+1 MUST as half-satisfied. Found by the Codex review bot on PR #358 (P1), not by the pass that wrote the row. Documentation only.**
> **The defect.** `ERR-019-002` split FR-TS-043 out of the KD-6 band — correctly, since §2.2 assigns it Stage 0+1 while the band published Stage 0 — but characterised it as a row that "spans both stages by construction", with its automated `tools/checklist-auditor.py` mechanics due to **"arrive at Stage 0+1"**. That treats Stage 0+1 as pending. It is not: **§7.1 defines Stage 0+1 as the first `src/` code commit (KD-5)** and this repository passed it long ago — the identical fact `ERR-019-001` established for the FR-TS-075 … 080 band and marked **Reached**. §7.1 also lists `tools/checklist-auditor.py` among its transition deliverables, and **the script does not exist**: verified by direct search, every reference to it in the tree is documentation. So FR-TS-043's Stage 0+1 obligation is **active and unmet**, and the row reported it as satisfied-in-part. **This is `ERR-019-001`'s defect class re-created by the repair of `ERR-019-002` — the second time in this sequence that a fix for this class reproduced it**, and the reason a bot caught it rather than the four review passes that had already gone over these tables.
> **The fix.** §5.2's row reads `ACTIVE (Stage 0+1) — non-conformant`, states the stage is REACHED per §7.1/KD-5, and names the missing script; §5.6's row says the automated half is unimplemented at a reached stage and that no automated report exists; §5.2's preamble carried the same "arrive at Stage 0+1" framing and is corrected with them. **Deliberately not deferred behind a new prerequisite** — gating FR-TS-043 on "the auditor being implemented" is the circular, fail-open patch `ERR-019-001` rejected for FR-TS-075, because FR-TS-043 *is* the obligation to have those mechanics. `section-5.md` **v0.7 → v0.8**, `spec-error-log.md` **v2.49 → v2.50**.
> **The gap is RECORDED, NOT FIXED, and is wider than the one row.** Checking the sibling deliverable rather than only the row the bot named: **`tools/spec5-schema-auditor.py` (§5.4.3) is also absent**, and with `tools/run-tests-local.sh` — already open under `ERR-019-001` — **all three of §7.1's named tooling deliverables are missing**. New entry at `open-issues.md` with the three owner decisions it implies; building either auditor is an implementation pass, not a table edit.
> **The repaired count check caught its own author.** Filing that entry took `open-issues.md` from 19 to 20 active, and `check_drift.sh` — repointed and made status-affecting earlier today — immediately reported `FAIL … claims 19 active / 53 archived — counted 20 / 53`. The claim and the `project-reference.md` index bullet moved with it, and `doc-consistency-check.py` then caught two `file-manifest.md` maintained pointers still citing `spec-error-log.md` v2.49. Both tools did on their first real outing exactly what this session rebuilt them to do.
> **Verified:** `doc-consistency-check.py` PASS, `assembly-tier-check.py` PASS, `recurring-defect-lint.py` 0 ERROR, tooling tests 166/166, `check_drift.sh` exit 0 and clean. No `.cs`, `.asmdef`, or `tools/dotnet-ci` file moved; no schema, executable semantics, runtime code, workflow, required status, `SPEC_INDEX.md` row, or spec status changed. A3.4 reapproval and A8 enforcement remain pending.
>
> **Last Updated (prior):** September 4, 2026, late — **Residual from the same external review: the duplicate-claim path was still exiting 0, and one comment still used the retired metric name. Tooling and documentation only.**
> **`UNPARSED` on a duplicated claim was still a silent pass.** The previous commit made a *missing* claim `BROKEN` and status-affecting but left the `n_claims -ne 1` branch printing `UNPARSED` without setting `check_broken`, so a claim line appearing **twice** exited 0. That is the same structural class as the other two: the check cannot tell which claim is authoritative, so it verified nothing. It now sets `check_broken=1` and says so in those words. The EXIT-CODE CONTRACT block is rewritten to name all three structural sub-cases explicitly — claim file missing, no claim line, more than one claim line — so the next reader does not have to infer the set from the code.
> **The review also caught that my mutation set was incomplete**, which is the more useful half of the finding: the four cases proven last commit covered healthy, missing, wrong and restored, and simply never constructed a duplicate. **Five paths are now proven by execution:** healthy → exit 0 / OK; claim line absent → exit 1 / BROKEN; claim present but wrong → exit 0 / FAIL (advisory, by design); **claim line duplicated → exit 1 / BROKEN**; restored → exit 0 / OK. No live branch emits `UNPARSED` any more — the four surviving occurrences are comments recording the history.
> **Terminology swept.** The script's own class-3 header comment still read "OPEN ISSUES active/resolved counts" against the `active/archived` semantics adopted in the previous commit; corrected. `SKILL.md` was already consistent.
> **Verified:** `doc-consistency-check.py` PASS, `assembly-tier-check.py` PASS, `recurring-defect-lint.py` 0 ERROR, tooling tests 166/166, `check_drift.sh` exit 0 and clean. No `.cs`, `.asmdef`, or `tools/dotnet-ci` file moved; no spec file touched; no schema, executable semantics, runtime code, workflow, required status, `SPEC_INDEX.md` row, or spec status changed.
>
> **Last Updated (prior):** September 4, 2026, night — **Three review findings from an external pass (ChatGPT) on the previous commit, all valid, all fixed: a semantically false metric, a workflow contract still pointing at the wrong file, and a structural check failure that exited 0. Documentation and tooling only.**
> **(1) `53 resolved` was semantically false, and is now `53 archived`.** The two football-judgment records archived in the previous commit are annotated *"Neither is a resolved issue"*, yet the metric counted every archive bullet as `resolved` — so the surfaces agreed mechanically while disagreeing about what the number meant. The count measures **archive membership**; it is renamed to `archived` in `check_drift.sh`, in the published claim, and in `doc-consistency-check.py`'s measured key. **No resolved-vs-superseded split is published, because no reliable mechanical test for one exists** — grepping the archive for supersession markers returns 11 of 53 bullets and most are false positives matching unrelated prose (`"Superseded files from old naming convention"`, `"#18 IN REVIEW — RESOLVED"`). Publishing a hand-derived composition figure would have created a new drift surface to fix the naming of an old one. **Recorded, not fixed:** `doc-consistency-check.py`'s cardinality group for this number stays **dormant** — proven, not assumed, by setting the claim to 99 and watching it still PASS — because `project-reference.md` is not in that checker's `CURRENT_STATE` scan list. Its old `**N resolved**` pattern had likewise been matching **no surface at all**, so that group has been checking nothing for some time. Adding the file to `CURRENT_STATE` would bring *all* its cardinality claims into scope at once; the September 3 entry below already recorded that as a separate decision, and it stays one. The claim is not unguarded meanwhile — `check_drift.sh` checks it, and that is proven to fire.
> **(2) The `landing-close-out` skill was still telling agents to update root `CLAUDE.md`'s OPEN ISSUES.** The previous commit fixed the script and left the workflow contract that would have recreated the same drift: `SKILL.md` said root `CLAUDE.md` "only holds OPEN ISSUES now" and its item 2 was headed *"Root `CLAUDE.md` — OPEN ISSUES."* That section does not exist there and has not since the compact restructure. Item 2 now names `docs/tracking/open-issues.md`, states what it used to say and why that was harmful, and enumerates the two surfaces that move with an entry — the `project-reference.md` index bullet and its `**N active** / **M archived**` count. Fixing the tool without fixing the instruction is how the tool gets re-broken by the next agent following the documented process.
> **(3) `BROKEN` now exits 1; a count disagreement still exits 0.** Structural failure of the check — it cannot find the surface carrying the claim, so it verified *nothing* — is a contract violation, not a judgment call, and is now status-affecting alongside the header-chain guard. A count **disagreement** (`FAIL`) stays advisory, because whether stale counts are acceptable to land on genuinely is the writer's judgment. The EXIT-CODE CONTRACT block is rewritten to say so, and `SKILL.md` now states plainly that a green exit still requires scanning the report for `FAIL`. **All four paths proven by execution:** healthy → exit 0 / OK; claim line absent → exit 1 / BROKEN; claim present but wrong → exit 0 / FAIL; restored → exit 0 / OK.
> **A correction to this session's own testing.** A first combined run of those four cases appeared to show the count-disagreement case wrongly returning BROKEN, and that reading was reported before it was checked. It was a faulty harness, not a defect: the display `grep -om1 '^\*\*…'` was matching the file's first bold line (`**Tactical Director** is a football…`) rather than the claim line. Re-run with assertions on the file content at each step, the behaviour is as designed. The bad reading is recorded rather than quietly dropped, because "the test was wrong" is a conclusion that needs the same evidence as "the code was wrong".
> **Verified:** `doc-consistency-check.py` PASS, `assembly-tier-check.py` PASS, `recurring-defect-lint.py` 0 ERROR, tooling tests 166/166, `check_drift.sh` exit 0 and clean. No `.cs`, `.asmdef`, or `tools/dotnet-ci` file moved; no spec file touched; no schema, executable semantics, runtime code, workflow, required status, `SPEC_INDEX.md` row, or spec status changed.
>
> **Last Updated (prior):** September 4, 2026, evening — **The last two A3.3 leftovers closed: the football-judgment triplicate collapsed, and the open-issues count check repaired after being found pointed at a file that has not carried the data for weeks. Documentation and tooling only.**
> **`check_drift.sh`'s open-issues count check has been checking nothing, and real drift accumulated behind it.** Its class-3 check read the `**N active** / M resolved` claim out of root `CLAUDE.md` — which has carried no OPEN ISSUES section since the compact restructure — so every run printed `UNPARSED` and compared nothing. This session reported that `UNPARSED` as "pre-existing" on five consecutive runs without asking why. The claim actually lives in `docs/agent-guides/project-reference.md`, where it read **15 active / 46 resolved** against a true 21/51. The check is repointed, and the two cases are now reported differently: a surface that carries no claim is **`BROKEN` — "this check is currently checking NOTHING… do not read this as a pass"**, not a shrug. **Both new paths were proven by execution, not by reading** — removing the claim line prints BROKEN, corrupting it to 7/3 prints `FAIL … counted 19 / 53`, and restoring prints OK. The script's EXIT-CODE CONTRACT is unchanged: this check stays advisory, and the no-chain guard remains the only status-affecting condition.
> **The football-judgment proxy review had three entries in `open-issues.md`, not two.** The August 22, 2026 de-duplication collapsed one and left two more, so the file's raw bullet count read 21 against 19 distinct issues — which is exactly why its index in `project-reference.md` could not be reconciled with it. The survivor is the entry with the newest counts (29 findings / 19 specs / 21 workable). The other two are moved to `open-issues-resolved.md` **verbatim**, under one blockquote annotation, on this file's own PR #305 precedent: a superseded parallel record, not a resolved issue. **The one fact they carried that the survivor did not — the ERR-008-022 and ERR-008-023 landings of August 6 and 7 — is merged into the survivor's title in the same commit, so nothing is lost.** The second moved record already carried its own `⚠️ SUPERSEDED August 17, 2026` annotation; it is archived with that annotation intact and unedited. **Verified verbatim by execution:** both moved blocks are byte-identical substrings of the archive, and exactly one line in `open-issues.md` differs outside the two removals — the merged title.
> **Counts re-derived by direct count, not predicted:** `grep -c '^- \*\*'` returns **19** on `open-issues.md` (was 21) and **53** on `open-issues-resolved.md` (was 51), `doc-consistency-check.py` measures the same 19 / 53, `check_drift.sh` now reports `OK … matches`, and the figure agrees with the 19 bullets in `project-reference.md`'s index resynced earlier today. The three surfaces agree for the first time in this session.
> **Verified:** `doc-consistency-check.py` PASS, `assembly-tier-check.py` PASS, `recurring-defect-lint.py` 0 ERROR, tooling tests 166/166, `check_drift.sh` clean — **and, for the first time this session, with no `UNPARSED` line in it.** No `.cs`, `.asmdef`, or `tools/dotnet-ci` file moved; no spec file touched; no schema, executable semantics, runtime code, workflow, required status, `SPEC_INDEX.md` row, or spec status changed.
>
> **Last Updated (prior):** September 4, 2026, later still — **Self-review of the A3.3 follow-up: the `ERR-019-002` fix reproduced `ERR-019-002`'s own defect class, and the footer added to #19 §2.2 asserted something false about its own table. Both corrected. Documentation only.**
> **The `ERR-019-002` fix shipped prose contradicting the table it had just corrected.** §5.2's preamble named the Stage 0 subset as "the KD-6 mandate FRs (FR-TS-040 … 045)", and its column-semantics note said that for procedural FRs "FR-TS-040 … 052 … both columns read Stage 0". Splitting FR-TS-043 out to Stage 0+1 falsified both sentences — so for one commit §5.2 published prose directly contradicting the table beneath it, which is precisely the defect `ERR-019-002` was filed about. The preamble now names FR-TS-040 … 042, 044, 045 and states explicitly that the activation ranges are **not** the §2.2 partition ranges; the note now names FR-TS-043 as the single row where the two columns deliberately differ, and says why. The `ERR-019-002` entry's closing pointer is corrected too: it cited `section-5.md` v0.6 as the resolved state when the fix in fact spans **v0.6 and v0.7**, and it now records the reproduction rather than leaving a later reader at an incomplete version. `section-5.md` **v0.6 → v0.7**.
> **The #19 §2.2 footer described a table that does not exist.** It claimed the partition table "carries the ranges and their per-partition counts, which sum to 97" — that table's columns are `FR Range | Topic | Rule mechanics in | Verification in`, with **no count column**. The phrasing was lifted from Spec #20 §2.2.10, whose table does have one, without checking this one. Corrected to state what is verifiable of the table as it stands: the twelve ranges are contiguous, non-overlapping, and span FR-TS-001 … 097 exactly — **re-derived mechanically, not read** — with the total stated in the footer precisely *because* the table carries no counts. `section-2.md` **v0.4 → v0.5**.
> **Checked and clean, so recorded rather than left implicit:** no other prose in #19 was falsified by the splits — `outline-detailed.md` §5.2 describes the Stage 0 subset without naming ranges, and `section-3.md`'s quoted FR-TS-061 … 067 criterion is verbatim-correct against a band this pass did not touch. Both tables still cover all 97 FRs **exactly once** (no row double-counted by the splits) with zero §2.2 mismatches. The #20 outline corrections were re-verified against `section-3.md` §3.5.2 itself rather than against the `ERR-020-003` record they were drafted from: the ten-tier order and the `──► reads "is available to"` label are exact. The `doc-consistency-check.py` PASS is **not** evidence on the `ERR-019-002` pointer — the checker excuses citations inside log-body record regions, so it never saw it; that one was caught by reading.
> **Verified:** `doc-consistency-check.py` PASS, `recurring-defect-lint.py` 0 ERROR, `check_drift.sh` clean apart from the pre-existing root `CLAUDE.md` `UNPARSED`, tooling tests 166/166, `assembly-tier-check.py` PASS. No `.cs`, `.asmdef`, or `tools/dotnet-ci` file moved; no schema, executable semantics, runtime code, workflow, required status, `SPEC_INDEX.md` row, or spec status changed.
>
> **Last Updated (prior):** September 4, 2026, later — **A3.3 follow-up: every issue the reconciliation pass recorded rather than fixed is now fixed, by owner direction. One ERR filed and resolved; one recorded disposition deliberately reversed. Documentation only.**
> The pass below recorded five things it declined to change — three as "new drafting, for A3.4", one as already dispositioned, one as out of scope. The owner directed that all of them be fixed. They are, and two of them turned out to be larger than the gate report had them.
> **`ERR-019-002` filed and RESOLVED — and the defect is twice the size first reported.** §5.2's Activation Stage was recorded as disagreeing with §2.2 for eight rows. Re-checked mechanically, **§5.6 carries the identical eight**, exactly as `ERR-019-001` spanned both tables; the first report had checked only §5.2. §5.2's own column-semantics note states its Activation Stage column *matches the same-name column in §2.2*, so this is a defect against the table's own contract, not a summarisation choice. Six **Stage 0 MUSTs** (FR-TS-013, 015, 018, 020, 022, 038) were buried in Stage 0+1 bands and published `Inactive`/`Schema only` behind criteria they do not wait on — all six are spec-drafting obligations binding on the current spec set, and FR-TS-038 is checkable against the committed Appendix B today. **FR-TS-039**, the partition's only MAY, was published Stage 0+1 behind D2 when §2.2 assigns Stage 1+ and its own text defers to **D8** — wrong stage and wrong deferral. **FR-TS-043** ran the other way: published Stage 0 inside an **ACTIVE** band against §2.2's Stage 0+1, advertising a requirement as gating merges today. All eight are split to their own rows in **both** tables, FR-TS-043 with its two columns answering their own questions rather than being forced to one value. **Both tables now reproduce §2.2 for all 97 rows, verified mechanically** (re-derive §2.2's per-FR value, diff against both tables: zero mismatches, 97/97 coverage). **No FR statement, level, or §2.2 value changed** — the tables moved to §2.2, not the reverse; amending §2.2 to match would have laundered eight table errors into normative changes to an approved baseline. Unlike `ERR-019-001`, this one opens no concealed conformance gap. §5.7's FR-TS-011 … 020 cross-reference is updated for the split. `section-5.md` **v0.5 → v0.6**.
> **#19 §3.6.5 now carries the exception boundary.** The coverage-exemption *procedure* — the thing a reviewer actually follows — was the one coverage-exemption site that neither stated nor pointed at the §2.1 boundary that §2.1, §2.3 and §7.2 all carry. A boundary bullet is added citing §2.1 as the owner and reproducing the four things an exemption cannot waive. **It creates no new obligation and says so in its own text:** §2.1 already governs coverage exemptions in the same words. `section-3.md` **v0.6 → v0.7**.
> **#19 §2.2 gains the FR table footer it never had.** #20 §2.2.10 states its total and forbids renumbering at the catalogue itself; #19 left 97 derivable only from the partition table and recorded only in §9.1.2, and carried no renumbering prohibition anywhere. The footer is added, mirroring #20's, with the appended-at FR-TS-098 rule and the root `CLAUDE.md` renumbering-cascade citation. **No FR is added, removed, renumbered or restated and no count moves** — 97 is what the partition table already summed to. The asymmetry predates A3; it is closed here because the two specs reapprove as one bundle. `section-2.md` **v0.3 → v0.4**.
> **The `ERR-020-002` "deliberately not changed" disposition is REVERSED, by owner direction, and the reversal is recorded at the disposition itself.** All three retired `Physics → Mechanics → AI → UI` sites in #20's outlines now state the adopted ten-tier order. This is a decision reversal, not a missed sweep: the August 17, 2026 record had listed these exact lines and explained why they stayed, and the gate below found them, cited that record, and left them. Leaving that paragraph standing while the tree contradicted it would have made the next sweep re-litigate this, so it is **annotated in place, not deleted** — the reasoning stays as the record of why they were kept for eighteen days, with an explicit ⚠️ that a later sweep should expect corrected sites rather than exceptions. `outline-detailed.md`'s §3.5.2 line additionally lost **"Upward references = compile error rule"**, which `section-4.md` v1.2 had already established is false — an acyclic upward reference compiles cleanly, and `tools/assembly-tier-check.py` is what blocks it. No normative rule moved in either direction. `outline-detailed.md` **v1.6 → v1.7**, `outline-mid.md` **v1.4 → v1.5**.
> **`project-reference.md`'s OPEN ISSUES index resynced against its owning file.** It carried 14 bullets against 19 distinct entries. Six were missing outright — **including both live #19/#20 governance issues** (the FR-TS-075/079 conformance gap and #20's KD-4 deferral), plus #44 Discipline, depleted-squad cover, the DisciplineConfig deferral and `EnvironmentFingerprint.floatModelHash`. Two carried claims their owning entry explicitly retracts: the foul/card bullet still published the "~7 red cards per 9 minutes" figure resolved in July 2026, and the approval-tags bullet still read "created locally, not yet pushed" against an entry titled *NOT "created locally, awaiting push"*. One was resolved and archived (injury/aging) and is removed. The index now carries **19**, matching the distinct set.
> **Recorded, not fixed — found while resyncing that index, and not a defect I was asked to resolve.** `open-issues.md` itself carries **three** overlapping football-judgment-proxy-review entries (two claiming 32 findings across 24 specs, one claiming 29 across 19), which is why its raw bullet count reads 21 against 19 distinct issues. A previous pass already de-duplicated this entry from two to one, per this chain's own August 22, 2026 record; it is back at three. Collapsing them means deciding which of three overlapping records is authoritative and which figures survive — an owner call about issue content, not an index repair.
> **Verified at this head:** `doc-consistency-check.py` **PASS** (`ERR-` index rows 227 → **228**, active open issues 21, production assemblies 35, spec folders 53), `assembly-tier-check.py` **PASS** (35/35 placed, 0 upward, acyclic), `recurring-defect-lint.py` **0 ERROR**, tooling unit tests **166/166**, `check_drift.sh` clean apart from the pre-existing root `CLAUDE.md` OPEN ISSUES `UNPARSED`. The A3.3 gates were re-run over the edited candidate and still hold: 81 / 97 unique contiguous FRs, #20 §5.5 at 81 covered, Appendix G.4 still schema-valid. Three `file-manifest.md` maintained pointers moved with the version bumps in the same commit — the checker caught them, which is what it is for. **No gate run is owed:** no `.cs`, `.asmdef`, or `tools/dotnet-ci` file moved. No schema, executable semantics, runtime code, workflow, required status, `SPEC_INDEX.md` row, or spec status changed. A3.4 reapproval and A8 enforcement remain pending.
>
> **Last Updated (prior):** September 4, 2026 — **A3.3 reconciliation gate run over the combined #19 + #20 amendment candidate; two findings corrected, one recorded as already dispositioned, three recorded for A3.4. Documentation only.**
...
> (`foul-discipline-balance-design.md` **v1.1** *(since v1.4)*, new §7 item **2a** *(renumbered to item **3** at v1.2 — `2a.` is not a valid list marker)*). That note's own item 2 said
...
> green throughout.
>
> **`match-engine-wiring-backlog.md` **v1.10** *(since v1.32)* records the consequence none of these three holds could
...
> re-pointed: `match-engine-wiring-backlog.md` is at **v1.11** *(since v1.32)*, whose §5 note carries the corrected
