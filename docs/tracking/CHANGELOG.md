...
> **One naming collision is recorded for whoever implements §12.** The tree's only `Caption` symbol is `RestartCaptionTicks` in `src/match-viewer/RestartBanner.cs` — a *visual* banner timer with nothing to do with the accessibility caption region. It is the same false-positive class as §8.1's `HeadingSpinTransfer` and `BudgetRollupEntry`, and a bare grep for "caption" would report a caption surface that does not exist.
