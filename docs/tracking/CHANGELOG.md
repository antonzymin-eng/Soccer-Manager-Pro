...
> **Last Updated:** September 9, 2026, later — **The project owner accepted the UX planning package. `ux-detailed-plan.md` v1.3 → v1.4. Documentation only — no `src/`, `.asmdef`, spec or `SPEC_INDEX.md` row moved, so no gate is owed.**
> **The decision.** The owner accepted the package on September 9, 2026 as it stands at `ux-high-level-plan.md` **v1.2** and `ux-detailed-plan.md` **v1.4**, with the F1–F3 outputs `ux-baseline-evidence.md` v0.3, `ux-experience-architecture.md` v0.1 and `ux-shared-system.md` v0.3. This discharges the one condition `open-issues.md` had held open on this workstream since F0, and it was the last item owed before PR #362 merges. Substantial UX **production from S0-C onward is authorized**; **F4 is still the next work package** and the acceptance does not reorder it.
> **Recorded where it can be found, and scoped where it can be misread.** New **§17** in the execution authority states the acceptance and its limits, because a plan approval read as an outcome approval is the obvious way this goes wrong: Gates A–J are unchanged and **none is passed by it**; **Gate G still binds at two independent participants**, with the "provisional" bypass still removed; **S0 Gate J remains unreachable** until B8/P4b host verification → B9b/P5b → B10/P6 produces real evidence, of which acceptance supplies none; the §10.1 UX-workstream role owner and the two Gate-G participants remain unassigned and are F4 outputs, not decisions made here; §8.1's `FUTURE-BLOCKED` S2 rows are unmoved, so transfers, scouting, finances and staff stay implementation-absent; and the career-return navigation move F2 identified is still owed by the **client** workstream.
> **Root `CLAUDE.md`'s routing bullet 6 is corrected in the same commit.** It told every agent session that "the detailed plan's F0 tracking close-out precedes F1 and any further substantial UX production" — a gate discharged on September 6 and now doubly stale. It states the accepted / F0–F3-complete / F4-next position and delegates live status to the plan and `open-issues.md`, which is where it belongs; the always-loaded guide is the worst place to keep a status claim that has to be maintained.
> **The `open-issues.md` entry is amended, not archived** — its title now leads with the acceptance rather than pointing the next agent at a decision already made, and the entry stays active because F4 through S1 are unstarted and the F2 client dependency is unresolved. Same treatment the audio G0 acceptance received on September 7. Active count unchanged at **23 active / 53 archived**.
> **Determinism/runtime declaration:** documentation-only owner-decision record; no `.cs`, `.asmdef`, asset, snapshot/serialization schema, RNG stream/domain/draw site/draw order, runtime behaviour, assembly graph, `SPEC_INDEX.md` row or spec status changed. **No gate result is claimed** and none is applicable — this landing contains no compilable file.
> **Verification:** `doc-consistency-check.py` PASS, `check_drift.sh` exit 0 with counts agreeing at 23 / 53, `recurring-defect-lint.py` unchanged at the 14 inherited `FR-CS-058` ERRORs described in the entry below.
>
> **Last Updated (prior):** September 9, 2026 — **PR #362 pre-merge correction: the UX high-level plan's F1 effort band reconciled with the execution authority. Documentation only — no `src/`, `.asmdef`, spec or `SPEC_INDEX.md` row moved, so no gate is owed.**
> **What landed.** `docs/design/ux-high-level-plan.md` **v1.1 → v1.2**. An external pre-merge review of PR #362 found §8's timebox table still stating **1–2 working days** for F1, while `ux-detailed-plan.md` — the single execution authority — had revised that band to **3–4 working days** at its own v1.2 on September 6, 2026, with its reasoning recorded (F1.1 spans six assemblies and five spec surfaces, and the v1.1 dependency review had already demonstrated that establishing what exists here is slower than it looks). The finding is correct and the high-level table was the stale side.
> **The cause is worth more than the fix.** §8 **duplicated** the bands rather than citing them, so the September 6 revision of the detailed plan had no reason to touch this file and nothing mechanical could notice: `doc-consistency-check.py` compares version citations and machine-derivable cardinalities, and an effort band in prose is neither. §0 of the high-level plan already applies the single-definition rule to Gates A–J — *"it deliberately does not repeat the execution gates; Gates A–J are defined once"* — and the bands were the one place it was not applied. §8 now names `ux-detailed-plan.md` §10.2 as the authority it mirrors and states that on disagreement the detailed plan wins. **All twelve rows were compared, not just the reported one: F1 is the only band that differed.**
> **Two smaller things landed with it.** §11's closing line *"F1 is the next UX action"* now carries a dated status note — those conditions landed September 6, 2026 and F0–F3 are complete with F4 next — because that sentence is the document's own indexable statement of where the workstream stands, the same class of stale summary this file's September 7 entry corrected in the `open-issues.md` title. And the header `**Version:**` / `**Last Updated:**` fields are re-derived in the same commit as the version-history row; not doing that is precisely the desync this workstream had to repair in `ux-detailed-plan.md` at v1.3.
> **Not changed:** the owner's acceptance of the UX planning package is still open, and it still gates UX *production* from S0-C onward rather than the completed audit phases. This correction does not supply it. Active issue count unchanged at **23**; no roadmap row, `SPEC_INDEX.md` row or `ERR` moved.
> **Determinism/runtime declaration:** documentation-only; no `.cs`, `.asmdef`, asset, snapshot/serialization schema, RNG stream/domain/draw site/draw order, runtime behaviour, assembly graph, `SPEC_INDEX.md` row or spec status changed. **Blast radius: checked, nothing moved** — no scenario, tick window, per-90 rate band, calibration corpus or `FR-PO-052` baseline is touched. **No gate result is claimed** and none is applicable: this landing contains no compilable file.
> **Verification on the edited tree:** `doc-consistency-check.py` **PASS** (the CI-wired one — it caught this entry's own first draft citing the execution plan at a superseded version in `open-issues.md`, which was rephrased rather than excused — and then caught the same pattern here, where naming that file beside its old version number re-created the very citation the sentence was reporting as fixed), `check_drift.sh` **exit 0** with chains single-labelled and counts agreeing at 23 active / 53 archived, and `recurring-defect-lint.py` **14 ERROR / 123 WARN / 27 INFO**. **All 14 ERRORs are inherited from `main`, not introduced here:** every one is `FR-CS-058` on a `src/club-finances/` file from the #40 landing (PR #375), each carrying a `#region VersionHistory` block with no version row. They reproduce with this branch's changes stashed, this landing touches no `.cs` file, and `recurring-defect-lint.py` is not CI-wired, so nothing gates on them today — recorded here because the September 6 entry claimed 0 ERROR tree-wide and that claim is no longer true.
>
...
> **Last Updated (prior):** September 7, 2026, later still — **F3 shared UX system landed and inventoried; its three load-bearing claims verified against source, and a header/version-history desync in the execution authority corrected. Documentation only — no `src/`, `.asmdef`, spec or `SPEC_INDEX.md` row moved, so no gate is owed.**
> **What landed.** `docs/design/ux-shared-system.md` **v0.3** — the F3 output (UX-B), defining only the shared S0/S1 primitives the detailed plan requires: action hierarchy and state, navigation/tab/focus and keyboard/no-trap behaviour, dense tables with sort/filter/selection, inputs and selectors, contextual help, modal/drawer, feedback, loading/empty/partial/stale/error, pseudo-locale/reflow and maximum text scale, colour-independent semantics, glyph and missing-art fallbacks, caption/HUD coexistence, and the 1366/1920×1080/2560 desktop validation cases. It is explicitly not a component library, not a runtime API, and its validation cases are **not** platform-support promises. With F1 and F2 this puts the UX workstream at **F0–F3 complete, F4 next**. Inventoried in `file-manifest.md`; the `open-issues.md` UX entry is amended in place (active count unchanged at **22**).
> **The three load-bearing claims were verified against the tree rather than accepted on report.** **F3-002 holds:** `src/ui-framework/` contains **zero** occurrences of focus, keyboard, tabindex or keydown — the assembly is `NavigationShell`, `ScreenId`, `ManagerIntent` and the dispatcher/view-model seams and has no focus model at all, so keyboard and focus semantics genuinely are UX/rendering requirements rather than current shell behaviour, and F3 is correct to leave #38's pure shell alone. **F3-003 is corroborated by `SPEC_INDEX.md` itself:** its row titles #49 *"Localization & Accessibility (seam + template-contract slice)"*, and an approved seam/template-contract slice does not settle option content or store — which is exactly the overclaim F3's own AR-1 pass caught and corrected at v0.2. **F3-005 holds:** `src/` contains no `Audio*` symbol whatsoever, so APPROVED #51 has no consumer and caption/HUD coexistence is correctly held as `FUTURE-BLOCKED`.
> **One naming collision is recorded for whoever implements §12.** The tree's only `Caption` symbol is `RestartCaptionTicks`, declared at **`src/match-client-web/MatchClientWebConstants.cs:65`** and consumed at `MatchClientPage.cs:123` — a `[GT]` tick count controlling how long a restart caption stays on screen, i.e. a *visual* banner timer with nothing to do with the accessibility caption region. It is the same false-positive class as §8.1's `HeadingSpinTransfer` and `BudgetRollupEntry`, and a bare grep for "caption" would report a caption surface that does not exist. **⚠️ Corrected September 7, 2026, before this entry was superseded:** as first written this line placed the symbol in `src/match-viewer/RestartBanner.cs`, which contains no `Caption` identifier at all. The wrong path was inferred from a `grep -l` *file list* rather than read from the declaration — the third instance in this workstream of the same error class as the #30 folder-name inference and §8.1's keyword false positives, and the most pointed, since it put a misleading source pointer inside the very entry warning an implementer about misleading source pointers. Found by the repository's Codex PR review and verified against the declaration before correcting.
> **A defect in this workstream's own earlier bookkeeping is corrected here, not patched silently.** The v1.2 revision of `ux-detailed-plan.md` appended its version-history row and edited its status line but left the header at `**Version:** 1.1` and `**Last Updated:** September 4, 2026`. The manifest, this changelog, `open-issues.md` and PR #362's body all cited the file as v1.2, so the document's own header contradicted every record pointing at it, and `doc-consistency-check.py` passed throughout because it cross-checks citations rather than header-against-history agreement. Both fields are re-derived at **v1.3**, whose history row states what went wrong.
> **Determinism/runtime declaration:** no snapshot or serialization schema change, no new RNG stream, domain tag, draw site or draw-order change, no runtime behaviour change. **Blast radius: checked, nothing moved** — no scenario, tick window, per-90 rate band, corpus fit or `FR-PO-052` perf baseline is touched. **No gate result is claimed**; no `.cs`, `.asmdef` or `tools/dotnet-ci` file moved. The repository-wide functional-gate timeout is tracked separately and is not this branch's defect: it reproduces on `main`, and diagnostic PR #369 (coverage removed for that branch only) completed the full workflow green in **47m24s**, establishing the coverage-enabled path as the coarse cause.
> **Last Updated (prior):** September 7, 2026, later — **F2 experience architecture landed and inventoried; its career-return finding verified against source and filed as a client dependency. Documentation only — no `src/`, `.asmdef`, spec or `SPEC_INDEX.md` row moved, so no gate is owed.**
> **What landed.** `docs/design/ux-experience-architecture.md` **v0.1** — the F2 output, recording the current four-screen/five-move PM-1 graph verbatim and holding it separate from the target PM-2 career shell, with interaction, attention, blocked-action, save and help rules. With F1's `ux-baseline-evidence.md` v0.3 this puts the UX workstream at **F0–F2 complete, F3 next**. Both files are now inventoried in `file-manifest.md`; the `open-issues.md` UX entry is amended in place (active count unchanged at **22**).
> **The F2 finding is the part worth carrying, and it was verified rather than accepted on report.** A future career shell cannot reuse the PM-1 graph's `Pop()` to return from Post-Match to a Career Home. `NavigationShell.Pop()` returns to whatever lies *beneath* the current screen and asserts nothing about the destination; `ClientScreenFlow.RequireCurrent` guards only the screen a move *departs from*. Traced through the live stack — `[MainMenu] → Push TacticsSetup → Replace MatchView → Replace PostMatchReport → Pop` — `ReturnToMainMenu()` reaches Main Menu **solely because Main Menu is the stack root and the two intervening edges are `Replace` rather than `Push`**. Put a Career Home at that root and the identically-named method lands there silently, while its name, its XML documentation and its guard all still say Main Menu: the **ERR-029-005 silent-identity class** that `ClientScreenFlow`'s own constructor transposition check exists to prevent, and which nothing in the current code could catch.
> **The fix shape is already dictated by the code's own rule,** so this is recorded rather than designed around: `ClientScreenFlow`'s class documentation states that a new destination is *"a new named move here, not a shell poke in the binding"*. The career return is therefore a **new named move with its own guard**, not a reinterpretation of `ReturnToMainMenu()`. This is a client implementation dependency owned outside UX — as is the already-recorded `src/ui-framework/IntentKind.cs` omission of `AdvanceRound` beneath commentary still asserting #30 has no source assembly. **Neither is patched by the UX workstream, and F2 correctly declined to patch them.**
> **Determinism/runtime declaration:** no snapshot or serialization schema change, no new RNG stream, domain tag, draw site or draw-order change, no runtime behaviour change. **Blast radius: checked, nothing moved** — no scenario, tick window, per-90 rate band, corpus fit or `FR-PO-052` perf baseline is touched. **No gate result is claimed**; no `.cs`, `.asmdef` or `tools/dotnet-ci` file moved.
> **Verified:** `check_drift.sh` exit 0 and clean; `doc-consistency-check.py` PASS; `recurring-defect-lint.py` 0 ERROR. The navigation trace above was read directly from `src/ui-framework/NavigationShell.cs` and `src/client-app/ClientScreenFlow.cs` at this commit.
> **Last Updated (prior):** September 7, 2026 — **`main` synchronised into the long-lived `design/ux-foundation` branch (152 commits), and the F1 evidence pack given the tracking entry it landed without. Documentation only — no `src/` file, `.asmdef`, spec or `SPEC_INDEX.md` row moved on either side of the merge, so no gate is owed.**
> **Why the branch conflicted.** `design/ux-foundation` split at `3851a410` and ran 15 commits while `main` ran 152, and both sides prepend to the same append-only tracking surfaces. Exactly three hunks conflicted — this chain's head, and two regions of `file-manifest.md` (its head entry and the Design References table). No content conflicted anywhere else: not in `open-issues.md`, not in `project-reference.md`, and in no `src/`, spec or tooling file. **Resolved per `steward/SKILL.md` §Conflicts in tracking documents: both histories kept, newest first, exactly one bare `**Last Updated:**` label, and old entries corrected by successors rather than rewritten.** Nothing from either side was dropped.
> **One ordering caveat, stated rather than papered over.** The branch's own September 6 entry (the UX F0 close-out) and `main`'s four September 6 art-pipeline entries carry no times, so their exact interleaving is not recoverable from the record. The branch entry is placed immediately below `main`'s newest rather than guessed into the middle of `main`'s run; `main`'s internal order is preserved untouched.
> **The F1 landing's missing half.** `docs/design/ux-baseline-evidence.md` **v0.3** — the UX-A Baseline & Evidence Pack completing F1.1–F1.5 — landed at `3ed72fb` with no `file-manifest.md` row, which its own PR body flagged as owed before merge. The row is added here, in this commit, rather than left for the merge to carry silently. **Its principal finding is worth recording in the chain itself:** the Early Access gap is not missing season simulation — `src/season-save/` already carries `SeasonLoop.AdvanceAndPlayNextRound`, `SeasonViewModel` and the league/calendar/table/bootstrap/save surfaces — it is the **client composition layer**: a season adapter/dispatcher, an `AdvanceRound` intent, and the Career Home / New Game / Continue / Load / Save screen identities and bindings. `src/ui-framework/IntentKind.cs` still omits `AdvanceRound` beneath commentary asserting #30 has no source assembly, which is the same stale-surface class this branch already corrected twice.
> **Determinism/runtime declaration:** no snapshot or serialization schema change, no new RNG stream, domain tag, draw site or draw-order change, no runtime behaviour change on either side of the merge. **Blast radius: checked, nothing moved** — no scenario, tick window, per-90 rate band, corpus fit or `FR-PO-052` perf baseline is touched.
> **Verified on the merged tree:** `check_drift.sh` exit 0 and clean (1 bare label; open-issues counts recounted and matching); `doc-consistency-check.py` PASS; `recurring-defect-lint.py` 0 ERROR. **No gate result is claimed** — no `.cs`, `.asmdef` or `tools/dotnet-ci` file moved in the branch's 15 commits, so the Linux shim gate is not owed by this landing; `main`'s own gate state is unchanged by a merge that adds only documentation.
...
>
> **Last Updated (prior):** September 6, 2026 — **UX planning package converged and routed; F0 tracking close-out landed. Documentation only — no `src/` file, `.asmdef`, spec or `SPEC_INDEX.md` row moved, so no gate is owed.**
> **What landed.** `docs/design/ux-high-level-plan.md` **v1.1** (UX strategy, milestone/Early-Access cut, dependency posture) and `docs/design/ux-detailed-plan.md` **v1.1**, since advanced to **v1.4** (the single execution authority — work packages F0–F4, journey slices S0–S2, Gates A–J, validation severity/disposition, test-data extremes, QA handoff, change control, execution order), plus `docs/design/ux-foundation.md` reduced to a superseded stub pointing at both, `docs/design/ui-mockups/Main Menu.html` recorded as **provisional pre-plan reference work**, its `README.md` routed through the plans, and a root `CLAUDE.md` routing bullet. The plans create **no** simulation, view-model, command, save or navigation contract; `docs/design/` remains non-normative and the spec wins on any disagreement.
> **The entry worth reading is the review's two false claims, not its seven adopted ones.** An external dependency review raised nine findings against v1.0. Seven were adopted. **Two were factually wrong and were corrected rather than adopted, and the error class is reusable:** (1) it asserted **#30 Season & Competition Loop has no `src/` assembly**, which is false — #30's implementation is **`src/season-save/`** (`SeasonLoop.AdvanceAndPlayNextRound`, `LeagueTable`, `FixtureScheduler`, `SeasonCalendar`, `SeasonViewModel`, `LeagueBootstrap`) — the claim having been inferred from the absent `src/season-competition-loop/` folder and from `path-to-playable-roadmap.md` §0's **July 25** table, both stale; (2) it read **P4b as unlanded**, where the accurate statement is landed as code August 15, 2026 and host-unverified, exactly as `open-issues.md`'s presentation-layer entry already had it. **Both plans now carry the generalisation — do not infer assembly absence from folder naming, and classify each capability individually at Gate A rather than declaring a journey globally blocked** — and the corrected consequence: PM-2's real gap is the player-facing projection / dispatch / navigation / binding layer, not the season domain. A planning document that had declared S1 blocked on an unbuilt #30 would have deferred the Early-Access journey behind work that was already done.
> **Adopted from the review.** S0 is tied to roadmap **B8/P4b host verification → B9b/P5b → B10/P6**, with Gate I meaning *ready for P5b implementation* and Gate J explicitly unreachable until that on-host evidence exists; **Gate G is binding at two independent participants** and the unlimited "provisional" bypass — which would have let every slice ship unvalidated while the plan still claimed a usability gate — is removed; `interactive-unity-client-design.md`, `ui-client-framework-design.md` and `ui-framework-t0-implementation-plan.md` are named as inherited authority, so F2 cannot re-derive the `ClientScreenFlow` five-edge graph that is already gate-compiled and test-locked at 15 tests; `match-viewer` / `match-client-web` are named as the host-free real-data prototype vehicle and **explicitly not** a second shipping client; the Gate A–J definitions now exist in the detailed plan only (both documents shrank — detailed 1298 → 980 lines, high-level 554 → 352); §10 states role owners and effort bands.
> **Blast radius: checked, nothing moved.** No scenario, tick window, per-90 rate band, corpus fit or `FR-PO-052` perf baseline is touched — the landing adds no per-tick work and no behaviour. No `SNAPSHOT_SCHEMA_VERSION` change, no new RNG stream, domain tag, draw site or draw-order change. **No roadmap row moved:** B8, B9b and B10 are unchanged, and deliberately so — the plans record a dependency *on* those rows without advancing them, because PR #362 is still draft and an unaccepted plan must not edit the roadmap it is waiting on.
> **The count moved with it:** `open-issues.md` 21 → **22 active**, archive unchanged at **53**, with `project-reference.md`'s claim and index bullet moved in the same commit. **What remains open is the acceptance itself** — F1 has not started, and the new entry carries three owner decisions: accept or reject the package; whether the S0 Gate-I packet is scheduled against B9b before or after B8's host run, both needing the pinned host under a per-landing cert budget; and who fills the UX workstream role and supplies the two Gate-G participants the plan now has no way to pass without. **⚠️ Corrected September 6, 2026, same day and before F1 began — that sentence overstated what F0 left open, and the plan's own text is the refutation.** `ux-detailed-plan.md` names the tracking close-out as the **only** prerequisite to F1, and this landing is it: **F0 is closed and F1 may begin.** Of the three "owner decisions", only the acceptance is real, and it gates substantial UX *production* from S0-C onward rather than the F1 audit. The second — B8 → B9b → B10 ordering — is a genuine dependency for S0 Gates I/J that constrains nothing in a capability audit. The third contradicted the document it was summarising: §4 already places Gate-G participant identification in **F4**, so the requirement stays binding while *who* they are is an F4 output, not something owed now. PR draft status was never a technical dependency at all. Corrected in `open-issues.md`, `project-reference.md` and the plan itself at **v1.2**, which also revises F1's effort band 1–2 → **3–4 working days** (F1.1 spans six assemblies and five spec surfaces, and this review sequence has now twice shown that establishing what exists here is slower than it looks) and adds **§8.1**, a provisional S2 implementation table derived by symbol search rather than folder naming: transfers, scouting, finances and staff are implementation-absent, while **board objectives are present** in `src/season-save/` — the same differently-named-assembly pattern as #30, and one a keyword search reports backwards in both directions.
> **Verified:** `check_drift.sh` exit 0 and clean (1 bare label; 22/53 recounted and matching). **No gate result is claimed for this landing** — no `.cs`, `.asmdef` or `tools/dotnet-ci` file moved, so the Linux shim gate is not owed and quoting a previous run's numbers here would be false. `README.md` untouched: this landing falsifies none of its dated snapshot's facts (no assembly count, assembly-less figure, Unity target or governance stage changed). `src/CLAUDE.md` and `CHANGELOG-src.md` untouched for the same reason — no code changed.
