...
> **Last Updated:** September 18, 2026 — **PR #416 Step 5 evidence-ref archive and per-ref uniqueness audit; documentation/evidence only.** New `docs/tracking/evidence/pr416-ref-archive/` captures **85 exact Git blobs** from the 24 surviving evidence refs: 72 current-tip branch-side files plus 13 exact historical run-time files for six refs that advanced after cited runs. The audit uses merge-base→ref directionality rather than current-main→old-ref comparison. Snapshot files are byte-identical but carry a terminal `.txt` quarantine suffix so historical C#/workflow/Markdown cannot enter ordinary scanners. `MANIFEST.tsv` is the 85-row blob authority; `ref-disposition.tsv` is the 24-row disposition authority: **21 `deletable`, 0 `retain`, 3 `policy-retained`** (close-chance retirement, narrow Rolling candidate, state-only pre-force candidate), with `delete_now=false` on every row. The provenance and live cleanup issue now point to the archive and explicitly require post-merge verification from `main` alone before any deletion. No evidence ref is deleted or deletion-authorized by this landing; Step 4 remains closed; active/archive issue counts remain **28 / 55**. No production/source, test semantics, APPROVED spec, assembly edge, durable field, snapshot/save schema, RNG stream/domain/draw site/order, owner-held bound, or `[GT]` value changed.
>
