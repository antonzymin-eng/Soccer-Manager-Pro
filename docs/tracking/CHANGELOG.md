...
> **Last Updated:** September 7, 2026, later — **F2 experience architecture landed and inventoried; its career-return finding verified against source and filed as a client dependency. Documentation only — no `src/`, `.asmdef`, spec or `SPEC_INDEX.md` row moved, so no gate is owed.**
