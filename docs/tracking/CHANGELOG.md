...
> **Last Updated:** August 10, 2026 (**THE SHOT-LANE REGRESSION, BISECTED — Acceptance-3's two named suspects REFUTED; the live regression is C1.** `close-chance-creation-design.md` §11 (v1.6): an 18-seed × 6-tree bisect (the 6-seed close-chance corpus + 12 fresh seeds, 90 minutes each, 108 full matches; the `64513e4` tree held fixed and only `OptionGenerator.cs`/`UtilityWeights.cs`/`DecisionTreeConstants.cs` + suite swapped, so the -021/-022/-023 shot-lane chain is not confounded with #41 T2 / `LineupSelector.TrySelect` / `MatchEngine.ConfigureSquads`) separates the Acceptance-3 regression (`open-issues.md`, `spec-error-log.md` v1.100) from C1 (`ERR-012-011`). **Seed `0xD1A6D05E` is bit-identical pre-chain, after -021 and after -022 (n=156, +0.1168), and moves only at -023 (−0.2320)** — the recorded regression is ERR-008-023 alone, and it is a **trajectory resample, not a mechanism**: no DRIBBLE path reads `GoalOpeningScore`, the 8-sector scan's tie resolves to `AgentFacingDirection` (ERR-008-024), and the whole swing rides on four extra final-third shots (`ft3Shoot` 15 → 19). Over 18 paired seeds the chain's directional effect is **−0.027 ± 0.039** (t = −0.70, 8 up / 10 down) against a between-seed sd of ≈ 0.17; the Acceptance-3 pooled −0.119 is the 4.6th percentile of the two-seed pooled estimator over all 153 pairs of those 18 seeds. **−021 is inert on 5 of 6 seeds**, independently confirming ERR-008-022's own diagnosis — the goal-centre-plane bound was discarding the blockers -021 existed to weight; it "did nothing at all until −022 gave it blockers to price." **The live regression failing `sim_match_engine_close_chance` on main today is C1 / `ERR-012-011`, and it IS a mechanism**: 16 of 18 seeds down, **−0.189 ± 0.038, t = −5.05**, reproducing the gate's −0.165 on the scenario pair exactly, with mean dribbles per seed 181 → 328 and the mechanism already recorded in §10 (C1's `InPoss` `PullFactor` column is less advanced than the `TransToAtk` column it replaced, deepest composed slot 23.0 → 25.7 m). Two bit-identical control results fall out: `ba4e194 == 64513e4` on all 18 seeds (the #29/#41 balance pass, B9c and the 275-error lint sweep are inert on the match engine) and `15f4ffd` (HEAD) `== e1480ee` on all 18 seeds, which **verifies by execution** the ERR-008-024 refusal's behaviour-neutrality claim, previously supported only by diff inspection. **Nothing re-tuned (KD-W1)**, and the ERR-008-024 tie-break fix was deliberately not re-attempted (it stalls `sim_match_engine_play_develops` at tick 18465 with zero goals; §10.5). **This landing is documentation-only in this session** — `spec-error-log.md` (→ v1.100, repairing the malformed ERR-008-021 double-entry and the missing ERR-008-023 body) and `close-chance-creation-design.md` (→ v1.6, §11) were landed separately; this entry syncs `match-engine-wiring-backlog.md` (→ v1.4), `open-issues.md`, root `CLAUDE.md`'s OPEN ISSUES index, `CHANGELOG-src.md`, and `file-manifest.md` to match. No schema change, no new RNG stream / domain tag / draw site / draw-order change — the bisect changed no production code. **GATE: FAILED — and the failure is pre-existing on main, not this landing's.** Whole tree, 32 suites, run locally in this session (the .NET 8 SDK installs cleanly from the Ubuntu archive — `apt-get update` first — per `src/CLAUDE.md`). Build 0 errors; 31 of 32 suites green; quarantine unchanged. The single failure is `MatchEngine.Tests` **447 passed / 1 failed / 10 skipped** (49 m 4 s), and the one failure is `sim_match_engine_close_chance` — 2 of its 3 predicates, `meanCosine = -0.165` against the −0.16 bound and `goalwardShare = 0.407` against 0.42. That is the C1 regression this entry localizes, already recorded as a live main failure in `match-engine-wiring-backlog.md`, and the gate's −0.165 reproduces the §11 probe's −0.1647 on the same seed pair to three decimals. This session's only `src/` change is comment-only (`MatchEngineCloseChanceScenarios.cs` v1.2 — verified: zero non-comment lines in the diff), so it moves no predicate, bound or seed and cannot have caused the failure. Prior entry below.)
>
