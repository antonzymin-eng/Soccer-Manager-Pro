...
> **Last Updated:** September 11, 2026 — **W5 + W7 implementation and two-round W5 remediation (`ERR-013-011`).** PR #398 wires #13 BACKWARD_PASS input and #26 kickoff selection. First review fixed frame ownership but introduced an unsatisfiable 60 Hz/10 Hz tick equality; second review exposed that plus AI-before-Resolve/Events order and the separate two-heartbeat debounce contradiction for a one-shot pass event. Final #13 contract accepts only `[N-AI_PHASE_STRIDE,N)` at AI physics tick `N`, latches a qualifying pass only through its already-started dwell, and keeps the retained event wholly world-frame with evaluator-side target normalization. Positive EventBus→next-stride→commit coverage proves acceptance. W7 remains after manager configuration and before tick 1, with AI-reference and neutral-baseline digest locks. Snapshot schema remains **21 → 22** for latest-ring persistence; the new timing carriers are ephemeral, so v22 bytes do not change. No RNG/draw-order change. Backlog W3–W10 falls **8 → 6**; KD-W1 remains active. **GATE:** current-head PR CI is authoritative and must be green before merge; Unity-runner jobs remain environment-gated when no license is configured.
>
