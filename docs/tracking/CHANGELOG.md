...
> **Last Updated:** September 10, 2026 — **A wrong sentence in the wiring backlog is corrected, and the measurement lane that the next three items depend on is built. Documentation + CI tooling only; no `src/` code changed.**
>
> **The correction is the load-bearing half.** `match-engine-wiring-backlog.md` **v1.13 → v1.14**. Since v1.0 the W4 entry had closed *"The keeper is simply not on that path."* Direct source read at `81c6ae0` refutes it: `MatchEngine.cs:3074` runs `_perception.OnHeartbeat` over the full `SQUAD_SIZE` including both keepers (keeper index == team id, `:922`), and `BallPerceptionEvaluator` already computes an occluded ball visibility for each of them every heartbeat. **Nothing needs putting on the perception path.** W4 is a *downstream-consumption and tick-order* problem, and the old framing pointed an implementer at a seam that does not exist. Three constraints are now recorded, none of which were visible under the old wording, and **the first two make the obvious implementation a regression rather than a fidelity gain**: (1) `FilteredView.BallVisible` is `range AND FoV AND not-occluded`, and keeper facing is `AUTO_ALIGN` — velocity direction, else the movement-target vector, else unchanged when stopped (`AgentMovementSystem.cs:454-465`) — so against an 80° base half-angle (`PerceptionConstants.cs:57-65`) a keeper settled on his #12-composed slot has a shot from in front of goal *outside* his cone, and gating the save on `BallVisible` would blind keepers to ordinary **unobstructed** shots while presenting as a working occlusion feature; **how often is UNMEASURED**, which is itself an argument for taking the gate-firing instrument first. (2) `SaveArmed` has **two** production callers, not one — `MatchEngine.cs:3311` (the DT-emitted-SAVE gate in `RunMechanicsAI`, entered at `:3046`) and `:4461` (the W1 rush exclusion via `DriveGkHeadingTactical` at `:3072`) — and **both precede** the `:3074` heartbeat, so a view read at either site is the *previous* heartbeat: 100 ms stale at 10 Hz, the same order as the reaction latency W4 exists to model. (3) `:4457-4460` states the invariant that the two sites share **one** predicate, or the save/rush exclusion drifts into the ERR-011-007 shape. **No ERR filed**: the backlog is an AUDIT document, not an APPROVED spec, so the correction has no spec to back-propagate into. **The §5 wire-order table is deliberately untouched** — a revision (measurement lane first; W12 split into a targeted runtime census **W12a** and the static written-but-never-read sweep **W12b**; W6 ahead of W4) is under discussion and **not adopted**.
>
> **The lane.** New `.github/workflows/measure.yml`, `workflow_dispatch` only: pick one of the eight `TD_*` instrument switches, give a VSTest filter and a stated reason, and it runs that instrument on `ubuntu-latest` under .NET 8 and uploads the raw output as an artifact with a provenance header (commit, ref, run id, filter, reason, timestamp). **It is deliberately separate from `ci.yml` and deliberately cannot write to the repository** (`permissions: contents: read`): the instruments are opt-in and expensive, they assert nothing by design (the ERR-030-014 convention), and a job that cannot fail does not belong in a blocking gate where it would read as passing evidence. Accepting a result and committing it under `docs/tracking/corpus-data/` stays a human act.
>
> **`tools/dotnet-ci/run-gate.sh` gains `--test-logger`**, and this is the reason the lane works at all. The env-gated instruments report by writing to stdout; at `dotnet test`'s default verbosity that output is **discarded**, the run goes green, and the measurement is silently lost — a failure this project has already paid for once (`file-manifest.md`, *"learned the hard way"*). The flag is an explicit argument rather than an environment variable, because that file rejects ambient configuration by design. It is also surfaced in the existing `TD_GATE_DRY_RUN=1` output, which is what made it provable in an environment with no SDK.
>
> **Blast radius: checked, nothing moved.** No `src/` file, no scenario, no tick window, no per-90 band, no `[GT]`. No `SNAPSHOT_SCHEMA_VERSION` change, no new RNG stream, domain tag or draw site, and no draw-order change. No per-tick work added, so the FR-PO-052 perf baseline is untouched. No calibration invalidated. `run-gate.sh`'s default behaviour is byte-identical when the new flag is absent.
>
> **Gate: PASSED, and a standing environment claim is corrected in the process.** **The correction first, because it has been the recorded excuse for an unmeasured landing since August 4:** this project's W1 entry states that *"the authoring environment has no .NET SDK and the agent proxy denies the installer"*, and that is **too broad**. The proxy denies `dot.net` / `builds.dotnet.microsoft.com` (`connect_rejected`) — verified again today — but the **Ubuntu archive route works**: `apt-get update && apt-get install -y dotnet-sdk-8.0` yields a working **8.0.131** in about a minute. `tools/dotnet-ci/README.md`'s manifest row had already scoped the finding correctly (*"the 'no SDK; installer 403' conclusion was scoped to `dot.net`, not the archive"*) and it was not read. **The trap that makes this look like unavailability: `apt-get install` alone 404s on a stale package index**, and reads exactly like the package not existing; `apt-get update` first is the whole fix. Recorded here in the narrative chain because a wrong unavailability finding costs every future pass a measurement. **The gate then ran, on the real path, as the lane itself:** `TD_CONVERSION_DIAGNOSTIC=1 run-gate.sh --test-filter 'FullyQualifiedName~GoalConversionDiagnostic' --test-logger 'console;verbosity=detailed'` → meta-integrity OK, 71 csproj generated (35 test projects), whole-tree build 0 errors, **1 passed / 0 failed**, 5.92 min, **quarantine empty**, **`Gate PASSED`**. **The instrument's report reached the log**, which is the flag's entire purpose and is the actual acceptance criterion for this slice. **The silent-loss claim is not folklore here — it was reproduced as a negative control.** The identical command with `--test-logger` removed (`--fast`, same filter, same env gate) returns **1 passed / 0 failed, `Gate PASSED`, exit 0** — and `REPORT A`, `REPORT B` and the tercile tables appear **zero** times in its log. So the un-flagged form is green, passing, and carries no measurement whatsoever, which is precisely the failure mode: it is indistinguishable from a successful measurement unless someone greps for the report. No `src/` code changed, so no suite-count delta is claimed and no realism figure here is read as a finding — the instrument asserts nothing by design. **Also run, all green:** **261 tooling tests + 32 subtests** (`python3 -m pytest tools/tests/`), `yamllint` under CI's own config over `.github/` with **zero findings on the new file**, `python3 tools/doc-consistency-check.py --repo .` **PASS** — it caught the v1.14 bump leaving **seven** stale citations across `open-issues.md`, `CHANGELOG.md` and `file-manifest.md`, all seven refreshed here, which is that checker earning its CI slot — `recurring-defect-lint.py` clean, and `TD_GATE_DRY_RUN=1` proving the new flag parses, rejects its missing-value case, and reaches the composed arguments. **Known constraint, stated rather than discovered later: GitHub offers `workflow_dispatch` only for workflows on the default branch, so `measure.yml` is not dispatchable until it is on `main`.** Its first dispatched run is post-merge and no measurement is claimed from it yet. **What this does NOT mean:** since the SDK is installable locally, the lane is not a precondition for the next items — W12a and the still-owed W1 close-range conversion pre/post pair can be measured without waiting for it. The lane's value is repeatability and provenance, not access.
