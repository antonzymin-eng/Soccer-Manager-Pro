...
> **Purpose:** The `**Last Updated (prior):**` entry chain formerly carried in the header of
...
> **Last Updated:** September 10, 2026 — **Localization boundary validation closes fifty malformed-input regressions.** `LocaleId`, `LocalizationKey`, `NamedSlot`, `NamedSlotSet`, and `LocalizedTextRequest` now fail loud on malformed, unbounded, partially default, or contradictory identities. A shared allocation-free validator and a new Localization.Tests assembly lock 50 enumerated malformed cases plus default-slot and canonical-value coverage (52 passing tests). No simulation state, save schema, RNG stream, draw order, or gameplay behavior changed. The targeted generated .NET suite passes; the whole-tree fast gate was started and all reported suites passed, but was stopped during the long MatchEngine suite.
>
