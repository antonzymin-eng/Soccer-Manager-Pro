...
> **Last Updated:** September 18, 2026 — **PR #416 Step 5 evidence-ref cleanup verification is now mechanical; tooling/CI + documentation only.** New `tools/dotnet-ci/check_pr416_evidence_refs.py` separates the two deletion prerequisites instead of relying on prose: Gate A requires all **24 live `evidence/pr416-*` refs**, derives branch-exclusive history against durable `main`, enumerates intermediate changed-path blob states, verifies exact current-tip reconciliation, checks the preserved commit subject/date fields, verifies all **31** cited Actions run mappings against GitHub, and fails unless the independently established **266 branch-exclusive blob instances** are covered by `main` + the archive + the three policy-retained refs. Gate B verifies the landed archive from `main` alone: **100** manifest rows (**72 current / 13 run / 15 history**), exact archived Git blob identities, the **24-row 21/0/3** disposition ledger, the **31-row** run ledger, and the durable interpretation documents. New `.github/workflows/pr416-step5-evidence-cleanup.yml` runs Gate B against fetched `origin/main` before Gate A on the checker PR; `tools/tests/test_pr416_evidence_refs.py` locks the intermediate-history enumeration behavior. This change does **not** authorize or delete any evidence ref: all 24 disposition rows remain `delete_now=false`, and the three policy-retained refs remain excluded. No production/source, test semantics, APPROVED spec, assembly edge, durable field, snapshot/save schema, RNG stream/domain/draw site/order, owner-held bound, or `[GT]` value changed.
>
