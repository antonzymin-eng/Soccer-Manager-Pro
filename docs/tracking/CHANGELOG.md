...
> **Last Updated:** September 7, 2026 — **`main` synchronised into the long-lived `design/ux-foundation` branch (152 commits), and the F1 evidence pack given the tracking entry it landed without. Documentation only — no `src/` file, `.asmdef`, spec or `SPEC_INDEX.md` row moved on either side of the merge, so no gate is owed.**
