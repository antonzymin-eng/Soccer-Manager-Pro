...
> **Last Updated:** September 19, 2026 — **Post-#420 owner-held gate lifecycle correction, durable isolation payload, and cleanup reconciliation; tooling/tests/tracking only.** PR #420's exact-head evidence is now durable in repository bytes, not only prose: `docs/tracking/evidence/pr420-owner-held-isolation.md` pins run `35420947427`, head `2e12fa3e…`, artifact `10578621101`, and SHA-256 `8a441096…71570`; `docs/tracking/evidence/pr420-owner-held-isolation/` contains the exact **1.1 MB Actions ZIP routed through a path-specific Git LFS rule** (pointer OID = artifact SHA-256, size 1,106,934), a normalized sorted `scope / test_name / outcome / multiplicity` TSV (SHA-256 `846d72d7…cea7`), and `SHA256SUMS`. The archived payload verifies **70 TRX files, 3,730 ordinary result records / 1 dedicated result, and exact `sim_match_engine_close_chance` multiplicity 0 ordinary / 1 dedicated**. The gate-integrity issue remains narrowed rather than closed because the historical pinned-main leakage mechanism is still unidentified. The #420 lifecycle path is repaired without weakening policy: an empty existing owner-held ledger means **no exception exists**, so no owner-held exclusion is applied and the ordinary suite owns the result; a missing ledger file still fails closed. An accidentally emptied-but-present ledger remains indistinguishable from deliberate final-row removal at runtime and is therefore controlled by repository review/history plus the emitted empty-ledger status. The verifier's success-only isolation line derives observed multiplicities rather than hardcoding `0/1`; failing leak/duplicate multiplicities stay on their earlier error paths. All verifier evidence/status stdout is flushed before stderr disposition errors. Tests lock ordinary leakage, duplicate dedicated execution, empty-vs-missing ledger semantics, and merged-stream ordering `TRX → ISOLATION → disposition error`; the ordering test now explicitly removes `PYTHONUNBUFFERED` so its buffering precondition cannot silently become vacuous. **Step 5 cleanup is also closed:** PR #427 merged as `c470eace…`, its head `d1d79b6…` is an ancestor of current `main`, the landed checker enforces the completed 21-deleted / 3-policy-retained topology, and the former live cleanup issue moves verbatim to the archive. That move is **28 active / 56 archived → 27 / 57**. The same pass files a new live tooling/process issue after a shallow agent clone falsely reported fully merged branch `Copy-Main-All-Specs-From-Claude` as unrelated to `main`; authoritative remote comparison confirms branch head `caaf5cf0…` is the merge base and `main` is 2,497 commits ahead / 0 behind from that base. Ref-only fetches do not unshallow a clone, and current-tree blob absence is not reachability evidence. Final counts are therefore **28 active / 57 archived**. The three PR416 policy-retained refs remain retained; `close-chance-retirement` is not deleted or reclassified. The current `sim_match_engine_close_chance` row and recorded diagnostics/bounds are unchanged. The functional job's expected unexpected-green RED is a separate owner-held disposition, not a failure of lifecycle/topology validation; exact-head topology lines and retained TRX own that claim. `.gitattributes` changes only to route that single evidence ZIP through Git LFS. No production/source semantics, APPROVED spec, assembly edge, save/snapshot schema, RNG behavior, owner-held bound, or `[GT]` value changed.
>
