...
>
> Review of merged #459/#460 verified the frozen A run and helper equivalence but exposed two evidence limits and three tracking/structural omissions. The Stage A record now names the claim→pass→claim loop as a known A→B confound (862 hand claims; 501/861 hand-origin passes interrupted by a later hand claim; 13/861 committed receivers controlled; 32.086-frame mean hold), records that 576/580 immediately PassKick-preceded claims were the same keeper reclaiming his own pass and none directly followed the opponent keeper's pass, and requires a test-only supplemental identity split for the exact 501-outcome population before B measurement. It also records that all observed A policies were `SlowDown`: the frozen six-seed A→B rerun is therefore SlowDown evidence only, and B must prove all six #21 policies in deterministic composed fixtures. The stale helper-next-step text and missing Created date are corrected.
>
> #461's B contract is tightened before review: the initial distance-blind `lerp(vMin,vMax,power)` goalkeeper speed is replaced by #5's existing distance-sensitive distance/power shape with no second KickPower/fatigue/weak-foot multiplier; launch angle likewise keeps the existing distance-sensitive profile formula. The central possession seam gets explicit completion/cancellation semantics: no teardown when the possessor is unchanged; ordinary loss tears down only a live outgoing hand episode; successful GK-distribution CONTACT is a typed normal-completion cause and may not self-cancel #5/#11. The SlowDown-only corpus coverage gap is made a normative all-six-policy B test obligation. ERR-011-015/016/017 remain spec-first/code-pending.
>
