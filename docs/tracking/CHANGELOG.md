...
> **Last Updated:** September 23, 2026 — **PR #447 begins durable W3/#439 evidence archival; no evidence refs deleted.**
>
> The exact six-seed pre-W3 structural-comparator table and final landed-W3 table are now committed under `docs/tracking/evidence/pr439-w3/` with full SHA-256 coverage. The archive records the original post-W3 run/artifact/production SHA as well as the final rerun; local artifact verification confirms the two post-W3 `report.txt`, `results.tsv` and `results.json` files are byte-identical, while logs/TRX are not asserted identical. Interpretation is intentionally narrower than a raw table diff: the pre-W3 W3 counters were structural literal zeros, not live measurements; `headerAttempts` is contacts+failures rather than HeaderIntent commits; and the measured landing state also includes the W6 reattachment correction, #442/#443 merge and `ERR-010-004`/`ERR-011-014`, so trajectory/foul/aerial deltas are not attributed to W3 alone. A timestamped 16-ref ledger is committed under `docs/tracking/evidence/pr439-ref-archive/`. Phase 1 records refs but does not protect them: branch-exclusive commit/blob archival, cited-run heads, explicit dispositions and a live-ref completeness verifier remain required before any `evidence/pr439-*` deletion. Evidence/governance only; no gameplay `[GT]` or production behavior change.
>
