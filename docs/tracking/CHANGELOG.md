...
>
> The repository-wide claim is intentionally narrow: this is **repository-wide integrity-contract registration**, not a claim that all evidence bytes are SHA-manifested. W12 remains governed by `check_w12_evidence.py`; the PR #416 ref archive remains governed by `check_pr416_evidence_refs.py`; `artifact-SHA256SUMS` covers only its named payloads. README documentation now states those boundaries and provides the strict full-manifest regeneration command.
>
