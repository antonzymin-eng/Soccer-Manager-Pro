...
> **Last Updated:** September 3, 2026 — **A3.1b post-merge review findings on PR #351 resolved; documentation only.**
> Two P2 findings against the post-merge correction itself. **(1)** That correction restated KD-4 in `outline-detailed.md` alone, leaving the authoritative `section-1.md` §1.3 saying no source code exists, no static-analysis tooling is required, and tooling waits for the Stage 0+1 transition — against §5.1's own recorded tree (35 production assemblies, 947 `.cs` files, `dotnet format` advisory and `tools/dotnet-ci/run-gate.sh` blocking on every push), which licensed a reviewer to treat a live blocking check as optional. KD-4 is now restated normatively in `section-1.md` v1.2 and matched in `outline-mid.md` v1.5, so all three outline tiers and the section file agree; the custom analyzer set, `.editorconfig`, `BannedSymbols.txt` and KD-5/D1's numeric thresholds remain Stage 0+1 deliverables. **(2)** §5.5's FR-CS-074 row had *replaced* "A4 canonical-selector / identity resolver" with the owner/point/state resolver instead of adding to it, while §5.4.8 item 1 and §3.5.6 still make the identity mechanics mandatory; both resolvers are now named. Also swept the last two `future \`src/CLAUDE.md\`` residues (`section-1.md` §1.4, `outline.md` executive summary) and refreshed the `file-manifest.md` outline-tier inventory row, which had gone stale for three files. `SPEC_INDEX.md` stays untouched pending A3.4. No runtime/code/schema/CI-enforcement change.
>
