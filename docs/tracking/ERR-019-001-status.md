...
- PR CI now wires blocking auditor enforcement to the actual approval event. The functional job checks out full history, passes the PR base SHA as `TD_APPROVAL_BASE_REF`, and `tools/testing-strategy-approval-scope.py` identifies spec sections that changed from non-approved/missing to `APPROVED`. Only those spec directories are rerun through both auditors without `--survey-only`; an unavailable base commit fails closed.
...
- the current candidate survey reports 219 checklist findings plus 329 §5 schema findings, 548 total. They remain survey debt rather than retroactive merge blockers; A3.4 must re-run and size the then-current corpus before a broad approval walk.
- `.githooks/pre-commit` evaluates the staged Git index in a persistent `.git/testing-strategy/precommit-snapshot`. Bootstrap pays the one-time full materialization outside the normal 60-second acceptance measurement. Later runs compare the previous/current index tree, remove deleted tracked paths with a linear manifest `comm`, and refresh only changed index paths. Unchanged tracked files retain their mtimes while untracked generated/bin/obj outputs survive, allowing MSBuild incremental state to remain reusable.
...
1. **Pre-commit timing acceptance is unproven.** The zero-cache and whole-tree-mtime defects are removed, but timeout/caching structure does not prove that a complete passing incremental composition finishes within 60 seconds on the certified developer host. A successful measured run is still required.
