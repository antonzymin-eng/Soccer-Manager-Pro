...
- `MatchClientBehaviour : MonoBehaviour` (in `match-client-unity`) — the PlayerLoop host the project
  ...
  clause 2); map `TeamId` to a palette. It should contain no DECISION a test would want to reach
  (L11, round 3: the file now has ~15 `RejectWiring` branches, a 3-arm material check, and a
  try/catch around `BuildScene` — all deliberately added by H5/H6/M1/L6, none reachable by any test,
