...
- `MatchClientBehaviour : MonoBehaviour` (in `match-client-unity`) — the PlayerLoop host the project
  ...
  the extent — see AR-P4a-H1), place the camera from `PitchCameraRig.ComputePose` with a
  `transform.LookAt`, turn a click into a ray for `PitchViewProjection.TryGroundHit`, assign
  `transform.position`/`localScale` from the
