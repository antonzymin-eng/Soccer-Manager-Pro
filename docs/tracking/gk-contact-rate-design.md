...
| AR-6 (CI-gate fallout on main run 419 — 1 failure, the first BOUND change in this scenario's history; owner-approved rebaseline) | 0H+1M | **M-1: `no-deep-dive-early-miss` tripped (`deepDiveEarly=1`) at the ERR-008-021/-022/-023 main merge** — the shot-lane chain changed which shots are taken, and the new population produced one crossed episode whose dive resolved **616.7 ms** before arrival (seed 0xD1A6D05E; the other crossed episode measured −16.7 ms). 616.7 ms is *inside* the pre-fix 456–2000 ms class, so this is not the AR-4/AR-5 instrument class: the miss class this scenario declared extinct has genuinely recurred, at 1 per match-equivalent against the pre-fix 9-of-15. Rebaselined `== 0` → `<= 1` (owner call, August 7, 2026) rather than widening `DeepDiveEarlyMs` past the episode — a 620 ms+ bound would have hollowed the predicate. The recurrence is queued for the KD-W1 calibration pass (the §7 item-3 rush wire changes this geometry again regardless). Bounds otherwise unchanged; scenario v1.1 |
| AR-5 (CI-gate fallout — 1 failure, the same instrument class as AR-4, found by the Linux gate on PR #282) | 0H+1M | **M-1: the #37 MatchAnalytics liveness window no longer carried away possession.** `MatchAnalyticsObserverNeutralityTests.TapIsLive_PossessionRecordsReachTheAggregatorAndResolveBothTeams` asserts both teams' possession is non-zero over a 30-second window (1 800 ticks) on its fixed seed; this pass moved that seed's opening trajectory so the away side's FIRST possession now lands between ticks 1 800 and 2 400 (measured by a per-window probe: away 0.000% at 900/1 200/1 500/1 800 → 3.625% at 2 400 → 3.389% at 3 600), so the window measured away possession at exactly 0 and the assertion — written precisely so the test would "not ride on a single early transition" — rode on exactly that transition. Window re-measured 1 800 → 3 600 ticks (60 s; 1.5× margin over the measured onset, home 9.4% / away 3.4% / sum < 100 at the new window); assertions unchanged (`MatchAnalyticsObserverNeutralityTests` v1.1). The third case of the AR-4 class: a trajectory-calibrated instrument window surfacing its assumption after a behaviour change to the most-composed assembly |
...
| 1.4 | 2026-08-07 | — | AR-6 (main run 419 fallout, ERR-008-023 downstream): `no-deep-dive-early-miss` rebaselined `== 0` → `<= 1` (owner call) — one crossed episode measured 616.7 ms early under the post-chain shot population, inside the pre-fix class range, so the ms bound was deliberately NOT widened. First bound change in this scenario's history; the recurrence itself is KD-W1 calibration-pass work. `MatchEngineKeeperContactScenarios.cs` v1.1. |
#endregion
