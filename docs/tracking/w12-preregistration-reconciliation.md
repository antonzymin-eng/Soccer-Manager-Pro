...
That audit found most of the stale branch superseded rather than missing: current `main` already retains the exact pre/post Actions ZIPs, the migrated `tools/dotnet-ci/check_w12_evidence.py`, regression coverage under `tools/tests/test_w12_evidence.py`, and enforcement through the required `Spec hygiene checks` context. The stale branch's Base64 reconstruction payload, duplicate root-level checker, and modified comparison document therefore remain deliberately unreplayed.

A later salvage review identified one useful branch-only record that was not superseded: `docs/tracking/evidence/w12/w12-gate-firing-census.json`, which the earlier `w6-controlled-ball-preregistration.md` already names as its locked baseline source. PR #432 restores that machine-readable census on top of current `main` and strengthens the existing checker/tests so the copy fails closed against the committed ZIP evidence. It does **not** create a second enforcement lane: the draft path-filtered W12 workflow was removed after review because `tools/tests/test_w12_evidence.py` is already discovered on every PR/push inside the required `Spec hygiene checks` context.
