...
- **KD-CC6a — PARKED, ratified by owner decision August 17, 2026: `pointQuality` stays out until the
...
| 1.3 | 2026-08-18 | — | **Reviewed adversarial-review finding H13 — the KD-CC6a HEADING still carried the satisfied condition v1.2 corrected in the body.** v1.2 restated the unparking condition inside KD-CC6a's text (the close-range conversion comparison on identical seeds, not the rush-geometry measurement discharged August 12, 2026) but left the decision's bolded title reading "stays out until W1's rush geometry is measured" — so a reader skimming titles concluded the park was releasable. Title corrected in place with a dated annotation; no design content changed, no `[GT]` moved. The same stale condition was corrected the same day at its two restatement sites outside this file (root `CLAUDE.md` OPEN ISSUES summary; `docs/tracking/CHANGELOG.md` HELD (6) heading). |
#endregion
