...
> **Owner decision, September 20, 2026:** §10.9 item 7 supersedes item 6's held-red disposition. The acceptance bounds and seeds are unchanged; only the owner-held exception is retired, returning the test to the ordinary blocking sweep.
...
collapse case that would have signalled a genuine regression. **Provenance of that collapse figure,
which the first draft of this section left unstated:** the advisory read that specified this
falsifier predicted term-off ≈ −0.20 for a collapsed mechanism, and −0.165 − (−0.20) = **0.035**.
It is a prediction registered before the run, not a number reverse-engineered afterwards to yield
"seven" — but it is a prediction, not a derivation, and the ratio is only as meaningful as it.
The intact-case prediction "≈ −0.45" is likewise that read's own approximation; computed exactly
from its stated +0.30 it is −0.465. Neither changes any conclusion here. `DirectionQuality_DRIBBLE` is doing essentially the same work it did before C1.
...
**6. Owner call, August 11, 2026: CONFIRMED — hold red, do not rebaseline a third time.** Item 4's
recommendation is adopted as the decision, not merely recorded as one. `sim_match_engine_close_chance`
stays failing (`meanCosine` **−0.165** against the −0.16 bound; `goalwardShare` **0.407** against 0.42;
both PASS at pre-C1 `ba4e194`) and its bounds are **not** moved. A predicate already rebaselined once
(§9 Acceptance-3, −0.10 → −0.16, owner call August 7, 2026) and already once deleted outright under the
same rule when it stopped being a lock (§9 Acceptance-1's box predicate) does not get a second
rebaseline to make it pass; the failure is queued for the KD-W1 calibration pass — against the complete
engine, per the wiring backlog's own sequencing — rather than fitted around piecemeal today. No `[GT]`
moves and no code changes as a result of this call; it is a disposition, not a fix. This closes the
close-chance half of the owner call the match-engine wiring backlog's C1 entry was left awaiting
(`match-engine-wiring-backlog.md`, `open-issues.md`, mirrored in root `CLAUDE.md`'s OPEN ISSUES index).
**Not covered by this call:** the `sim_match_engine_shot_outcomes` `fast-balls-deflect-off-bodies`
reachability predicate that C1 drove to zero is a separate lock in the same note and stays open,
awaiting its own owner call.

**7. Owner call, September 20, 2026: SUPERSEDED — retire the owner-held exception; keep the bounds.**
Item 6 remains the historical August 11 decision. It is superseded now because the predicate no longer
describes a standing red state: exact retirement-evidence run `35344056248` on
`evidence/pr416-close-chance-retirement` measured **2,494** final-third dribbles,
`meanCosine=-0.038` and `goalwardShare=0.496`, against unchanged floors **−0.16 / 0.42**.
The only source delta on that evidence ref is commit `ffe676fd…`, a `Console.WriteLine` inserted
*after* both values are computed and *before* the predicates, so it observes rather than perturbs them.
The corresponding margins are **0.122 / 0.076**. The cosine margin is only about **1.1×** the
two-seed pooled trajectory-resampling sd **0.110** already documented above, so a later
trajectory-moving change can legitimately push this floor red again; that future failure must be
investigated mechanistically and is not, by itself, proof of a regression or permission to rebaseline.
No seed, predicate, bound, `[GT]`, gameplay code, or scenario logic moves. The policy change is only
that `sim_match_engine_close_chance` returns to the ordinary blocking sweep and no longer has a
dedicated owner-held execution.
...
| 2.4 | 2026-09-20 | — | **Owner decision supersedes §10.9 item 6's held-red disposition without changing the acceptance test.** Exact retirement evidence run `35344056248` measured 2,494 dribbles, meanCosine **−0.038** and goalwardShare **0.496** against unchanged floors −0.16 / 0.42. The evidence ref differs in the scenario only by an observation-only `Console.WriteLine` after the values are computed. The owner-held row is retired and the test returns to the ordinary blocking sweep. The 0.122 cosine margin is only ~1.1× the documented 0.110 two-seed resampling sd, so a future trajectory-moving failure remains a sensitivity to investigate rather than automatic evidence for rebaseline/regression. No seed, predicate, bound, `[GT]`, gameplay code, or scenario logic changed. |
#endregion
