...
| `RegenGenerator.cs` | `static (PlayerRecord record, PlayerLifecycle life) GenerateRegen(DeterministicRngService rng, int streamIndex, int clubId, int newPlayerId, uint worldDay)` — the single-player analogue of `RosterGenerator.Generate` (§3.3); **same** `Reserve(streamIndex, PROGRESSION_REGEN_FIELDS)`/`DrawReserved`/`CloseReservation` pattern + `NameCatalogue`; takes an explicit `newPlayerId` (never reuses a retiree's — KD-3) and **returns the lifecycle too**, carrying the drawn `PotentialAbility` (a record-only return would drop the regen's PA ceiling). See the drill-down §3.7 (authoritative). |
...
= 0 handles it. *(The #28 section files have already settled the timing: §4.3 / FR-PG-020 register the
stream at the **first regen for a club, never earlier** — construction-time registration would be the
zero-draw phantom-surface class FR-LW-031 forbids (a club can go many seasons with no retirement, and a
new-game save predates every boundary). So the `regenStreamRegistered` presence flag is **load-bearing**
and stays; it is not a droppable "always 1" case.)*
