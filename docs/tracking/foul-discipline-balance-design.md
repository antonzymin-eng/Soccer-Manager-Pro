...
>
> **Current calibration phase (September 22, 2026):** The frozen post-W2 baseline characterization has now run on exact landed `main` SHA `c56e5e1adff4e240b54c9abbb5ebea9c0290f65e` (run `35765635130`, job `106874328764`) and is durably retained at `docs/tracking/evidence/foul-card-six-seed/`. Across six full matches the shipped engine produced **50 fouls = 8.33/90, 5 cautions = 0.83/90 and 1 total dismissal = 0.167/90**, with **42 collision FROM_BEHIND + 8 W2 SLIDE_TACKLE** fouls and **42/0 priced-candidate identity checks/mismatches**. Both preregistered tackle-during-cooldown counters observed zero, but that sparse exposure does **not** decide whether the production bypass is intended or defective; the owner disposition required before any final fit remains open. This is characterization, not the #435 §5 final calibration regression: no governed `[GT]` change is authorized until KD-W1's complete-engine condition is met, and any #435 §8 invalidator requires the same frozen source-complete corpus to be rerun.
...
| `FoulCallProbability` | *(new)* | **0.015 at v1.0; 0.030 current** | v1.0 was calibrated on a LIVE run, not the offline sweep — see §6. §5.Z.13 later changed collision emission from sustained-overlap-per-tick to one-event-per-contact; 0.015 then produced ~0.4 fouls/90 and was recalibrated to the current 0.030. The post-W2 preregistration starts from 0.030 and treats emission granularity as an invalidation trigger. |
...
3. **THE TRIPWIRE FIRED, and nobody was watching — recorded August 13, 2026, decision taken August 17,
   ...
   committed diagnostic (`FoulRateDiagnosticTests`, `TD_FOUL_DIAGNOSTIC=1`, 6 seeds × 54 000 ticks):
   **fouls 35.0 / yellows 5.0 / reds 1.00 per 90** against this pass's own post-fix 21.0 / 3.0 / 1.0 —
   fouls and yellows both **~67% above** where this note left them, and reds still **4×** football's
   ~0.25. The acceptance bands cannot see any of it: `MatchEngineDisciplineScenarios` caps fouls at 90,
...
**✅ OWNER DECISION, August 17, 2026: hold the drift. Arm W2 first, then calibrate ONCE.** No interim
   ...
   **Accepted cost:** the card rate — and, since #44, the suspension rate derived from it — stays
   ...
   throughout. **What un-holds it:** W2 armed at a non-zero `TackleContactRadiusM` with the
