...
> **Last Updated:** July 22, 2026 (v0.4 — **LANDED.** Implemented per the converged design; the code
...
| 0.4 | 2026-07-22 | — | **LANDED.** New `src/season-save/` assembly (`SeasonSaveConstants`/`SeasonSaveBlobs`/`SeasonSaveCodec`/`SeasonSaveContents`/`SeasonSaveManager`) + `MatchSaveManager.Encode`/`Restore` public blob API (KD-5). Code self-AR clean on the faithful-mirror surface. New `SeasonSaveManagerTests` (18). Booking-cursor case not re-driven at season level (nested match-blob property, KD-2). Full dotnet gate PASSED, 0 failures (whole tree green; SDK via apt). Closes snapshot-deserialize N2 / Phase G-Phase 3. |
