...
> **Status:** CLOSED OUT — lands with PR #439 as **W3: wired but dormant** (owner classification); #441 owns Head reachability and #440 stays deferred. v0.18 carries the #441 per-seed header-reachability census and W3 artifact member hashes into `pr439-w3/`. v0.17 records the completed durable evidence/ref archive and live-verification gate; v0.16 established its attribution limits. v0.15 records the #442/#443 contract landing, the non-rebase merge of `main` into #439, and the post-merge verification. v0.14 and earlier record the raw-log-audited production corpus, the W2 gate diagnosis, the M7 mutation proof, the #441 localization and the #442 protocol.
...
The #442 owner approval, if granted, must also lock the **landing order and result interpretation**
before any six-seed W2 outcome is read:

1. #442 is a **separate PR from current `main`**; its six-seed `CorpusSeeds` / two-seed
   `RestoreSeeds` contract is committed before measurement.
2. The identical committed contract is exercised on the #442 base/main parent and on a separate
