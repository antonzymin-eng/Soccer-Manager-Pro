...
> **Last Updated:** July 20, 2026 (v0.6 — **PROMOTED to `match-engine-design.md` Phase G; Phase 1
> KD-8 writer half LANDED** — `SNAPSHOT_SCHEMA_VERSION` 16 → 17 serializes the `match-flow.card-severity`
...
> Phase 1 IN PROGRESS** (Stage 0+1 integration scaffolding; NOT a numbered spec, same governance
...
> `RngStreamState` cursor; `MatchEngine.cs` v1.40 / `MatchEngineConstants.cs` v1.24). The reader
> (`DeserializeWorldState`), the `RestoreState` counterpart seams, the `RestoreFromSnapshot` factory,
> and the G3 round-trip determinism test are the remaining Phase-1 slices.
> **Author:** —
