...
| Version | Date | Change |
|---------|------|--------|
| v0.7 | August 6, 2026 | **T2 AR passes 3–5 — 1H + 4M + 7L, all fixed.** The #29-side findings sharpen v0.6's own lesson rather than adding a new one, which is itself the point. **The High:** `SeasonLoop`'s career-wired match boot had never executed anywhere — every career test runs `QuickSimAll` and every engine-mode test in `season-save` builds the loop without a career — so the §3.3 match-entry-fatigue seam's **sole production call site** shipped unrun. `SeasonLoop.BootFixtureEngine` is extracted `internal` so it can be asserted on in milliseconds instead of a 90-minute match. **The Medium that matters to #29's own contract:** `MatchEngineEntryFatigueTests` could not distinguish `entryFatigue[local]` from `entryFatigue[k]`, because the fixture squad lays positions out in slot order — so the KD-1 projection's *indexing*, the thing that breaks the moment #30's availability filter renumbers the locals, was untested in both suites at once. **And the correction to v0.6:** that entry recorded `RosterGeneration` as the answer to the stale-`ScheduleFor`-handle defect. It was not — it made the loss *detectable by a caller who remembered to compare a counter*, which is the same documented-not-enforced shape v0.6 itself names as the general lesson. `ScheduleFor` is now `internal` and the FR-TR-023 surface is `PlayerCareerStates.TrySetFocus`, a command resolving fresh per call: the handle cannot be held, so it cannot go stale. `FromBlocks` likewise now **copies** the state arrays, closing the second-writer route the save-side `internal` accessors left open. **Carrying v0.6's lesson forward, amended:** a public accessor whose contract is documented rather than enforced is the defect — and a *counter that makes the violation visible* is not enforcement. |
| v0.6 | August 6, 2026 | **T2 AR pass — 3H + 4M + 4L, all fixed; pass 2 clean.** The #29-side findings: `PlayerCareerStates.FromBlocks` never checked that player ids ascend while every lookup is a binary search, so an unordered block made `SyncToRoster` overwrite carried players' conditioning with `Create()` — silent and total, and precisely the FR-TR-025 lifecycle failure the day-0 trap discipline exists to prevent, arriving from the other direction. `TrainingBlocks()` went `internal`: it hands out the live `TrainingState[]`, so a public accessor made every holder of `SeasonLoop.Career` a second writer, defeating FR-TR-004 / FR-TR-023's sole-writer contract exactly as `SeasonState` would have been defeated without its `internal` mutators. And `ScheduleFor`'s handle — the FR-TR-023 command surface — binds arrays the season boundary replaces, so a cached schedule silently discarded focus changes; a `RosterGeneration` counter makes that detectable. **The general lesson worth carrying:** every one of those is a *public accessor whose contract was documented rather than enforced*. |
...
| v0.8 | August 7, 2026 | **The #29/#41 balance pass**: `InjuryRiskMax` is no longer #41's draw denominator (ERR-041-011 pinned that `[FIXED]` at 1,000,000; ERR-029-007 corrects this side's text) — the shared ceiling is now the 1.6%/day occurrence-probability cap (16,000; raised from the D3 first-guess 10,000 at the balance-pass AR to restore discrimination headroom), `[CROSS]`-mirrored by #41 as before (ERR-041-003, discharged). #29's own step, tables and neutrality locks are untouched; the matchday ordering note changes shape (ERR-030-027: the fixture day's slot-2 training step now runs pre-round inside `AdvanceAndPlayNextRound`, idempotently re-entered by the next advance). (Authored as a second "v0.5" with a stray column; renumbered v0.8 at the balance-pass AR pass 2.) |
