...
| v0.5 | August 7, 2026 | — | **The #29/#41 balance pass**: `InjuryRiskMax` is no longer #41's draw denominator (ERR-041-011 pinned that `[FIXED]` at 1,000,000; ERR-029-007 corrects this side's text) — the shared ceiling is now the 1%/day occurrence-probability cap, `[CROSS]`-mirrored by #41 as before (ERR-041-003, discharged). #29's own step, tables and neutrality locks are untouched; the matchday ordering note changes shape (ERR-030-027: the fixture day's slot-2 training step now runs pre-round inside `AdvanceAndPlayNextRound`, idempotently re-entered by the next advance). |
