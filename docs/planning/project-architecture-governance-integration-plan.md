...
**Current state: REVIEWED, CONDITION 4 REOPENED, NOT CLOSED.** The candidate provides ten schema documents, seven versioned state registries, reference semantics v2.0.0, and the bounded schema validator `tools/architecture-governance/schema_validator.py`. The verification split is 147 governance + 9 phantom-stream context + 8 assembly-tier fixtures.

**Conditions 1, 2, 3 and 5 are satisfied.** Nine review rounds are recorded in the durable ledger under series `A2-SCHEMA-FREEZE`, each bound to the material subject digest of the tree it actually reviewed and each digest recomputed from the commit its scope names. Twenty-two findings, all `Blocker` / `Resolved`. Every independent round has found a defect the preceding non-independent work did not; round 8 found three in the frozen contract itself, and round 9 found that one of round 8's own fixes had closed the §3.9 `inactive → migration` edge.

**Condition 4 was claimed at plan v0.20, retracted at v0.21, and remains open at v0.26.** Independent review found that round 3 reviewed `678f0f2`, the material subject then moved by 150 lines — including the `A2-R3-001` fix — and the commit asserting completion had itself never been reviewed. The gate's pushed-candidate wording is deliberately stronger than FR-AG-018's, and the party satisfying a condition does not get to relax it. Row 4 becomes claimable only after a fresh review of the artifact as pushed.
